...
public class SerialRxMessageQ_V2 {

	volatile Queue<String> msgQueue = new LinkedList<>();
	...
	public SerialRxMessageQ_V2(CommunicatorV2 inpSerialPortObj) {
		serialPortObj = inpSerialPortObj;

	}

	public CommunicatorV2 getSerialPortObj() {
	 ...
	 * error("IsExpectedErrorResponseReceived :Exception:"+ e.getMessage());
	 * }
	 * };
	...
	public Boolean IsResponseReceived() {
		...
		while ((!responseRecieved) && (responseWaitCount != 0)) {
			// ApplicationLauncher.logger.debug("IsResponseReceived: responseWaitCount: " +
			// responseWaitCount);
			// ApplicationLauncher.logger.debug("IsResponseReceived: responseRecieved: " +
			// responseRecieved);
			try {
				Thread.sleep(250);
	 ...
	 * e.printStackTrace();
	 * ApplicationLauncher.logger.
	 * error("SerialRxMessageQ IsExpectedResponseReceived :Exception:"+
	 * e.getMessage());
	 * }
	 * };
	 ...
	 * e.printStackTrace();
	 * ApplicationLauncher.logger.
	 * error("SerialRxMessageQ IsUnknownResponseRecieved :Exception:"+
	 * e.getMessage());
	 * }
	 * };
	...
	public Boolean isUnknownResponseRecieved() {
		ApplicationLauncher.logger.debug("isUnknownResponseRecieved V2: Entry");
		return this.UnknownResponseRecieved;
	}
	...
	public void addMsgQueue(String inputMessage) {
		...
		// ApplicationLauncher.logger.info("SerialRxMessageQ V2: inputMessage:
		// "+inputMessage);
		// ApplicationLauncher.logger.info("SerialRxMessageQ V2: inputMessage-String:
		// "+GuiUtils.asciiToHex(inputMessage));
	...
	public void removeProcessedMsgFromQueue() {
		...
		// ApplicationLauncher.logger.info("SerialRxMessageQ:
		// removeProcessedMsgFromQueue invoked");
		// ApplicationLauncher.logger.info("SerialDataMsgHV_Manager:
	 ...
	 * 
	 * ApplicationLauncher.logger.
	 * info("SerialDataMsgHV_Manager2: addMsgQueue invoked");
	 * //msgQueue.add(inputMessage);
	 * 
	 * }
	 */
	...
	public void ProcessRxMsg() {
		...
		if (lastMessage != null) {
			...
			ApplicationLauncher.logger.debug("ProcessRxMsg: lastMessage hex :" + GuiUtils.asciiToHex(lastMessage));
			...
			ApplicationLauncher.logger.debug(
					"ProcessRxMsg: getExpectedResult hex :" + GuiUtils.asciiToHex(serialPortObj.getExpectedResult()));
			...
			if (lastMessage.length() == serialPortObj.getExpectedLength()) {
				if (lastMessage.equals(serialPortObj.getExpectedResult())) {
					...
					ApplicationLauncher.logger.debug("ProcessRxMsg: lastMessage2A:" + lastMessage);
				}
			} else if ((lastMessage.startsWith(serialPortObj.getExpectedResult()))
					&& (serialPortObj.getExpectedResult().length() > 0)) {
				...
				if (lastMessage.contains(serialPortObj.getExpectedError1Result())) {
					...
					if (lastMessage.contains(serialPortObj.getExpectedError2Result())) {
						...
						setResponseRecieved(true);
						// ApplicationLauncher.logger.debug("ProcessRxMsg: Unexpected message dropping
						// the packet1:"+lastMessage);
				...
				if (lastMessage.contains(serialPortObj.getExpectedError2Result())) {
					...
					ApplicationLauncher.logger
							.debug("ProcessRxMsg: Unexpected message dropping the packet3:" + lastMessage);
					removeProcessedMsgFromQueue();
				}
			} else {
				// ApplicationLauncher.logger.debug("ProcessRxMsg: Unexpected message dropping
				// the packet4:"+lastMessage);
				// removeProcessedMsgFromQueue();
				ApplicationLauncher.logger.debug("ProcessRxMsg: Unexpected message7: " + lastMessage);
				setUnknownResponseRecieved(true);
				setResponseRecieved(true);
			}
		} else {
		...
		// ApplicationLauncher.logger.debug("ProcessRxMsg: Exit :
		// ExpectedResponseRecieved:" + ExpectedResponseRecieved);
	}
