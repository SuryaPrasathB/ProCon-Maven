...
/*			if(MaintenanceModeExecController.isSerialDisplayProcess()) {
	...
	class SerialRxPhysicalTask extends TimerTask {
		public void run() {
			...
			//ApplicationLauncher.logger.debug("ReadRxPhysicalData V2: "+ rxLabel+": "+SerialPortObj.getDeviceType());
			if(MaintenanceModeExecController.isSerialDisplayProcess()) {
				//if(ProjectManualModeExecController.isProGenDemoExecution()) {
				if(ProcalFeatureEnable.MAINTENANCE_MODE_ENABLED) {	
			...
			//ApplicationLauncher.logger.debug("ReadRxPhysicalData:  responseMessageTerminator:"+responseMessageTerminator);
