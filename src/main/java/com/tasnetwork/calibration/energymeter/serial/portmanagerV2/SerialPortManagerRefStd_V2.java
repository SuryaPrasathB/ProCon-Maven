...
public class SerialPortManagerRefStd_V2 {
	...
	public SerialRxPhysical_V2 rxPhysical_RefStd = new SerialRxPhysical_V2(commRefStandard, rxMsgQ_RefStd, false,
			"RefStd", DeleteMeConstant.HV_ER_TERMINATOR, "");
	...
	private void createObjects() {
		...
		rxPhysical_RefStd = new SerialRxPhysical_V2(commRefStandard, rxMsgQ_RefStd, false, "RefStdV2",
	...
	public boolean setSerialCommV2(CommunicatorV2 SerialPortObj, String SerialPort_ID, Integer BaudRate,
		...
		try {
			...
			if (SerialPortObj.isDeviceConnected() == true) {
				...
				ApplicationLauncher.logger
						.debug("setSerialCommV2 : device com port " + SerialPort_ID + " not connected");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Map<String, Object> refStdSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec,
			String expectedDataInHex, boolean isResponseExpected, String rxMessageTerminatorInHex) {
		...
		// String ExpectedDataInHex = GUIUtils.StringToHex("K");//GUIUtils.StringToHex(
		// VoltageValue)+ConstantPrimaryVICI_Meter.VI_ER_TERMINATOR;//getER_HV_StartAck
			...
			while ((retryCount > 0) && (responseData.equals(DeleteMeConstant.NO_RESPONSE) &&
				...
				if (rxMsgQ_RefStd.IsResponseReceived()) {
					if (rxMsgQ_RefStd.isExpectedResponseReceived()) {
						...
						// ApplicationLauncher.logger.info("powerSourceSendCommandProcess: VICI Received
						...
						String CurrentReadData = rxMsgQ_RefStd.getLastReadMessage();
						// ApplicationLauncher.logger.info("powerSourceSendCommandProcess: ErrorResponse
						// Received:"+CurrentLDU_Data);
						// ApplicationLauncher.logger.info("powerSourceSendCommandProcess:
						...
						if (responseData != null) { // Gopi2
							ApplicationLauncher.logger
									.debug("refStdSendCommandProcess :Unexpected Message: dropping the message:"
											+ responseData);
							// ApplicationLauncher.logger.debug("refStdSendCommandProcess :Unexpected
		...
		if (rxMsgQ_RefStd.getLastReadMessage() == null) {
			...
			// ApplicationLauncher.logger.info("refStdSendCommandProcess: last
			// read:"+rxMsgQ_RefStd.getLastReadMessage());
		}
