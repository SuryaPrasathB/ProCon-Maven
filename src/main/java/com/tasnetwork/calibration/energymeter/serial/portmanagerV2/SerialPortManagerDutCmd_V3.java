...
public class SerialPortManagerDutCmd_V3 {
	...
	public SerialRxPhysical_V2 rxPhysical_Dut = new SerialRxPhysical_V2(commDut,rxMsgQ_Dut,false,"DutV2",DeleteMeConstant.HV_ER_TERMINATOR,"");
	//public SerialRxPhysical rxPhysical_Dut = new SerialRxPhysical(commPowerSrc,rxMsgQ_Dut,false,"Dut",DeleteMeConstant.HV_ER_TERMINATOR,"");
...
/*		commLDU = new Communicator(ConstantApp.SERIAL_PORT_LDU);
		...
		rxMsgQ_Dut = new SerialRxMessageQ_V2(commDut);
		rxPhysical_Dut = new SerialRxPhysical_V2(commDut,rxMsgQ_Dut,false,portInternalAliasId,ConstantDut.ER_TERMINATOR,"");
/*		if(scannedSerialPortMap.isEmpty()){
	...
	public void disconnectDut(){
		...
		if(isDutSerialStatusConnected()){
			//ApplicationLauncher.logger.debug("disconnectDut :DutSerialStatusConnected:" + isDutSerialStatusConnected());
	...
	/*public void disconnectDut(int dutPositionNo){
		ApplicationLauncher.logger.debug("disconnectDut V2 :Entry");
		if(isDutSerialStatusConnected()){
			ApplicationLauncher.logger.debug("disconnectDut :DutSerialStatusConnected:" + isDutSerialStatusConnected() + " -> position: " +dutPositionNo);
			disableSerialRxPhysical_DutMonitor();
			//DisplayDataObj.setDutReadDataFlag(false);  
			Sleep(10);
			disconnectDutSerialComm();
	...
	public DutResponse dutComInitV2(String InputComm,String BaudRate) {
		...
		try {
					...
					//ApplicationLauncher.logger.debug("setDutSerialStatusConnected :Hit3:" +dutResponse.getStatus()+  " , Comm -"+ InputComm);
	...
	public DutResponse setSerialCommV2(CommunicatorV2 SerialPortObj, String SerialPort_ID, Integer BaudRate,Boolean ReadHexFormat){
	...
	public void dutSendCommand(String payLoadInHex,int timeDelayInMilliSec){
		...
		//`ApplicationLauncher.logger.debug("dutSendCommand V2: payLoadInHex: " + payLoadInHex);
...
/*				String eachDataInHex = "";
				...
				//ApplicationLauncher.logger.debug("writeStringToDut : asciiToHex: "  + GuiUtils.asciiToHex(myStr));
				
				
				
				
				//ApplicationLauncher.logger.debug("writeStringToDut : StringToHex: "  + GuiUtils.StringToHex(myStr));
				//ApplicationLauncher.logger.debug("writeStringToDut : StringToHex: "  + GuiUtils.StringToHex(myStr));
...
/*				String eachDataInHex = "";
				...
				//ApplicationLauncher.logger.debug("writeStringToDutV2 : threadTimeStamp: "  + threadTimeStamp);
	...
	public void dutSetExpectedData(String ExpectedResponse){
		...
		//ApplicationLauncher.logger.debug("dutSetExpectedDataV2 :commDut.getDeviceType(): " +commDut.getDeviceType());
		//ApplicationLauncher.logger.debug("dutSetExpectedDataV2 :getExpectedResult: " +rxMsgQ_Dut.getSerialPortObj().getExpectedResult());
		
		//SerialPortObj.setExpectedError1Result(ExpectedError1Data);
		//SerialPortObj.setExpectedError2Result(ExpectedError2Data);
		//ApplicationLauncher.logger.debug("dutSetExpectedDataV2: setExpectedResult:"+SerialPortObj.getExpectedResult());
		//ApplicationLauncher.logger.debug("dutSetExpectedDataV2: setExpectedLength:"+SerialPortObj.getExpectedLength());
	...
	public void dutSetExpectedError1Data(String ExpectedError1Response){
		...
		//SerialPortObj.setExpectedError1Result(ExpectedError1Data);
		//SerialPortObj.setExpectedError2Result(ExpectedError2Data);
		//ApplicationLauncher.logger.debug("dutSetExpectedError1Data: getExpectedError1Result:"+SerialPortObj.getExpectedError1Result());
	...
	public void dutSetExpectedError2Data(String ExpectedError2Response){
		...
		//SerialPortObj.setExpectedError1Result(ExpectedError1Data);
		//SerialPortObj.setExpectedError2Result(ExpectedError2Data);
		//ApplicationLauncher.logger.debug("dutSetExpectedError2Data: getExpectedError2Result:"+SerialPortObj.getExpectedError2Result());
		//ApplicationLauncher.logger.debug("HVCI_SetExpectedError1Data: setExpectedLength:"+SerialPortObj.getExpectedLength());
	...
	public Map<String,Object> dutSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
		...
		ApplicationLauncher.logger.debug("dutSendCommandProcess :commDut.getExpectedResult(): " +rxMsgQ_Dut.getSerialPortObj().getExpectedResult());
		...
		if(isDutSerialStatusConnected()){
				...
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_Dut.IsResponseReceived()) {
						if(rxMsgQ_Dut.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("dutSendCommandProcess <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("dutSendCommandProcess <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_Dut.getLastReadMessage());
							...
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("dutSendCommandProcess :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("dutSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("dutSendCommandProcess <"+sourceThread +">: Ack No Data Received");
					}
					if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
						...
						if(ProjectExecutionController.getUserAbortedFlag()){
							...
							ApplicationLauncher.logger.debug("dutSendCommandProcess <"+sourceThread +">: user aborted - detected");
						}else if(!isDutSerialStatusConnected()){
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("dutSendCommandProcess <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("dutSendCommandProcess-X <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_Dut.getLastReadMessage()==null) {
			ApplicationLauncher.logger.debug("dutSendCommandProcess <"+sourceThread +">: last read null");
		}else {
			//ApplicationLauncher.logger.info("dutSendCommandProcess: last read:"+rxMsgQ_Dut.getLastReadMessage());
		}
	...
	public Map<String,Object> dutSendCommandProcessV2(String payLoadInHex, int timeDelayInMilliSec, String expectedDataInHex,
			boolean isResponseExpected, String rxMessageTerminatorInHex,String sourceThread){
		...
		if(isDutSerialStatusConnected()){
			...
			if(isResponseExpected){
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					ApplicationLauncher.logger.debug("dutSendCommandProcess V2: IsResponseReceived:" +rxMsgQ_Dut.IsResponseReceived() + " : " +sourceThread);
					ApplicationLauncher.logger.debug("dutSendCommandProcess V2: isExpectedResponseReceived:" + rxMsgQ_Dut.isExpectedResponseReceived() + " : " +sourceThread);
					if(rxMsgQ_Dut.IsResponseReceived()) {
						...
						if(rxMsgQ_Dut.isExpectedResponseReceived()){
							...
							ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_Dut.getLastReadMessage());
							...
							rxMsgQ_Dut.removeProcessedMsgFromQueue();
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("dutSendCommandProcess V2 :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("dutSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +">: Ack No Data Received");
					}
					if(parentManager.isUserAborted()) {
						//responseData = DeleteMeConstant.NO_RESPONSE;
						retryCount = 0;
						ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +">: user aborted ");
					}else {
						Sleep(200);
					}
...
/*					if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
						...
						if(ProjectExecutionController.getUserAbortedFlag()){
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +">: user aborted - detected");
						}else if(!isDutSerialStatusConnected()){
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("dutSendCommandProcess V2-Y <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_Dut.getLastReadMessage()==null) {
	...
	public void setDutSerialStatusConnected(boolean DutSerialStatus) {
		//ApplicationLauncher.logger.debug("setDutSerialStatusConnected:"+ DutSerialStatus);
