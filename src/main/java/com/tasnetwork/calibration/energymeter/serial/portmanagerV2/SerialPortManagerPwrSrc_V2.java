...
public class SerialPortManagerPwrSrc_V2 {
	...
	public SerialRxPhysical_V2 rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource, rxMsgQ_PwrSrc, false,
			"PwrSrcV2", DeleteMeConstant.HV_ER_TERMINATOR, "");
	...
	private void createObjects() {
		...
		rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource, rxMsgQ_PwrSrc, false, "PwrSrcV2",
	...
	public void disconnectPwrSrc() {
		...
		if (isPwrSrcSerialStatusConnected()) {
			ApplicationLauncher.logger
					.debug("disconnectPwrSrc :PwrSrcSerialStatusConnected:" + isPwrSrcSerialStatusConnected());
	...
	public Map<String, Object> powerSourceSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec,
			String expectedData,
				...
				while (retryCount > 0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					if (rxMsgQ_PwrSrc.IsResponseReceived()) {
						if (rxMsgQ_PwrSrc.isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug(
									"powerSourceSendCommandProcess V2 <" + sourceThread + ">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug(
									"powerSourceSendCommandProcess V2 <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = new String(rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if (responseData != null) { // Gopi2
								ApplicationLauncher.logger.debug("powerSourceSendCommandProcess V2 :Unexpected Message<"
										+ sourceThread + ">: dropping the message:" + responseData);
								// ApplicationLauncher.logger.debug("powerSourceSendCommandProcess :Unexpected
						...
						ApplicationLauncher.logger
								.debug("powerSourceSendCommandProcess V2 <" + sourceThread + ">: Ack No Data Received");
					}
					if ((!payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) &&
						...
						if (BayUtils.isUserAborted()) {
							...
							ApplicationLauncher.logger.debug(
									"powerSourceSendCommandProcess V2 <" + sourceThread + ">: user aborted - detected");
						} else if (!isPwrSrcSerialStatusConnected()) {
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("powerSourceSendCommandProcess V2 <" + sourceThread
		...
		if (rxMsgQ_PwrSrc.getLastReadMessage() == null) {
			...
			// ApplicationLauncher.logger.info("powerSourceSendCommandProcess: last
