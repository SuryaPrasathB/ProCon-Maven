...
public class DutCmdMessengerV3 {
	...
	public DutResponse dutMsngrSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, 
		...
		if(getDutCmdSpmObj().isDutSerialStatusConnected()){
			...
			if(isResponseExpected){
				while (expectedRetryCount>0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					ApplicationLauncher.logger.debug("dutMsngrSendCommandProcess :expectedRetryCount<"+sourceThread +">:" + expectedRetryCount);
		...
		if(getDutCmdSpmObj().getRxMsgQ_Dut().getLastReadMessage()==null) {
			...
			ApplicationLauncher.logger.info("dutMsngrSendCommandProcess: last read<"+sourceThread +">:"+getDutCmdSpmObj().getRxMsgQ_Dut().getLastReadMessage());
		}
	...
	public DutResponse dutMsngrSendCommandProcessV2(int dutPositionNo,DutCommand dutCommand,String sourceThread) {
		...
		if(dutWriteSerialNoToDutEnabled) {
			...
			ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : dutSerialNumberInAscii : <" + dutSerialNumberInAscii + "> : position id : " + dutPositionNo);
		}
		...
		if(dutWriteRtcToDutEnabled) {
	        ...
	        ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : formattedTime : <" + formattedTime + "> : position id : " + dutPositionNo);
	        if(rtcConvertTotHex) {
	        	 String formatedTimeInHex = bindTwoCharToHex(formattedTime);
			     ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : formatedTimeInHex : <" + formatedTimeInHex + "> : position id : " + dutPositionNo);
			     rtcInAscii = GuiUtils.hexToAsciiV2(formatedTimeInHex);
			     ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : rtcInAscii-binding : <" + rtcInAscii + "> : position id : " + dutPositionNo);
	        }else {
	        	 rtcInAscii = formattedTime;
			     ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : rtcInAscii-2 : <" + rtcInAscii + "> : position id : " + dutPositionNo);
	        }
	        
	        
		}
		
		String absoluteCommandInAscii = targetCommandInAscii + dutSerialNumberInAscii + rtcInAscii +commandTerminatorInAscii;
		String absoluteCommandInHex =  GuiUtils.asciiToHex(absoluteCommandInAscii);
		ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : absoluteCommandInHex : <" + absoluteCommandInHex + "> : position id : " + dutPositionNo);
        
		ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : absoluteCommandInAscii : " + absoluteCommandInAscii + " : position id : " + dutPositionNo);
		ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : expectedResponseInHex length: " + expectedResponseInHex.length()+ " : position id : " + dutPositionNo);
		...
		//ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : isDutSerialStatusConnected: "+ getDutCmdSpmObj().isDutSerialStatusConnected());
		...
		if(status){
			String responseData = (String)responseReturn.get("result");
			ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : responseData <"+sourceThread +">: " + responseData);
			//ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : responseData <"+sourceThread +">hex : " + GuiUtils.asciiToHex(responseData));
			//if(responseData.equals(GuiUtils.hexToAsciiV2(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))) {
			//if(responseData.equals(GuiUtils.hexToAsciiV2(ConstantPowerSourceBofa.ER_CMD_STOP_VOLT_CURRENT_POSITIVE_IN_HEX))) {
			/*if(responseData.equals(expectedData)) {
			...
			if(dutResponseMandatory){
				...
				//	expectedRetryCount--;
					//VI_SendStartCommand(CommandVI_PayLoad,SelectedPhase);
				//	ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 :expectedRetryCount<"+sourceThread +">:" + expectedRetryCount);
					//int TimeOutInSec= 60;vjhvjgv
					//SerialDataLDU lduData = VICI_ReadDataWithErrorResponse(ExpectedData.length(),ExpectedData,ExpectedError1Data,ExpectedError2Data,TimeOutInSec);
					//ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 :Hit2");
					if(getDutCmdSpmObj().getRxMsgQ_Dut().IsResponseReceived()) {
						...
						if(getDutCmdSpmObj().getRxMsgQ_Dut().isExpectedResponseReceived()){
							...
							ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2: <"+sourceThread +">:Ack Response Success");
							//String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							//ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: VICI Received Data1:"+CurrentLDU_Data);
							//StripLDU_SerialData(lduData.getReceivedLength());
							//responseStatus=DeleteMeConstant.SUCCESS_RESPONSE;
							...
							if(dutReadSerialNoFromDutEnabled) {
								...
								ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: Expected recieved: readSerialNoInAscii:<"+readSerialNoInAscii+ "> : position id : " + dutPositionNo);
							...
							if(dutReadRtcFromDutEnabled) {
								...
								readRtcInHex = readRtcInHex.replaceFirst(expectedResponseInHex, "");// replace starting header
								readRtcInHex = readRtcInHex.replaceAll(expectedResponseTerminatorInHex+"$","");///replace last terminator
								ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: Expected recieved: readRtcInHex-binding:<"+readRtcInHex+ "> : position id : " + dutPositionNo);
								
								String readRtcInAscii = "";
								if(rtcConvertTotHex) {
									readRtcInAscii = bindHexToTwoChar(readRtcInHex);
									ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: Expected recieved: readRtcInAscii-binding:<"+readRtcInAscii+ "> : position id : " + dutPositionNo);
									
								}else {
									readRtcInAscii = GuiUtils.hexToAsciiV2(readRtcInHex);
									ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: Expected recieved: readRtcInAscii-2:<"+readRtcInAscii+ "> : position id : " + dutPositionNo);
							...
							ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =getDutCmdSpmObj().getRxMsgQ_Dut().getLastReadMessage();
							//ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: ErrorResponse Received:"+CurrentLDU_Data);
							//ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: ExpectedError1Data:"+ExpectedError1Data);
							//StripLDU_SerialData(CurrentLDU_Data.length());
							...
							String CurrentReadData =getDutCmdSpmObj().getRxMsgQ_Dut().getLastReadMessage();
							ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : isUnknownResponseRecieved : CurrentReadData<"+sourceThread +">: "+ CurrentReadData);
							responseStatus=CurrentReadData;
							dutResponse.setResponseData(responseStatus);
							dutResponse.setStatus(false);
							getDutCmdSpmObj().getRxMsgQ_Dut().removeProcessedMsgFromQueue();
							if(responseStatus!=null) { //Gopi2
								ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 :Unexpected Message: dropping the message<"+sourceThread +">:"+responseStatus);
								ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 :Unexpected Message: dropping the message<"+sourceThread +">:"+GuiUtils.HexToString(responseStatus));
							}
						...
						dutResponse.setResponseData(responseStatus);
						dutResponse.setStatus(true);
						ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2 : <"+sourceThread +">: Ack No Data Received");
					}
					
					
					//ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : <"+sourceThread +">test1: ");
					...
					//	     ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_CURRENT_IN_HEX)) ){
							if(ProjectExecutionController.getUserAbortedFlag()){
								...
								dutResponse.setResponseData(responseStatus);
								dutResponse.setStatus(false);
								ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2 V2 <"+sourceThread +">: user aborted - detected");
							}
		...
		//ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 : expectedDataInHex: " + expectedDataInHex);
		/*String expectedData = "";
		if(isResponseExpected){
			expectedData = GuiUtils.hexToAsciiV2(expectedDataInHex);
		}*/
...
/*		String rxMessageTerminatorInHex = ConstantDut.ER_TERMINATOR;//GuiUtils.hexToAscii(ConstantPowerSourceBofa.END_BYTE);
		Map<String,Object> responseReturn = new HashMap<String,Object>();
		if(getDutCmdSpmObj().isDutSerialStatusConnected()){
			responseReturn = getDutCmdSpmObj().dutSendCommandProcessV2( payLoadInHex, timeDelayInMilliSec,
					expectedData,isResponseExpected, rxMessageTerminatorInHex, sourceThread);
			status = (boolean)responseReturn.get("status");
			if(status){
				String responseData = (String)responseReturn.get("result");
		...
		/*}else{
			ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessV2 V2 <"+sourceThread +">: serial port not connected or disconnected");
		...
		if(getDutCmdSpmObj().getRxMsgQ_Dut().getLastReadMessage()==null) {
			...
			ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: last read<"+sourceThread +">:"+getDutCmdSpmObj().getRxMsgQ_Dut().getLastReadMessage());
		}
		
		
		int timeOut = haltTimeInSec;// Integer.parseInt(haltTimeInSecStr);
		ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: timeOut Entry: "+ " : position id : " + dutPositionNo);		
		while((timeOut>0) && (!ProjectExecutionController.getUserAbortedFlag())) {
			ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: timeOut countDown: "+timeOut + " : position id : " + dutPositionNo);
			timeOut--;
			Sleep(1000);
		}
		ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: timeOut Exit: " + " : position id : " + dutPositionNo);
	...
	public String bindHexToTwoChar(String hexStr) {
	    ...
	    ApplicationLauncher.logger.info("dutMsngrSendCommandProcessV2: bindHexToTwoChar : dateTimeStr " + result.toString());
