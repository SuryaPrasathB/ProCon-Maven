...
public class ProcalFeatureEnable {
	...
	public static boolean CONVEYOR_REST_CLIENT_ENABLED = true;
	...
	public static boolean RESULT_STATUS_DISPLAY_ENABLE_FEATURE = false;// This is to display result status on the table
																		// and reports
	public static boolean WARMUP_NO_OF_PULSES_DISPLAY_ENABLE_FEATURE = false;// true;
	public static boolean REF_STD_PHASE_180_FLAG = false; // For radiant reference standard 30 degree lag will be
															// displayed as -30 degree. In case for other reference
															// standard if the display is 30 degree for the same input,
															// then this flag should be set to true

	public static boolean EMH_PWR_SRC_VOLT_UNBALANCE_ZEROVOLT_WORKAROUND = true;// For the HT CT meters for the
																				// unbalance voltage for Volt-R only,
																				// Volt-Y only and Volt-B only Test
																				// point failed with exceeding error
																				// limit.
	// For example for Volt-R only test point, Y phase volt and B phase volt were
	// set to 43 volt by the power source, hence built a logic to maintain other
	// phase voltage to 1 to 10 volt, which can be configured in config.json as
	...
	public static boolean REF_STD_CONST_CALCULATE = false;// true;//making this true will calculate the ref std const
															// value for Wh and for false the value will be read from
															// the config.json lookup
	public static boolean RACK_MCT_NCT_ENABLED = true;// feature to enable Main CT and Neutral CT feature

	public static boolean CREEP_KWH_READING_PROMPT_ENABLED = false;// for Creep-NoLoad Test , to Enable/Disable the user
																	// prompt for kWH reading from Meter DUT
	public static boolean STA_KWH_READING_PROMPT_ENABLED = false;// for Starting Current Test , to Enable/Disable the
																	// user prompt for kWH reading from Meter DUT
	...
	public static boolean REF_STD_DATA_DISPLAYED_IN_RUN_SCREEN_ENABLED = true;// to enable the reference standard
																				// instant metrics in Run Screen

	public static JSONObject TEST_PROFILE_ENABLE_FEATURE = new JSONObject();

	public static boolean REPORT_SUPPORTING_DATA_POPULATE_ENABLED = true; // feature to populate voltage and current ,
																			// user name , time stamp on the report
	...
	public static boolean SANDS_REFSTD_NEGATIVE_PF_DISPLAY_ISSUE_EXIST = true;// in sands some time PF values are
																				// displayed with negative -work around
																				// to fix the display
	...
	public static boolean MODBUS_PLC_SLAVE_MODE = true; // plc has no intelligence, it will enable/disable output based
														// on java command
