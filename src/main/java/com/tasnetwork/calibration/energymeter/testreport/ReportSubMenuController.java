...
public class ReportSubMenuController implements Initializable {
	...
	public void LoadAllChild_FXML() {
		...
		try {
			...
			reportheaderconfig_childpane.getChildren()
					.add(getNodeFromFXML("/fxml/testreport/ReportHeaderConfig" + ConstantApp.THEME_FXML));
			reportexcelconfig_childpane.getChildren()
					.add(getNodeFromFXML("/fxml/testreport/ReportExcelConfig" + ConstantApp.THEME_FXML));
			reportfilelocation_childpane.getChildren()
					.add(getNodeFromFXML("/fxml/testreport/ReportFileLocation" + ConstantApp.THEME_FXML));
		} catch (Exception e) {
			e.printStackTrace();
