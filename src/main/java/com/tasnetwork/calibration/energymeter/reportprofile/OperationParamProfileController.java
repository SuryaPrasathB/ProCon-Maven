...
public class OperationParamProfileController implements Initializable {
	...
	@FXML
	private ComboBox cmbBxOperationParamProfileName;
	@FXML
	private Button btnLoad;
	@FXML
	private Button btnAddParamProfile;
	@FXML
	...
	@FXML
	private TableView<OperationParam> tvOperationParamProfile;
	@FXML
	private TableColumn<OperationParam, String> columnSerialNo;
	@FXML
	private TableColumn columnParamType;
	@FXML
	private TableColumn columnParamKeyName;
	@FXML
	private TableColumn columnParamPopulateType;
	...
	private void loadTableViewProperty() {
		...
		ref_columnParamKeyName.setOnEditCommit(new EventHandler<CellEditEvent<OperationParam, String>>() {
			public void handle(CellEditEvent<OperationParam, String> t) {
				OperationParam rowData = ((OperationParam) t.getTableView().getItems()
	...
	private void loadDefaultData() {
		 ...
		 * debug("OperationParamProfileController: loadDefaultData: keyParamName1: " +
		 * keyParamName1);
		 * ApplicationLauncher.logger.
		 * debug("OperationParamProfileController: loadDefaultData: keyParamName2: " +
		 * keyParamName2);
		 */
	}
		...
		if (paramProfileName != null) {
			...
			if (!paramProfileName.isEmpty()) {
				...
				ApplicationLauncher.logger.debug(
						"btnSaveOnClick: paramProfileName is empty,Param Profile name is empty, kindly update and try again!!!  - prompted");
				// WindowManager.InformUser(ErrorCodeMapping.ERROR_CODE_6000,
				// ErrorCodeMapping.ERROR_CODE_6000_MSG,AlertType.ERROR);
				WindowManager.InformUser("Empty Param Profile Name",
						"Param Profile name is empty, kindly update and try again!!!", AlertType.ERROR);

			}
		} else {
			ApplicationLauncher.logger.debug(
					"btnSaveOnClick: Param Profile Name Null, Param Profile name is null, kindly update and try again!!! - prompted");
			// WindowManager.InformUser(ErrorCodeMapping.ERROR_CODE_6000,
			// ErrorCodeMapping.ERROR_CODE_6000_MSG,AlertType.ERROR);
			WindowManager.InformUser("Param Profile Name Null",
					"Param Profile name is null, kindly update and try again!!!", AlertType.ERROR);
		...
		if (ref_tvOperationParamProfile.getItems().size() > 0) {
			...
			if (paramProfileName != null) {
				...
				if (!paramProfileName.isEmpty()) {
					 ...
					 * ApplicationLauncher.logger.debug("btnSaveOnClick: Entry " +
					 * e.getTableSerialNo() + " -> key = <" + e.getKeyParam() +
					 * "> , getPopulateType -> " + e.getPopulateType() + " , paramType = " +
					 * e.getParamType() );
					 * if(e.getPopulateType().equals(ConstantReportV2.
					 * POPULATE_DATA_TYPE_ONLY_HEADERS)){
					 * 
					 * e.setPopulateOnlyHeaders(true);
					 * e.setPopulateAllDut(false);
					 * }else
					 * if(e.getPopulateType().equals(ConstantReportV2.POPULATE_DATA_TYPE_ALL_DUT)){
					...
					ArrayList<String> keyParamEmptySerialNoList = (ArrayList<String>) ref_tvOperationParamProfile
							.getItems().stream()
					...
					if (keyParamEmptySerialNoList.size() > 0) {
						...
						ApplicationLauncher.logger
								.debug("btnSaveOnClick: keyParamEmptySerialNoList: " + keyParamEmptySerialNoList);
						// WindowManager.InformUser(ErrorCodeMapping.ERROR_CODE_6000,
						// ErrorCodeMapping.ERROR_CODE_6000_MSG,AlertType.ERROR);
						WindowManager.InformUser("Empty KeyParam found",
								"Emtpy <Key Name> found in below serial number:\n\nSerial No list: "
										+ StringUtils.join(keyParamEmptySerialNoList, ","),
								AlertType.ERROR);

					} else {

						ArrayList<OperationParam> keyParamDataList = (ArrayList<OperationParam>) ref_tvOperationParamProfile
								.getItems().stream()
								// .map(e -> e.getKeyParam())
								.collect(Collectors.toList());
						// Map<String, List<Integer>> dataMap = IntStream.range(0,
						// keyParamDataList.size())
						// .boxed()
						...
						duplicateKeyNameList.stream()
								.forEach(e -> {
									ApplicationLauncher.logger.debug("btnSaveOnClick: dulicateKeyParamMap: "
											+ e.getTableSerialNo() + " -> " + e.getKeyParam());
						...
						if (duplicateKeyNameList.size() > 0) {
							...
							duplicateKeyNameListMap.keySet().stream().forEach(e -> {
								String data = "Duplicate Key Name: " + e + " , Serial No List: "
										+ (Collection<String>) duplicateKeyNameListMap.get(e);
							...
							ref_tvOperationParamProfile.getItems().stream().forEach(e -> {
								ApplicationLauncher.logger.debug("btnSaveOnClick: Entry " + e.getTableSerialNo()
										+ " -> key = <" + e.getKeyParam() + "> , getPopulateType -> "
										+ e.getPopulateType() + " , paramType = " + e.getParamType());
								if (e.getPopulateType().equals(ConstantReportV2.POPULATE_DATA_TYPE_ONLY_HEADERS)) {
									...
									e.setPopulateAllDut(true);
									// e.setResultTypeAverage(false);
								} else if (e.getPopulateType()
										.equals(ConstantReportV2.POPULATE_DATA_TYPE_ALL_DUT_AVERAGE)) {
									e.setPopulateOnlyHeaders(false);
									e.setPopulateAllDut(true);
							...
							//
							WindowManager.InformUser("Success", "Saved to Database", AlertType.INFORMATION);
						...
						// ArrayList<OperationParam> groupByKeyParamDataList = (Map)
						// keyParamDataList.stream()
						// .collect(Collectors.groupingBy(OperationParam::getKeyParam ));
	...
	public void cmbBxOperationParamProfileNameOnChange() {
		...
		ref_tvOperationParamProfile.getItems().clear();
		// String customerId =
		// ConstantAppConfig.REPORT_PROFILE_DEFAULT_ACTIVE_CUSTOMER_ID;

		// List<OperationParam> operationParamDataList =
		// getReportOperationParamService().findByCustomerId(customerId);
	}
	...
	public void btnImportOnClick() {
		...
		FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("Json files", "*.json");// new
		...
		getOperationProcessDataModel().getOperationLocalInput().forEach(e -> {
			ApplicationLauncher.logger
					.info("btnImportOnClick: localInput : getOperationProcessKey: " + e.getOperationProcessKey());
		...
		getOperationProcessDataModel().getOperationLocalOutput().forEach(e -> {
			ApplicationLauncher.logger
					.info("btnImportOnClick: localOutput: getOperationProcessKey: " + e.getOperationProcessKey());
		...
		getOperationProcessDataModel().getOperationMasterOutput().forEach(e -> {
			ApplicationLauncher.logger
					.info("btnImportOnClick: masterOutput: getOperationProcessKey: " + e.getOperationProcessKey());
		...
		// ApplicationLauncher.logger.debug("btnImportOnClick: file_location: " +
		// file_location);
	...
	public void btnDeleteOnClick() {
		...
		if (selectedIndex != -1) {
			...
			try {
				...
				ApplicationLauncher.logger
						.debug("btnDeleteOnClick: item deleted from db: " + selectedOperationParam.getKeyParam());
				reOrderSerialNumberOperationParam();
				ref_tvOperationParamProfile.getItems().stream().forEach(e -> {
					ApplicationLauncher.logger.debug("btnDeleteOnClick: Entry " + e.getTableSerialNo() + " -> key = <"
							+ e.getKeyParam() + "> , getPopulateType -> " + e.getPopulateType() + " , paramType = "
							+ e.getParamType());
					if (e.getPopulateType().equals(ConstantReportV2.POPULATE_DATA_TYPE_ONLY_HEADERS)) {
	...
	private void reOrderSerialNumberOperationParam() {
		...
		for (int i = 0; i < ref_tvOperationParamProfile.getItems().size(); i++) {
			...
			ref_tvOperationParamProfile.getItems().get(i)
					.setTableSerialNo(String.valueOf(getSerialNo().incrementAndGet()));
			// ApplicationLauncher.logger.debug("reOrderSerialNumberTestFilter :
			// serialNumber2: " +
			// ref_tvOperationParamProfile.getItems().get(i).getTableSerialNo());
			// serialNumber++;
		}
