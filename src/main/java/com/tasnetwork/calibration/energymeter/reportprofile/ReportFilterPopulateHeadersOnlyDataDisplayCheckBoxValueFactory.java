...
public class ReportFilterPopulateHeadersOnlyDataDisplayCheckBoxValueFactory implements Callback<TableColumn.CellDataFeatures<ReportMeterMetaDataTypeSubModel, CheckBox>, ObservableValue<CheckBox>> {
	...
	public ObservableValue<CheckBox> call(CellDataFeatures<ReportMeterMetaDataTypeSubModel, CheckBox> param) {
		...
		// setGraphic(checkBox);
		checkBox.selectedProperty().setValue(rowData.isPopulateOnlyHeader());
		checkBox.selectedProperty().addListener((ov, old_val, new_val) -> {
			rowData.setPopulateOnlyHeader(new_val);
		});
