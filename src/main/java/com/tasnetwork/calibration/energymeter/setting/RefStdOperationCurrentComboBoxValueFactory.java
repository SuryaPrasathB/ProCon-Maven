...
public class RefStdOperationCurrentComboBoxValueFactory implements Callback<TableColumn.CellDataFeatures<RefStdPulseConstantModel, ComboBox<String>>, ObservableValue<ComboBox<String>>> {
	    ...
	    public ObservableValue<ComboBox<String>> call(CellDataFeatures<RefStdPulseConstantModel, ComboBox<String>> param) {
			...
			operationtype.addAll(ConstantApp.REF_STD_OPERATION_CURRENT_LIST);//(ConstantApp.TESTPOINT_RUNTYPE_PULSEBASED);
/*			if(ProcalFeatureEnable.RUN_TYPE_TIME_BASED_ENABLED){
				testruntype.add(ConstantApp.TESTPOINT_RUNTYPE_TIMEBASED);
			}*/
			...
			Combo_Box.setOnAction((e) -> {
	             ApplicationLauncher.logger.info("RefStdOperationCurrentComboBoxValueFactory: "+ Combo_Box.getSelectionModel().getSelectedItem());
