...
/*	public static void ScanDeviceButtonStatus(Boolean value){
	...
	static class getValidateCredResponseSuccessTask extends TimerTask {
		...
		public void run() {
			...
			ScanDeviceCompletedPostProcess();
			if(ConstantCustomer.getCurrentCustomer().getTargetDevice().equals(ConstantProTamp.getCurrentConnectedDeviceName())){
				ConstantProTamp.ScanDeviceFound = true;
			}
			ApplicationLauncher.logger.info("getValidateCredResponseSuccessTask: Device "+ CurrentResponseData.getDevice_name()+" found");
			
			if (ScanDeviceClickedByUser){
				ScanDeviceClickedByUser = false;
				if(ConstantProTamp.ScanDeviceFound ){
					ApplicationLauncher.InformUser("Success","Device "+ CurrentResponseData.getDevice_name()+" found",AlertType.INFORMATION);
				}else{
					ApplicationLauncher.logger.info("getValidateCredResponseSuccessTask: Error 603: Device "+ CurrentResponseData.getDevice_name()+" found, but not matching with configuration. Kindly contact the vendor");
					ApplicationLauncher.InformUser("Error 603","Error 603: Device "+ CurrentResponseData.getDevice_name()+" found, but not matching with configuration. Kindly contact the vendor",AlertType.INFORMATION);
	...
	static class getValidateCredResponseSlaveCommFailedTask extends TimerTask {
		...
		public void run() {
			...
			ApplicationLauncher.logger.info("getValidateCredResponseSlaveCommFailedTask: Error 402: Device "+ CurrentResponseData.getDevice_name()+" found with error. Slave communication failed");
			
			if (ScanDeviceClickedByUser){
				ScanDeviceClickedByUser = false;
				ApplicationLauncher.InformUser("Error 402","Error 402: Device "+ CurrentResponseData.getDevice_name()+" found with error. Slave communication failed",AlertType.ERROR);
