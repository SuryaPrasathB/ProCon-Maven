...
public class BayDeviceConfigQrPalletComTypeComboBoxValueFactory implements Callback<TableColumn.CellDataFeatures<BayDeviceConfig, ComboBox<String>>, ObservableValue<ComboBox<String>>> {
	    ...
	    public ObservableValue<ComboBox<String>> call(CellDataFeatures<BayDeviceConfig, ComboBox<String>> param) {
			...
			Combo_Box.setValue(rowData.getQrPalletComType());

			Combo_Box.setOnAction((e) -> {
	             ApplicationLauncher.logger.info(Combo_Box.getSelectionModel().getSelectedItem());
	             rowData.setQrPalletComType(Combo_Box.getSelectionModel().getSelectedItem());
	        });
