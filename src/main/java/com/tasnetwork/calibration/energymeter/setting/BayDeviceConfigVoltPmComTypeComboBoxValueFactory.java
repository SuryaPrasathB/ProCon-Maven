...
public class BayDeviceConfigVoltPmComTypeComboBoxValueFactory implements Callback<TableColumn.CellDataFeatures<BayDeviceConfig, ComboBox<String>>, ObservableValue<ComboBox<String>>> {
	    ...
	    public ObservableValue<ComboBox<String>> call(CellDataFeatures<BayDeviceConfig, ComboBox<String>> param) {
			...
			Combo_Box.setValue(rowData.getVoltPmComType());

			Combo_Box.setOnAction((e) -> {
	             ApplicationLauncher.logger.info(Combo_Box.getSelectionModel().getSelectedItem());
	             rowData.setVoltPmComType(Combo_Box.getSelectionModel().getSelectedItem());
	        });
