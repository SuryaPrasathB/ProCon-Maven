...
public class TestParamSetupController implements Initializable {
	...
	public void SaveOnClick() {
		...
		if (titledPaneTimerTestPoint.isVisible()) {
			...
			TimerTP_OffTimeInSec = (Integer.parseInt(cmbBxTimerTP_OffTimeMin.getSelectionModel().getSelectedItem())
					* 60) +
		...
		if (!RefStandardConstantStr.isEmpty()) {
			if (GUIUtils.isNumber(RefStandardConstantStr)) {
				...
				ApplicationLauncher.logger.info(
						"TestParamSetupController: Enter valid number on Reference Standard : No of impulses/unit : Prompted");
				WindowManager.InformUser("Save failed",
						"Enter valid number on Reference Standard : No of impulses/unit", AlertType.ERROR);
			}
		} else {
			ApplicationLauncher.logger.info(
					"TestParamSetupController: Enter the value for Reference Standard : No of impulses/unit : Prompted");
			WindowManager.InformUser("Save failed", "Enter the value for Reference Standard : No of impulses/unit",
					AlertType.ERROR);
		}
