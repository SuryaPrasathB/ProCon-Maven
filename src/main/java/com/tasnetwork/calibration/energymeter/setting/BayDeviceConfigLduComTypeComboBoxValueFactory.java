...
public class BayDeviceConfigLduComTypeComboBoxValueFactory implements Callback<TableColumn.CellDataFeatures<BayDeviceConfig, ComboBox<String>>, ObservableValue<ComboBox<String>>> {
	    ...
	    public ObservableValue<ComboBox<String>> call(CellDataFeatures<BayDeviceConfig, ComboBox<String>> param) {
			...
			Combo_Box.setValue(rowData.getLduComType());

			Combo_Box.setOnAction((e) -> {
	             ApplicationLauncher.logger.info(Combo_Box.getSelectionModel().getSelectedItem());
	             rowData.setLduComType(Combo_Box.getSelectionModel().getSelectedItem());
	        });
