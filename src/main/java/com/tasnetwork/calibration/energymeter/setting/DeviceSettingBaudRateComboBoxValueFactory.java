...
public class DeviceSettingBaudRateComboBoxValueFactory implements Callback<TableColumn.CellDataFeatures<DeviceSetting, ComboBox<String>>, ObservableValue<ComboBox<String>>> {
	    ...
	    public ObservableValue<ComboBox<String>> call(CellDataFeatures<DeviceSetting, ComboBox<String>> param) {
			...
			Combo_Box.getItems().addAll(ConstantApp.BaudRateConstantString);
			Combo_Box.getSelectionModel().select(0);
			Combo_Box.setValue(rowData.getBaudRate());

			Combo_Box.setOnAction((e) -> {
	             //ApplicationLauncher.logger.info(Combo_Box.getSelectionModel().getSelectedItem());
	             rowData.setBaudRate(Combo_Box.getSelectionModel().getSelectedItem());
	        });
