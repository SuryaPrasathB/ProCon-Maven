...
/*		int dutPositionNo =1;
		//
		DutCmdManager dutCmdManager = new DutCmdManager();//DeviceDataManagerController.getDutCmdManagerV3();
		...
		if(dutResponse.isStatus()) {
			...
			ApplicationLauncher.logger.debug("dutCommandTesting: getStatus: " + dutResponse.getStatus());
			
			ApplicationLauncher.logger.debug("dutCommandTesting: getResponseData: " + dutResponse.getResponseData());
			String dutResponseDataInHex = GuiUtils.asciiToHex(dutResponse.getResponseData());
			ApplicationLauncher.logger.debug("dutCommandTesting: dutResponseDataInHex: " + dutResponseDataInHex);
			dutCmdManager.dutCmdDisconnectPort_V2(dutPositionNo);
		}else {
			ApplicationLauncher.logger.debug("dutCommandTesting: Access failed : getStatus: " + dutResponse.getStatus());
			
			ApplicationLauncher.logger.debug("dutCommandTesting: Access failed : getResponseData: " + dutResponse.getResponseData());
...
/*		String bayKey = ConstantConveyor.VERIFICATION_BAY_KEY;
		...
		//String palletDistinctId = "20250720T160639_0079_LSCSM06P023";//"20250719T210233_0122_LSCSM06P020";
...
/*		String inpDistinctId = "20250721T213454_0130_LSCSM06P023";
		Optional<PalletManage> myPalletManageOptional =  MySqlServiceManager.getPalletManageService().findByPalletDistinctId(inpDistinctId);
    	...
    	try {
			testReportConveyorController.exportPalletMeterResult(myPalletManageOptional.get());
		} catch (IOException e) {
...
/*        if(myPalletManageOptional.isPresent()){
        	...
        	Unloading.logger.info("S05_qR_Code_Scanning_of_Pallet : found result myPalletManage : " + myPalletManage.getPalletDistinctId());
        	//BayUtils bayUtils = new BayUtils();
    		...
    		if (metersData != null && !metersData.isEmpty()) {
    			...
    			Unloading.logger.debug("S05_qR_Code_Scanning_of_Pallet: No meter data found for batch update for bay: " );//+ getMyBayKey());
    		}
        }else {
        	Unloading.logger.info("S05_qR_Code_Scanning_of_Pallet : result not found for Pallet : " + palletQrData);
        }*/
...
//		try {
			...
			for(int i=0; i<testcaselist.length(); i++){
				...
				ApplicationLauncher.logger.debug("assertPrintTestTimePeriod: readTestCaseName: " + readTestCaseName);
				if(readTestCaseName.startsWith(splittedData[0])){
					if(readTestCaseName.endsWith(splittedData[1])){
						ApplicationLauncher.logger.debug("assertPrintTestTimePeriod: readTestCaseName matching ");
						...
						if(!readData.isEmpty()){
							...
							if(keyParam.equals("test_period_in_sec")){
								value = value/60;
								ApplicationLauncher.logger.debug("assertPrintTestTimePeriod: test_period_in_sec: value: " + String.valueOf(value));
								return String.valueOf(value);
							}else if(keyParam.equals("warmup_period_in_sec")){
								value = value/60;
								ApplicationLauncher.logger.debug("assertPrintTestTimePeriod: warmup_period_in_sec: value: " + String.valueOf(value));
								return String.valueOf(value);
							}
							ApplicationLauncher.logger.debug("assertPrintTestTimePeriod: value: " + String.valueOf(value));
							return String.valueOf(value);
						}
					}
...
/*				if(getDeployedEM_ModelType().contains(ConstantApp.METERTYPE_ACTIVE)){
	...
	public static void timeBasedAssert(){
		...
		String AverageCycle = DisplayDataObj.manipulateNoOfPulsesForTimeBased(meterConstantInImpulsesPerKiloWattHour,timeDurationInSec,totalTargetPowerInKiloWatt); 
...
//		try{
//			int calibSize = DeviceDataManagerController.getLscsCalibrationConfigParsedKey().getVoltageCalibration().size();
...
//		String responseR_Phase = "52000000003F8FA15E000000000000000000000000000000004247D808C8";
//		String responseY_Phase = "59000000003D1F34AE80000000000000008000000000000000424923A76E";
//		String responseB_Phase = "42000000003D2339A9800000000000000080000000000000004248320DF9";
...
//		
//		//String responseTotalPowerAndAngle = "54370CC51EB6E871493AA350AFBBDCA8F24387316342B400004387000043B09F7F4279E57D4256B83607";
///*		String responseTotalPowerAndAngle =   "5439AFF665B6A90C923C211D9CBD0BCB934387FA5A42B4000042B4000043B1A46A426D0B43435A769E6D";
//		String rDegree = mSandsRefStd.ParseDegreeR_PhaseDatafromRefStd(responseTotalPowerAndAngle);
//		ApplicationLauncher.logger.debug("assertSandsRefStdResponse: rDegree: "+ rDegree);
//		
//		String yDegree = mSandsRefStd.ParseDegreeY_PhaseDatafromRefStd(responseTotalPowerAndAngle);
//		ApplicationLauncher.logger.debug("assertSandsRefStdResponse: yDegree: "+ yDegree);
//		
//		String bDegree = mSandsRefStd.ParseDegreeB_PhaseDatafromRefStd(responseTotalPowerAndAngle);
//		ApplicationLauncher.logger.debug("assertSandsRefStdResponse: bDegree: "+ bDegree);*/
