...
public class ParallelTaskManager {
	...
	public DutControlProcess startWriteMeterSerialNoToDutProcess(int dutAddress){
		...
		dutprocess.setDutProcessExecutionStarted(true);
		Timer processExecutionTimer = new Timer();
		processExecutionTimer.schedule(new ProcessFtWriteMeterSerialNoToDutExecuteTimerTask(dutprocess,dutAddress), 50);
		Sleep(100);
		//dutprocess.executeDutControlProcess(dutAddress);
		addDutControlProcessList(dutprocess);
		processExecutionTimer.cancel();
	...
	public DutControlProcess startWriteMeterHardwareIdNoToDutProcess(int dutAddress){
		...
		dutprocess.setDutProcessExecutionStarted(true);
		Timer processExecutionTimer = new Timer();
		processExecutionTimer.schedule(new ProcessFtWriteMeterHardwareIdNoToDutExecuteTimerTask(dutprocess,dutAddress), 50);
	...
	class monitorDutControlProcess extends TimerTask{
		...
		public void run() {
			...
			if(!isDutAllControlProcessCompleted()){
				...
				ApplicationLauncher.logger.debug("monitorDutControlProcess: monitorDutControlProcessTimer cancelling....");
				//Sleep(20000);
	...
	public void monitorDutControlProcessTask(){
		...
		if(!ProjectExecutionController.getUserAbortedFlag()){
			ApplicationLauncher.logger.debug("monitorDutControlProcessTask : getDutControlProcessList().size() : " +getDutControlProcessList().size());
			...
			ApplicationLauncher.logger.debug("monitorDutControlProcessTask : noOfDutProcessCompleted : " + noOfDutProcessCompleted );
...
/*				DeviceDataManagerController.setRefStdLogResultsEnabled(false);
	...
	public void validateMeterIdProcess() {
		...
		if((isBlackListedMeterIdFoundOverAllStatus()) || (isMeterIdAlreadyCalibratedFoundOverAllStatus()) || 
			...
			if(isBlackListedMeterIdFoundOverAllStatus()) {
				...
				ApplicationLauncher.logger.info("validateMeterIdProcess: invoking exit process due to black listed meter id found");
				ApplicationLauncher.logger.debug("validateMeterIdProcess: ERROR_CODE_5001 :" + ConvErrorCodeMapping.ERROR_CODE_5001_MSG + "\n\nMeter Id: " + meterIdList + " : Prompted");
				ApplicationLauncher.InformUser(ConvErrorCodeMapping.ERROR_CODE_5001, ConvErrorCodeMapping.ERROR_CODE_5001_MSG + "\n\nMeter Id: " + meterIdList,AlertType.ERROR);
			...
			if(isMeterIdAlreadyCalibratedFoundOverAllStatus()) {
				...
				ApplicationLauncher.logger.info("validateMeterIdProcess: invoking exit process due to meter id found already calibrated");
				ApplicationLauncher.logger.debug("validateMeterIdProcess: ERROR_CODE_5002 :" + ConvErrorCodeMapping.ERROR_CODE_5002_MSG + "\n\nMeter Id: " + meterIdList + " : Prompted");
				ApplicationLauncher.InformUser(ConvErrorCodeMapping.ERROR_CODE_5002, ConvErrorCodeMapping.ERROR_CODE_5002_MSG + "\n\nMeter Id: " + meterIdList,AlertType.ERROR);
			...
			if(isEmptyMeterIdFoundOverAllStatus()) {
				...
				ApplicationLauncher.logger.info("validateMeterIdProcess: invoking exit process due to meter id found empty");
				ApplicationLauncher.logger.debug("validateMeterIdProcess: ERROR_CODE_5003 :" + ConvErrorCodeMapping.ERROR_CODE_5003_MSG + "\n\nMeter Id: " + meterIdList + " : Prompted");
				ApplicationLauncher.InformUser(ConvErrorCodeMapping.ERROR_CODE_5003, ConvErrorCodeMapping.ERROR_CODE_5003_MSG + "\n\nMeter Id: " + meterIdList,AlertType.ERROR);
	...
	public void validateForBlackListedMeterId(){
		...
		//if(ProjectExecutionController.getCurrentTestPointName().contains(ConstantCalibration.CALIB_READ_METER_ID)){
			for(int i = 0; i < getDutControlProcessList().size() ; i++){
				...
				boolean blackListedMeterIdFound =isMeterIdExistInBlackList(getDutControlProcessList().get(i).getDutSerialNumber());
	...
	public void validateForAlreadyCalibratedMeterId(){
		ApplicationLauncher.logger.debug("validateForAlreadyCalibratedMeterId :Entry");
			...
			for(int i = 0; i < getDutControlProcessList().size() ; i++){
				boolean meterIdAlreadyCalibrated = isMeterIdExistInAlreadyCalibratedList(getDutControlProcessList().get(i).getDutSerialNumber());
...
/*		if(ConstantConfig.METER_ID_BLACKLIST_VALIDATION){
	...
	public boolean isMeterIdExistInAlreadyCalibratedList(String readMeterId){
		boolean status = false;
/*		ApplicationLauncher.logger.debug("isMeterIdExistInAlreadyCalibratedList: Entry"  );
		if(ConstantConfig.METER_ID_VALIDATE_ALREADY_CALIBRATED){
				status = MySQL_Controller.sp_ValidateDutAlreadyCalibrated( ProjectExecutionController.getSelectedDeployment_ID(), readMeterId, ConstantReport.RESULT_DATA_TYPE_ERROR_VALUE);
				if (status){
					ApplicationLauncher.logger.debug("isMeterIdExistInAlreadyCalibratedList: meter id found in calibrated list: " + readMeterId  );
					return status;
				}
		}*/
...
/*				MySQL_Controller.sp_add_calibration_resultV2(dbWriteData.getCurrentProjectName(), 
						...
						dbWriteData.getValidationType2ResultStatus(),dbWriteData.getValidationType2ResultData(),
						dbWriteData.getOverAllResultStatus(),dbWriteData.getErrorMin2(), 
						dbWriteData.getErrorMax2(),dbWriteData.getAverageType(),dbWriteData.getAverageCount());*/
			}else{
				...
				/*MySQL_Controller.sp_add_resultV2(dbWriteData.getCurrentProjectName(), 
						dbWriteData.getCurrentTestPointName(), dbWriteData.getCurrentTestAliasID(),
						dbWriteData.getRackId(),dbWriteData.getResultStatus(),dbWriteData.getErrorCount(),
						dbWriteData.getErrorValue(),dbWriteData.getFailureReason(),dbWriteData.getDataType(),
						dbWriteData.getExecutionMctNctMode(),dbWriteData.getEnergyFlowMode(),
						dbWriteData.getSelectedDeployment_ID(),dbWriteData.getSequenceNumber(),
...
/*	public static void updateRefStdResult(){
		...
		for(int i = 0; i < getDutControlProcessList().size() ; i++){
			if(getDutControlProcessList().get(i).isDutProcessExecutionCompleted()){
				ApplicationLauncher.logger.debug("updateRefStdResult : Index : " + i);
				//getDutControlProcessList().get(i).setRefStdResult(DisplayDataObj.getRefStdResult());
			}
		}
