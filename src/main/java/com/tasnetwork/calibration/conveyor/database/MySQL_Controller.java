...
public class MySQL_Controller {
	...
	public static boolean DB_ltcreateSrcDevice(String SrcType,String ModelName,String MeterType,String SerialNo,String classValue,String Asset_ID,String DeviceActive, String UpdatedBy ) {
		...
		try {
			...
			ex.printStackTrace();
			ApplicationLauncher.logger.error("DB_ltcreateSrcDevice: Exception: " + ex.getMessage());
			return false;
	...
	public static JSONObject sp_get_completed_result_data(String filterDataType1,String deploymentID) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				result_data = SQLConnect.sp_get_completed_result_data(filterDataType1, deploymentID);
			...
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_project_components( project_name,
						test_case_name, test_type, test_alias_id, test_position_id,
						 inf_emin,  inf_emax,
						 rPhaseSelectedLoad,  rPhaseTotalLoad,
						 yPhaseSelectedLoad,  yPhaseTotalLoad,
						 bPhaseSelectedLoad,  bPhaseTotalLoad
	...
	public static JSONObject sp_ltgetproject_componentsV2(String projectname, String aliasid) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				test_details_json = SQLConnect.sp_ltgetproject_componentsV2(projectname,  aliasid);


			}
		} catch (Exception ex) {
			ex.printStackTrace();
			ApplicationLauncher.logger.error ("sp_ltgetproject_componentsV2: Exception:"+ex.getMessage());
			return test_details_json;
	...
	public static boolean sp_add_testparam_config (int OccuranceTimeInSec, int RestorationTimeInSec, int TimerTP_OnTimeInSec, 
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_pt_add_testparam_config( OccuranceTimeInSec,  RestorationTimeInSec,  TimerTP_OnTimeInSec, 
        				 TimerTP_OffTimeInSec,  TimerTP_NoOfCycle, RefStandardConstant,  RefStdNoOfPulses, 
        				 TP_PulsatingDC_Freq ) ;


				if (SP_Response){
					ApplicationLauncher.logger.info ("sp_add_server_settings: Server Settings create: Success");
					return true;
				} else {
					ApplicationLauncher.logger.info ("sp_add_server_settings: Server Settings create: Failure");
					return false;
				} 
			}
		} catch (Exception ex) {	  
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_add_server_settings : Exception:"+ex.getMessage());
			return false;
	...
	public static boolean sp_add_server_settings (String serverHttpProtocol,String serverIP,String serverPort, int refreshGUI_Frequency
			) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_pt_add_server_settings(serverHttpProtocol, serverIP, serverPort,  refreshGUI_Frequency) ;
	...
	public static boolean sp_add_device_settings (int id, String device_type, String model_name, String port_name, String baud_rate, String qrPortName) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_device_settings( id,  device_type,  model_name,  port_name,  baud_rate,qrPortName) ;
	...
	public static boolean sp_add_device_settings_v2 ( String device_type, String model_name, String port_name, String baud_rate, String clusterName,
			String bayName,
			String positionNo,	String qrPortName,String deviceId) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_device_settings_v2(   device_type,  model_name,  port_name,  baud_rate, clusterName,
						 bayName,
						 positionNo,	qrPortName, deviceId) ;


				if (SP_Response){
					ApplicationLauncher.logger.info ("sp_add_device_settings_v2 Settings create: Success");
					return true;
				} else {
					ApplicationLauncher.logger.info ("sp_add_device_settings_v2 Settings create: Failure");
					return false;
				} 
			}
		} catch (Exception ex) {	  
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_add_device_settings_v2 : Exception:"+ex.getMessage());
			return false;
	...
	public static JSONObject sp_get_testparam_config() {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				testparam_config = SQLConnect.sp_pt_get_testparam_config();
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_get_testparam_config : Exception:"+ex.getMessage());
	...
	public static JSONObject sp_get_server_setting() {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				device_setting = SQLConnect.sp_pt_get_server_setting();
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_get_server_setting : Exception:"+ex.getMessage());
	...
	public static JSONObject sp_getdevice_setting_v2(String deviceId) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				device_setting = SQLConnect.sp_getdevice_setting_v2(deviceId);
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_getdevice_setting_v2 : Exception:"+ex.getMessage());
			return device_setting;
	...
	public static JSONObject sp_get_cname_port_setting(String device_type) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				device_setting = SQLConnect.sp_get_qr_port_setting(device_type);
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_get_qr_port_setting : Exception:"+ex.getMessage());
			return device_setting;
	...
	public static boolean sp_add_result (String project_name, String TestPointID, String test_case_name,
			String alias_id, String rack_id,
			String test_result, int error_id, String error_value,
			String FailureReason,String data_type) {
		...
		try {
			...
			boolean SP_Response = false;
			SP_Response = SQLConnect.sp_ltadd_result( project_name, TestPointID, test_case_name, alias_id,rack_id,  
					test_result, error_id, error_value,FailureReason,data_type) ;
	...
	/*public static boolean sp_add_deploy_test_cases (String project_name, String test_case, String test_type, String alias_id, String sequence_no, String is_deployed, String testPointID) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_deploy_test_cases( project_name, test_case, test_type, alias_id, sequence_no, is_deployed,testPointID) ;
...
*/
	...
	public static boolean sp_add_deploy_test_cases (String lastUpdatedDeploymentID, String project_name, String test_case, String test_type, String alias_id, String sequence_no,String testPointId, String is_deployed) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_deploy_test_cases( lastUpdatedDeploymentID, project_name, test_case, test_type, alias_id, sequence_no, testPointId, is_deployed) ;
	...
	public static boolean sp_add_deploy_devices (String project_name,
			String device, int rack_id, 
			int ctr_ratio, int ptr_ratio,
			int meter_const,
			String is_deployed) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_deploy_devices( project_name,
						device, rack_id, ctr_ratio,
						ptr_ratio, meter_const, is_deployed) ;
...
/*	public static JSONObject sp_getdeploy_devices(String project_name) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				resultjson =  SQLConnect.sp_ltgetdeploy_devices(project_name);			  		
	...
	public static boolean sp_add_summary_data (String project_name, String testcasename, 
			String testtype, String aliasid, int sequenceno,String testPoint_ID) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltadd_summary_data( project_name, testcasename,
						testtype, aliasid, sequenceno,testPoint_ID) ;
	...
	public static boolean sp_add_result_v1_1 (String project_name, String testPointId,String test_case_name,
			String alias_id, String rack_id,
			String test_result, int error_id, String error_value,
			String FailureReason,String data_type,
			String occuranceMode,String restorationMode,
			String deploymentId) {
		...
		try {
			...
			boolean SP_Response = false;
			SP_Response = SQLConnect.sp_ltadd_result_v1_1( project_name, testPointId, test_case_name, alias_id,rack_id,  
					test_result, error_id, error_value,FailureReason,data_type, occuranceMode, restorationMode,
					deploymentId) ;
			...
			if (SP_Response){
				...
				ApplicationLauncher.logger.info ("sp_add_result_v1_1: Failure");
				return false;
			} 

		} catch (Exception ex) {	  
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_add_result_v1_1 : Exception:"+ex.getMessage());
			return false;
...
/*				if (SP_Response){
	...
	public static boolean sp_add_users(String username, String password, String access_level, String created_by, 
			String date_created) {
		...
		try {
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_lt_add_users(username, 
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_add_users : Exception:"+ex.getMessage());
			return false;
	...
	public static JSONObject sp_get_users() {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				resultjson =  SQLConnect.sp_ltget_users();			  		
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_ltget_users : Exception:"+ex.getMessage());
			return resultjson;
	...
	public static JSONObject sp_get_user_access_level(String user_name, String password) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				resultjson =  SQLConnect.sp_ltget_user_access_level(user_name, password);			  		
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_get_user_access_level : Exception:"+ex.getMessage());
			return resultjson;
	...
	public static boolean sp_delete_users (String user_name) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltdelete_users( user_name) ;
			...
			ex.printStackTrace();	  
			ApplicationLauncher.logger.error ("sp_delete_users : Exception:"+ex.getMessage());
			return false;
	...
	public static boolean sp_add_report_header_config (String testtype, String headertype,
			String headervalue) {
		...
		try {
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_lt_add_report_header_config( testtype, 
	...
	public static JSONObject sp_getreport_header_config(String test_type) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				resultjson =  SQLConnect.sp_ltgetreport_header_config(test_type);			  		
	...
	public static boolean sp_delete_report_header_config(String test_type) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltdelete_report_header_config(test_type) ;
	...
	public static boolean sp_add_report_excel_config (String testtype, String celltype,
			String cellvalue) {
		...
		try {
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_lt_add_report_excel_config( testtype, 
						celltype, cellvalue) ;
	...
	public static JSONObject sp_getreport_excel_config(String test_type) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				resultjson =  SQLConnect.sp_ltgetreport_excel_config(test_type);			  		
	...
	public static boolean sp_delete_report_excel_config(String test_type) {
		...
		try {
			...
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_ltdelete_report_excel_config(test_type) ;
	...
	public static boolean sp_add_report_file_location (String test_type,String templ_file_loc, String save_file_loc) {
		...
		try {
			boolean SP_Response = false;
			if(SQLConnect.ConnectMySQL()){
				SP_Response = SQLConnect.sp_lt_add_report_file_location( test_type,templ_file_loc, save_file_loc) ;
	...
	public static JSONObject sp_getreport_file_location(String test_type) {
		...
		try {
			...
			if(SQLConnect.ConnectMySQL()){
				...
				resultjson =  SQLConnect.sp_ltgetreport_file_location( test_type);			  		
