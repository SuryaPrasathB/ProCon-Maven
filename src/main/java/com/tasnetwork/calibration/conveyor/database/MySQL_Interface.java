...
public class MySQL_Interface {
	 ...
	 */
	// private static final String mSQL_URL =
	// ConstantApp.DB_URL+ConstantConfig.DB_NAME;
	private static final String mSQL_URL = ConstantConveyorConfig.DB_URL + ConstantConveyorConfig.DB_NAME
			+ ConstantConveyorConfig.DB_URL_TAIL_OPTION;
	...
	public boolean ConnectMySQL() {
		...
		try {
			...
			if (isDbConnected()) {
				...
				String url1 = ConstantAppConfig.DB_URL + ConstantAppConfig.DB_NAME
	...
	public boolean sp_ltadd_result_v1_1(String project_name, String testPointId, String test_case_name,
		...
		try {
			...
			// sp_add_result(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			CallableStatement statement = ConnectManager
					.prepareCall("{call sp_add_result_V1_1(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			statement.setString(1, project_name);
	...
	public boolean sp_ltadd_project_components(String project_name, String test_case_name, String test_type,
		...
		try {
			...
			CallableStatement statement = ConnectManager
					.prepareCall("{call sp_add_project_components(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			statement.setString(1, project_name);
			statement.setString(2, test_case_name);
	...
	public boolean sp_ltgetproject_componentsSaveAs(String CurrentProjectName, String ToBeSavedProjectName) {
		...
		try {
			...
			// CallableStatement statement = ConnectManager.prepareCall("{call
			// sp_getproject_componentsSaveAs(?,?)}");
				...
				while (hadResults) {
					...
					while (resultSet.next()) {
						...
						switch (TestCaseType) {
							 ...
							 * jobj.put("testruntype", resultSet.getString("test_run_type"));
							 * ApplicationLauncher.logger.
							 * debug("MYSQL_Interface: InfluenceVolt: testruntype: " +
							 * resultSet.getString("test_run_type"));
							 * json_list.put(jobj);
							 * count++;
							 * break;
							 ...
							 * 
							 * 
							 * jobj.put("r_phase_selected_load",
							 * resultSet.getString("r_phase_selected_load"));
							 * jobj.put("r_phase_total_load", resultSet.getString("r_phase_total_load"));
							 * 
							 * jobj.put("y_phase_selected_load",
							 * resultSet.getString("y_phase_selected_load"));
							 * jobj.put("y_phase_total_load", resultSet.getString("y_phase_total_load"));
							 * 
							 * jobj.put("b_phase_selected_load",
							 * resultSet.getString("b_phase_selected_load"));
							 * jobj.put("b_phase_total_load", resultSet.getString("b_phase_total_load"));
							 */
							/*
							 * jobj.put("inf_emin", resultSet.getString("inf_emin"));
							 ...
							 * jobj.put("inf_deviation", resultSet.getString("inf_deviation"));
							 * jobj.put("testruntype", resultSet.getString("test_run_type"));
							 * jobj.put("inf_voltage_unbalance_u1",
							 * resultSet.getString("inf_voltage_unbalance_u1"));
							 * jobj.put("inf_voltage_unbalance_u2",
							 * resultSet.getString("inf_voltage_unbalance_u2"));
							 * jobj.put("inf_voltage_unbalance_u3",
							 * resultSet.getString("inf_voltage_unbalance_u3"));
							 */
							/*
							 ...
							 * jobj.put("voltage", resultSet.getString("inf_voltage"));
							 * ApplicationLauncher.logger.
							 * debug("MYSQL_Interface: Repeatability:testruntype: " +
							 * resultSet.getString("test_run_type"));
							 * json_list.put(jobj);
							 * count++;
							 * break;
	...
	public JSONObject sp_ltgetproject_componentsV2(String projectname, String aliasid) {
		...
		try {
			...
			try {
				...
				statement.close();
				// ApplicationLauncher.logger.info ("sp_ltgetproject_componentsV2:Error 201:
				// Source EM Model Reading: Failure");
				return test_details_json;
			}
	...
	public boolean sp_pt_add_testparam_config(int OccuranceTimeInSec, int RestorationTimeInSec, int TimerTP_OnTimeInSec,
		...
		try {
			...
			CallableStatement statement = ConnectManager
					.prepareCall("{call sp_pt_add_testparam_config(?,?,?,?,?,?,?,?)}");
	...
	public boolean sp_pt_add_server_settings(String serverHttpProtocol, String serverIP, String serverPort,
	...
	public boolean sp_ltadd_device_settings(int id, String device_type, String model_name, String port_name,
			String baud_rate, String qrPortName) {
	...
	public boolean sp_ltadd_device_settings_v2(String device_type, String model_name, String port_name,
			String baud_rate,
			String clusterName,
			String bayName,
			String positionNo,
		...
		try {
			...
			CallableStatement statement = ConnectManager
					.prepareCall("{call sp_add_device_setting_v2(?,?,?,?,?,?,?,?,?)}");
			// statement.setInt(1, id);
			...
			try {
				...
				while (hadResults) {
					...
					while (resultSet.next()) {
						...
						testparam_config.put("freq_tp_pulsating_dc_freq",
								resultSet.getInt("freq_tp_pulsating_dc_freq"));
					}
			...
			try {
				...
				while (hadResults) {
					...
					while (resultSet.next()) {
						...
						// ApplicationLauncher.logger.debug ("sp_ltgetsummary_data: testpoint_id"+
						// resultSet.getString("testpoint_id"));
						jobj.put("testpoint_id", resultSet.getString("testpoint_id"));
						testcases.put(jobj);
						count++;
	...
	/*
	 * public boolean sp_ltadd_deploy_test_cases (String project_name, String
	 * test_case, String test_type, String alias_id,String sequence_no,
	 * String is_deployed, String testPointID) {
	 * 
	 * ApplicationLauncher.logger.info("sp_ltadd_deploy_test_cases: is_deployed: " +
	 * is_deployed);
	 * 
	 * try {
	 * 
	 * CallableStatement statement =
	 * ConnectManager.prepareCall("{call sp_add_deploy_test_cases(?,?,?,?,?,?,?)}");
	 * statement.setString(1, project_name);
	 ...
	 * } catch (Exception ex) {
	 * ex.printStackTrace();
	 * ApplicationLauncher.logger.error("sp_ltadd_deploy_test_cases : Exception1 :"+
	 * ex.getMessage());
	 * return false;
	 * 
	 * }
	 ...
	 * } catch (Exception ex) {
	 * ex.printStackTrace();
	 * ApplicationLauncher.logger.error("sp_ltadd_deploy_test_cases : Exception2 :"+
	 * ex.getMessage());
	 * return false;
	 * }
	 * }
	...
	public boolean sp_ltadd_deploy_test_cases(String lastUpdatedDeploymentID, String project_name, String test_case,
			String test_type, String alias_id, String sequence_no, String testPointId, String is_deployed) {
		...
		try {
			...
			CallableStatement statement = ConnectManager
					.prepareCall("{call sp_add_deploy_test_cases(?,?,?,?,?,?,?,?)}");
			statement.setString(1, lastUpdatedDeploymentID);
			statement.setString(2, project_name);
	 ...
	 * 
	 * CallableStatement statement =
	 * ConnectManager.prepareCall("{call sp_delete_deploy_test_cases(?)}");
	 * statement.setString(1, project_name);
	 * 
	 * 
	 * boolean hadResults = statement.execute();
	 ...
	 * ex.printStackTrace();
	 * ApplicationLauncher.logger.error
	 * ("sp_ltdelete_deploy_test_cases : Exception1 :"+ ex.getMessage());
	 * return false;
	 * 
	 * }
	 * 
	 ...
	 * ex.printStackTrace();
	 * ApplicationLauncher.logger.error
	 * ("sp_ltdelete_deploy_test_cases : Exception2 :"+ ex.getMessage());
	 * return false;
	 * }
	 * }
	 */
		...
		try {
			...
			for (int i = 0; i < tp_setup_values.size(); i++) {
				try {
					...
					e1.printStackTrace();
					ApplicationLauncher.logger
							.error("sp_lt_add_backup_file_location : JSONException1" + e1.getMessage());
				}
			}
