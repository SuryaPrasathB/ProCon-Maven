...
public class ConveyorDebugController implements Initializable {
   ...
   public static TableView<StateFlowRow> tableStatePlanner_FtBay_UI = new TableView<StateFlowRow>();
	...
	/*private void initializeStateCodeMap() {
	 ...
	 stateCodeMap.put("S13_ensure_divertor_relay_turned_on_FT_Bay", "STATE_CODE_017");
	 ...
	 stateCodeMap.put("S18_ensure_divertor_relay_turned_off_FT_Bay", "STATE_CODE_022");
	 stateCodeMap.put("S19_wait_for_reset_button_ip_Rejection_Bay", "STATE_CODE_023");
	 ...
	 */
	/*private final ObservableList<StateFlowRow> stateFlowRows = FXCollections.observableArrayList();*/
	...
	//private Map<String,ArrayList<String>> bayNameListMap = new HashMap<String,ArrayList<String>>();
	...
	public void loadAllChild_FXML(){
		...
		Platform.runLater(() -> {
			try{
				...
				statePlannerChild.getChildren().add(getNodeFromFXML("/fxml/conveyor/StatePlanner" + ConstantApp.THEME_FXML));
				//InitCounter--;
	...
	private void dataSetupInit() {
		...
		/*		TestIntefaceStatus dataSet1 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY, ConstantBayStateManage.FT_BAY_HP_SEQ_01,ConstantConveyor.DEVICE_TYPE_CLUSTER_OUTPUT, "0","STPR_B4_FT_BAY", ConstantConveyor.COMM_STATUS_NOT_APPLICABLE, "Success", "On", ConstantConveyor.COMM_EXECUTION_STATUS_COMPLETED);
		TestIntefaceStatus dataSet2 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY,ConstantBayStateManage.FT_BAY_HP_SEQ_02,ConstantConveyor.DEVICE_TYPE_QR_SCANNER,  "0","Com9-FT_BAY_QR_SCNR_PALLET", "Closed",   "Success",  "PLT-123456", ConstantConveyor.COMM_EXECUTION_STATUS_COMPLETED);
		TestIntefaceStatus dataSet3 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY,ConstantBayStateManage.FT_BAY_HP_SEQ_03, ConstantConveyor.DEVICE_TYPE_DUT, "1","Com10-FT_BAY_DUT1", "Closed", "Success", "SN-987654", ConstantConveyor.COMM_EXECUTION_STATUS_COMPLETED);
		TestIntefaceStatus dataSet4 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY,ConstantBayStateManage.FT_BAY_HP_SEQ_04,ConstantConveyor.DEVICE_TYPE_DUT,  "1","Com10-FT_BAY_DUT1", "Closed",   "Success",  "Dut-Relay-On", ConstantConveyor.COMM_EXECUTION_STATUS_COMPLETED);
		TestIntefaceStatus dataSet5 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY,ConstantBayStateManage.FT_BAY_HP_SEQ_05, ConstantConveyor.DEVICE_TYPE_CLUSTER_OUTPUT, "0","SRC_START_STOP_FT_BAY",  ConstantConveyor.COMM_STATUS_NOT_APPLICABLE, "Success", "FtSrcOn", ConstantConveyor.COMM_EXECUTION_STATUS_COMPLETED);
		TestIntefaceStatus dataSet6 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY,ConstantBayStateManage.FT_BAY_HP_SEQ_06,ConstantConveyor.DEVICE_TYPE_CLUSTER_INPUT,  "1","LDU_SCANNER1_IP_FT_BAY",  ConstantConveyor.COMM_STATUS_NOT_APPLICABLE,   "",  "", ConstantConveyor.COMM_EXECUTION_STATUS_INP);
		TestIntefaceStatus dataSet7 = new TestIntefaceStatus(String.valueOf(getSerialNoTestStatusAtomic().getAndIncrement()), ConstantConveyor.FT_BAY_KEY,ConstantBayStateManage.FT_BAY_HP_SEQ_07, ConstantConveyor.DEVICE_TYPE_CLUSTER_INPUT, "1","STPR_B4_FT_BAY", "", "", "", ConstantConveyor.COMM_EXECUTION_STATUS_PENDING);
	...
	public void guiInit() {
		...
		//ref_colLpPositonNo.setCellValueFactory(new PropertyValueFactory<PositionLoadedModel, String>("positionNo"));
		//ref_colLpSerialNo.setCellValueFactory(new PropertyValueFactory<PositionLoadedModel, String>("serialNo"));
	...
	public void loadDataFromConfig() {			
		...
		for (Terminal eachTerminal: getBayConfigModel().getTerminal()) {
			if (eachTerminal.getTerminalId().equals(ConstantConveyorConfig.MY_TERMINAL_ID)) {
				for (ClusterDetail eachClusterDetail: eachTerminal.getClusterDetails()) {							// Get cluster details
					ref_cmbBxPosLoadedClusterSelection.getItems().add(eachClusterDetail.getName());					// Add details to Combo Box
					getClusterNameIdListMap().put(eachClusterDetail.getName(), eachClusterDetail.getClusterId());	
		...
		if (getClusterBayNameListMap().size() > 0) {
			if (getClusterBayNameListMap().containsKey(ref_cmbBxPosLoadedClusterSelection.getSelectionModel().getSelectedItem().toString())) {
				ref_cmbBxPosLoadedBaySelection.getItems().addAll(getClusterBayNameListMap().get(ref_cmbBxPosLoadedClusterSelection.getSelectionModel().getSelectedItem().toString()));
			}			
		}
	...
	public void setClusterBayNameIdMap(Map<String, Map<String, String>> clusterBayNameIdtMap) {
			...
			if(ref_tbViewInputPortData.getItems().size()>0) {
				...
				for(int i =0; i< ref_tbViewInputPortData.getItems().size(); i++) {
					...
					if(inputPortDetails.isReadBay()) {
						...
						RestApiJsonBodyResponse clusterResponseData  = getInputDataFromBay(clusterServer, deviceId,bayId,inputPortId) ;
	...
	class posLoadedlusterSelectionOnChangeTask extends TimerTask {
		public void run() {
			Platform.runLater(()->{
				String selectedClusterName = (String)ref_cmbBxPosLoadedClusterSelection.getSelectionModel().getSelectedItem();
				if(getClusterBayNameListMap().size()>0){
					ref_cmbBxPosLoadedBaySelection.getItems().clear();
					if(getClusterBayNameListMap().containsKey(selectedClusterName)){
						ref_cmbBxPosLoadedBaySelection.getItems().addAll(getClusterBayNameListMap().get(selectedClusterName));
					}
					ref_cmbBxPosLoadedBaySelection.getSelectionModel().select(0);
	...
	class PosLoadedDeviceTypeSelectionOnChangeTask extends TimerTask {
		public void run() {
			Platform.runLater(()->{
				...
				String selectedClusterName = (String)ref_cmbBxPosLoadedDeviceTypeSelection.getSelectionModel().getSelectedItem();
				if(getClusterBayNameListMap().size()>0){
					//ref_txtClusterIpAddress.setText("");
					//ref_txtClusterPortNo.setText("");
					//if(getClusterBayNameListMap().containsKey(selectedClusterName)){
	...
	public static void enableFtStartButton() {
		try {
			...
			e.printStackTrace();
			ApplicationLauncher.logger.error("enableFtStartButton : Exception: " + e.getMessage());
		}
	}
	...
	public void btnSampleDataOnClick(){
		...
		for (int j = 0; j < 15; j++) {
			...
			allData.add(testIntefaceStatus) ;
			int newRecordSerialNo = ConveyorDebugController.addToTestStatusGui(testIntefaceStatus);
	...
	public void btnRefreshOnClick(){
		...
		List<ResultSummary>  resultSummaryList= MySqlServiceManager.getResultSummaryService().findAll();//

		resultSummaryList.stream().forEach(e->{
			ApplicationLauncher.logger.debug("Each Customer: " + e.getCustomerId() + " : name : " +e.getCustomerName());
		});


		List<ResultSummary>  resultSummaryWithCustomerId= MySqlServiceManager.getResultSummaryService().findByCustomerId("2");//

		resultSummaryWithCustomerId.stream().forEach(e->{
			ApplicationLauncher.logger.debug("Each Customer with id: " + e.getCustomerId() + " : name : " +e.getCustomerName());
		});*/
	...
	class RefreshOnClickTimerTask extends TimerTask {
		...
		public void run() {
			Platform.runLater(()->{
				...
				ApplicationLauncher.logger.debug("RefreshOnClickTimerTask getClusterBayNameIdMap(): " + getClusterBayNameIdMap());
				...
				String selectedTestType = (String)cmbBxPosLoadedDeviceTypeSelection.getSelectionModel().getSelectedItem();
				ref_tvLoadedPosition.getItems().clear();
				...
				if(selectedTestType.equals(ConstantConveyor.DEVICE_TYPE_QR_SCANNER)){
					...
					ArrayList<QrScanner> qrScannerList = 		 
							(ArrayList<QrScanner>) getBayConfigModel().getTerminal().stream()
							.filter(e1->e1.getTerminalId().equals(ConstantConveyorConfig.MY_TERMINAL_ID))
					...
					if(qrScannerList.size()>0){
						//ApplicationLauncher.logger.debug("RefreshOnClickTimerTask qrScannerList: QR: " + qrScannerList);
						for(int i=0; i< qrScannerList.size();i++){
							positionLoadedModel = new PositionLoadedModel();
							ApplicationLauncher.logger.debug("RefreshOnClickTimerTask qrScannerList: QR: getPositionId:" + qrScannerList.get(i).getPositionId());
				...
				if(selectedTestType.equals(ConstantConveyor.DEVICE_TYPE_DUT)){
					...
					ArrayList<DutDevice> dutDeviceList = 		 
							(ArrayList<DutDevice>) getBayConfigModel().getTerminal().stream()
							.filter(e1->e1.getTerminalId().equals(ConstantConveyorConfig.MY_TERMINAL_ID))
					...
					if(dutDeviceList.size()>0){
						...
						for(int i=0; i< dutDeviceList.size();i++){
							positionLoadedModel = new PositionLoadedModel();
							ApplicationLauncher.logger.debug("RefreshOnClickTimerTask dutDeviceList: EM: getPositionId:" + dutDeviceList.get(i).getPositionId());
							positionLoadedModel.setSerialNo(String.valueOf(serialNumber++));
				...
				if(selectedTestType.equals(ConstantConveyor.DEVICE_TYPE_OHM_METER)){
					...
					ArrayList<MegaOhmMeter> megaOhmMeterList = 		 
					...
					if(megaOhmMeterList.size()>0){
						ApplicationLauncher.logger.debug("RefreshOnClickTimerTask megaOhmMeterList: OM: " + megaOhmMeterList);
						for(int i=0; i< megaOhmMeterList.size();i++){
							positionLoadedModel = new PositionLoadedModel();
							ApplicationLauncher.logger.debug("RefreshOnClickTimerTask megaOhmMeterList: OM: getPositionId:" + megaOhmMeterList.get(i).getPositionId());
							positionLoadedModel.setSerialNo(String.valueOf(serialNumber++));
				...
				/*if(selectedTestType.equals(ConstantConveyor.DEVICE_TYPE_OHM_METER)){
					...
					if(voltMeterList.size()>0){
						ApplicationLauncher.logger.debug("RefreshOnClickTimerTask voltMeterList: VM: " + voltMeterList);
						for(int i=0; i< voltMeterList.size();i++){
							ApplicationLauncher.logger.debug("RefreshOnClickTimerTask voltMeterList: VM: getPositionId:" + voltMeterList.get(i).getPositionId());

						}
					}else{
						ApplicationLauncher.logger.debug("RefreshOnClickTimerTask No record found for Volt Meter");
					}
				}*/
				...
				for ( int i = 0; i < ref_tvLoadedPosition.getItems().size(); i ++){
					ApplicationLauncher.logger.debug("RefreshOnClickTimerTask: " + ref_tvLoadedPosition.getItems().get(i).getcName() + " -> status :" + ref_tvLoadedPosition.getItems().get(i).isPositionSelected());
	...
	public static TextField getTextFieldByKey(String bayKey) {
	    switch (bayKey) {
	        case "CALB": return ref_txtCalibBayPallet;
	        case "COMB": return ref_txtCommBayPallet;
	        case "FTB": return ref_txtFTBayPallet;
	        case "HVB": return ref_txtHVBayPallet;
	        case "IRB": return ref_txtIRBayPallet;
	        case "LOADB": return ref_txtLoadingBayPallet;
	        case "RJTB": return ref_txtRejectionBayPallet;
	        case "SCTNLT1P1": return ref_txtSCTNLTBay1Pallet1;
	        case "SCTNLT1P2": return ref_txtSCTNLTBay1Pallet2;
	        case "SCTNLT1P3": return ref_txtSCTNLTBay1Pallet3;
	        case "SCTNLT1P4": return ref_txtSCTNLTBay1Pallet4;
	        case "SCTNLT2P1": return ref_txtSCTNLTBay2Pallet1;
	        case "SCTNLT2P2": return ref_txtSCTNLTBay2Pallet2;
	        case "SCTNLT2P3": return ref_txtSCTNLTBay2Pallet3;
	        case "SCTNLT2P4": return ref_txtSCTNLTBay2Pallet4;
	        case "UNLDB": return ref_txtUnloadingBayPallet;
	        case "Verific1": return ref_txtVerificBayPallet1;
	        case "Verific2": return ref_txtVerificBayPallet2;
	        case "Verific3": return ref_txtVerificBayPallet3;
	        case "Verific4": return ref_txtVerificBayPallet4;
	        case "Waiting1": return ref_txtWaitingBayPallet1;
	        case "Waiting2": return ref_txtWaitingBayPallet2;
	        case "Waiting3": return ref_txtWaitingBayPallet3;
	        case "Waiting4": return ref_txtWaitingBayPallet4;
	        default: return null;
	    }
	}
	...
	public static List<TextField> getMultiPalletTextFields(String bayKey) {
	    switch (bayKey) {
	        case "VERIFICB":
	            return Arrays.asList(ref_txtVerificBayPallet1, ref_txtVerificBayPallet2, ref_txtVerificBayPallet3, ref_txtVerificBayPallet4);
	        case "STNLD1B":
	            return Arrays.asList(ref_txtSCTNLTBay1Pallet1, ref_txtSCTNLTBay1Pallet2, ref_txtSCTNLTBay1Pallet3, ref_txtSCTNLTBay1Pallet4);
	        case "STNLD2B":
	            return Arrays.asList(ref_txtSCTNLTBay2Pallet1, ref_txtSCTNLTBay2Pallet2, ref_txtSCTNLTBay2Pallet3, ref_txtSCTNLTBay2Pallet4);
	        default:
	...
	public void ButtonDeleteRowOnClick() {
	    ...
	    StateFlowRow selectedRow = tableStatePlanner.getSelectionModel().getSelectedItem();
	...
	public void ButtonDeleteRowOnClick() {
	    ...
	    if (!stateFlowRows.isEmpty()) {
	        // Remove the last row (or you can modify this to remove a specific row based on some condition)
	...
	/*    public static class StateFlowRow {
        ...
        public StateFlowRow(String path, String state, String stateErrorCode, String success, String successErrorCode, String failed, String failedErrorCode) {
	...
	public void setTableStatePlannerFtBay_UI(TableView<StateFlowRow> tableStatePlannerFtBay_UI) {
