    ...
    public boolean start() {
        try {
            ...
            if (!isPortAvailable(modbusServer.getPort())) {
                ...
                // JOptionPane.showMessageDialog(null, "❌ ERROR: Port " + modbusServer.getPort()
                // + " is already in use!\nStop any other process and try again.",
                // "Port Conflict", JOptionPane.ERROR_MESSAGE);

                WindowManager.InformUser("Error-K01-Port Conflict",
                        "❌ ERROR: Port " + modbusServer.getPort()
            ...
            try {
                ...
                ApplicationLauncher.logger
                        .error("❌ ERROR: Port is already in use. Please stop any other process using port 502.");
                /*
                 * JOptionPane.showMessageDialog(null,
                 * "❌ ERROR: Port 502 is already in use!\nPlease stop any other process using it and try again."
                 * ,
                 * "Port Conflict", JOptionPane.ERROR_MESSAGE);
                 */
            ...
            public void run() {
                ...
                for (int i = 0; i < modbusServer.holdingRegisters.length; i++) {
                    if (modbusServer.holdingRegisters[i] != previousHoldingRegisters[i]) {
                        ApplicationLauncher.logger
                                .debug("🔄 Register[" + i + "] changed: " + previousHoldingRegisters[i] +
