...
public class ModbusTcpClient {
	...
	public boolean modbusReadPlcData() throws ModbusException,    java.net.UnknownHostException,    java.net.SocketException,    java.io.IOException{
	...
	public boolean modbusReadPlcFaultOccured() throws ModbusException,    java.net.UnknownHostException,    java.net.SocketException,    java.io.IOException{
		...
		try
		{
			...
			if(modbusClient.isConnected()){
				...
				//ModbusTcpClient.logger.debug("modbusReadPlcFaultOccured: Fault Occured: "+modbusClient.ReadCoils(10, 1)[0]);

				
				int numberOfBitsToRead = 1;
				boolean value = false;
				boolean faultOccured = false;
			//	faultOccured = modbusClient.ReadCoils(ConstantModBusPlc.ADDRESS_BIT_FAULT_OCCURED_STATUS, numberOfBitsToRead )[0];
				//faultOccured = ! value;
	...
	public void modbusPlcMonitorFaultTrigger() {
		...
		//ModbusTcpClient.logger.info("modbusPlcMonitorFaultTrigger :getSerialLDU_ComRefreshTimeInMsec:" + getSerialLDU_ComRefreshTimeInMsec());
	...
	class modbusPlcMonitorFaultTask extends TimerTask {
		public void run() {
			...
			//ModbusTcpClient.logger.debug("MaximumNumberOfDeviceConnected: " + MaximumNumberOfDeviceConnected);
			ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: Entry "); 
			//ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: isPlcReadDataContinous: " + isPlcReadDataContinous()); 
			//ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: isPlcConnected: " + isPlcCommSuccess()); 
			if (isPlcReadDataContinous() && isPlcCommSuccess()){
				//ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: isPlcReadDataContinous True entry");
				if(plcSemLock){
					//ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: plcSemLock True entry");
					plcSemLock = false;
					try {
					...
					try{
							...
							if(!ProjectExecutionController.getUserAbortedFlag()){
								if(isPlcReadDataContinous()){
									//ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: validating device to be read");
									//if(DisplayDataObj.getDevicesToBeRead().size() !=0){
										//ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask:Scheduling task again");
										plcReadTimer.schedule(new modbusPlcMonitorFaultTask(), getDataRefreshTimeInMsec());
									//}/*else{
									//	ModbusTcpClient.logger.debug("modbusPlcMonitorFaultTask: send refresh command");
		...
		try{
			...
			if(status) {
				if(isReadFaultOccured()){
					...
					modbusReadPlcData();
					setPlcReadDataContinous(false);
					//ModbusTcpClient.logger.debug("monitorPlcFaultOccurance : setting UserAbortedFlag");
					ProjectExecutionController.setUserAbortedFlag(true);
					//ModbusTcpClient.logger.info("monitorPlcFaultOccurance: user prompted: Closure-Success , Project closed succesfully");
					String plcFaultReason = fetchPlcFaultReason();
					ModbusTcpClient.logger.info("monitorPlcFaultOccurance: Plc fault occured:" + "Execution aborted!. Plc failed due to below reason: \n"+plcFaultReason +" : prompted!" );
					ApplicationLauncher.InformUser("Plc fault occured", "Execution aborted!. Plc failed due to below reason: \n"+plcFaultReason,AlertType.ERROR);
				
					//ModbusTcpClient.InformUser("PLC-Fault Occured","Aborting execution due to fault occurance in PLC!", AlertType.ERROR);
				}
			}
		}catch(Exception E){
			E.printStackTrace();
	...
	//public static boolean modbusTcpSendWriteCoilCmd(int writeAddress,boolean writeData) throws ModbusException,    java.net.UnknownHostException,    java.net.SocketException,    java.io.IOException{
	public boolean modbusTcpSendWriteCoilCmd(int writeAddress,boolean writeData) {
		...
		try
		{
			...
			if(modbusClient.isConnected()){
				...
				e.printStackTrace();
				ModbusTcpClient.logger.error("modbusTcpSendWriteCoilCmd: Exception: Address: " + writeAddress + " : " +e.getMessage());
		        ...
		        try {
		            ...
		            ModbusTcpClient.logger.error("modbusTcpSendWriteCoilCmd: Retry after reconnection failed: Address: " + writeAddress + " : " + retryEx.getMessage());
	...
	public BayResponse modbusTcpSendReadCoilCmd(int readAddress) {
		...
		try
		{
			...
			if(modbusClient.isConnected()){
				//if(toBeStarted){
				int numberOfBitsToRead = 1;
				//modbusClient.ReadCoils(ConstantModBusPlc.ADDRESS_BIT_START_CMD_ACK_STATUS, 1)[0];
				...
				e.printStackTrace();
				ModbusTcpClient.logger.error("modbusTcpSendReadCoilCmd: Exception: Address: " + readAddress + " : " +e.getMessage());
		        ...
		        try {
		            ...
		            ModbusTcpClient.logger.error("modbusTcpSendReadCoilCmd: Retry after reconnection failed: Address: " + readAddress + " : " + retryEx.getMessage());
		            retryEx.printStackTrace();
		        }
	...
	public BayResponse modbusTcpSendReadHoldingRegistersCmd(int readAddress) {
		...
		try
		{
			...
			if(modbusClient.isConnected()){
				...
				// Attempt to reconnect and retry once
		        try {
		            ModbusTcpClient.logger.debug("modbusTcpSendReadHoldingRegistersCmd: Attempting reconnection after exception...");
		            ...
		            if (modbusConnect()) {
						...
						int unsignedValue = value & 0xFFFF;  
							...
							bayResponse.setResponseData(String.valueOf(unsignedValue));
							bayResponse.setStatus(true);
		                ModbusTcpClient.logger.debug("modbusTcpSendReadHoldingRegistersCmd: Retry after reconnection successful.");
		            } else {
		                ModbusTcpClient.logger.error("modbusTcpSendReadHoldingRegistersCmd: Reconnection failed.");
		            }

		        } catch (Exception retryEx) {
		            ModbusTcpClient.logger.error("modbusTcpSendReadHoldingRegistersCmd: Retry after reconnection failed: Address: " + readAddress + " : " + retryEx.getMessage());
		            retryEx.printStackTrace();
		        }
		}
		return bayResponse;
	...
	public BayResponse modbusTcpSendWriteHoldingRegistersCmd(int writeAddress, Integer writeValue) {
		...
		try
		{
			...
			if(modbusClient.isConnected()){
				modbusClient.WriteSingleRegister(writeAddress, writeValue);//(startingAddressRead, quantityRead, startingAddressWrite, values)//.WriteHoldingRegisters(writeAddress, writeValue);
					Sleep(100);
					//status = true;
				...
				// Attempt to reconnect and retry once
		        try {
		            ModbusTcpClient.logger.debug("modbusTcpSendWriteHoldingRegistersCmd: Attempting reconnection after exception...");
		            ...
		            if (modbusConnect()) {
						...
						//bayResponse.setResponseData(String.valueOf(value));
						bayResponse.setStatus(true);
		                ModbusTcpClient.logger.debug("modbusTcpSendWriteHoldingRegistersCmd: Retry after reconnection successful.");
		            } else {
		                ModbusTcpClient.logger.error("modbusTcpSendWriteHoldingRegistersCmd: Reconnection failed.");
		            }

		        } catch (Exception retryEx) {
		            ModbusTcpClient.logger.error("modbusTcpSendWriteHoldingRegistersCmd: Retry after reconnection failed: Address: " + writeAddress + " : " + retryEx.getMessage());
		            retryEx.printStackTrace();
		        }
