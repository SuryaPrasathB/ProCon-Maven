...
public class PlcServerBayTestController implements Initializable,InputPortTableViewRefresher,OutputPortTableViewRefresher {
	...
	//ObservableList<InputPort> inputPortCoilDataList = FXCollections.observableArrayList();
...
/*	@FXML CheckBox chkBxWriteEnabledSample;
	...
	@FXML Button btnWriteHoldingRegister;
...
/*		@FXML
	...
	public void guiInit() {
		...
		ref_tblColOutputActive.setCellValueFactory(new OutputPortActiveCheckBoxValueFactory(PlcServerBayTestController.this));
		...
		ref_tblColInputActive.setCellValueFactory(new InputPortActiveCheckBoxValueFactory(PlcServerBayTestController.this));
				...
				for(int i =0; i< ref_tbViewOutputPortData.getItems().size(); i++) {
					...
					if(outputPortDetails.isUpdateBay()) {
						...
						if(GuiUtils.isNumber(outputPortId)) {
							...
							//deviceResponded = ModbusTcpClient.modbusTcpSendWriteCoilCmd(plcCoilAddress,outputPortDetails.isOutputActive());
							if(!deviceResponded) {
...
/*
			...
			//if(ref_chkBxReadEnabledSample.isSelected()) {
				...
				/*BayResponse bayResponse2 = ModbusTcpClient.modbusTcpSendReadHoldingRegistersCmd(holdingReadAddress);
			...
			/*if(ref_chkBxWriteEnabledSample.isSelected()) {
				...
				BayResponse bayResponse3 = ModbusPlcDeviceData.modbusTcpSendWriteHoldingRegistersCmd(holdingWriteAddress,holdingWriteValue);
...
/*    private void updateHoldingRegisters(int[] holdingRegisters) {
        Platform.runLater(() -> {
            ...
            System.out.println("ðŸ“¡ GUI Updated: Holding Register[" + address + "] = " + holdingRegisters[address]);
        });
    }*/
	       ...
	       // int address = 301; // Example register to display
	        ...
	        for (int i = 0; i < holdingRegisters.length; i++) {
                    ...
                    if(serverInputHoldingRegisterAddressList.contains(i)) {
                        ...
                        ApplicationLauncher.logger.debug("ðŸ“¡ GUI Updated: Holding Register[" + i + "] = " + holdingRegisters[i]);
                    }
	...
	class ModbusStartServerTask extends TimerTask{
		...
		public void run() {
			...
			if(status) {
		        ...
		        getModbusTcpServer().setHoldingRegisterUpdateListener(holdingRegisters -> updateHoldingRegisters(holdingRegisters));
...
/*			modbusServer = new ModbusServer();
	        ...
	        ObservableList<InputPort> inputPortList = FXCollections.observableArrayList();
	        for (int i = 1; i <= 32; i++) {
	            InputPort inputPort = new InputPort();
	        ...
	        btnWriteHoldingRegister.setOnAction(event -> {
	            try {
	                ...
	                if (address >= 0 && address < modbusServer.holdingRegisters.length) {
	                    ...
	                    ApplicationLauncher.logger.debug("âœ… Holding Register[" + address + "] updated to " + value);
	                } else {
	        ...
	        modbusServer.setNotifyHoldingRegistersChanged(new IHoldingRegistersChangedDelegator() {
	            private int[] previousValues = modbusServer.holdingRegisters.clone(); // Store old values
	        ...
	        modbusServer.setNotifyCoilsChanged(new ICoilsChangedDelegator() {
	            private boolean[] previousCoils = modbusServer.coils.clone(); // Store old coil states
	                ...
	                Platform.runLater(() -> {
	                    ...
	                    for (int i = 1; i <= 32; i++) {
	                        if (modbusServer.coils[i] != previousCoils[i]) {
	                            ApplicationLauncher.logger.debug("Coil[" + i + "] changed from " + previousCoils[i] +
	        ...
	        try {
	            ApplicationLauncher.logger.debug("ðŸš€ Starting Modbus TCP Server on port 502...");
	            modbusServer.Listen();
			...
			//Platform.runLater(() -> {
/*			if ( !(ref_txtClusterIpAddress.getText().isEmpty()) && !(ref_txtClusterPortNo.getText().isEmpty()) ) {
					boolean status=false;
					try{
						...
						if(ModbusPlcDeviceData.getModbusClient().isConnected()) {
						  ...
						  e.printStackTrace();
						  ApplicationLauncher.logger.error("modbusConnectTask: : UnknownHostException: "+e.getMessage());
					}catch(IOException e) {
						  e.printStackTrace();
						  ApplicationLauncher.logger.error("modbusConnectTask: : IOException: "+e.getMessage());
					}
					...
					ApplicationLauncher.InformUser("Error-3010","Invalid Server Ip or Invalid Port No",AlertType.ERROR);
...
/*				boolean status=false;
				try{
					...
					Platform.runLater(() -> {
					  ...
					  e.printStackTrace();
					  ApplicationLauncher.logger.error("modbusDisconnectTask: : UnknownHostException: "+e.getMessage());
				}catch(IOException e) {
					  e.printStackTrace();
					  ApplicationLauncher.logger.error("modbusDisconnectTask: : IOException: "+e.getMessage());
				}*/
/*				status=stop_confirmation();
				if(status){
					StopOnClickSuccess();
...
/*	public ObservableList<InputPort> getInputPortCoilDataList() {
	...
	public void setInputPortCoilDataList(ObservableList<InputPort> inputPortCoilDataList) {
