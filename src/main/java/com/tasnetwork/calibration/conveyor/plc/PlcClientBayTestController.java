...
public class PlcClientBayTestController
	...
	/*
	 * Map<String,Boolean> modbusTcpServerConnectedMap = new HashMap<String,
	 * Boolean>();
	 * Map<String,ModbusTcpClient> modbusTcpServerObjectMap = new HashMap<String,
	 * ModbusTcpClient>();
	 * 
	...
	@FXML
	CheckBox chkBxReadEnabledSample;
	...
	@FXML
	CheckBox chkBxWriteEnabledSample;
	...
	public void guiInit() {
		...
		ref_tblColOutputActive.setCellValueFactory(
				new OutputPortActiveCheckBoxValueFactory(PlcClientBayTestController.getInstance()));
		...
		ref_tblColInputActive
				.setCellValueFactory(new InputPortActiveCheckBoxValueFactory(PlcClientBayTestController.getInstance()));
	...
	class SendDataToBayTaskInParallel extends TimerTask {
		public void run() {
			...
			if (ref_chkBxReadEnabledSample.isSelected()) {
				...
				ModbusRequestProcessor.addRequest(serverKey, () -> {
					BayResponse response = BayUtils.getModbusTcpClientManager().modbusReadHoldingRegister(clusterServer,
							holdingReadAddress);

					Platform.runLater(() -> {
						ref_txtReadStatusSample.setText(response.getStatus() ? "Success" : "Failed");
						if (response.getStatus())
							ref_txtReadValueSample.setText(response.getResponseData());
					});
			...
			if (ref_chkBxWriteEnabledSample.isSelected()) {
				...
				ModbusRequestProcessor.addRequest(serverKey, () -> {
					...
					Platform.runLater(
							() -> ref_txtWriteStatusSample.setText(response.getStatus() ? "Success" : "Failed"));
	...
	class SendDataToBayTask extends TimerTask {
		public void run() {
			...
			if (ref_tbViewOutputPortData.getItems().size() > 0) {
				...
				outputPorts.parallelStream().forEach(outputPortDetails -> {
					...
					if (GuiUtils.isNumber(outputPortId)) {
						...
						WindowManager.InformUser(
								"Error-1010",
								"Kindly check key : " + outputPortDetails.getPortId()
										+ " on conveyor config file on <outputPort> section",
								AlertType.ERROR);
					}
				});
			...
			if (ref_tbViewInputPortData.getItems().size() > 0) {
				...
				ref_tbViewInputPortData.getItems().parallelStream()
						...
						.forEach(inputPortDetails -> {
							...
							if (GuiUtils.isNumber(inputPortId)) {
								final int plcCoilAddress = Integer.parseInt(inputPortId); // ✅ Use 'final' to avoid
																							// lambda issues
								...
								ModbusRequestProcessor.addRequest(serverKey, () -> {
									...
									Platform.runLater(() -> {
										if (!bayResponse.getStatus()) {
											...
											stateDesc.set(coilState ? inputPortDetails.getOnStateDesc()
													: inputPortDetails.getOffStateDesc());
											ApplicationLauncher.logger.info("SendDataToBayTask: inputPortId: "
													+ inputPortDetails.getPortId() + " : value : " + coilState);
										}
								...
								Platform.runLater(() -> {
									...
									ApplicationLauncher.logger.info(
											"SendDataToBayTask: invalid inputPortId: " + inputPortDetails.getPortId());
									WindowManager.InformUser(
											"Error-2010",
											"Kindly check key : " + inputPortDetails.getPortId()
													+ " on conveyor config file on <inputPort> section",
											AlertType.ERROR);
								});
							}
						});
			...
			if (ref_chkBxReadEnabledSample.isSelected()) {
				...
				BayResponse bayResponse2 = BayUtils.getModbusTcpClientManager().modbusReadHoldingRegister(clusterServer,
						holdingReadAddress);
				// bayResponse2 =
				// BayUtils.getModbusTcpClientManager().getModbusTcpClient().modbusTcpSendReadHoldingRegistersCmd(holdingReadAddress);
			...
			if (ref_chkBxWriteEnabledSample.isSelected()) {
				...
				BayResponse bayResponse3 = BayUtils.getModbusTcpClientManager()
						.modbusWriteHoldingRegister(clusterServer, holdingWriteAddress, holdingWriteValue);
				// bayResponse3 =
				// BayUtils.getModbusTcpClientManager().getModbusTcpClient().modbusTcpSendWriteHoldingRegistersCmd(holdingWriteAddress,holdingWriteValue);
	...
	class LoadOnClickTask extends TimerTask {
		public void run() {
			Platform.runLater(() -> {
				...
				if (clusterOpt.isPresent()) {
					ClusterDetail clusterDetail = clusterOpt.get();
					ApplicationLauncher.logger.info("LoadOnClickTask : clusterDetail().getClusterIpAddress(): "
					...
					ApplicationLauncher.logger
							.info("LoadOnClickTask: getModbusTcpServerStatus : " + BayUtils.getModbusTcpClientManager()
					...
					if (isConnected) {
						...
						ApplicationLauncher.logger
								.info("LoadOnClickTask: Connected to " + clusterDetail.getClusterIpAddress());
					} else {
						Platform.runLater(() -> ref_txtComStatus.setText("Disconnected"));
						ref_txtComStatus.setStyle("-fx-text-fill: red;");
						ApplicationLauncher.logger
								.info("LoadOnClickTask: Not connected to " + clusterDetail.getClusterIpAddress());
					}
				...
				if (bayOpt.isPresent()) {
					...
					// ArrayList<OutputPort> outputPortListClone = (ArrayList<OutputPort>)
					// outputPortList.clone();
			...
			Platform.runLater(() -> {
				...
				if (clusterOpt.isPresent()) {
					...
					if (isConnected) {
						...
						/*
						 * ApplicationLauncher.logger.
						 * info("modbusConnectTask: Establishing connection to " + key);
						 * BayResponse response = BayUtils.getModbusTcpClientManager()
						 * .modbusConnect(clusterIpAddress, clusterPortNo);
						 * 
						 * if (response.getStatus()) {
						 * ref_txtComStatus.setText("Connected");
						 * ApplicationLauncher.logger.info("modbusConnectTask: Connection successful - "
						 * + response.getResponseData());
						 * } else {
						 * ref_txtComStatus.setText("Connection Failed");
						 * ApplicationLauncher.logger.error("modbusConnectTask: Connection failed - " +
						 * response.getResponseData());
						 * }
						 */
						...
						new Thread(() -> {
							...
							Platform.runLater(() -> {
								if (response.getStatus()) {
									...
									ref_txtComStatus.setStyle("-fx-text-fill: green;");
									ApplicationLauncher.logger.info(
											"modbusConnectTask: Connection successful - " + response.getResponseData());
								} else {
									ref_txtComStatus.setText("Connection Failed");
									ref_txtComStatus.setStyle("-fx-text-fill: red;");
									ApplicationLauncher.logger.error(
											"modbusConnectTask: Connection failed - " + response.getResponseData());
								}
	 ...
	 * }
	 * 
	 * public void setModbusTcpServerConnectedMap(Map<String, Boolean>
	 * modbusTcpServerConnectionStatusMap) {
	 ...
	 * }
	 * 
	 * public void setModbusTcpServerObjectMap(Map<String, ModbusTcpClient>
	 * modbusTcpServerObjectMap) {
