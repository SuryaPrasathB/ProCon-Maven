...
public class S22_error_Handling implements FtBayState {
	...
	public BayResponse handleRequest() {
		...
		Ft.logger.info(String.format("[%s] : [ERROR_HANDLING] : [SEQUENCE_ENTRY] - Initiating error handling sequence.",
				getMyBayKey()));
		...
		Ft.logger.debug(String.format(
				"[%s] : [ERROR_HANDLING] : [GUI_STATUS_UPDATE] - Added new GUI status record with serial no: %d for state %s.",
				getMyBayKey(), newRecordSerialNo, palletAvailableTest_I_F_Status.getStateName()));
		...
		Ft.logger.debug(String.format(
				"[%s] : [ERROR_HANDLING] : [GUI_STATUS_UPDATE] - Updating GUI status to COMPLETED for sequence: %s.",
				getMyBayKey(), palletAvailableTest_I_F_Status.getStateName()));

		// Structured log for sequence exit
		Ft.logger.info(String.format(
				"[%s] : [ERROR_HANDLING] : [SEQUENCE_EXIT] - Error handling sequence completed. Final Status: %s, Error Code: %s",
				getMyBayKey(), bayResponse.getStatus(), bayResponse.getErrorCode()));
		return bayResponse;
	}
	...
	public S22_error_Handling() {
		...
		Ft.logger.debug(String.format(
				"[%s] : [ERROR_HANDLER_CONSTRUCTOR] : [DEFAULT_INIT] - Default constructor called.", getMyBayKey()));
	}

	public S22_error_Handling(String errorCode) {
		// Structured info log for constructor entry with error code
		Ft.logger.info(String.format(
				"[%s] : [ERROR_HANDLER_CONSTRUCTOR] : [INIT_WITH_CODE] - Initializing error handler for code: %s",
		...
		switch (errorCode) {
			...
			default:
				...
				Ft.logger.debug(String.format(
						"[%s] : [ERROR_HANDLER_CONSTRUCTOR] : [ACTION] - Task for error code %s cancelled.",
						getMyBayKey(), errorCode));
				break;
		}
	...
	public void Sleep(int timeInMsec) {
		Ft.logger.debug(
				String.format("[%s] : [UTILITY] : [SLEEP_ENTRY] - Sleeping for %d ms.", getMyBayKey(), timeInMsec));
		try {
			...
			Ft.logger.error(String.format("[%s] : [UTILITY] : [SLEEP_INTERRUPTED] - Sleep interrupted: %s",
	...
	private class FunctionalTestBayStop extends TimerTask {
		...
		public void run() {
			Ft.logger.debug(String.format(
					"[%s] : [FunctionalTestBayStop] : [RUN] - FunctionalTestBayStop task executed.", getMyBayKey()));
		}
	}
}
