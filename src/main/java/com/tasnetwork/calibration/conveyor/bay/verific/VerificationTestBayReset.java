...
public class VerificationTestBayReset extends TimerTask {
	...
	private void manageVerificationTestBayResetStates() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService()
				.findByBayKeyAndExecutionMode(ConstantConveyor.VERIFICATION_BAY_KEY, "RESET");// findByBayKey(ConstantConveyor.VERIFICATION_BAY_KEY);
		...
		if (getTableStatePlanner_VerificBay().size() > 0) {
			...
			Verification.logger.debug(
					"VerificationTestBayReset : manageVerificationTestBayResetStates : getTableStatePlanner2 : Size : "
			...
			Verification.logger.debug(
					"VerificationTestBayReset : manageVerificationTestBayResetStates : No states found in the planner");
		}
	...
	public BayResponse processCurrentState() {
		...
		Verification.logger
				.debug("processCurrentState : " + VerificTestBayResetStateManager.getState().getClass().getSimpleName()
