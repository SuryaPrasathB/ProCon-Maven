...
public class S056_Neutral_Calibration implements CalibrationBayState {
	...
	public BayResponse handleRequest() {
		Calib.logger.info("S055_Neutral_Calibration : Entry");

		//BayUtils.delay(15000); // 15sec wait time for current to set
		
		for(int i = 1; i <= 15; i ++) {
			Calib.logger.info("S055_Neutral_Calibration : Waiting for Neutral CT Current Set: " + i + "secs");
			BayUtils.delay(1000);
		}
		...
		if (status) {
			Calib.logger.info("S055_Neutral_Calibration : Calibration Test Successful");
			bayResponse.setStatus(true);
			...
			Platform.runLater(() -> {
	            StateExecutorController.ref_tf_CALIB_prompt.setText("Remove Optical Reader");
	        });
	        
			for(int i = 1; i <= 60; i ++) {
				Calib.logger.info("S055_Neutral_Calibration : Waiting for Neutral Calib: T-" + (60-i) + "secs");
				BayUtils.delay(1000);
				Calib.logger.info("S055_Neutral_Calibration : Waiting to remove Optical Reader : T-" + (60-i) + "secs");
			}
			...
			Calib.logger.info("S055_Neutral_Calibration : Calibration Test Failed");
		...
		Calib.logger.info("S055_Neutral_Calibration : Exit");
		return bayResponse;
	}
	...
	public Map<String,Object> neutralCalibrationTask() {
		Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Entry");
		...
		if(ProconFeatureEnable.CALIB_DUT_EXECUTION_PROCESS_IN_PARALLEL){
			...
			while ( (!ProjectExecutionController.getUserAbortedFlag()) && (dutWaitTimeCounter < dutWaitTimeDurationMaxInSec) && (!dutAllProcessExecutionCompleted) && !Calib.isStopProcessRequestedCalibBay()){
				Sleep(1000);
			...
			if(ProconFeatureEnable.DEV_SYS_EM1_CONNECTED) {
				...
				Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Meter 1 : status : " + status1);
				Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Meter 2 : status : " + status2);
				Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Meter 3 : status : " + status3);
				Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Meter 4 : status : " + status4);
				Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Meter 5 : status : " + status5);
				Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationTask : Meter 6 : status : " + status6);
			}
		...
		Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationProcess : status : " + status);

		Calib.logger.debug("S055_Neutral_Calibration : neutralCalibrationProcess : Exit");
