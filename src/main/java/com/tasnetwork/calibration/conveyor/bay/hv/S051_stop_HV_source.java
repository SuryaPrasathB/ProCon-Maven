...
public class S051_stop_HV_source implements HvtBayState {
   ...
   /* String LOW   = "START";
    ...
    /*private Map<String, Object> stopHvSource() {
        ...
        HighVoltageTestBay.logger.debug("S12_turn_on_start_pin_hv_bay : stopHvSource : Entry");
		...
		if (status) {
	        HighVoltageTestBay.logger.debug("S12_turn_on_start_pin_hv_bay : stopHvSource : Turned on Source Stop Pin : Success");
			BayUtils.delay(500);
		} else {
	        HighVoltageTestBay.logger.debug("S12_turn_on_start_pin_hv_bay : stopHvSource : Failed to turn on Source Stop Pin");
	        status = false;
		}
		status = true;
		status = turn_off_stop_pin_hv_bay();
		if (status) {
	        HighVoltageTestBay.logger.debug("S12_turn_on_start_pin_hv_bay : stopHvSource : Turned Off Source Stop Pin : Success");

		} else {
	        HighVoltageTestBay.logger.debug("S12_turn_on_start_pin_hv_bay : stopHvSource : Failed to turn off Source Stop Pin");
	        status = false;
		}
		
		HighVoltageTestBay.logger.debug("S12_turn_on_start_pin_hv_bay : stopHvSource : Exit");
