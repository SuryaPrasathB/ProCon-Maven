...
public class UnloadingBayStop extends TimerTask {
	...
	// Logger.getLogger(UnloadingBay.class.getPackage().getName());
	...
	private void manageUnloadingBayStopStates() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService()
				.findByBayKeyAndExecutionMode(ConstantConveyor.UNLOADING_BAY_KEY, "STOP");// findByBayKey(ConstantConveyor.UNLOADING_BAY_KEY);
		...
		if (getTableStatePlanner_UnloadingBay().size() > 0) {
			...
			UnloadingBayState currentState = UnloadingBayState.createState(currentStateName); // Create the state
																								// instance
			setNextState(currentState); // Set the first state

			Unloading.logger.debug("UnloadingBayStop : manageUnloadingBayStopStates : getTableStatePlanner2 : Size : "
					+ getTableStatePlanner_UnloadingBay().size());
	 ...
	 * BayResponse bayStatus = processCurrentState();
	 * 
	 * UnloadingBay.logger.debug("singleStateTestRun : bayStatus : Status : " +
	 * bayStatus.getStatus());
	 * UnloadingBay.logger.debug("singleStateTestRun : bayStatus : Error Code : " +
	...
	public BayResponse processCurrentState() {
		...
		Unloading.logger
				.debug("processCurrentState : " + unloadingBayStopStateManager.getState().getClass().getSimpleName()
	...
	// public TableView<StateFlow> tableStatePlanner_FtBay = new
	// TableView<StateFlow>();
