 ...
 */
public class S26_close_stop_latch_b4_FT_Bay implements FtBayState {
    ...
    public BayResponse handleRequest() {
        ...
        Ft.logger.info(String.format("[%s] : [CLOSE_STOPPER_BR-HV] : [SEQUENCE_ENTRY] - Attempting to close stop latch B4 at FT Bay.", getMyBayKey()));

        BayResponse bayResponse = new BayResponse();
        bayResponse.setStatus(true);
        bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);

        Ft.logger.debug(String.format("[%s] : [CLOSE_STOPPER_BR-HV] : [DELAY] - Delaying for 2000 ms before latch operation.", getMyBayKey()));
        ...
        Ft.logger.debug(String.format("[%s] : [CLOSE_STOPPER_BR-HV] : [GUI_UPDATE] - Updated GUI status after attempting to close stop latch B4.", getMyBayKey()));
        ...
        if (closeStopLatchB4_FtBay_status) {
            Ft.logger.info(String.format("[%s] : [CLOSE_STOPPER_BR-HV] : [SUCCESS] - Stop Latch B4 successfully closed.", getMyBayKey()));
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
            ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayEntryStopper(getMyBayKey(),false);
        } else {
            Ft.logger.error(String.format("[%s] : [CLOSE_STOPPER_BR-HV] : [FAILED] - Failed to close stop latch B4. Error Code: %s", getMyBayKey(), ConvErrorCodeMapping.ERROR_CODE_FT_027));
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_027);
        }

        // Structured log for sequence exit
        Ft.logger.info(String.format("[%s] : [CLOSE_STOPPER_BR-HV] : [SEQUENCE_EXIT] - Close stop latch B4 sequence completed. Final Status: %s", getMyBayKey(), bayResponse.getStatus()));
        return bayResponse;
    }
    //============================================================================================================================================
