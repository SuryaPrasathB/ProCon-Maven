...
public class S01_check_for_pallets_at_SCT_NLT_Bay1 implements STA_NoLoadTestBay1State {
    ...
    public BayResponse handleRequest() {
		...
		boolean isPalletAvailableAt_SCT_NLTBay1 = (boolean)responseReturn.get("status");
	    
        while (!isPalletAvailableAt_SCT_NLTBay1 &&
        		(!ConstantConveyor.ALL_LOOP_BREAK_FLAG)) {
        	  STA_NoLoadTestBay1.logger.info("S01_check_for_pallets_at_SCT_NLT_Bay1 : No pallet Available at SCT NLT Bay1");
              BayUtils.delay(1000);
        ...
        if (isPalletAvailableAt_SCT_NLTBay1) {
            STA_NoLoadTestBay1.logger.info("S01_check_for_pallets_at_SCT_NLT_Bay1 : Pallet Available");
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
        } else {
            STA_NoLoadTestBay1.logger.info("S01_check_for_pallets_at_SCT_NLT_Bay1 : Pallet Not Available");
        ...
        while ( (!stableDetection) && (!ConstantConveyor.ALL_LOOP_BREAK_FLAG) 
            ...
            if (isPalletAvailableAt_SCT_NLTBay1) {
            	...
            	if(logEnabled) {
                StaNld_Bay1.logger.info("S01_check_for_pallets_at_SCT_NLT_Bay1 : No pallet Available at SCT NLT Bay1");
            	}
            	BayUtils.delay(1000);
            }
            logEnabled= false;
    ...
    private Map<String,Object> isPalletAvailableAt_SCT_NLTBay1() {
        ...
        if (portInfo != null) {
            ...
            /*StaNld_1.logger.debug("PortId    : " + portInfo.getPortId());
            StaNld_1.logger.debug("ClusterId : " + portInfo.getClusterId());
            StaNld_1.logger.debug("BayId     : " + portInfo.getBayId());*/
            StaNld_Bay1.logger.debug("isPalletAvailableAt_SCT_NLTBay1 : getClusterId: " +portInfo.getClusterId() + " -> getBayId: " + portInfo.getBayId() + " -> getPortId: " + portInfo.getPortId() );
