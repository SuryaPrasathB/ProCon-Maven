...
public class S28_turn_off_motor_STA_NLDT_Bay1 implements STA_NoLoadTestBay1State{
    ...
    public BayResponse handleRequest() {
        StaNld_Bay1.logger.info("S28_turn_off_motor_STA_NLDT_Bay1 : Entry");
        ...
        if (turn_off_divertor_relay_FT_Bay) {
            StaNld_Bay1.logger.info("S28_turn_off_motor_STA_NLDT_Bay1 : Motor Turned Off");
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
        } else {
            StaNld_Bay1.logger.info("S28_turn_off_motor_STA_NLDT_Bay1 : Failed to Turn Off Motor");
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_017);
        }

        StaNld_Bay1.logger.info("S28_turn_off_motor_STA_NLDT_Bay1 : Exit");
        return bayResponse;
    }
    ...
    private Map<String,Object> turn_off() {
        StaNld_Bay1.logger.debug("S28_turn_off_motor_STA_NLDT_Bay1 : turn_off : Entry");
		...
		if (ProconFeatureEnable.MOTOR_CONTROL_ENABLE) {
			...
			if (set_motor_not_required) {
				StaNld_Bay1.logger.info("S28_turn_off_motor_STA_NLDT_Bay2 : turn_off: set_motor_not_required : Success");
				status = true;
				bayResponse.setStatus(true);
				bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
			} else {
				StaNld_Bay1.logger.info("S28_turn_off_motor_STA_NLDT_Bay2 : turn_off : Failed to set_motor_not_required ");
				bayResponse.setStatus(false);
	        ...
	        if (portInfo != null) {
	            ...
	            StaNld_Bay1.logger.debug("S28_turn_off_motor_STA_NLDT_Bay1 : Output port not found");
	        ...
	        StaNld_Bay1.logger.debug("S28_turn_off_motor_STA_NLDT_Bay1 : turn_off : status : " + status);
	        //============================================================================================  
		}
