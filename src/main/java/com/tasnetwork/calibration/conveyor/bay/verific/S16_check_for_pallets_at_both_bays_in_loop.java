...
public class S16_check_for_pallets_at_both_bays_in_loop implements VerificTestBayState {
		...
		while (!stableDetection &&
			...
			if (isPalletAvailableAt_SCT_NLT_Bay1) {
				...
				if (System.currentTimeMillis() - startTimeBay1 >= ConstantConveyor.STABLE_PALLET_TIME_IN_MSEC) {
					...
					Verification.logger
							.info("S16_check_for_pallets_at_both_bays_in_loop : Pallet Available at SCT NLT Bay 1");
			...
			if (isPalletAvailableAt_SCT_NLT_Bay2) {
				...
				if (System.currentTimeMillis() - startTimeBay2 >= ConstantConveyor.STABLE_PALLET_TIME_IN_MSEC) {
					stableDetection = true; // Mark as stable if available for stable time
					Verification.logger
							.info("S16_check_for_pallets_at_both_bays_in_loop : Pallet Available at SCT NLT Bay 2");
					bayResponse.setStatus(true);
			...
			if (!stableDetection) {
				...
				Verification.logger
						.info("S16_check_for_pallets_at_both_bays_in_loop : Checking for pallet availability...");
				BayUtils.delay(1000);
	...
	private Map<String, Object> isPalletAvailableAt_SCT_NLT_Bay1() {
		...
		if (portInfo != null) {
			...
			Verification.logger.debug(
					"S08_check_for_pallets_at_SCT_NLT_Bay1 : isPalletAvailableAt_SCT_NLT_Bay1 : Output port not found");
		...
		Verification.logger
				.debug("S08_check_for_pallets_at_SCT_NLT_Bay1 : isPalletAvailableAt_SCT_NLT_Bay1 : state : " + state);
		...
		Verification.logger
				.debug("S08_check_for_pallets_at_SCT_NLT_Bay1 : isPalletAvailableAt_SCT_NLT_Bay1 : status : " + status);
	...
	private Map<String, Object> isPalletAvailableAt_SCT_NLT_Bay2() {
		Verification.logger
				.debug("S16_check_for_pallets_at_both_bays_in_loop : isPalletAvailableAt_SCT_NLT_Bay1 : Entry");
		...
		if (portInfo != null) {
			...
			Verification.logger.debug(
					"S16_check_for_pallets_at_both_bays_in_loop : isPalletAvailableAt_SCT_NLT_Bay1 : Output port not found");
			return responseReturn;
		}
		...
		Verification.logger.debug(
				"S16_check_for_pallets_at_both_bays_in_loop : isPalletAvailableAt_SCT_NLT_Bay1 : state : " + state);
		...
		Verification.logger.debug(
				"S16_check_for_pallets_at_both_bays_in_loop : isPalletAvailableAt_SCT_NLT_Bay1 : status : " + status);

		responseReturn.put("status", status);

		Verification.logger
				.debug("S16_check_for_pallets_at_both_bays_in_loop : isPalletAvailableAt_SCT_NLT_Bay1 : Exit");
		return responseReturn;
	}
