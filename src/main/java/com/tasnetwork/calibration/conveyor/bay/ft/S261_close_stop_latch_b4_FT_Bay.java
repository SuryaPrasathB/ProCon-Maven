...
/**
 * State class responsible for closing stop latch B4 at the FT Bay.
 */
public class S261_close_stop_latch_b4_FT_Bay implements FtBayState {
    ...
    public BayResponse handleRequest() {
        ...
        Ft.logger.info(String.format("[%s] : [CLOSE_STOPPER_B4-RJ] : [SEQUENCE_ENTRY] - Attempting to close stop latch B4 at FT Bay.", getMyBayKey()));

        BayResponse bayResponse = new BayResponse();
        bayResponse.setStatus(true);
        bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);

        Ft.logger.debug(String.format("[%s] : [CLOSE_STOPPER_B4-RJ] : [DELAY] - Delaying for 2000 ms before latch operation.", getMyBayKey()));
        ...
        Map<String,Object> responseReturn = close_StopLatchB4_FtBay();
        ...
        Ft.logger.debug(String.format("[%s] : [CLOSE_STOPPER_B4-RJ] : [GUI_UPDATE] - Updated GUI status after attempting to close stop latch B4.", getMyBayKey()));

        boolean closeStopLatchB4_FtBay_status = (boolean)responseReturn.getOrDefault("status", false); // Get status safely

        if (closeStopLatchB4_FtBay_status) {
            Ft.logger.info(String.format("[%s] : [CLOSE_STOPPER_B4-RJ] : [SUCCESS] - Stop Latch B4 successfully closed.", getMyBayKey()));
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
            ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayEntryStopper(getMyBayKey(),false);
        } else {
            Ft.logger.error(String.format("[%s] : [CLOSE_STOPPER_B4-RJ] : [FAILED] - Failed to close stop latch B4. Error Code: %s", getMyBayKey(), ConvErrorCodeMapping.ERROR_CODE_FT_027));
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_027);
        }

        // Structured log for sequence exit
        Ft.logger.info(String.format("[%s] : [CLOSE_STOPPER_B4-RJ] : [SEQUENCE_EXIT] - Close stop latch B4 sequence completed. Final Status: %s", getMyBayKey(), bayResponse.getStatus()));
        return bayResponse;
    }
    //============================================================================================================================================

    private Map<String,Object> close_StopLatchB4_FtBay() {
        ...
        testInterfaceStatus.setStateName(ConstantBayStateManage.BAY_HP_SEQ_00); // Consider updating to a more specific state if available for S26
        ...
        if (portInfo != null) {
            ...
            if(status){
                ...
                testInterfaceStatus.setDeviceResponseData("Latch B4 Closed");
            }else{
                ...
                if (!testInterfaceStatus.getDeviceResponseStatus().equals("Error")) {
                    ...
                    testInterfaceStatus.setDeviceResponseData("Latch B4 Not Closed");
                }
            }
