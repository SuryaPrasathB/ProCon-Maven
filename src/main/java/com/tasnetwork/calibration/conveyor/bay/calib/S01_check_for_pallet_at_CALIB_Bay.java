...
public class S01_check_for_pallet_at_CALIB_Bay implements CalibrationBayState {
	...
	/*  String LOW = "OPEN";
    String HIGH = "CLOSE";*/
	//private boolean simulateCalibBayHappyPath = true;
	...
	public BayResponse handleRequest() {
		...
		while (!isPalletAvailableAt_CalibBay &&
				...
				(!CalibrationBay2.isStopProcessRequestedCalibBay())) {   
			CalibrationBay.logger.info("S01_check_for_pallet_at_CALIB_Bay : No pallet Available at CALIB Bay");
			BayUtils.delay(1000);
		...
		if (isPalletAvailableAt_CalibBay) {
			CalibrationBay.logger.info("S01_check_for_pallet_at_CALIB_Bay : Pallet Available");
			bayResponse.setStatus(true);
			bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
		} else {
			CalibrationBay.logger.info("S01_check_for_pallet_at_CALIB_Bay : Pallet Not Available");
	...
	private Map<String,Object>  isPalletAvailableAt_CalibBay() {
		...
		if (portInfo != null) {
			...
			if(logEnabled) {
				Calib.logger.debug("isPalletAvailableAt_CalibBay : getClusterId: " +portInfo.getClusterId() + " -> getBayId: " + portInfo.getBayId() + " -> getPortId: " + portInfo.getPortId() );
			}
		} else {
