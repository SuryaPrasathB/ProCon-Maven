...
public class S02_qR_Code_Scanning_of_Pallet implements CalibrationBayState {
 ...
 /*       setSequencePathId("p1");
        ...
        String do_QR_Code_Scanning_Pallet_CalibBay = (String) responseReturn.get("responseData");

        StateExecutorController.updateTestInterfaceStatusOnGui(responseReturn,ConstantConveyor.COMM_EXECUTION_STATUS_COMPLETED);
        
        String status = do_QR_Code_Scanning_Pallet_CalibBay ; // Call the function to scan pallet QR codes

        if (status.equals("GOOD")) {
            CalibrationBay.logger.info("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Successful");
            ...
            CalibrationBay.logger.info("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Failed");
            CalibrationBay.logger.info("S02_qR_Code_Scanning_of_Pallet : Issue with Pallet Side");
            // Logic for failure case (status is false)
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_CALIB_006);  // Failure error code
        }
        else if(status.equals("SCNR_NW")){
            CalibrationBay.logger.info("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Failed");
            CalibrationBay.logger.info("S02_qR_Code_Scanning_of_Pallet : Issue with Scanner Side");
            // Logic for failure case (status is false)
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_CALIB_006);  // Failure error code
        }
...
/*        TerminalBayProfileModel myTerminalProfile = new TerminalBayProfileModel();
    	...
    	//String scannedData = qrScannerObj.scan_QR_code(ConstantBayPortNameMapping.QR_SCNR_CALIB_BAY_PALLET);
   ...
   //================================= 	
		 ...
		 if(ProconFeatureEnable.CALIB_QR_SCANNER_EXECUTION_PROCESS_IN_PARALLEL){
			 ...
			 int dutWaitTimeDurationMaxInSec = 300;
				...
				while ( (!ProjectExecutionController.getUserAbortedFlag()) && 
					...
					Calib.logger.debug("calibrationTask: dutWaitTimeCounter: "+ dutWaitTimeCounter + "/" + dutWaitTimeDurationMaxInSec+ " : dutAllProcessExecutionCompleted: " + dutAllProcessExecutionCompleted);
				...
				if(dutManager.isDutAllControlProcessCompleted()){
					//if(dutManager.isProjectExitProcess()){
						Calib.logger.debug("calibrationTask: All dut task completed");
						 for(int positionNo =0; positionNo<= maxDeviceConnected; positionNo++){
							 Calib.logger.debug("calibrationTask: result position Id : " + positionNo + " : getDutResultSummary : "  + dutManager.getDutResultSummary(positionNo));
							 Calib.logger.debug("calibrationTask: result position Id : " + positionNo + " : getDutResultResponse : "  + dutManager.getDutResultResponse(positionNo));
	  ...
	  //================================= 	
        ...
        Calib.logger.debug("S02_qR_Code_Scanning_of_Pallet : do_QR_Code_Scanning_Pallet_CalibBay : scannedData : " + scannedData);
