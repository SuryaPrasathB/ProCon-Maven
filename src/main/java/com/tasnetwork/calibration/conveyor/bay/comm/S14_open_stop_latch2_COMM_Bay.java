...
public class S14_open_stop_latch2_COMM_Bay implements CommTestBayState {

    @Override
    public BayResponse handleRequest() {
        Comm.logger.info("S14_open_stop_latch2_COMM_Bay : Entry");
        ...
        if (openStopLatch_COMM_Bay) {
            Comm.logger.info("S14_open_stop_latch2_COMM_Bay : Stop Latch Opened");
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
        } else {
            Comm.logger.info("S14_open_stop_latch2_COMM_Bay : Failed to Open Stop Latch");
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_COMM_008);
        }

        Comm.logger.info("S14_open_stop_latch2_COMM_Bay : Exit");
        return bayResponse;
    }

    private Map<String, Object> open_StopLatch_COMM_Bay() {
        Comm.logger.debug("S14_open_stop_latch2_COMM_Bay : commBay_StopLatch_Status : Entry");
