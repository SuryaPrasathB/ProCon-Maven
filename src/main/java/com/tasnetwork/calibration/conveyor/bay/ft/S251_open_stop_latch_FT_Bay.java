...
/**
 * State class responsible for opening the stop latch at the FT Bay.
 */
public class S251_open_stop_latch_FT_Bay implements FtBayState {
	...
	public BayResponse handleRequest() {
		...
		Ft.logger.info(String.format("[%s] : [OPEN_STOPPER-RJ] : [SEQUENCE_ENTRY] - Attempting to open stop latch at FT Bay.", getMyBayKey()));

		BayResponse bayResponse = new BayResponse();
		bayResponse.setStatus(true);
		bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
		// Removed original delay as it was commented out in the provided code, but if needed, logs should be added.
		// BayUtils.delay(2000); // Consider adding a log if this delay is re-enabled
		...
		Ft.logger.info("S251_open_stop_latch_FT_Bay: Waiting for Halt Pallet at FT-Entry");
		...
		Ft.logger.info("S251_open_stop_latch_FT_Bay: Waiting for Halt Pallet at FT-Exit");
		if(!Ft.isStopProcessRequestedFtBay()) {
			...
			Ft.logger.debug(String.format("[%s] : [OPEN_STOPPER-RJ] : [GUI_UPDATE] - Updated GUI status after attempting to open stop latch.", getMyBayKey()));
	
			boolean openStopLatch_FtBay_status = (boolean) responseReturn.getOrDefault("status", false); // Get status safely
	
			if (openStopLatch_FtBay_status) {
				Ft.logger.info(String.format("[%s] : [OPEN_STOPPER-RJ] : [SUCCESS] - Stop Latch successfully opened.", getMyBayKey()));
				bayResponse.setStatus(true);
				bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
				ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(getMyBayKey(),true);
				// Log pallet movement
				Ft.logger.info(String.format("[%s] : [PALLET_TRACKER] : [SWITCH] - Attempting to switch pallet from %s to %s.", getMyBayKey(), myBayKey, ConstantConveyor.REJECTION_BAY_KEY));
				palletTracker.switchPalletToNextBay(myBayKey, ConstantConveyor.REJECTION_BAY_KEY);
	            
	            // Log screen update action
	            Ft.logger.info(String.format("[%s] : [GUI_UPDATE] : [REJECTION_BAY_SCREEN] - Calling updateRejetionBayScreen().", getMyBayKey()));
				updateRejetionBayScreen();
	
	            Ft.logger.debug(String.format("[%s] : [OPEN_STOPPER-RJ] : [DELAY] - Delaying for 1000 ms after pallet switch.", getMyBayKey()));
				BayUtils.delay(1000);
			} else {
				Ft.logger.error(String.format("[%s] : [OPEN_STOPPER-RJ] : [FAILED] - Failed to open stop latch. Error Code: %s", getMyBayKey(), ConvErrorCodeMapping.ERROR_CODE_FT_027));
				bayResponse.setStatus(false);
				bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_027);
			}
		}
		// Structured log for sequence exit
		Ft.logger.info(String.format("[%s] : [OPEN_STOPPER-RJ] : [SEQUENCE_EXIT] - Open stop latch sequence completed. Final Status: %s", getMyBayKey(), bayResponse.getStatus()));
		return bayResponse;
	}
	//============================================================================================================
	private Map<String, Object> open_StopLatch_FtBay() {
		...
		testInterfaceStatus.setStateName(ConstantBayStateManage.BAY_HP_SEQ_00); // Consider updating to a more specific state if available for S251
		testInterfaceStatus.setcName(ConstantBayPortNameMapping.FT_PORT_NAME_STPR_AT); // Corrected from B4 to AT as it's the target port
		...
		if (portInfo != null) {
			...
			if (status) {
				...
				testInterfaceStatus.setDeviceResponseData("Latch Opened");
			} else {
				...
				if (!testInterfaceStatus.getDeviceResponseStatus().equals("Error")) {
					...
					testInterfaceStatus.setDeviceResponseData("Latch Not Opened");
				}
			}
	...
	public void updateRejetionBayScreen() {
        Ft.logger.debug(String.format("[%s] : [GUI_UPDATE] : [REJECTION_BAY_SCREEN_METHOD] - Executing updateRejetionBayScreen() method.", getMyBayKey()));
		// This method would contain logic to update the GUI specific to the Rejection Bay.
		// For example, refreshing sensor displays or pallet positions on the screen.
	}
