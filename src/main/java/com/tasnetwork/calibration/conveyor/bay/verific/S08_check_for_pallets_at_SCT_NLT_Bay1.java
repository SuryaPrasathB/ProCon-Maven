		...
		if (stableDetectionForNoPallet) {
            ...
            if(ConveyorDataManager.isSta1PalletsAllCleared()){
            	...
            	while ( (!Verification.isStopProcessRequestedVerificBay()) &&
            			...
            			(waitTimeInSec>0) &&
            			!ConstantConveyor.ALL_LOOP_BREAK_FLAG) {
            		Verification.logger.info("S08_check_for_pallets_at_SCT_NLT_Bay1 : STA1: awaiting for Pallet all clear");
            		BayUtils.delay(1000);
            		waitTimeInSec--;
            	}
            	Verification.logger.info("S08_check_for_pallets_at_SCT_NLT_Bay1 : STA1: all clear awiting exit: " +ConveyorDataManager.isSta1PalletsAllCleared());
            
            	if( (waitTimeInSec ==0) && (!ConveyorDataManager.isSta1PalletsAllCleared()) ) {
            		Verification.logger.info("S08_check_for_pallets_at_SCT_NLT_Bay1 : STA1: Timed Out: still Sta1AllPallets not Cleared : isSta1PalletsAllCleared:" +ConveyorDataManager.isSta1PalletsAllCleared());
            		 bayResponse.setStatus(false);
                     bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_013);
                     Verification.logger.info("S08_check_for_pallets_at_SCT_NLT_Bay1 : STA1: Check for Sta2 availability");
            	}
            }
