...
public class CommunicationTestBayReset extends TimerTask{
	...
	private void manageCommunicationTestBayResetStates() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.COMMUNICATION_BAY_KEY, "RESET");//findByBayKey(ConstantConveyor.COMMUNICATION_BAY_KEY);
		...
		if(getTableStatePlanner_CommBay().size()>0) {
			...
			Comm.logger.debug("CommunicationTestBay : manageCommunicationTestBayResetStates : getTableStatePlanner2 : Size : " + getTableStatePlanner_CommBay().size());
			...
			Comm.logger.debug("CommunicationTestBayReset : manageCommunicationTestBayResetStates : No states found in the planner");
		}
...
/*	public static void singleStateTestRun(CommTestBayState currentState){
	...
	public BayResponse processCurrentState(){
		...
		Comm.logger.debug("processCurrentState : " + commTestBayResetManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		Comm.logger.debug("processCurrentState : " + commTestBayResetManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}
