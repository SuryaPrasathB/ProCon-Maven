...
public class LoadingBayReset extends TimerTask{
	//public static Logger logger = Logger.getLogger(LoadingBay.class.getPackage().getName()); 
	private LoadingBayContext loadingBayResetStateManager = new LoadingBayContext();  
	public static boolean abort_Loading_Bay = false ;
	public void run() {
		Loading.logger.debug("LoadingBay2 : Entry"); 

		manageLoadingBayResetStates();
		//DevSysEnergyMeter.sendReadNeutralCurrentCommand();
	}

	private void manageLoadingBayResetStates() {

		Loading.logger.debug("LoadingBayReset : manageLoadingBayResetStates : Entry");

		//setTableStatePlanner_LoadingBay(StatePlannerController.getTableStatePlannerLoadingBay_UI());
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.LOADING_BAY_KEY, "RESET");//findByBayKey(ConstantConveyor.LOADING_BAY_KEY);
		...
		if(getTableStatePlanner_LoadingBay().size()>0) {
			...
			Loading.logger.debug("LoadingBayReset : manageLoadingBayResetStates : getTableStatePlanner2 : Size : " + getTableStatePlanner_LoadingBay().size());
	
	
			while (!Loading.isResetProcessCompletedLoadingBay() &&
			...
			Loading.logger.debug("LoadingBayReset : manageLoadingBayResetStates : getTableStatePlanner2  : No states found in the planner");
		}
		
		Loading.logger.debug("LoadingBayReset : manageLoadingBayResetStates : Exit");
...
/*	public static void singleStateTestRun(LoadingBayState currentState){
	...
	public void setNextState(LoadingBayState newState) {
		...
		// Set the new state
		loadingBayResetStateManager.setState(newState);  
	...
	public BayResponse processCurrentState(){
		// Process the current state
		BayResponse bayStatus = loadingBayResetStateManager.processPresentState();

		Loading.logger.debug("processCurrentState : " + loadingBayResetStateManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		Loading.logger.debug("processCurrentState : " + loadingBayResetStateManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}

	public LoadingBayState getPreviousState(){
		return loadingBayResetStateManager.getLastProcessedBayState();
	}
