...
public class S02_qR_Code_Scanning_of_Pallet_Bay1 implements STA_NoLoadTestBay1State {
	...
	public BayResponse handleRequest() {
		...
		String do_QR_Code_Scanning_Pallet_SctNltBay1 = (String)responseReturn.get("responseData");


		String status = do_QR_Code_Scanning_Pallet_SctNltBay1;  // Call the function to scan pallet QR codes

		if (status.equals("GOOD")) {
			STA_NoLoadTestBay1.logger.info("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Successful");
			...
			STA_NoLoadTestBay1.logger.info("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Failed");
			STA_NoLoadTestBay1.logger.info("S02_qR_Code_Scanning_of_Pallet : Issue with Pallet Side");
			// Logic for failure case (status is false)
			bayResponse.setStatus(false);
			bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_SCT_NLT_BAY1_006);  // Failure error code
		}
		else if(status.equals("SCNR_NW")){
			STA_NoLoadTestBay1.logger.info("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Failed");
			STA_NoLoadTestBay1.logger.info("S02_qR_Code_Scanning_of_Pallet : Issue with Scanner Side");
			// Logic for failure case (status is false)
			bayResponse.setStatus(false);
			bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_SCT_NLT_BAY1_006);  // Failure error code
		}*/
		...
		StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet : awaiting for all pallets movement and exit confirmation from WaitingVerific bay : " +verificToSta1TransitTimeInSec + " Sec");
		while( (verificToSta1TransitTimeInSec>0) && 
			...
			BayUtils.delay(1000);
			StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet : awaiting : verificToSta1TransitTimeInSec: "+ verificToSta1TransitTimeInSec);
			StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet : awaiting : isVerific1PalletsAllCleared(): "+ ConveyorDataManager.isVerific1PalletsAllCleared());
			StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet : awaiting : StaNld_Bay1.isStopProcessRequestedSctNltBay1(): "+ StaNld_Bay1.isStopProcessRequestedStaNldBay1());
			StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet : awaiting : *********************************************"); 
			
		}
		
		StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet :  verificToSta1TransitTimeInSec: "+ verificToSta1TransitTimeInSec);
		StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet :  isVerific1WaitingBayPalletsAllCleared(): "+ ConveyorDataManager.isWaitingVerific1BayPalletsAllCleared());
		StaNld_Bay1.logger.info("S02_qR_Code_Scanning_of_Pallet :  StaNld_Bay1.isStopProcessRequestedSctNltBay1(): "+ StaNld_Bay1.isStopProcessRequestedStaNldBay1());
...
/*		TerminalBayProfileModel terminalBayProfile = new TerminalBayProfileModel();
		...
		terminalBayProfile = MySqlServiceManager.getTerminalProfileSettingService().findByBayKey(ConstantConveyor.STA_NLD1_BAY_KEY);
		...
		StaNld_Bay1.logger.debug("S02_qR_Code_Scanning_of_Pallet : do_QR_Code_Scanning_Pallet_SctNltBay1 : status : " + status);
