...
public class CalibrationBayBypass extends TimerTask {
	...
	public BayResponse processCurrentState() {
		...
		Calib.logger.debug("processCurrentState : " + calibBayBypassStateManager.getState().getClass().getSimpleName()
	...
	public void manageCalibrationBayBypassStates2() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService()
				.findByBayKeyAndExecutionMode(ConstantConveyor.CALIBRATION_BAY_KEY, ConstantStateModes.BAY_BYPASS);
		...
		if (getTableStatePlanner_FtBay().size() > 0) {
			...
			CalibrationBayState currentState = CalibrationBayState.createState(currentStateName); // Create the state instance
			setNextState(currentState); // Set the first state

			Calib.logger.debug("CalibrationBay : manageCalibrationBayBypassStates2 : getTableStatePlanner2 : Size : "
				...
				if (bayStatus.isStatus()) {
					...
					if (nextStateName != null && !nextStateName.isEmpty()) {
						if (nextStateName.equals("S10_error_Handling")) {
							CalibrationBayState nextState2 = CalibrationBayState.createErrorState(nextStateName, errorCode);
