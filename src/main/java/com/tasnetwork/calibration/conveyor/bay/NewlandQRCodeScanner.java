...
public class NewlandQRCodeScanner {
	...
	private boolean qR_code_scanner_init(){
		...
		for (int i = 0; i < ConstantBayPortNameMapping.NUM_OF_QR_CODE_SCANNERS_TERM_1; i++) {
			...
			if (!status) {
				ApplicationLauncher.logger.debug("qR_code_scanner_init: configRS232Comm for : " + scannerID[i] + " Failed");
			}
		}
		...
		for (int i = 0; i < ConstantBayPortNameMapping.NUM_OF_QR_CODE_SCANNERS_TERM_1; i++) {
			...
			if (receivedData == null || receivedData.isEmpty()) {   return false;   }       // Invalid input
	           
  	        if (receivedData.length() == 1) {  return false;   }                            // Not enough data
			...
			if (!status) {
				ApplicationLauncher.logger.debug("qR_code_scanner_init: analog Trigger for : " + scannerID[i] + " Failed");
			}
		}								
	...
	/*public String scan_QR_code(String scannerID){
		...
		//	status =  false;
			ApplicationLauncher.logger.debug("scan_QR_code: " + scannerID +  "Failed : No QR Code Available");
	...
	public String scan_QR_code(int positionNum ){
		...
		if (positionNum == 1) {
			...
			// portInfo = BayUtils.getInputPortDetails(ConstantBayPortNameMapping.FT_PORT_NAME_LDU_SCANNER2_IP ); 
			 receivedData = "TestQr2";
		}
		else if (positionNum == 3) {
			// portInfo = BayUtils.getInputPortDetails(ConstantBayPortNameMapping.FT_PORT_NAME_LDU_SCANNER3_IP ); 
			 receivedData = "TestQr3";
		}
		else if (positionNum == 4) {
			// portInfo = BayUtils.getInputPortDetails(ConstantBayPortNameMapping.FT_PORT_NAME_LDU_SCANNER4_IP ); 
			 receivedData = "TestQr4";
		}
		else if (positionNum == 5) {
			 //portInfo = BayUtils.getInputPortDetails(ConstantBayPortNameMapping.FT_PORT_NAME_LDU_SCANNER5_IP ); 
			 receivedData = "TestQr5";
		}
		else if (positionNum == 6) {
			// portInfo = BayUtils.getInputPortDetails(ConstantBayPortNameMapping.FT_PORT_NAME_LDU_SCANNER6_IP ); 
...
/*		if(DisplayDataObj.ValidateAllComPortAccessible()){
			...
			status = sendReadCommandQrCodeScanner(ANALOG_TRIGGER_SETTING, isResponseExpected, EXPECTED_DATA_IN_HEX);  // ANALOG_TRIGGER_SETTING
		}*/
					...
					try{
						for (int i = 0; i < retry_count; i++ ) {
							...
							if(status) {
								...
								qrData = NewlandQRCodeScanner.extractScannedData((String)responseMap.get("responseData"));
								ApplicationLauncher.logger.debug("analogTriggerV2: qrData2: <"+qrData + "> : Index : " + i);
...
/*		if(DisplayDataObj.ValidateAllComPortAccessible()){
	...
	private boolean triggerStop(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(TRIGGER_STOP_SETTINGS, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // TRIGGER_STOP_SETTINGS
	...
	//============================================================================================	  
		...
		private boolean enableCommandProgramming(String scannerID){
			...
			boolean isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(ENABLE_COMMAND_PROGRAMMING, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // ENABLE_COMMAND_PROGRAMMING
	...
	private boolean disableCommandProgramming(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(DISABLE_COMMAND_PROGRAMMING, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // DISABLE_COMMAND_PROGRAMMING
	...
	private boolean enable1dBarCode(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(ENABLE_1D_BAR_CODES, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);  // ENABLE_1D_BAR_CODES
	...
	private boolean disable1dBarCode(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(DISABLE_1D_BAR_CODES, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // DISABLE_1D_BAR_CODES
	...
	private boolean enable2dBarCode(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(ENABLE_2D_BAR_CODES, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // ENABLE_2D_BAR_CODES
	...
	private boolean disable2dBarCode(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(DISABLE_2D_BAR_CODES, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // DISABLE_2D_BAR_CODES
	...
	private boolean enableAllBarCode(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(GOOD_READ_BEEP_ENABLE, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // GOOD_READ_BEEP_ENABLE
	...
	private boolean disableAllBarCode(String scannerID){
		...
		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(GOOD_READ_BEEP_DISABLE, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX); // GOOD_READ_BEEP_DISABLE
	...
	//============================================================================================	
		private boolean enableGoodReadBeep(String scannerID){
			...
			boolean isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(ENABLE_ALL_BAR_CODES, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // ENABLE_ALL_BAR_CODES
		...
		private boolean disableGoodReadBeep(String scannerID){
			...
			boolean isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(DISABLE_ALL_BAR_CODES, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // DISABLE_ALL_BAR_CODES
		...
		private boolean saveAsUserDefault(String scannerID){
			...
			boolean isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(SAVE_AS_USER_DEFAULT, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // SAVE_AS_USER_DEFAULT
	...
	private boolean configRS232Comm(String scannerID, String baudRate, String verificType, String dataBits, String numOfStopBits){
		ApplicationLauncher.logger.debug("configRS232Comm : Entry");

		boolean status =  false;

		boolean isResponseExpected = true;
		status = sendReadCommandQrCodeScanner(BAUD_RATE_9600_BPS, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // BAUD_RATE_9600_BPS

		if (status) {
			BayUtils.delay(10);
			 isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(RS232_NO_VERIFY, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // RS232_NO_VERIFY
		} else {
			status =  false;
		...
		if (status) {
			BayUtils.delay(10);
			 isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(RS232_8_DIGITS, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // RS232_8_DIGITS
		} else {
			status =  false;
		...
		if (status) {
			BayUtils.delay(10);
			 isResponseExpected = true;
			status = sendReadCommandQrCodeScanner(RS232_ONE_STOP_BIT, isResponseExpected, NOT_GOOD_READ_EXPECTED_DATA_IN_HEX);   // RS232_ONE_STOP_BIT
		} else {
			status =  false;
...
/*		String addressStr = BofaManager.asciiToHex(String.valueOf((char)(address + ConstantPowerSourceBofa.LDU_ADDRESS_ADDITION))) ;
		...
		String responseStatus = pwrSrcBofaMessenger.qrMsngrSendCommandProcess(payLoadInHex,timeDelayInMilliSec,expectedDataInHex,error1ExpectedDataInHex,isResponseExpected,"test-T1");
		...
		if (responseStatus.equals(DeleteMeConstant.SUCCESS_RESPONSE)) {
			...
			if(!isResponseExpected){
				if(responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
					ApplicationLauncher.logger.info("sendDataToBofaAfterSemaPhoreAcquired : no response expected success");
					status = true;
				}
			}
		}
	...
	public static String extractScannedData(String hexData) {
	    ...
	    String[] hexBytes = hexData.trim().split("(?<=\\G..)"); // Split every 2 characters
	  //  ApplicationLauncher.logger.info("extractScannedData: hexBytes : " + Arrays.toString(hexBytes));

	    // Handle the case where only the start byte '06' is received
	    //if (hexBytes.length == 1 && hexBytes[0].equalsIgnoreCase(NOT_GOOD_READ_EXPECTED_DATA_IN_HEX)){//"06")) {
	    /*if (hexBytes.length == 1 && hexBytes[0].equalsIgnoreCase(NOT_GOOD_READ_EXPECTED_DATA_IN_HEX)){//"06")) {
	        ApplicationLauncher.logger.info("extractScannedData: Only start byte '06' received. Returning '06' directly.");
	        return NOT_GOOD_READ_EXPECTED_DATA_IN_HEX; // Return just the start byte if that's the only data
	    }*/
	    
	    if(hexData.equals(NOT_GOOD_READ_EXPECTED_DATA_IN_HEX)){
	    	return GUIUtils.hexToAsciiV2(NOT_GOOD_READ_EXPECTED_DATA_IN_HEX.replace(DeleteMeConstant.HV_ER_TERMINATOR, ""));
	    }
	    ...
	    // Verify terminators
/*	    if (!hexBytes[0].equalsIgnoreCase(NOT_GOOD_READ_EXPECTED_DATA_IN_HEX)|| //"06") ||
	        !hexBytes[hexBytes.length - 2].equalsIgnoreCase("0D") ||
	        !hexBytes[hexBytes.length - 1].equalsIgnoreCase("0A")) {
	        ApplicationLauncher.logger.info("extractScannedData: Invalid format or terminator");
	        return null; // Invalid format
	    }*/

	    // Calculate data length and extract data bytes
/*	    int dataLength = hexBytes.length - ((GOOD_READ_EXPECTED_BEGIN_DATA_IN_HEX.length()/2) + 2); // Exclude 06 (start byte) and 0D 0A (terminators)
	    ApplicationLauncher.logger.info("extractScannedData: Data length : " + dataLength);

	    byte[] scannedBytes = new byte[dataLength];
	    for (int i = 0; i < dataLength; i++) {
	        try {
	           // scannedBytes[i] = (byte) Integer.parseInt(hexBytes[i + 1], 16); // Start after 06
	            scannedBytes[i] = (byte) Integer.parseInt(hexBytes[i], 16); // Start after 06
	        } catch (NumberFormatException e) {
	            ApplicationLauncher.logger.error("extractScannedData: Invalid hex byte at index " + (i + 1) + " : " + hexBytes[i + 1], e);
	            return null; // Invalid data
	        }
	    }*/

	   // ApplicationLauncher.logger.info("extractScannedData: Extracted bytes : " + Arrays.toString(scannedBytes));

	    // Convert bytes to string
	    String result = GUIUtils.hexToAsciiV2(hexData.replace(GOOD_READ_EXPECTED_BEGIN_DATA_IN_HEX, "")); // new String(scannedBytes, StandardCharsets.UTF_8);
