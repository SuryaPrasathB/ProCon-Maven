...
public class S22_close_stop_latch2_Verific_Bay implements VerificTestBayState {
    ...
    public BayResponse handleRequest() {
    	...
    	Verification.logger.info("S22_close_stop_latch2_Verific_Bay : Entry");
        ...
        if (closeStopLatch_Verific_Bay) {
        	Verification.logger.info("S22_close_stop_latch2_Verific_Bay : Stop Latch Closed");
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
            
            BayUtils.delay(3000);
        } else {
        	Verification.logger.info("S22_close_stop_latch2_Verific_Bay : Failed to Close Stop Latch");
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_020 );
        }
        
        Verification.logger.info("S22_close_stop_latch2_Verific_Bay : getPalletsPassedVerificBay()" + S19_open_stop_latch_Verific_Bay.getPalletsPassedVerificBay());

        if (S19_open_stop_latch_Verific_Bay.getPalletsPassedVerificBay() == ConstantConveyor.NUM_PALLETS_VERFIC_BAY) {
        	Verification.logger.info("S22_close_stop_latch2_Verific_Bay : All pallets passed out of Verific Bay");
        	bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_022 );
            S19_open_stop_latch_Verific_Bay.setPalletsPassedVerificBay(0);
            ...
            Verification.logger.info("S22_close_stop_latch2_Verific_Bay : palletMoveMentverificToSta1 : palletMoveMentWaiting stop:");
            ...
            if (ProconFeatureEnable.MOTOR_CONTROL_ENABLE) {
				...
				if (set_motor_not_required) {
					Verification.logger
							.info("S22_close_stop_latch2_Verific_Bay : set_motor_not_required : Success");
					bayResponse.setStatus(false);
		            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_022 );
				} else {
					Verification.logger
							.info("S22_close_stop_latch2_Verific_Bay : Failed to set_motor_not_required ");
					bayResponse.setStatus(false);
        ...
        Verification.logger.info("S22_close_stop_latch2_Verific_Bay : Exit");
        return bayResponse;
    }

    private Map<String, Object> close_StopLatch_Verific_Bay() {
    	Verification.logger.debug("S22_close_stop_latch2_Verific_Bay : verificBay_StopLatch_Status : Entry");
        ...
        if (portInfo != null) {
            ...
            Verification.logger.debug("S22_close_stop_latch2_Verific_Bay : close_StopLatch_Verific_Bay : state : " + state);
            ...
            Verification.logger.debug("S22_close_stop_latch2_Verific_Bay : close_StopLatch_Verific_Bay : status : " + status);

        } else {
        	Verification.logger.debug("S22_close_stop_latch2_Verific_Bay : Output port not found");
        ...
        Verification.logger.debug("S22_close_stop_latch2_Verific_Bay : verificBay_StopLatch_Status : status : " + status);
        Verification.logger.debug("S22_close_stop_latch2_Verific_Bay : verificBay_StopLatch_Status : Exit");
        return responseReturn;
    }
}
