...
public class CalibrationBayStop extends TimerTask{
	//public static Logger logger = Logger.getLogger(CalibrationBay.class.getPackage().getName()); 
	private CalibrationBayContext calibBayStopStateManager = new CalibrationBayContext();  


	public static boolean abort_Calib_Bay = false ;
	public void run() {
		Calib.logger.debug("CalibrationBayStop : Entry"); 

		manageCalibrationBayStopStates();
		//DevSysEnergyMeter.sendReadNeutralCurrentCommand();
	}

	private void manageCalibrationBayStopStates() {

		Calib.logger.debug("CalibrationBayStop : manageCalibrationBayStopStates : Entry");

		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.CALIBRATION_BAY_KEY, "STOP");//findByBayKey(ConstantConveyor.CALIBRATION_BAY_KEY);
		...
		if(getTableStatePlanner_CalibBay().size()>0) {
			...
			Calib.logger.debug("CalibrationBayStop : manageCalibrationBayStopStates : getTableStatePlanner2 : Size : " + getTableStatePlanner_CalibBay().size());
			...
			Calib.logger.debug("CalibrationBayStop : manageCalibrationBayStopStates : No states found in the planner");
		}
		Calib.logger.debug("CalibrationBayStop : manageCalibrationBayStopStates : Exit");
	...
	public void setNextState(CalibrationBayState newState) {
		...
		// Set the new state
		calibBayStopStateManager.setState(newState);  
	...
	public BayResponse processCurrentState(){
		// Process the current state
		BayResponse bayStatus = calibBayStopStateManager.processPresentState();

		Calib.logger.debug("processCurrentState : " + calibBayStopStateManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		Calib.logger.debug("processCurrentState : " + calibBayStopStateManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}

	public CalibrationBayState getPreviousState(){
		return calibBayStopStateManager.getLastProcessedBayState();
	}
