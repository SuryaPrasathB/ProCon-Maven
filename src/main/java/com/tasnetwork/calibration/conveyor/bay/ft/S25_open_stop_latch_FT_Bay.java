 ...
 */
public class S25_open_stop_latch_FT_Bay implements FtBayState {
    ...
    public BayResponse handleRequest() {
        ...
        Ft.logger.info(String.format("[%s] : [OPEN_STOPPER-HV] : [SEQUENCE_ENTRY] - Attempting to open stop latch at FT Bay.", getMyBayKey()));
        ...
        Ft.logger.info("S25_open_stop_latch_FT_Bay: Waiting for Halt Pallet at FT-Entry");
		...
		Ft.logger.info("S25_open_stop_latch_FT_Bay: Waiting for Halt Pallet at FT-Exit");
		if(!Ft.isStopProcessRequestedFtBay()) {
	        ...
	        Ft.logger.debug(String.format("[%s] : [OPEN_STOPPER-HV] : [GUI_UPDATE] - Updated GUI status after attempting to open stop latch.", getMyBayKey()));
	        ...
	        if (openStopLatch_FtBay_status) {
	            Ft.logger.info(String.format("[%s] : [OPEN_STOPPER-HV] : [SUCCESS] - Stop Latch successfully opened.", getMyBayKey()));
	            bayResponse.setStatus(true);
	            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
	            ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(getMyBayKey(),true);
	            // Log pallet movement
	            Ft.logger.info(String.format("[%s] : [PALLET_TRACKER] : [SWITCH] - Attempting to switch pallet from %s to %s.", getMyBayKey(), myBayKey, ConstantConveyor.HV_BAY_KEY));
	            palletTracker.switchPalletToNextBay(myBayKey, ConstantConveyor.HV_BAY_KEY);
	            Ft.logger.debug(String.format("[%s] : [PALLET_TRACKER] : [DELAY] - Delaying for 1000 ms after pallet switch.", getMyBayKey()));
	            BayUtils.delay(1000);
	        } else {
	            Ft.logger.error(String.format("[%s] : [OPEN_STOPPER-HV] : [FAILED] - Failed to open stop latch. Error Code: %s", getMyBayKey(), ConvErrorCodeMapping.ERROR_CODE_FT_027));
        ...
        Ft.logger.info(String.format("[%s] : [OPEN_STOPPER-HV] : [SEQUENCE_EXIT] - Open stop latch sequence completed. Final Status: %s", getMyBayKey(), bayResponse.getStatus()));
        return bayResponse;
    }
...
//============================================================================================================
    ...
    private Map<String, Object> open_StopLatch_FtBay() {
        ...
        testInterfaceStatus.setStateName(ConstantBayStateManage.BAY_HP_SEQ_00); // Consider updating to a more specific state if available for S25
