...
public class S043_Get_Test_Point_Status implements VerificTestBayState  {
	...
	// --- Simulation Flags ---
	// Set this to "NONE", "CIRCUIT_OPEN", or "CONTIGUOUS_BATCH" for simulation testing
	...
	/**
	 * Private static inner class to mock ProcalRemoteResponse for simulation purposes.
	 * This allows us to control the behavior of getTestPointStatus() and getAllResultsJson()
	 * without requiring actual remote calls or complex constructors of the real ProcalRemoteResponse.
	 * Assumes ProcalRemoteResponse has an accessible default (no-argument) constructor.
	 */
	private static class MockProcalRemoteResponse extends ProcalRemoteResponse {
		...
		public MockProcalRemoteResponse(TestPointStatus statusToReturn) {
			...
			// This will compile if ProcalRemoteResponse has a public or protected no-arg constructor.
			// If ProcalRemoteResponse only has parameterized constructors, this line might need adjustment
			// to call a valid super constructor, potentially with 'null' or dummy arguments.
			super();
	...
	public BayResponse handleRequest() {
		...
		if (!SIMULATION_MODE.equals(MODE_NONE)) {
			...
			initialSimulatedStatus.setCurrentCircuitOpenDetectedInBatch(SIMULATION_MODE.equals(MODE_CIRCUIT_OPEN)); // Set based on mode
			...
			Verification.logger.info("S043_Get_Test_Point_Status : Initializing myProcalRemoteResponse in simulation mode.");
		} else {
			...
			Verification.logger.info("S043_Get_Test_Point_Status : getPresentTestPointId-1 : " + myProcalRemoteResponse.getTestPointStatus().getPresentTestPointId());
			Verification.logger.info("S043_Get_Test_Point_Status : getCompletedTestPointCount-1 : " + myProcalRemoteResponse.getTestPointStatus().getCompletedTestPointCount());
			Verification.logger.info("S043_Get_Test_Point_Status : getTotalTestPointCount-1 : " + myProcalRemoteResponse.getTestPointStatus().getTotalTestPointCount());
			Verification.logger.info("S043_Get_Test_Point_Status : getTotalTestingTimeInSec-1 : " + myProcalRemoteResponse.getTestPointStatus().getTotalTestingTimeInSec());
			
		}

		boolean isPalletToBeReleasedToNextBay = true;
		String message = "Current circuit open detected";
		int  delayCount = 10;
		// Loop until all tests completed, stop process requested, or a critical batch failure is detected
		while (!(myProcalRemoteResponse.getTestPointStatus().isAllTestExecutionCompleted()) &&
			...
			if (!SIMULATION_MODE.equals(MODE_NONE)) {
				...
				if (simulationLoopCounter >= 3 && SIMULATION_MODE.equals(MODE_NONE)) { // Only simulate completion in NONE for real scenario
					simulatedStatus.setAllTestExecutionCompleted(true);
					Verification.logger.info("S043_Get_Test_Point_Status : Simulating all test execution completed (Loop: " + simulationLoopCounter + ").");
				} else if (simulationLoopCounter >= 3 && !SIMULATION_MODE.equals(MODE_NONE)){ // for simulation, make it complete to exit loop
					simulatedStatus.setAllTestExecutionCompleted(true);
					Verification.logger.info("S043_Get_Test_Point_Status : Simulating all test execution completed for simulation mode (Loop: " + simulationLoopCounter + ").");
				}
				else {
				...
				if (SIMULATION_MODE.equals(MODE_CIRCUIT_OPEN)) {
					// This case should ideally be handled by the initial check, but ensures it's set
					simulatedStatus.setCurrentCircuitOpenDetectedInBatch(true);
					Verification.logger.info("S043_Get_Test_Point_Status : Simulating CIRCUIT_OPEN detection inside loop.");
					...
					if (contiguousFaultCounter >= 2) { // Simulate after 2 polling attempts
						...
						Verification.logger.info("S043_Get_Test_Point_Status : Simulating CONTIGUOUS_BATCH fault detection (Count: " + contiguousFaultCounter + ").");
					}
				}
				myProcalRemoteResponse = new MockProcalRemoteResponse(simulatedStatus); // Update mock response for this iteration
			} else {
				// Normal operation: send actual request to Procal
				myProcalRemoteResponse = procalRemoteSender.sendCommResultRefreshToProcal(verificClusterServer, endpoint);
			}

			// Check for current circuit open detected in batch - this is a critical failure that stops the process
			if (myProcalRemoteResponse.getTestPointStatus().isCurrentCircuitOpenDetectedInBatch()) {
				Verification.logger.error("S043_Get_Test_Point_Status : Current circuit open detected in batch. Setting batchFailedRelayOpen flag and exiting loop.");
				batchFailedRelayOpenOneTime = true;
				message = "Current circuit open detected";
				isPalletToBeReleasedToNextBay= releasePalletToNextBayForBatchFailed(message);


				break; // Exit the while loop
			}

			// Check for contiguous batch fault - this prompts the user with a UI alert and waits for interaction
			if (myProcalRemoteResponse.getTestPointStatus().isFaultDetectedInContiguousBatch()) {
				Verification.logger.warn("S043_Get_Test_Point_Status : ALERT: Two batches continuous failure detected. Showing UI prompt.");
				batchFailedRelayOpenContiguous = true;
				/*				// Use CountDownLatch to wait for UI interaction if running in a non-UI thread
				final CountDownLatch latch = new CountDownLatch(1);

				Platform.runLater(() -> {
					try {
						Alert alert = new Alert(AlertType.WARNING); // Use WARNING type for the alert
						alert.setTitle("Action Required - Batch Failure");
						alert.setHeaderText(null); // No header text
						alert.setContentText("Please check source and meter conditions in VERIFICATION BAY. Two batches continuous failure detected.");

						// Set custom button for "Start Test"
						ButtonType startTestButton = new ButtonType("Start Test");
						alert.getButtonTypes().setAll(startTestButton); // Replace default buttons with "Start Test"

						// Optionally add an icon to the dialog stage
						// Note: ConstantVersion and image path are assumed to be correctly set up
						...
						// Show the dialog and wait for user interaction (OK button)
						Optional<ButtonType> result = alert.showAndWait();

						// Latch counts down when the alert is dismissed (e.g., user clicks Start Test or closes it)
						latch.countDown();

					} catch (Exception e) {
						Verification.logger.error("S043_Get_Test_Point_Status : Failed to display batch failure alert dialog: " + e.getMessage(), e);
						latch.countDown(); // Release the latch even on error to prevent indefinite blocking
					}
				});

				try {
					latch.await(); // Wait for the UI thread to finish showing the alert and user interaction
				} catch (InterruptedException e) {
					Verification.logger.error("S043_Get_Test_Point_Status : Interrupted while waiting for UI alert: " + e.getMessage(), e);
					Thread.currentThread().interrupt(); // Restore the interrupted status
				}*/

				/*boolean isPalletToBeReleasedToNextBay= releasePalletToNextBayForBatchFailed();
				...
				/*				Verification.logger.info("S043_Get_Test_Point_Status : UI alert dismissed. Attempting to restart test.");

				// Action on "Start Test" after UI alert is dismissed - Send start command to Procal
				String procalServerResponse;
				if (SIMULATION_MODE.equals(MODE_NONE)) {
					procalServerResponse = procalRemoteSender.sendStartCommandToProcal(verificClusterServer, endpoint);
				} else {
					// Simulate start command response in simulation modes
					procalServerResponse = "startTestExecuteInitiated"; // Always succeed in simulation for simplicity
					Verification.logger.info("S043_Get_Test_Point_Status : Simulating 'startTestExecuteInitiated' response.");
				}


				if (procalServerResponse.equals("startTestExecuteInitiated")) {
					BayUtils.delay(1000);
					Verification.logger.info("S043_Get_Test_Point_Status : Start Execution Success after contiguous fault and user acknowledgement. Resuming polling.");
					contiguousFaultCounter = 0; // Reset counter after simulated restart
					BayUtils.delay(10000); // Add delay after successful start command
				} else {
					Verification.logger.error("S043_Get_Test_Point_Status : Start Execution Failed after contiguous fault and user acknowledgement. Continuing polling, but test might be stuck.");
				}*/
			...
			Verification.logger.info("S043_Get_Test_Point_Status : getPresentTestPointId : " + myProcalRemoteResponse.getTestPointStatus().getPresentTestPointId());
			Verification.logger.info("S043_Get_Test_Point_Status : getCompletedTestPointCount : " + myProcalRemoteResponse.getTestPointStatus().getCompletedTestPointCount());
			Verification.logger.info("S043_Get_Test_Point_Status : getTotalTestPointCount : " + myProcalRemoteResponse.getTestPointStatus().getTotalTestPointCount());
			Verification.logger.info("S043_Get_Test_Point_Status : getTotalTestingTimeInSec : " + myProcalRemoteResponse.getTestPointStatus().getTotalTestingTimeInSec());
			Verification.logger.info("S043_Get_Test_Point_Status : presentTpResult : " + presentTpResult);
			//int totalTestPoint = presentTpResult.stream().filter(e->e.getPositionId() != ConstantApp.LIVE_TABLE_EXECUTION_STATUS_ID)
			//											.map(mapper)
		...
		if (batchFailedRelayOpenOneTime || batchFailedRelayOpenContiguous) {
			Verification.logger.info("S043_Get_Test_Point_Status : batchFailedRelayOpenOneTime: " + batchFailedRelayOpenOneTime);
			Verification.logger.info("S043_Get_Test_Point_Status : batchFailedRelayOpenContiguous: " + batchFailedRelayOpenContiguous);
			...
			if(batchFailedRelayOpenContiguous) {
				Verification.logger.info("S043_Get_Test_Point_Status : Execution Failed due to BATCH FAILED - Relay Open - multiple time");
				...
				Verification.logger.info("S043_Get_Test_Point_Status : Execution Failed due to BATCH FAILED - Relay Open - one time");
				bayResponse.setStatus(false);
				
				if (userAbortedPalletRelease) {
					bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_033); // Using existing failure code
					userAbortedPalletRelease = false;
				}
				else {
					bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_030); // Using existing failure code
				}
			}
			...
			if (SIMULATION_MODE.equals(MODE_NONE)) {
				...
				while ( (retryCount>0) && 
					...
					if(myProcalRemoteResult.contains(expectedContent)){
						if(myProcalRemoteResult.length() > 40 ) {
							...
							Verification.logger.info("S043_Get_Test_Point_Status : length not sufficient: "+ myProcalRemoteResult);
						}
					}
				}	
				...
				// In simulation, return a dummy JSON array for all results, for testing processing
				myProcalRemoteResult = "{\"Results\":[]}"; // You can populate this with dummy pass/fail results as needed for testing
				Verification.logger.info("S043_Get_Test_Point_Status : Simulating empty JSON results for all results call.");
			}
			...
			if(expectedResultRecieved){
				if(PROCESS_RESULT_WITH_DUT_SERIAL_NO_MAPPING_ENABLED) {
					Verification.logger.info("S043_Get_Test_Point_Status : executing result processing with dutSerialNo");
					
					
					addTestPointMeterResults(myProcalRemoteResult, false);
				
				}else {
					Verification.logger.info("S043_Get_Test_Point_Status : executing result processing with lduPositionMapping");
			...
			Verification.logger.info("S043_Get_Test_Point_Status : Execution Failed (e.g., due to stop process request or ALL_LOOP_BREAK_FLAG being set)");
			bayResponse.setStatus(false);
	...
	private boolean releasePalletToNextBayForBatchFailed(String message) {
		...
		if (ProconFeatureEnable.VERIFIC_BATCH_FAILED_USER_PROMPT_ENABLED) {
			...
			//Verification.logger.debug("releasePalletToNextBayForBatchFailed : setCALIB_CURRENT_STABLE: false");
			Platform.runLater(()->{
				...
				dialog.resultProperty().addListener((obs, oldVal, newVal) -> {
					if (Boolean.TRUE.equals(newVal)) {
						...
						Verification.logger.debug("releasePalletToNextBayForBatchFailed: release pallet prompt user hit: YES");
					} else {
						ConveyorDataManager.setVerific1BatchFailedReleasePalletRequested(false);
					...
					Verification.logger.debug("releasePalletToNextBayForBatchFailed: setVerific1BatchFailedPromptUserInputReceived: true");
				});
			...
			Verification.logger.debug("releasePalletToNextBayForBatchFailed : awaiting for user input for power stable: Entry");
			...
			if(Verification.isStopProcessRequestedVerificBay()) {
				...
				Verification.logger.debug("releasePalletToNextBayForBatchFailed : stop requested: releaseMeterForNextBay: " + releaseMeterForNextBay);
			}else {
				releaseMeterForNextBay = ConveyorDataManager.isVerific1BatchFailedReleasePalletRequested();
				Verification.logger.debug("releasePalletToNextBayForBatchFailed :  releaseMeterForNextBay: " + releaseMeterForNextBay);

			}

			Verification.logger.debug("releasePalletToNextBayForBatchFailed : awaiting for user input for power stable: Exit");
	 ...
	 * a specific batch failure (relay open) where all positions are marked as failed.
	 *
	 * @param myProcalRemoteResult The JSON string containing test results from Procal. Null if isBatchFailedRelayOpen is true.
	 * @param isBatchFailedRelayOpen A boolean flag indicating if the call is for a batch failure due to relay open.
	 */
	private void addTestPointMeterResults(String myProcalRemoteResult, boolean isBatchFailedRelayOpen) {
		...
		try {
			...
			if (myPalletManageList == null || myPalletManageList.isEmpty()) {
				Verification.logger.warn("S043_Get_Test_Point_Status : No active pallets found for bay key: " + presentBayKey + ". Cannot add results.");
				// If no pallets are found, we cannot proceed with result addition, even for batch fail.
			...
			if (isBatchFailedRelayOpen) {
				Verification.logger.info("S043_Get_Test_Point_Status : Handling BATCH FAILED - Relay Open scenario for all 24 positions.");
				String batchFailMessage = ConstantConveyor.REASON_RESULT_BATCH_FAILED;//"BATCH FAILED - Relay Open in VERIFIC";
				String testType = ConstantConveyor.VERIFICATION_RESULT_KEY; // Using existing constant for test type
				String testCaseName = ConstantConveyor.SUMMARY_VERIFICATION_RESULT_TEST_NAME; // Using existing constant for summary test case name
				...
				for (int devicePos = 1; devicePos <= 24; devicePos++) {
					...
					if (mappedPosition != -1 && selectedPallet != null) {
						Verification.logger.info("S043_Get_Test_Point_Status : Marking position " + devicePos + " (mapped to " + mappedPosition + " on pallet " + selectedPallet.getPalletQrId() + ") as BATCH FAILED.");
						...
						Verification.logger.warn("S043_Get_Test_Point_Status : Could not map position " + devicePos + " for batch failure. Skipping result marking for this position.");
					}
				}
			...
			if (myProcalRemoteResult == null || myProcalRemoteResult.isEmpty()) {
				Verification.logger.warn("S043_Get_Test_Point_Status : Normal processing: Received null or empty result string. Nothing to process.");
				return;
			}
...
/*				procalResult = JsonPath
			...
			Verification.logger.info("S043_Get_Test_Point_Status : myPalletManageList : size:" + myPalletManageList.size());

			// Map to track if all tests for a given device passed
			Map<Integer, Boolean> devicePassStatus = new HashMap<>();

			// First pass: Determine overall pass/fail status for each device based on all its test cases
			...
			for (Result eachResult: procalResult.getResults()) {
				try {
					...
					e.printStackTrace();
					Verification.logger.error("S043_Get_Test_Point_Status procalResult : Exception-for-1: " + e.getMessage());
			...
			List<String> mandatoryTestPointList = DeviceDataManagerController.getConveyorConfigParsedKey().getReportPrintDetailedTestPointWhiteList();

			// Group results by device for easier validation
			Map<Integer, List<Result>> resultsByDevice = procalResult.getResults().stream()
...
/*			    List<Result> deviceResults = resultsByDevice.getOrDefault(lduPosition, Collections.emptyList());
			 ...
			 // Get existing test names for this device
			    ...
			    for (String whiteListPattern : mandatoryTestPointList) {
			        ...
			        if (!patternMatched) {
			            ...
			            //Verification.logger.warn("Device at LDU position {} missing required test pattern: {}", 
			...
			// Second pass: Process individual test results and add them to the pallet tracker
			int lduPositionNo = 0;
			...
			//for (int verificDataCount = 0; verificDataCount < procalResult.getResults().size(); verificDataCount++) {
			for (Result eachResult: procalResult.getResults()) {
				...
				try {
					...
					// Skip if this test case for this device was already processed (to handle potential duplicates in Procal response)
					if (!processedTestCases.add(uniqueKey)) {
						Verification.logger.warn("S043_Get_Test_Point_Status : addTestPointMeterResults : Skipping duplicate test case: " + uniqueKey);
					...
					// Determine test type based on constant configuration (e.g., "LOE" if starts with "ACCURACY_ALIAS_NAME")
					testType = testCaseName.startsWith(ConstantConveyor.ACCURACY_ALIAS_NAME) ? ConstantConveyor.VERIFICATION_RESULT_KEY : "";

					//MappedResult mappedResult = getMappedPosition(lduPositionNo, pos, myPalletManageList);
					//int mappedPosition = mappedResult.getMappedPosition();
					//PalletManage selectedPallet = mappedResult.getSelectedPallet();
					if(palletRackpositionNo!=0){
						//if (mappedPosition != -1 && selectedPallet != null) {
						//Verification.logger.info("S043_Get_Test_Point_Status : addTestPointMeterResults : Adding result for Meter: " + lduPositionNo + " (Mapped: " + palletRackpositionNo + ")");
						...
						palletTracker.addResultToPalletMeterV1_1(palletRackpositionNo,  dutSerialNo , resultStatus,  resultValue,  getMyBayKey(), palletDistinctId,  testType,  testCaseName,error_min, error_max );
					}else{
						Verification.logger.info("S043_Get_Test_Point_Status : palletRackpositionNo : skipping : " + palletRackpositionNo );
					}

					/*				} else {
						Verification.logger.warn("S043_Get_Test_Point_Status : addTestPointMeterResults : No matching pallet found for position: " + lduPositionNo + ". Skipping result addition.");
					}*/
				}catch(Exception e) {
					e.printStackTrace();
					Verification.logger.error("S043_Get_Test_Point_Status procalResult : Exception-for-2: " + e.getMessage());
			...
			for (Map.Entry<Integer, Boolean> entry : devicePassStatus.entrySet()) {
				try {
					...
					/*MappedResult mappedResult = getMappedPosition(lduPositionNoKey, pos, myPalletManageList);
					int mappedPosition = mappedResult.getMappedPosition();
					PalletManage selectedPallet = mappedResult.getSelectedPallet();*/
					//int lduPositionNoKey
					Optional<Result> procalResultOpt= procalResult.getResults().stream().filter(e->e.getDevice_name().equals(String.valueOf(lduPositionNoKey)))
							.findFirst();
					...
					if (procalResultOpt.isPresent()) {
						...
						Verification.logger.info("S043_Get_Test_Point_Status : lduPositionNoKey : " + lduPositionNoKey + " -> dutSerialNo :" + dutSerialNo+  " -> palletRackpositionNo : " + palletRackpositionNo +" -> finalResultValue : <" + finalResultValue +"> , finalResultValue : <" + finalResultValue + "> , palletDistinctId: " + palletDistinctId  );
						
						//Verification.logger.info("S043_Get_Test_Point_Status : addTestPointMeterResults : Adding summary for Meter : " + palletRackpositionNo + " (Pallet: " + palletDistinctId + ")");
						...
						palletTracker.addMeterResultSummaryWithPalletDetailsV2(palletRackpositionNo,dutSerialNo, finalResultStatus, finalResultValue, testCaseName,  testType, palletDistinctId);


					} else {
						Verification.logger.warn("S043_Get_Test_Point_Status : addTestPointMeterResults : No matching pallet found for lduPositionNo: " + lduPositionNoKey + ". Skipping summary addition.");
					}

				}catch(Exception e) {
					e.printStackTrace();
					Verification.logger.error("S043_Get_Test_Point_Status procalResult : Exception-for-3: " + e.getMessage());

				}
			}

			/*} catch (org.json.JSONException e) {
			Verification.logger.error("Error parsing JSON in normal processing: " + e.getMessage(), e);
			 */
		} catch (Exception e) {
			Verification.logger.error("S043_Get_Test_Point_Status: Exception: An unexpected error occurred in addTestPointMeterResults: " + e.getMessage());
		}
	...
	private void addTestPointMeterResultsWithLduPosition(String myProcalRemoteResult, boolean isBatchFailedRelayOpen) {
		...
		try {
			...
			for (int verificDataCount = 0; verificDataCount < results.length(); verificDataCount++) {
				...
				// Skip if this test case for this device was already processed (to handle potential duplicates in Procal response)
				if (!processedTestCases.add(uniqueKey)) {
					Verification.logger.warn("S043_Get_Test_Point_Status : addTestPointMeterResultsWithLduPosition : Skipping duplicate test case: " + uniqueKey);
					continue; // Skip to next iteration
				}
				...
				if (mappedPosition != -1 && selectedPallet != null) {
					Verification.logger.info("S043_Get_Test_Point_Status : addTestPointMeterResultsWithLduPosition : Adding result for Meter: " + positionNo + " (Mapped: " + mappedPosition + ")");
					Verification.logger.info("S043_Get_Test_Point_Status : addTestPointMeterResultsWithLduPosition : " + positionNo + " : Result Value : " + resultValue + ", Error Min: " + error_min + ", Error Max: " + error_max);
					palletTracker.addResultToPalletMeterV2(mappedPosition, resultStatus, resultValue, error_min, error_max, getMyBayKey(), selectedPallet, testType, testCaseName);
				} else {
					Verification.logger.warn("S043_Get_Test_Point_Status : addTestPointMeterResultsWithLduPosition : No matching pallet found for position: " + positionNo + ". Skipping result addition.");
				}
			...
			for (Map.Entry<Integer, Boolean> entry : devicePassStatus.entrySet()) {
				...
				if (mappedPosition != -1 && selectedPallet != null) {
					Verification.logger.info("S043_Get_Test_Point_Status : addTestPointMeterResultsWithLduPosition : Adding summary for Meter : " + mappedPosition + " (Pallet: " + selectedPallet.getPalletQrId() + ")");
					palletTracker.addMeterResultSummaryWithPalletDetails(mappedPosition, finalResultStatus, finalResultValue, testCaseName,  testType, selectedPallet);
				} else {
					Verification.logger.warn("S043_Get_Test_Point_Status : addTestPointMeterResultsWithLduPosition : No matching pallet found for position: " + mappedPosition + ". Skipping summary addition.");
				}
			}

		} catch (org.json.JSONException e) {
			Verification.logger.error("addTestPointMeterResultsWithLduPosition: Error parsing JSON in normal processing: " + e.getMessage(), e);
		} catch (Exception e) {
            Verification.logger.error("addTestPointMeterResultsWithLduPosition: An unexpected error occurred in addTestPointMeterResultsWithLduPosition: " + e.getMessage(), e);
        }
	 ...
	 * This method assumes myPalletManageList contains the pallets in a specific order
	 * where index 0 is for Pallet 4, index 1 for Pallet 3, index 2 for Pallet 2, and index 3 for Pallet 1.
	 *
	 * @param positionNo The raw device position number (1-24).
	 * @param pos Integer representation of positionNo, used for `contains` checks.
	 * @param myPalletManageList A list of PalletManage objects representing active pallets in the bay.
	 * @return A MappedResult object containing the mapped position and the selected PalletManage object.
	 */
	private MappedResult getMappedPosition(int positionNo, int pos, List<PalletManage> myPalletManageList) {
		...
		try {
			...
			if (myPalletManageList.size() >= 4) {
				...
				Verification.logger.warn("getMappedPosition: Not enough pallets in myPalletManageList! Size: " + myPalletManageList.size() + ". Expected at least 4 for correct mapping.");
			}

			// Log the mapping details
			Verification.logger.info("getMappedPosition: Mapping Position " + positionNo + " -> Mapped to " + mappedPosition + " | Assigned to Pallet: " + (selectedPallet != null ? selectedPallet.getPalletQrId() : "None"));
