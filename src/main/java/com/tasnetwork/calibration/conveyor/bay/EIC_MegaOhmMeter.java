...
/*		JSONObject cNamePortSettingData = MySQL_Controller.sp_get_cname_port_setting(comPortCname);
		...
		try {
        	...
        	//cmbBxLDU_PortSelection2.setValue(saved_ldu2_setting.getString("port_name")); 	
				...
				ApplicationLauncher.logger.info("sendCommandToPanelIrMeter: port_name-1: Data not retrieved from DB");
			...
			e.printStackTrace();
			ApplicationLauncher.logger.error("sendCommandToPanelIrMeter: JSONException5-2:"+e.getMessage());
			//cmbBxLDU_PortSelection2.setValue("");
			ApplicationLauncher.logger.info("sendCommandToPanelIrMeter: port_name-2: Data not retrieved from database");
		...
		try {
        	...
        	//cmbBxLDU_PortSelection2.setValue(saved_ldu2_setting.getString("port_name")); 	
				...
				//cmbBxLDU_PortSelection2.setValue("");
				ApplicationLauncher.logger.info("sendCommandToPanelIrMeter: baudrate-1: Data not retrieved from DB");
			...
			ApplicationLauncher.logger.error("sendCommandToPanelIrMeter: JSONException5-2:"+e.getMessage());
			//cmbBxLDU_PortSelection2.setValue("");
			ApplicationLauncher.logger.info("sendCommandToPanelIrMeter: baudrate-2: Data not retrieved from database");
...
/*		if(DisplayDataObj.ValidateAllComPortAccessible()){
    ...
    public IR_ReadResult readMegaOhmMeter(DeviceSetting deviceSetting, String slaveId) {
        ...
        try {
            ...
            //ApplicationLauncher.logger.debug("EIC_MegaOhmMeter : readMegaOhmMeter : Command: " + commandFrame);
            //NewlandQRCodeScanner qrScannerObj = new NewlandQRCodeScanner();
            //EIC_MegaOhmMeter eicMegaOhmMeter = new EIC_MegaOhmMeter();
        	//String responseHex = sendCommandToPanelIrMeter(ConstantBayPortNameMapping.COM_PORT_EIC_MEGA,slaveId);
            ...
            // float value = extractFloatFromResponse(responseHex);
            // ApplicationLauncher.logger.debug("EIC_MegaOhmMeter : readMegaOhmMeter : Extracted Float Value: " + value);
        ...
        ApplicationLauncher.logger.debug("EIC_MegaOhmMeter : readMegaOhmMeter : responseHex " + responseHex);	
            ...
            ApplicationLauncher.logger.debug("EIC_MegaOhmMeter : readMegaOhmMeter : Error creating command frame: " + e.getMessage());
        }
    ...
    public String extractFloatFromResponse(String responseHex) {
     ...
     // Validate response length
       ...
       //  int intBits = Integer.parseUnsignedInt(dataHex, 16); // Parse as unsigned integer
