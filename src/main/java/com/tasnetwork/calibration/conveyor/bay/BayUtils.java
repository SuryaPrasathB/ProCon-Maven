...
public class BayUtils {
	...
	public static IoPortInfo getOutputPortDetails(String searchPortName) {
		...
		if (outputPortOpt.isPresent()) {
			...
			//ModbusTcpClient.logger.debug("getOutputPortDetails : getClusterId : " + outputPortDetails.getClusterId());
			//ModbusTcpClient.logger.debug("getOutputPortDetails : getBayId     : " + outputPortDetails.getBayId());
	...
	public static IoPortInfo getOutputPortDetailsWithPortNamePrefixAndPositionNo(String terminalId,String clusterId, 
		...
		/*		Optional<OutputPort> outputPortOpt = ConveyorDeviceDataManagerController.getBayConfigParsedKey().getTerminal().stream()
				.filter(e-> e.getTerminalId().equals(ConstantConveyorConfig.MY_TERMINAL_ID))
				.flatMap(terminal -> terminal.getOutputPort().stream())
				.filter(p -> p.getPortName().contains(searchPrefixPortName))
	...
	public static IoPortInfo getInputPortDetailsWithPortNamePrefixAndPositionNo(String terminalId,String clusterId, 
		...
		.forEach(e->{
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo: getClusterId    : " + e.getClusterId());
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo: getBayId    : " + e.getBayId());
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo: cName    : " + e.getPortName());
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo: getPositionId    : " + e.getPositionId());
		});
		DeviceDataManagerController.getBayConfigParsedKey().getTerminal().stream()
		//.filter(e-> e.getTerminalId().equals("01"))
		.filter(e-> Integer.parseInt(e.getTerminalId())==Integer.parseInt("01"))
		...
		.filter(p -> Integer.parseInt(p.getPositionId())==(5))
		.forEach(e->{
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo2: getClusterId    : " + e.getClusterId());
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo2: getBayId    : " + e.getBayId());
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo2: cName    : " + e.getPortName());
			ApplicationLauncher.logger.debug("getInputPortDetailsWithPortNamePrefixAndPositionNo2: getPositionId    : " + e.getPositionId());
		});*/
	...
	public static IoPortInfo getInputPortDetails(String searchPortName) {
		...
		if (inputPortOpt.isPresent()) {
			InputPort inputPortDetails = inputPortOpt.get();
			/*ModbusTcpClient.logger.debug("getInputPortDetails : getPortId    : " + inputPortDetails.getPortId());
			ModbusTcpClient.logger.debug("getInputPortDetails : getClusterId : " + inputPortDetails.getClusterId());
			ModbusTcpClient.logger.debug("getInputPortDetails : getBayId     : " + inputPortDetails.getBayId());*/
			
			
			//ModbusTcpClient.logger.debug("getInputPortDetails : getClusterId: " +inputPortDetails.getClusterId() + " -> getBayId: " + inputPortDetails.getBayId() + " -> getPortId: " + inputPortDetails.getPortId() );
	...
	/*public String setOutputDataToPlcBay(String clusterId, String bayId, String outputPortId, String outputStatus) {
		ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Entry ");
		String statusResponse = "";
		outputStatus = Constant_IO_ActionMapping.ON.equalsIgnoreCase(outputStatus) ? Constant_IO_ActionMapping.OFF : Constant_IO_ActionMapping.ON;
		//ConveyorClientManager cluster1ClientManager = ConveyorClientManager.getInstance(clusterId);
		//RestApiJsonBodyResponse clusterResponseData = new RestApiJsonBodyResponse();
		String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
		String  formattedOutputPortId = outputPortId;
		ClusterServer clusterServer = getServerDetails(terminalId, clusterId);
		ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Test1 ");
		if (clusterServer == null) {
			ApplicationLauncher.logger.debug("setOutputDataToPlcBay: cluster ip address details not found for terminalId: " + terminalId + " , clusterId: " + clusterId);
			return null;
		}
	    ...
	    ApplicationLauncher.logger.debug("setOutputDataToPlcBay : clusterId : " + clusterId);

	    Object clusterLock = ClusterUtils.getClusterLock(clusterId);  // Get lock for this cluster

	    //ApplicationLauncher.logger.debug("setOutputDataToPlcBay : clusterLock : " + clusterLock);

	    while ((retryCount != 0) && (!messageProcessed) && (!ProjectExecutionController.getUserAbortedFlag())) {
	        retryCount--;
	        ApplicationLauncher.logger.debug("setOutputDataToPlcBay : retryCount: " + retryCount + " : " + outputPortId);

	        synchronized (clusterLock) {  // Lock based on clusterId
	            ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Processing cluster: " + clusterId + " : outputId : " + outputPortId);


	            modbusPlcConnected = getModbusTcpClientManager().ensureModbusConnection(clusterServer);

	    		if(modbusPlcConnected){
	    			ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Test3 ");
	    					...
	    					if(GuiUtils.isNumber(formattedOutputPortId)) {
	    						...
	    						bayResponse = getModbusTcpClientManager().modbusWriteCoil(clusterServer, plcCoilAddress, outputValue);
	    						//boolean status = false;

	    						//status = getModbusTcpClientManager().getModbusTcpClient().modbusTcpSendWriteCoilCmd(plcCoilAddress,outputValue);
	    						//bayResponse.setStatus(status);
	    						if(!bayResponse.getStatus()) {
	    							ApplicationLauncher.logger.info("setOutputDataToPlcBay: outputPortId: device not responded : " + outputPortId);
	    							statusResponse = outputStatus.equalsIgnoreCase(Constant_IO_ActionMapping.ON) ? Constant_IO_ActionMapping.OFF : Constant_IO_ActionMapping.ON;// to get the inverse

	    						}else {
	    							ApplicationLauncher.logger.info("setOutputDataToPlcBay: success : outputPortId:  " + outputPortId + " : "  + bayResponse.isResponseBooleanData());

	    							//statusResponse = outputStatus;
	    							statusResponse = Constant_IO_ActionMapping.ON.equalsIgnoreCase(outputStatus) ? Constant_IO_ActionMapping.OFF : Constant_IO_ActionMapping.ON;

	    						}
	    					}else {
	    						statusResponse = outputPortId;
	    						ApplicationLauncher.logger.info("setOutputDataToPlcBay: invalid outputPortId: "+ outputPortId);
	    						//overAllStatus = false;
	    			...
	    			statusResponse = outputPortId;
	    			ApplicationLauncher.logger.info("setOutputDataToPlcBay: modbus failed to connect :  outputPortId: "+ outputPortId);
	    		}
	            ...
	            ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Completed processing for cluster: " + clusterId + " : outputId : " + outputPortId);
	        }
	    }



		ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Exit ");
		return statusResponse;
	}*/
	...
	public String setOutputWordToPlcBay(String clusterId, String bayId, String outputPortId, int outputValue) {
		...
		//ModbusTcpClient.logger.debug("setOutputWordToPlcBay : clusterId : " + clusterId);

		Object clusterLock = ClusterUtils.getClusterLock(clusterId);  // Get lock for this cluster

		//ModbusTcpClient.logger.debug("setOutputWordToPlcBay : clusterLock : " + clusterLock);
		...
		while ((retryCount != 0) && (!messageProcessed) && (!ProjectExecutionController.getUserAbortedFlag())) {
			...
			synchronized (clusterLock) {  // Lock based on clusterId
				...
				modbusPlcConnected = getModbusTcpClientManager().ensureModbusConnection(clusterServer);//isConnectionSetupGood(clusterServer.getIpAddress(), clusterServer.getPort());


				if(modbusPlcConnected){
					//ModbusTcpClient.logger.debug("setOutputWordToPlcBay: Test3 ");
					//if(!getModbusTcpClientManager().getModbusTcpServerConnectedMap().get(clusterServer.getIpAddress())){
					//ModbusTcpClient.logger.debug("setOutputWordToPlcBay: Test4 ");
					//BayResponse bayResponse = getModbusTcpClientManager().modbusConnect(clusterServer.getIpAddress(), clusterServer.getPort());
					//if (!bayResponse.getStatus()){
					//	ModbusTcpClient.logger.debug("getInputDataFromPlcBay: plc connection failed:  IpAddress: " + clusterServer.getIpAddress()
					//			+ " : port: " + clusterServer.getPort());
					//}else{
					//getModbusTcpClientManager().getModbusTcpClient().modbusTcpSendReadCoilCmd(inputPortId);
					...
					if(GuiUtils.isNumber(formattedOutputPortId)) {
						...
						if(!bayResponse.getStatus()) {
							...
							statusResponse = Constant_IO_ActionMapping.OFF ; //Constant_IO_ActionMapping.OLD_ON_NEW_OFF.equalsIgnoreCase(outputStatus) ? Constant_IO_ActionMapping.OLD_ON_NEW_OFF : Constant_IO_ActionMapping.OLD_OFF_NEW_ON;
						}else {
	...
	public String setOutputDataToPlcBay(String clusterId, String bayId, String outputPortId, String outputStatus) {
		...
		////outputStatus = Constant_IO_ActionMapping.OLD_ON_NEW_OFF.equalsIgnoreCase(outputStatus) ? Constant_IO_ActionMapping.OLD_OFF_NEW_ON : Constant_IO_ActionMapping.OLD_ON_NEW_OFF;
...
/*		ApplicationLauncher.logger.debug("setOutputDataToPlcBay: Test2 ");
		if(!getModbusTcpClientManager().getModbusTcpServerConnectedMap().containsKey(clusterServer.getIpAddress())){
			
			BayResponse bayResponse = getModbusTcpClientManager().modbusConnect(clusterServer.getIpAddress(), clusterServer.getPort());
			if(bayResponse.getStatus()) {
				ApplicationLauncher.logger.debug("setOutputDataToPlcBay: modbus connected : " + outputPortId);
			...
			if(!getModbusTcpClientManager().getModbusTcpServerConnectedMap().get(clusterServer.getIpAddress())){
				BayResponse bayResponse = getModbusTcpClientManager().modbusConnect(clusterServer.getIpAddress(), clusterServer.getPort());
				if(bayResponse.getStatus()) {
					ApplicationLauncher.logger.debug("setOutputDataToPlcBay: modbus connected-B : " + outputPortId);
					modbusPlcConnected = true;
				}
			}else{
				ApplicationLauncher.logger.debug("setOutputDataToPlcBay: modbus already connected-C : " + outputPortId);
				modbusPlcConnected = true;
			}
	    ...
	    //ModbusTcpClient.logger.debug("setOutputDataToPlcBay : clusterId : " + clusterId);
	...
	public String getInputDataFromPlcBay(IoPortInfo io_portInfo) {
		...
		String bayId = io_portInfo.getBayId();
		String inputPortId = io_portInfo.getPortId();
		ModbusTcpClient.logger.debug("getInputDataFromPlcBay: clusterId: " + clusterId + " -> bayId :" + bayId +" -> inputPortId: " + inputPortId);
		//ApplicationLauncher.logger.debug("getInputDataFromPlcBay: bayId: " + bayId);
		//ApplicationLauncher.logger.debug("getInputDataFromPlcBay: clusterId: " + clusterId);
		String statusResponse = "";
		//ConveyorClientManager cluster1ClientManager = ConveyorClientManager.getInstance(clusterId);
		String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
		ClusterServer clusterServer = getServerDetails(terminalId, clusterId);
		...
		/*		if(!getModbusTcpClientManager().getModbusTcpServerConnectedMap().containsKey(clusterServer.getIpAddress())){
			...
			BayResponse bayResponse = getModbusTcpClientManager().modbusConnect(clusterServer.getIpAddress(), clusterServer.getPort());
			if(bayResponse.getStatus()) {
				ApplicationLauncher.logger.debug("getInputDataFromPlcBay: modbus connected-A : " + inputPortId);
				modbusPlcConnected = true;
			}
		}else {
			if(!getModbusTcpClientManager().getModbusTcpServerConnectedMap().get(clusterServer.getIpAddress())){
				BayResponse bayResponse = getModbusTcpClientManager().modbusConnect(clusterServer.getIpAddress(), clusterServer.getPort());
				if(bayResponse.getStatus()) {
					ApplicationLauncher.logger.debug("getInputDataFromPlcBay: modbus connected-B : " + inputPortId);
					modbusPlcConnected = true;
				}
			}else{
				ApplicationLauncher.logger.debug("getInputDataFromPlcBay: modbus already connected-C : " + inputPortId);
				modbusPlcConnected = true;
			}
		...
		//ModbusTcpClient.logger.debug("getInputDataFromPlcBay : clusterId : " + clusterId);

		Object clusterLock = ClusterUtils.getClusterLock(clusterId);  // Get lock for this cluster

		//ApplicationLauncher.logger.debug("getInputDataFromPlcBay : clusterLock : " + clusterLock);

		while ((retryCount != 0) && (!messageProcessed) && (!ProjectExecutionController.getUserAbortedFlag())) {
			retryCount--;
			//ModbusTcpClient.logger.debug("getInputDataFromPlcBay : retryCount: " + retryCount + " : " + io_portInfo.getPortId());

			synchronized (clusterLock) {  // Lock based on clusterId
				//ModbusTcpClient.logger.debug("getInputDataFromPlcBay: Processing cluster: " + clusterId + " : inputPortId : " + io_portInfo.getPortId());
				...
				if(modbusPlcConnected){
					...
					//BayResponse bayResponse = getModbusTcpClientManager().modbusConnect(clusterServer.getIpAddress(), clusterServer.getPort());
					//if (!bayResponse.getStatus()){
					//	ApplicationLauncher.logger.debug("getInputDataFromPlcBay: plc connection failed:  IpAddress: " + clusterServer.getIpAddress()
					//			+ " : port: " + clusterServer.getPort());
					//}else{
					//getModbusTcpClientManager().getModbusTcpClient().modbusTcpSendReadCoilCmd(inputPortId);
					...
					if(GuiUtils.isNumber(inputPortId)) {
						...
						if(!bayResponse.getStatus()) {
							...
							////statusResponse = Constant_IO_ActionMapping.OLD_ON_NEW_OFF.equalsIgnoreCase(statusResponse) ? Constant_IO_ActionMapping.OLD_OFF_NEW_ON : Constant_IO_ActionMapping.OLD_ON_NEW_OFF;
						...
						//ModbusTcpClient.InformUser("Error-2010","Kindly check key : "+inputPortDetails.getPortId() +" on conveyor config file on <inputPort> section",AlertType.ERROR);
					}
					//	}
		...
		ApplicationLauncher.logger.debug("getInputDataFromPlcBay: Wait Time Entry: " + inputPortId);
		boolean isResponseReceived = false;
		...
		ApplicationLauncher.logger.debug("getInputDataFromPlcBay: Wait Time Exit: " + inputPortId);
		...
		if (isResponseReceived) {
			...
			ApplicationLauncher.logger.debug("getInputDataFromPlcBay: isResponseReceived : test1 : " + inputPortId);
			if (clusterResponseData.getStatusCode().equals(ConstantConveyor.HTTP_RESPONSE_SUCCESS)) {
				ApplicationLauncher.logger.debug("getInputDataFromPlcBay: isResponseReceived : test2: " + inputPortId);
				try {
					ApplicationLauncher.logger.debug("getInputDataFromPlcBay: isResponseReceived : inputPortId: " + inputPortId);
					statusResponse = clusterResponseData.getJsonBodyResponse().get(inputPortId).toString();
					ApplicationLauncher.logger.debug("getInputDataFromPlcBay: isResponseReceived : statusResponse: " + statusResponse);
				} catch (Exception e) {
					e.printStackTrace();
					ApplicationLauncher.logger.error("getInputDataFromPlcBay-1: Exception1 : " + e.getMessage() + " : inputPortId: " + inputPortId);
				}
			} else {
				ApplicationLauncher.logger.debug("getInputDataFromPlcBay: isResponseReceived : test3");
			}
		} else {
			ApplicationLauncher.logger.debug("getInputDataFromPlcBay-1: response failed: " + inputPortId);
			statusResponse = inputPortId;
		}*/
	...
	/*	public String getInputDataFromPlcBay(IoPortInfo io_portInfo) {
		...
		String bayId = io_portInfo.getBayId();
		String inputPortId = io_portInfo.getPortId();
		ApplicationLauncher.logger.debug("getInputDataFromPlcBay: Entry: " + inputPortId);
		String statusResponse = "";
		//ConveyorClientManager cluster1ClientManager = ConveyorClientManager.getInstance(clusterId);
		String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
		ClusterServer clusterServer = getServerDetails(terminalId, clusterId);
		boolean modbusPlcConnected = false;
		if (clusterServer == null) {
			ApplicationLauncher.logger.debug("getInputDataFromPlcBay: cluster ip address details not found for terminalId: " + terminalId + " , clusterId: " + clusterId);
			return null;
		}
	    ...
	    while ((retryCount != 0) && (!messageProcessed) && (!ProjectExecutionController.getUserAbortedFlag())) {
	        retryCount--;
	        ApplicationLauncher.logger.debug("getInputDataFromPlcBay : retryCount: " + retryCount + " : " + io_portInfo.getPortId());

	        synchronized (clusterLock) {  // Lock based on clusterId
	            ApplicationLauncher.logger.debug("getInputDataFromPlcBay: Processing cluster: " + clusterId + " : inputPortId : " + io_portInfo.getPortId());

	            // Process the message

	           // modbusPlcConnected = getModbusTcpClientManager().isConnectionSetupGood(clusterServer.getIpAddress(), clusterServer.getPort());
	            modbusPlcConnected = getModbusTcpClientManager().ensureModbusConnection(clusterServer);
	    		if(modbusPlcConnected){
	    					...
	    					if(GuiUtils.isNumber(inputPortId)) {
	    						...
	    						//bayResponse = getModbusTcpClientManager().getModbusTcpClient().modbusTcpSendReadCoilCmd(plcCoilAddress);
	    						if(!bayResponse.getStatus()) {
	    							ApplicationLauncher.logger.info("getInputDataFromPlcBay: inputPortId: device not responded ");
	    							statusResponse = inputPortId;

	    						}else {
	    							ApplicationLauncher.logger.info("getInputDataFromPlcBay: inputPortId:  " + io_portInfo.getPortId() + " : "  + bayResponse.isResponseBooleanData());
	    							statusResponse = bayResponse.getResponseData();
	    							statusResponse = Constant_IO_ActionMapping.ON.equalsIgnoreCase(statusResponse) ? Constant_IO_ActionMapping.OFF : Constant_IO_ActionMapping.ON;

	    							ApplicationLauncher.logger.info("getInputDataFromPlcBay: inputPortId:  " + statusResponse + " : "  + statusResponse);
	    						...
	    						statusResponse = inputPortId;
	    						ApplicationLauncher.logger.info("getInputDataFromPlcBay: invalid inputPortId: "+ io_portInfo.getPortId());
	    					}
	    			//	}

	    		}else {
	    			statusResponse = inputPortId;
	    			ApplicationLauncher.logger.info("getInputDataFromPlcBay: modbus failed to connect :  outputPortId: "+ inputPortId);
	    		}
	            ...
	            ApplicationLauncher.logger.debug("getInputDataFromPlcBay: Completed processing for cluster: " + clusterId + " : inputPortId : " + io_portInfo.getPortId());
	        }
	    }
...
/*	    	case ConstantConveyor.FT_BAY_KEY:
	...
	public void computeMeterOverAllStatus(String palletDistinctId ){
		...
		if(myPalletManageOpt.isPresent()) {
			...
			//PalletMeter  palletMeter = myPalletManage.getPalletMeterList();
			int positionNo= 0;
			for(int i = 0; i<  sortedPalletMeterList.size();i++) {
				...
				positionNo = palletMeter.getRackPositionNo();
				if(palletMeter!=null) {
					...
					if(allTestTypeCompleted) {
						...
						ApplicationLauncher.logger.debug("BayUtils: computeMeterOverAllStatus: presentMeterSerialNo: " + presentMeterSerialNo + " : failedResultFound : " + failedResultFound);
						boolean wfrResultFound = palletMeter.getPalletMeterResultsList().stream().filter(e->e.getMeterSerialNo().equals(presentMeterSerialNo))
								.anyMatch(e->e.getResultStatus().equals(ConstantReport.REPORT_POPULATE_WFR));
						ApplicationLauncher.logger.debug("BayUtils: computeMeterOverAllStatus: presentMeterSerialNo: " + presentMeterSerialNo + " : wfrResultFound : " + wfrResultFound);
						*/
						
						String failureReason = "";
						boolean batchFailedResultFound = palletMeter.getPalletMeterResultsList().stream().filter(e->e.getMeterSerialNo().equals(presentMeterSerialNo))
								.anyMatch(e->e.getResultValue().contains(ConstantConveyor.REASON_RESULT_BATCH_FAILED_DISPLAY_HEADER));
						...
						/*if(batchFailedResultFound) {
							...
							String batchFailedTestTypesCsv = palletMeter.getPalletMeterResultsList().stream()
								    ...
								    .filter(e -> e.getResultValue().contains(ConstantConveyor.REASON_RESULT_BATCH_FAILED_DISPLAY_HEADER))
								    .map(e -> e.getTestType() + ":" + ConstantConveyor.REASON_RESULT_BATCH_FAILED_DISPLAY_HEADER )
								    .distinct() // optional, if you want unique test types
								    .collect(Collectors.joining(","));
							 //failedTestTypesCsv = "Failed: " + failedTestTypesCsv;
							batchFailedTestTypesCsv = ConstantConveyor.REASON_RESULT_FAILED_DISPLAY + batchFailedTestTypesCsv; 
							 ...
							 failureReason = batchFailedTestTypesCsv;// + "\n" + wfrResultFoundCsv;
								...
								if(!failureReason.isEmpty()){// removing last newline char
								 ...
								 ApplicationLauncher.logger.debug("BayUtils: computeMeterOverAllStatus: batch : presentMeterSerialNo: " + presentMeterSerialNo + " : failureReason : " + failureReason);
							...
							boolean failedResultFound = palletMeter.getPalletMeterResultsList().stream().filter(e->e.getMeterSerialNo().equals(presentMeterSerialNo))
									.anyMatch(e->( (e.getResultStatus().equals(ConstantReport.REPORT_POPULATE_FAIL)) && (!e.getResultValue().contains(ConstantConveyor.REASON_RESULT_BATCH_FAILED_DISPLAY_HEADER))));
							...
							if(failedResultFound || wfrResultFound || batchFailedResultFound) {
								...
								if(batchFailedResultFound) {
									...
									batchFailedTestTypesCsv = palletMeter.getPalletMeterResultsList().stream()
										    ...
										    .collect(Collectors.joining(","));
									 //failedTestTypesCsv = "Failed: " + failedTestTypesCsv;
									batchFailedTestTypesCsv = ConstantConveyor.REASON_RESULT_BATCH_FAILED_DISPLAY_HEADER + ":" + batchFailedTestTypesCsv;//ConstantConveyor.REASON_RESULT_FAILED_DISPLAY + batchFailedTestTypesCsv; 
									 ApplicationLauncher.logger.debug("BayUtils: computeMeterOverAllStatus: batch failedResultFound : presentMeterSerialNo: " + presentMeterSerialNo + " : batchFailedTestTypesCsv : " + batchFailedTestTypesCsv);
									 /*failureReason = batchFailedTestTypesCsv;// + "\n" + wfrResultFoundCsv;
								...
								if(failedResultFound) {
									 failedTestTypesCsv = palletMeter.getPalletMeterResultsList().stream()
										    .filter(e -> e.getMeterSerialNo().equals(presentMeterSerialNo))
										    .filter(e -> ( (e.getResultStatus().equals(ConstantReport.REPORT_POPULATE_FAIL)) && (!e.getResultValue().contains(ConstantConveyor.REASON_RESULT_BATCH_FAILED_DISPLAY_HEADER))))
										    .map(e -> e.getTestType())
										    .distinct() // optional, if you want unique test types
										    .collect(Collectors.joining(","));
									 //failedTestTypesCsv = "Failed: " + failedTestTypesCsv;
	...
	public void archiveExistingResultInDb(String myBayKey, String palletDistinctId) {
		...
		if(myPalletManageOpt.isPresent()) {
			...
			for(PalletMeter eachPalletMeter: sortedPalletMeterList) {
				//ApplicationLauncher.logger.debug("BayUtils: archiveExistingResultInDb: Hit2");
...
/*				eachPalletMeter.getPalletMeterResultsList().removeIf(result -> {
				...
				List<PalletMeterResults> recordsToDelete = eachPalletMeter.getPalletMeterResultsList().stream()
					    .filter(e -> e.getBayStateKey().equals(myBayKey))
					    .collect(Collectors.toList());
/*				
				MySqlServiceManager.getPalletMeterResultsService().deleteAll(recordsToDelete);*/
			}
	...
	public void archiveResultsForBayKey(PalletMeter palletMeter, String bayKey) {
	    ...
	    palletMeter.getPalletMeterResultsList().removeIf(r -> {
	        ...
	        if (match) r.setPalletMeter(null);
	        return match;
	    });
	...
	public boolean updateConveyorMetrics(
		...
		String customerName = ConveyorDataManager.getTerminalBayConfig().getCustomerName();//DeviceDataManagerController.getConveyorConfigParsedKey().getCustomerName();//."DevSys"; // Fixed customerName
...
/*	                if (positionNo != null) {
	                    ...
	                    //MeterStatus meterStatus = status.equals("Pass") ? MeterStatus.PASSED : MeterStatus.FAILED;
	                    ...
	                    if(status.equals(ConstantReport.REPORT_POPULATE_PASS)) {
	                    	...
	                    	ApplicationLauncher.logger.debug("updateConveyorMetrics: Unloading: positionNo: "+ positionNo +" Pass hit1"); 
	                    }else if(status.equals(ConstantReport.REPORT_POPULATE_FAIL)) {
	                    	meterStatus = MeterStatus.FAILED;
	                    	ApplicationLauncher.logger.debug("updateConveyorMetrics: Unloading: positionNo: "+ positionNo +" Fail hit2"); 
	                    }else if(status.equals(ConstantReport.REPORT_POPULATE_WFR)) {
	                    	ApplicationLauncher.logger.debug("updateConveyorMetrics: Unloading: positionNo: "+ positionNo +" WFR hit3"); 
	                    	meterStatus = MeterStatus.FAILED;
	                    }else {
	                    	ApplicationLauncher.logger.debug("updateConveyorMetrics: Unloading: positionNo: "+ positionNo +" others hit4"); 
	                    	meterStatus = MeterStatus.FAILED;
	                    }
	                    ...
	                    ConveyorDataManager.getDashboardObject().
	                			updatePalletMeterStatusByBayAndPositionWithSerialNo(bayKey, positionNo, serialNo, meterStatus, 
	                					reason.replace(ConstantConveyor.REASON_RESULT_FAILED_DISPLAY, "").replace("\n", ""));//.replace(ConstantConveyor.REASON_RESULT_WFR_DISPLAY, ""));
	                } else {
	                    ApplicationLauncher.logger.debug("batchUpdateMeters: Unloading: Meter ID is null, skipping update for: " + serialNo);
...
/*    				if (positionNo != null) {
    					...
    					ApplicationLauncher.logger.error("updateConveyorMetrics: Rejection: Meter ID is null, skipping update for: " + serialNo);
    					allUpdatesInitiatedSuccessfully = false;
    				}*/
    			} catch (Exception e) {
        ...
        //ApplicationLauncher.logger.debug("batchUpdateMeters : calculatedAverage: " + calculatedAverage);
        ...
        try {
            ...
            //ApplicationLauncher.logger.info("ConveyorOutputMetrics (Daily) updated successfully for customer: " + customerName + ", bay: " + bayKey + " for today. Avg Hourly Output: " + String.format("%.2f", calculatedAverage));
            /*Platform.runLater(()->{
        ...
        //updateSummaryMetrics( metrics,  bayType, currentPalletTotalMeters, currentPalletPassedMeters, currentPalletFailedMeters);
	...
	public void updateSummaryMetrics(String bayKey, String palletDistinctId) {
        ...
        ConveyorOutputMetrics triggerMetric = MySqlServiceManager.getConveyorOutputMetricsService().findByPalletDistinctId(palletDistinctId)
        ...
        List<ConveyorOutputMetrics> dailyMetrics = MySqlServiceManager.getConveyorOutputMetricsService().findAllByBayTypeAndDateH(
                bayKey,
                metricTargetDate
        );
	...
	private AggregatedMetrics calculateAggregatedMetrics(List<ConveyorOutputMetrics> metrics) {
        ...
        result.averageHourlyOutput = operationalHours > 0 ? 
        		roundToTwoDecimals((double) result.totalMeters / operationalHours) : 0.0;
        ...
        result.operationalHours = operationalHours;
...
/*    private Date getStartOfDay(Date date) {
    ...
    public void markAsCompleteForPreviousBayPallet(String bayKey,Logger eachLogger ) {
		...
		if(palletManageList.size()>1) {
			List<String> palletDistinctIdList = palletManageList.stream().map(e->e.getPalletDistinctId())
					.collect(Collectors.toList());
