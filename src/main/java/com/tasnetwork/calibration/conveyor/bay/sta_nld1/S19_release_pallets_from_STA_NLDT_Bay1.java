...
public class S19_release_pallets_from_STA_NLDT_Bay1 implements STA_NoLoadTestBay1State {
	...
	int maxNoOfPalletsInStaNld1 = DeviceDataManagerController.getConveyorConfigParsedKey().getMaxNoOfPalletsInStaNld1();
	int postDelaySta1ExitOpenStopLatch1_InMsec= 1000 * DeviceDataManagerController.getConveyorConfigParsedKey().getPostDelaySta1ExitOpenStopLatch1_InSec();
	int postDelaySta1ExitOpenStopLatch2_InMsec= 1000 * DeviceDataManagerController.getConveyorConfigParsedKey().getPostDelaySta1ExitOpenStopLatch2_InSec();
	int postDelaySta1ExitCloseStopLatch1_InMsec= 1000 * DeviceDataManagerController.getConveyorConfigParsedKey().getPostDelaySta1ExitCloseStopLatch1_InSec();
	int postDelaySta1ExitCloseStopLatch2_InMsec= 1000 * DeviceDataManagerController.getConveyorConfigParsedKey().getPostDelaySta1ExitCloseStopLatch2_InSec();
    */
...
/*        if (S16_open_stop_latch_STA_NLDT_Bay1.getPalletsPassedSTA_NLDT_Bay1() == ConstantConveyor.NUM_PALLETS_STA_NLD1_BAY1) {
        	StaNld_1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : All Pallets Passed STA_NLDT_Bay1");
    ...
    private BayResponse releasePalletsFromStaNlt_Bay1() {
    	...
    	int postDelaySta1ExitCloseStopLatch2_InMsec= 1000 * DeviceDataManagerController.getConveyorConfigParsedKey().getPostDelaySta1ExitCloseStopLatch2_InSec();
	        ...
	        if (bayResponse.getStatus()) {
	            ...
	            //BayUtils.delay(3000);
	            noOfPalletsExited = 0;
	            while( (noOfPalletsExited <= maxNoOfPalletsInStaNld1)  && (!StaNld_Bay1.isStopProcessRequestedStaNldBay1()) ){
		            if(noOfPalletsExited < maxNoOfPalletsInStaNld1) {
		            	 //BayUtils.delay(2000);
		            	BayUtils.delay(postDelaySta1ExitCloseStopLatch2_InMsec);//5000);
		            	StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch_SCT_NLT_Bay1 : Hit1 ");
		            	bayResponse = open_StopLatch_SCT_NLT_Bay1();
		            	if (bayResponse.getStatus()) {
		            		BayUtils.delay(postDelaySta1ExitOpenStopLatch1_InMsec);//5000);
		            		StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch_SCT_NLT_Bay1 : Hit2 ");
		            		bayResponse = close_StopLatch_SCT_NLT_Bay1();
		            		if (bayResponse.getStatus()) {
		            			BayUtils.delay(postDelaySta1ExitCloseStopLatch1_InMsec);//5000);
		            			StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch2_SCT_NLT_Bay1 : Hit3 ");
		            			bayResponse = open_StopLatch2_SCT_NLT_Bay1();
		            			if (bayResponse.getStatus()) {
		            				StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch2_SCT_NLT_Bay1 : Hit4 ");
		            				BayUtils.delay(postDelaySta1ExitOpenStopLatch2_InMsec);//5000);
		            				bayResponse = close_StopLatch2_SCT_NLT_Bay1();
		            				noOfPalletsExited++;
		            				StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : noOfPalletsExited :" + noOfPalletsExited);
		            				if (!bayResponse.getStatus()) {
		            		            StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : Failed to Close Stop Latch2");
		            		            bayResponse.setStatus(false);
		            		            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_SCT_NLT_BAY1_018);
		            		        }
		            			}else {
		            	        	StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : Failed to Open Stop Latch2");
		            	            bayResponse.setStatus(false);
		            	            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_SCT_NLT_BAY1_019 );
		            	        }
		            		}else {
		                        StaNld_Bay1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : Failed to Close Stop Latch1");
		                        bayResponse.setStatus(false);
        ...
        //StaNld_1.logger.info("S19_release_pallets_from_STA_NLDT_Bay1 : getPalletsPassedSTA_NLDT_Bay1()" + passedPallets);
    ...
    private BayResponse close_StopLatch_SCT_NLT_Bay1() {
       ...
       // Map<String, Object> responseReturn = new HashMap<>();
        ...
        if (portInfo != null) {
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch_SCT_NLT_Bay1 : state : " + state);
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch_SCT_NLT_Bay1 : status : " + status);

        } else {
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch_SCT_NLT_Bay1 : Output port not found");
        }
    ...
    private BayResponse open_StopLatch_SCT_NLT_Bay1() {
        ...
        if (portInfo != null) {
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch_SCT_NLT_Bay1 : state : " + state);
            /*if (simulateSCTNLTBay1HappyPath) {
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch_SCT_NLT_Bay1 : status : " + status);

        } else {
        	StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch_SCT_NLT_Bay1: Output port not found");
	...
	private BayResponse close_StopLatch2_SCT_NLT_Bay1() {
        ...
        if (portInfo != null) {
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch2_SCT_NLT_Bay1 : state : " + state);
            /*if (simulateSCTNLTBay1HappyPath) {
                state = Constant_IO_ActionMapping.OFF;
            }*/
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch2_SCT_NLT_Bay1 : status : " + status);

        } else {
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : close_StopLatch2_SCT_NLT_Bay1: Output port not found");
        }
	...
	private BayResponse open_StopLatch2_SCT_NLT_Bay1() {
        ...
        if (portInfo != null) {
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch2_SCT_NLT_Bay1 : state : " + state);
            /*if (simulateSCTNLTBay1HappyPath) {
                state = Constant_IO_ActionMapping.ON;
            }*/
            ...
            StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch2_SCT_NLT_Bay1 : status : " + status);

        } else {
        	StaNld_Bay1.logger.debug("S19_release_pallets_from_STA_NLDT_Bay1 : open_StopLatch2_SCT_NLT_Bay1: Output port not found");
        	return bayResponse ;
        }
