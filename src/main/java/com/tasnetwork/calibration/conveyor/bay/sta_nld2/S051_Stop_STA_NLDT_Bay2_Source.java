...
public class S051_Stop_STA_NLDT_Bay2_Source implements STA_NoLoadTestBay2State{
	...
	/*private Map<String,Object> StopSTA_NLDTSource() {
		...
		STA_NoLoadTestBay2.logger.debug("S071_Stop_STA_NLDT_source : StopSTA_NLDTSource : Entry");
		...
		if (isSrcStoped_STA_NLDTBay) {
			STA_NoLoadTestBay2.logger.debug("S071_Stop_STA_NLDT_source : StopSTA_NLDTSource : Turned on Source Stop Pin : Success");
			BayUtils.delay(500);
		} else {
			STA_NoLoadTestBay2.logger.debug("S071_Stop_STA_NLDT_source : StopSTA_NLDTSource : Failed to turn on Source Stop Pin");
			isSrcStoped_STA_NLDTBay = false;
		}

		status = turn_off_Stop_pin_ft_bay();
		if (status) {
	        STA_NoLoadTestBay2.logger.debug("S08_functional_Test : StopSTA_NLDTSource : Turned Off Source Stop Pin : Success");

		} else {
	        STA_NoLoadTestBay2.logger.debug("S08_functional_Test : StopSTA_NLDTSource : Failed to turn off Source Stop Pin");
	        status = false;
		}
		...
		STA_NoLoadTestBay2.logger.debug("S08_functional_Test : StopSTA_NLDTSource : Exit");
	...
	//	Add Procal Source Stop Here
		...
		StaNld_Bay2.logger.debug("S071_turn_on_Stop_pin_ft_bay : StopSTA_NLDTSource : status : " + responseReturn.get("status"));
	...
	/*private Map<String,Object> turn_off_Stop_pin_ft_bay() {
		STA_NoLoadTestBay2.logger.debug("S071_turn_off_Stop_pin_ft_bay : ftBay_StopPin_Status : Entry");
		...
		if (portInfo != null) {
			...
			STA_NoLoadTestBay2.logger.debug("S071_turn_off_Stop_pin_ft_bay : Output port not found");
			return responseReturn ;
        }
		...
		STA_NoLoadTestBay2.logger.debug("S071_turn_off_Stop_pin_ft_bay : ftBay_StopPin_Status : status : " + status); 

		responseReturn.put("status", status);
		
		STA_NoLoadTestBay2.logger.debug("S071_turn_off_Stop_pin_ft_bay : ftBay_StopPin_Status : Exit");
		return responseReturn;
	}*/
	//============================================================================================================================================  
