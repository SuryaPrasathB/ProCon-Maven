...
public class S02_check_for_pallets_at_Verific_Bay implements WaitingBayState {
	...
	public BayResponse handleRequest() {
		...
		// !ConstantConveyor.isVerificationBayPalletsCleared() ) &&
			...
			// VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay :
			// VERIFICATION_BAY_PALLETS_CLEARED : " +
			// ConstantConveyor.isVerificationBayPalletsCleared());
			...
			if (logEnabled) {
				VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay : ALL_LOOP_BREAK_FLAG-1: "
						+ ConstantConveyor.ALL_LOOP_BREAK_FLAG);
				VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay : isVerific1PalletsAllCleared()-1: "
						+ ConveyorDataManager.isVerific1PalletsAllCleared());
				VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay : isPalletAvailableAt_VerificBay-1: "
						+ isPalletAvailableAt_VerificBay);
			}
			logEnabled = false;
		}

		VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay : ALL_LOOP_BREAK_FLAG-2: "
				+ ConstantConveyor.ALL_LOOP_BREAK_FLAG);
		VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay : isPalletAvailableAt_VerificBay-2: "
				+ isPalletAvailableAt_VerificBay);
		VerificWaiting.logger.debug("S02_check_for_pallets_at_Verific_Bay : isVerific1PalletsAllCleared()-2: "
				+ ConveyorDataManager.isVerific1PalletsAllCleared());
		...
		if (!isPalletAvailableAt_VerificBay && ConveyorDataManager.isVerific1PalletsAllCleared()) {
			VerificWaiting.logger.debug(
					"S02_check_for_pallets_at_Verific_Bay : No Pallet Available at Verific Bay and existing cleared");
			ConveyorDataManager.getDashboardObject().getBayIndicatorManager()
	...
	private Map<String, Object> isPalletAvailableAt_VerificBay() {
		...
		if (portInfo != null) {
			 ...
			 */
			if (logEnabled) {
				VerificWaiting.logger.debug("isPalletAvailableAt_VerificBay : getClusterId: " + portInfo.getClusterId()
