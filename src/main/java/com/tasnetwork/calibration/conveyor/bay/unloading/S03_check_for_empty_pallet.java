...
public class S03_check_for_empty_pallet implements UnloadingBayState {
	...
	private Map<String,Object> check_for_empty_pallet() {
		...
		int emptyPalletDetectionStableWaitTimeInMSec = 1000*  DeviceDataManagerController.getConveyorConfigParsedKey().getUnloadingPalletEmptyStableDetectionWaitTime_InSec();
		...
		if (stableDetectionForNoPallet) {
			...
			ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayAllPalletsExistInBay(getMyBayKey(),false);
			status = true;
			//bayResponse.setStatus(true);
			//bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
		} else {
	...
	private Map<String,Object> isPalletAvailableAt_UnloadingBay() {
		...
		if (portInfo != null) {
			...
			if(logEnabled) {
				Unloading.logger.debug("S03_check_for_empty_pallet : isPalletAvailableAt_UnloadingBay :Output port not found");
			}
			return responseReturn ;
		}
		...
		if(logEnabled) {
			Unloading.logger.debug("S03_check_for_empty_pallet : isPalletAvailableAt_UnloadingBay : status : " + status);
		}
		responseReturn.put("status", status);
		if(logEnabled) {
