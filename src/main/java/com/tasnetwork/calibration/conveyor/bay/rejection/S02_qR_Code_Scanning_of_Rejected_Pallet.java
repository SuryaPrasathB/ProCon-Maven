...
public class S02_qR_Code_Scanning_of_Rejected_Pallet implements RejectionBayState {
	...
	// Define the URL for your Python rejection application's single meter update API
	private static final String REJECTION_SINGLE_METER_API_URL = "http://127.0.0.1:5001/api/rejection_meters/"; // Base URL for PUT
...
/*	private ConveyorOutputMetricsService conveyorOutputMetricsService;
	...
	// This field represents the current bay's identifier. It must be set for each bay instance.
	...
	// Setter for myBayKey, often used if this class is instantiated and then configured
	...
	public BayResponse handleRequest() {
		...
		if ((bayResponse.getStatus()) ||
			...
			if(bayResponse.getStatus()) {
				...
				try{
					if(BayUtils.getRejectionBayPalletDistinctIdList().contains(palletQrId)) {
						...
						long palletCount = BayUtils.getRejectionBayPalletDistinctIdList().stream().filter(e-> e.endsWith(palletQrIdFinal))
								.count();
						Rejection.logger.debug("S02_qR_Code_Scanning_of_Pallet: palletCount : " + palletCount + " for palletQrId :" + palletQrId);
						if(palletCount>1) {
							Rejection.logger.debug("S02_qR_Code_Scanning_of_Pallet: duplicate pallets found for " +  palletQrId +" . Kindly check BayUtils.getRejectionBayPalletDistinctIdList():" +BayUtils.getRejectionBayPalletDistinctIdList());
						}else {
							palletDistinctId = BayUtils.getRejectionBayPalletDistinctIdList().stream().filter(e-> e.endsWith(palletQrIdFinal))
									.findFirst().get();
						}
			...
			if(palletDistinctId.isEmpty()) {
				Rejection.logger.info("S02_qR_Code_Scanning_of_Pallet : not success in fetching from getRejectionBayPalletDistinctIdList");
				boolean exitAppeared = false;
				Optional<PalletManage> myPalletManageOptional =  MySqlServiceManager.getPalletManageService().findTopByTodayDateAndPresentBayKeyAndPalletQrIdAndExitAppeared(
		        ...
		        if(myPalletManageOptional.isPresent()){
		        	...
		        	Rejection.logger.info("S02_qR_Code_Scanning_of_Pallet : found result  in database myPalletManage : " + myPalletManage.getPalletDistinctId());
		        	
		        	palletDistinctId = myPalletManage.getPalletDistinctId();
		        	
		        }else {
		        	Rejection.logger.info("S02_qR_Code_Scanning_of_Pallet : result not found with today date in database for Pallet : " + palletQrId);
		        }
				
			}else {
				
				BayUtils.getRejectionBayPalletDistinctIdList().remove(palletDistinctId);
				Rejection.logger.info("S02_qR_Code_Scanning_of_Pallet : removed palletDistinctId from getRejectionBayPalletDistinctIdList :" + BayUtils.getRejectionBayPalletDistinctIdList());
			}
			
			if(palletDistinctId.isEmpty()) {
				
				if(ConveyorDataManager.isRejectionBayScreenPalletResultDisplayUserRequested()) {
					palletDistinctId = new String (ConveyorDataManager.getRejectionBayScreenPalletResultDisplayUserRequestedPalletDistinctId());
					...
					*/
					...
					Rejection.logger.info("S02_qR_Code_Scanning_of_Pallet :processing user request for palletDistinctId: " + palletDistinctId);
				}
...
/*		    	List<Map<String, Object>> metersData = readPalletMetersData(getMyBayKey(),palletDistinctId);
		    	
				if (metersData != null && !metersData.isEmpty()) {
					batchUpdateRejectionMeters(palletDistinctId,palletQrId,metersData); // This will now update ConveyorOutputMetrics daily with average hourly output
				} else {
					Rejection.logger.debug("S02_qR_Code_Scanning_of_Pallet: No meter data found for batch update for bay: " + getMyBayKey());
				}*/
				...
				if (palletMeterList != null && !palletMeterList.isEmpty()) {
					...
					metricsUpdateRejectionMeters(palletDistinctId,palletQrId,palletMeterList); // This will now update ConveyorOutputMetrics daily with average hourly output
				...
				Rejection.logger.debug("S02_qR_Code_Scanning_of_Pallet: No Pallet details found for bay: " + getMyBayKey());
			...
			Rejection.logger.debug("S02_qR_Code_Scanning_of_Pallet: Qr scanning failed : getResponseData: " + bayResponse.getResponseData());
			sendIdleStatusUpdate();
		}
		
		
/*		List<Map<String, Object>> metersData = readPalletMetersData(getMyBayKey(),qrData);
		Rejection.logger.debug("S02_qR_Code_Scanning_of_Pallet: metersData: " + metersData);
		// If data is available, send it for batch update to the rejection app
		if (metersData != null && !metersData.isEmpty()) {
			batchUpdateRejectionMeters(qrData,metersData); // This will now update ConveyorOutputMetrics daily with average hourly output
		} else {
			Rejection.logger.debug("handleRequest: No meter data found for batch update for bay: " + getMyBayKey());
		}*/
	...
	private void updateRejectionDashboard(ArrayList<PalletMeter> palletMeterList) {
		...
		for ( PalletMeter eachpalletMeter: palletMeterList) {	
			try {
				...
				status = eachpalletMeter.getOverAllTestResultStatus();//(String) meterData.get("overallTestResultStatus");
				reason = eachpalletMeter.getErrorCode();//(String) meterData.get("errorCode");
				ApplicationLauncher.logger.debug("updateRejectionDashBoard: Rejection: positionNo: " + positionNo + " ,serialNo: " + serialNo + " , status: <" + status + "> , reason: " +reason);
				

				if (positionNo != null) {
					
					MeterStatus meterStatus = status.equals(ConstantReport.REPORT_POPULATE_PASS) ? MeterStatus.PASSED : MeterStatus.FAILED;
					if(status.equals(ConstantReport.REPORT_POPULATE_PASS)) {
						meterStatus = MeterStatus.PASSED;
						ApplicationLauncher.logger.debug("updateRejectionDashBoard: Rejection: positionNo: "+ positionNo +" Pass hit1"); 
					}else if(status.equals(ConstantReport.REPORT_POPULATE_FAIL)) {
						meterStatus = MeterStatus.FAILED;
						ApplicationLauncher.logger.debug("updateRejectionDashBoard: Rejection: positionNo: "+ positionNo +" Fail hit2"); 
					}else if(status.equals(ConstantReport.REPORT_POPULATE_WFR)) {
						ApplicationLauncher.logger.debug("updateRejectionDashBoard: Rejection: positionNo: "+ positionNo +" WFR hit3"); 
						meterStatus = MeterStatus.IDLE;
					}else {
						ApplicationLauncher.logger.debug("updateRejectionDashBoard: Rejection: positionNo: "+ positionNo +" others hit4"); 
						meterStatus = MeterStatus.IDLE;
					}
					...
					ApplicationLauncher.logger.error("updateRejectionDashBoard: Rejection: Meter ID is null, skipping update for: " + serialNo);
					//allUpdatesInitiatedSuccessfully = false;
				}
			} catch (Exception e) {
				ApplicationLauncher.logger.error("updateRejectionDashBoard: Rejection: Error initiating update for meter data: " + eachpalletMeter.getMeterSerialNo() + ". Exception: " + e.getMessage(), e);
				//allUpdatesInitiatedSuccessfully = false;
			}
        }
	}
	 ...
	 *
	 * @param selectedBayTypeKey The key identifying the bay type (e.g., "BAY_A") to retrieve the pallet.
	 * @return A list of maps, where each map represents a meter's data, or null if the pallet is not found or not active.
	 */
	public List<Map<String, Object>> readPalletMetersData(String selectedBayTypeKey, String palletDistinctId) {
		...
		try {
			//Rejection.logger.error("readPalletMetersData: getPresentPalletAtBayMap : " + PalletTrackerController.getPresentPalletAtBayMap());
			...
			//Optional<PalletManage> myPalletManageOptional =  MySqlServiceManager.getPalletManageService().findTopByPresentBayKeyAndPalletQrIdAndExitNotAppeared(
			...
			/*            if (myPalletManage == null) {
                Rejection.logger.error("readPalletMetersData: PalletManage not found for distinct ID: " + myPalletDistinctId);
                return null;
            }*/
			
			Optional<PalletManage> myPalletManageOptional =  MySqlServiceManager.getPalletManageService().findByPalletDistinctId(palletDistinctId);
	           
			if(myPalletManageOptional.isPresent()){
				myPalletManage = myPalletManageOptional.get();
				Rejection.logger.debug("Rejection : readPalletMetersData: result found for " + selectedBayTypeKey + " : palletDistinctId: " + palletDistinctId);
				//Rejection.logger.debug("Rejection : readPalletMetersData: result found for " + palletQrId + " : getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
				//Rejection.logger.debug("Rejection : readPalletMetersData: result found for " + palletQrId + " : getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
				//Rejection.logger.debug("Rejection : readPalletMetersData: result found for " + palletQrId + " : getPalletDistinctId: " + myPalletManage.g);
				//Rejection.logger.debug("Rejection : readPalletMetersData: result found for " + palletQrId + " : getPalletDistinctId: " + myPalletManage.getPalletDistinctId());

			}else{
				Rejection.logger.debug("readPalletMetersData: result not found for " + selectedBayTypeKey + " : " + palletDistinctId);
				return null;
			}
			...
			Rejection.logger.debug("Rejection: readPalletMetersData: updated setExitAppeared to true for getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
...
/*			for (PalletMeter meter : palletMeters) {
	...
	public void sendIdleStatusUpdate() {
    	...
    	new Thread(() -> {
    		try {
    			...
    			//Rejection.logger.debug("sendPalletWithMetersStatusUpdate: resultManipulatedMetersData: " + resultManipulatedMetersData);
    			//String status2 = status.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
    			//Rejection.logger.debug("sendPalletWithMetersStatusUpdate: status-2: <" + status2 + "> : positionNo: " + positionNo);
    			...
    			Rejection.logger.error("sendIdleStatusUpdate : Error sending Rejection meter update: " + e.getMessage() );
    			e.printStackTrace();
	...
	/**
	 * Sends an update to the rejection Python application's API using a curl command.
	 * This method will construct a curl command to update a specific meter's status
	 * and reason on the rejection dashboard. This is typically for single meter updates.
	 *
	...
	/*public static void sendRejectionMeterUpdate(int positionNo, String serialNo, String status, String reason) {
        new Thread(() -> {
            ...
            try {
                ...
                Rejection.logger.debug("Rejection: sendRejectionMeterUpdate : jsonPayloadForCurl: " + jsonPayloadForCurl);
                ...
                Rejection.logger.debug("Rejection: sendRejectionMeterUpdate : command: " + command.toString());
                ProcessBuilder pb = new ProcessBuilder(command);
                pb.inheritIO(); // Inherit I/O to see curl output in Java console

                Rejection.logger.debug("Sending Rejection API Update: " + Arrays.toString(command));
                Process process = pb.start();
                int exitCode = process.waitFor(); // Wait for curl command to complete
                Rejection.logger.debug("Curl command for rejection app exited with code: " + exitCode);

            } catch (IOException | InterruptedException e) {
            	Rejection.logger.error("Error sending rejection meter update: " + e.getMessage());
                e.printStackTrace();
                if (e instanceof InterruptedException) {
                    Thread.currentThread().interrupt(); // Restore interrupt status
                }
	...
	/*public static void sendRejectionMeterUpdate(int positionNo, String serialNo, String status, String reason) {
		new Thread(() -> {
			...
			try {
				...
				Rejection.logger.debug("Rejection: sendRejectionMeterUpdate : jsonPayloadForProcessBuilder: " + jsonPayloadForProcessBuilder);
				...
				Rejection.logger.debug("Rejection: sendRejectionMeterUpdate : command: " + Arrays.toString(command));
				ProcessBuilder pb = new ProcessBuilder(command);
				pb.inheritIO(); // Inherit I/O to see curl output in Java console
	...
	public void sendRestApiPalletUpdate(String palletQrCode) {
		...
		new Thread(() -> {
			try {
				//Rejection.logger.debug("sendRestApiMeterStatusUpdate: status-1: <" + status + "> : positionNo: " + positionNo);
				//ConveyorClientManager cluster1ClientManager = new ConveyorClientManager();
				String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
				String clusterId = ConstantConveyorConfig.CONVEYOR_REJECTION_CLUSTER_ID;//"4";
				Rejection.logger.debug("sendRestApiPalletUpdate: clusterId: <" + clusterId + "> : palletQrCode: " + palletQrCode);
				BayUtils bayUtils = new BayUtils();
				ClusterServer clusterServer =  bayUtils.getServerDetails(terminalId, clusterId);
				ConveyorClientManager cluster4ClientManager = ConveyorClientManager.getInstance(clusterId);
				String targetDisplay= ConstantConveyorConfig.REST_API_TAIL_END_REJECTION_DISPLAY;// "rejection_meters";
				//String status2 = status.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
				//Rejection.logger.debug("sendRestApiMeterStatusUpdate: status-2: <" + status2 + "> : positionNo: " + positionNo);
				cluster4ClientManager.getRestConvClient().sendPalletNumberUpdate(clusterServer,targetDisplay,palletQrCode);


				Rejection.logger.debug("sendRestApiPalletUpdate: Exit");
			} catch (Exception e) {
				Rejection.logger.error("sendRestApiPalletUpdate : Error sending rejection pallet update: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
				e.printStackTrace();
				if (e instanceof InterruptedException) {
					Thread.currentThread().interrupt(); // Restore interrupt status
				}
			...
			try {
				...
				for (Map<String, Object> meter : metersData) {
					try{
						...
						manipulatedStatus = 		manipulatedStatus.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
						...
						Rejection.logger.debug("sendPalletWithMetersStatusUpdate: serial number : " + meterJson.get("meterSerialNo") + " , status: <" + meterJson.get("overallTestResultStatus") + "> : positionNo: " + meterJson.get("rackPositionNo"));

					} catch (Exception e) {
						Rejection.logger.error("sendPalletWithMetersStatusUpdate : Exception in result manipulation: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
						e.printStackTrace();
				...
				cluster4ClientManager.getRestConvClient().sendPalletWithMetersStatusUpdate( clusterServer, targetDisplay,palletQrCode,  resultManipulatedMetersData);
				Rejection.logger.debug("sendPalletWithMetersStatusUpdate: Exit");
			} catch (Exception e) {
				Rejection.logger.error("sendPalletWithMetersStatusUpdate : Error sending Rejection meter update: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
				e.printStackTrace();
				if (e instanceof InterruptedException) {
					Thread.currentThread().interrupt(); // Restore interrupt status
				}
	...
	public void updatePalletStatusOnDisplayMonitor(String palletQrCode, ArrayList<PalletMeter> palletMeterList) {
		...
		new Thread(() -> {
			try {
				...
				//Rejection.logger.debug("updatePalletStatusOnDisplayMonitor: metersData: <" + metersData + ">");
				//ConveyorClientManager cluster1ClientManager = new ConveyorClientManager();
				String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
				String clusterId = ConstantConveyorConfig.CONVEYOR_REJECTION_CLUSTER_ID;//"4";
				BayUtils bayUtils = new BayUtils();
				...
				for(PalletMeter eachPalletMeter: palletMeterList) {
					try{
						...
						resultManipulatedMetersData.add(meterJson);
						Rejection.logger.debug("updatePalletStatusOnDisplayMonitor: serial number : " + meterJson.get("meterSerialNo") + " , status: <" + meterJson.get("overallTestResultStatus") + "> : positionNo: " + meterJson.get("rackPositionNo"));

					} catch (Exception e) {
						Rejection.logger.error("updatePalletStatusOnDisplayMonitor : Exception in result manipulation: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
						e.printStackTrace();

					}
				}

				Rejection.logger.debug("updatePalletStatusOnDisplayMonitor: resultManipulatedMetersData: " + resultManipulatedMetersData);
				//String status2 = status.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
				//Rejection.logger.debug("updatePalletStatusOnDisplayMonitor: status-2: <" + status2 + "> : positionNo: " + positionNo);

				cluster4ClientManager.getRestConvClient().sendPalletWithMetersStatusUpdate( clusterServer, targetDisplay,palletQrCode,  resultManipulatedMetersData);
				Rejection.logger.debug("updatePalletStatusOnDisplayMonitor: Exit");
			} catch (Exception e) {
				Rejection.logger.error("updatePalletStatusOnDisplayMonitor : Error sending Rejection meter update: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
				e.printStackTrace();
				if (e instanceof InterruptedException) {
					Thread.currentThread().interrupt(); // Restore interrupt status
				}
	...
	public void sendRestApiMeterStatusUpdate(int positionNo, String serialNo, String status, String reason) {
		...
		new Thread(() -> {
			try {
				...
				String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
				String clusterId = ConstantConveyorConfig.CONVEYOR_REJECTION_CLUSTER_ID;//"4";
				Rejection.logger.debug("sendRestApiMeterStatusUpdate: clusterId: <" + clusterId + "> : positionNo: " + positionNo);
				BayUtils bayUtils = new BayUtils();
				ClusterServer clusterServer =  bayUtils.getServerDetails(terminalId, clusterId);
				ConveyorClientManager cluster4ClientManager = ConveyorClientManager.getInstance(clusterId);
				String targetDisplay= ConstantConveyorConfig.REST_API_TAIL_END_REJECTION_DISPLAY;// "rejection_meters";
				String status2 = status.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
				Rejection.logger.debug("sendRestApiMeterStatusUpdate: status-2: <" + status2 + "> : positionNo: " + positionNo);
				cluster4ClientManager.getRestConvClient().sendMeterStatusUpdate(clusterServer,targetDisplay,positionNo, serialNo, status2, reason);
				Rejection.logger.debug("sendRestApiMeterStatusUpdate: Exit");
			} catch (Exception e) {
				Rejection.logger.error("sendRestApiMeterStatusUpdate : Error sending rejection meter update: " + e.getMessage() + " : Position :" +positionNo);
				e.printStackTrace();
				if (e instanceof InterruptedException) {
					Thread.currentThread().interrupt(); // Restore interrupt status
				}
	...
	/**
	 * Sends a batch update of meter statuses and reasons to the Python rejection application's API.
	 * This method iterates through a list of meter data and sends an individual curl command
	 * for each meter update.
	 * It also updates the ConveyorOutputMetrics table with daily aggregated metrics and a running
	 * average hourly output for the specified customer and bay type.
	 *
	 * @param metersToUpdate A list of maps, where each map contains "id", "meterSerialNo",
	 * "overallTestResultStatus", and "errorCode" for a meter.
	 * @return true if all updates were successfully initiated, false if any failed to initiate.
	 */
	
	public boolean batchUpdateRejectionMeters(String palletDistinctId,String palletQrCode,List<Map<String, Object>> metersToUpdate) {
    	...
    	//String customerName = DeviceDataManagerController.getConveyorConfigParsedKey().getCustomerName();//."DevSys"; // Fixed customerName
    	String bayType = getMyBayKey(); // Use the bayType from the current instance

    	if (bayType == null || bayType.isEmpty()) {
    		Rejection.logger.error("batchUpdateRejectionMeters: bayType (myBayKey) is null or empty. Cannot update daily metrics.");
    		return false;
    	}
    	...
    	for (Map<String, Object> meterData : metersToUpdate) {
    		try {
    			...
    			if (positionNo != null) {
    				...
    				Rejection.logger.error("batchUpdateRejectionMeters: Meter ID is null, skipping update for: " + serialNo);
    				allUpdatesInitiatedSuccessfully = false;
    			}
    		} catch (Exception e) {
    			Rejection.logger.error("batchUpdateRejectionMeters: Error initiating update for meter data: " + meterData + ". Exception: " + e.getMessage(), e);
    			allUpdatesInitiatedSuccessfully = false;
    		}
    	}
...
*/
    	Rejection.logger.debug("batchUpdateRejectionMeters: Exit. All updates initiated successfully: " + allUpdatesInitiatedSuccessfully);
    	return allUpdatesInitiatedSuccessfully;
    }
	
	public boolean metricsUpdateRejectionMeters(String palletDistinctId,String palletQrCode,ArrayList<PalletMeter> palletMeterList) {
    	...
    	if (bayType == null || bayType.isEmpty()) {
    		Rejection.logger.error("metricsUpdateRejectionMeters: bayType (myBayKey) is null or empty. Cannot update daily metrics.");
    		return false;
    	}
    	...
    	/*for (Map<String, Object> meterData : metersToUpdate) {
    		try {
    			...
    			if (positionNo != null) {
    				...
    				Rejection.logger.error("metricsUpdateRejectionMeters: Meter ID is null, skipping update for: " + serialNo);
    				allUpdatesInitiatedSuccessfully = false;
    			}
    		} catch (Exception e) {
    			Rejection.logger.error("metricsUpdateRejectionMeters: Error initiating update for meter data: " + meterData + ". Exception: " + e.getMessage(), e);
    			allUpdatesInitiatedSuccessfully = false;
    		}
    	}
*/
/*    	if(!restApiSendIndividualMeterStatus) {
    	...
    	Rejection.logger.debug("metricsUpdateRejectionMeters: Exit. All updates initiated successfully: " + allUpdatesInitiatedSuccessfully);
    	return allUpdatesInitiatedSuccessfully;
    }
	
	/*public boolean batchUpdateRejectionMeters(String palletQrCode,List<Map<String, Object>> metersToUpdate) {
		Rejection.logger.debug("batchUpdateRejectionMeters: Entry.");
		if (metersToUpdate == null || metersToUpdate.isEmpty()) {
			Rejection.logger.debug("batchUpdateRejectionMeters: No meters to update in batch.");
			return false;
		...
		// Critical: Ensure bayType is available before proceeding with metrics update
		if (bayType == null || bayType.isEmpty()) {
			Rejection.logger.error("batchUpdateRejectionMeters: bayType (myBayKey) is null or empty. Cannot update daily metrics.");
			return false;
		}
		...
		long diffInMillis = 0;
		// Find or create the daily metrics entry for the specific customer and bayType
		Optional<ConveyorOutputMetrics> metricsOptional = conveyorOutputMetricsService.findByCustomerNameBayTypeAndCurrentDate(customerName, bayType);
		 .orElseGet(() -> {
                                                 ...
                                                 // createdAt for the daily record will be set to the start of the day by the service
                                                 return newMetrics;
                                             });

		if(metricsOptional.isPresent()){
			Rejection.logger.debug("batchUpdateRejectionMeters: bayType " + bayType + " metrics exist");
			metrics= metricsOptional.get();
		}else{
			Rejection.logger.debug("batchUpdateRejectionMeters: bayType " + bayType + " metrics NOT exist");
		}
		...
		Rejection.logger.debug("batchUpdateRejectionMeters: metersToUpdate.size(): " +metersToUpdate.size());
		for (Map<String, Object> meterData : metersToUpdate) {
			try {
				Integer positionNo = (Integer) meterData.get("rackPositionNo");
				String serialNo = (String) meterData.get("meterSerialNo");
				String status = (String) meterData.get("overallTestResultStatus");
				String reason = (String) meterData.get("errorCode");
				Rejection.logger.debug("batchUpdateRejectionMeters: positionNo: " + positionNo + " ,serialNo: " + serialNo + " , status: <" + status + "> , reason: " +reason);
				if (ConstantReport.REPORT_POPULATE_PASS.equalsIgnoreCase(status)) {
					currentPalletPassedMeters++;
					Rejection.logger.debug("batchUpdateRejectionMeters: positionNo : " + positionNo + ": Pass hit1");
				} else if (ConstantReport.REPORT_POPULATE_FAIL.equalsIgnoreCase(status)) {
					currentPalletFailedMeters++;
					Rejection.logger.debug("batchUpdateRejectionMeters: positionNo : " + positionNo + ": Fail hit2");
				}else if (ConstantReport.REPORT_POPULATE_WFR.equalsIgnoreCase(status)) {
					currentPalletPassedMeters++;
					Rejection.logger.debug("batchUpdateRejectionMeters: positionNo : " + positionNo + ": WFR hit3");
				}else{
					currentPalletPassedMeters++;
					Rejection.logger.debug("batchUpdateRejectionMeters: positionNo : " + positionNo + ": Others hit4");
				}
				...
				if (positionNo != null) {
					...
					ConveyorDeviceDataManagerController.getDashboardObject().updatePalletMeterStatusByBayAndPositionWithSerialNo(
							getMyBayKey(), positionNo, serialNo, meterStatus, reason);
				} else {
					Rejection.logger.error("batchUpdateRejectionMeters: Meter ID is null, skipping update for: " + serialNo);
					allUpdatesInitiatedSuccessfully = false;
		...
		// After processing all meters for the current pallet, update the daily ConveyorOutputMetrics
		metrics.setPalletOutput(metrics.getPalletOutput() + 1); // Increment pallet count for the day
		metrics.setTotalNoOfMeters(metrics.getTotalNoOfMeters() + currentPalletTotalMeters);
		metrics.setPassedMeters(metrics.getPassedMeters() + currentPalletPassedMeters);
		metrics.setFailedMeters(metrics.getFailedMeters() + currentPalletFailedMeters);
		...
		// The current time 'now' represents the end of the active period for average calculation
		Date now = new Date();
		if(metricsOptional.isPresent()){
			diffInMillis = now.getTime() - metrics.getCreatedAt().getTime(); // Time elapsed since start of day
			Rejection.logger.debug("batchUpdateRejectionMeters: diffInMillis: " + diffInMillis);
		}
		double hoursElapsed = (double) diffInMillis / (1000 * 60 * 60); // Convert milliseconds to hours
		Rejection.logger.debug("batchUpdateRejectionMeters: hoursElapsed: " + hoursElapsed);
		// To avoid division by zero or inflated "per hour" numbers for very short durations
		// If the elapsed time is less than an hour, we'll consider it 1 hour for averaging purposes.
		...
		Rejection.logger.debug("batchUpdateRejectionMeters: calculatedAverage: " + calculatedAverage);
		metrics.setAverageHourlyOutput(calculatedAverage);


		try {
			conveyorOutputMetricsService.saveToDb(metrics);
			Rejection.logger.info("ConveyorOutputMetrics (Daily) updated successfully for customer: " + customerName + ", bay: " + bayType + " for today. Avg Hourly Output: " + String.format("%.2f", calculatedAverage));
			Platform.runLater(()->{
				ConveyorDeviceDataManagerController.getDashboardObject().refreshMetricsTable();
			});

		} catch (Exception e) {
			Rejection.logger.error("Error saving ConveyorOutputMetrics (Daily): " + e.getMessage(), e);
			allUpdatesInitiatedSuccessfully = false;
		}
		...
		// SUMMARY - UPDATE
			try {
				...
				Optional<ConveyorOutputMetricsSummary> summaryOptional = conveyorOutputMetricsSummaryService.findByCustomerNameBayTypeAndCurrentDate(customerName, bayType);
				...
				hoursElapsed = (double) diffInMillis / (1000 * 60 * 60); // Convert milliseconds to hours
		        Rejection.logger.debug("batchUpdateUnloadingMeters : summary : hoursElapsed: " + hoursElapsed);
		        // To avoid division by zero or inflated "per hour" numbers for very short durations
		        // If the elapsed time is less than an hour, we'll consider it 1 hour for averaging purposes.
		        // This gives a more realistic "rate" from the beginning of operation.
		        if (hoursElapsed < 1.0) {
				...
				Rejection.logger.debug("batchUpdateUnloadingMeters : summary : hoursElapsed: " + hoursElapsed);
				Rejection.logger.debug("batchUpdateUnloadingMeters : summary : TotalNoOfMeters: " + summary.getTotalNoOfMeters());
				Rejection.logger.debug("batchUpdateUnloadingMeters : summary : calculatedSummaryAverage: " + calculatedSummaryAverage);
				...
				Rejection.logger.info("ConveyorOutputMetricsSummary updated for customer: " + customerName + ", bay: " + bayType);
			} catch (Exception e) {
				Rejection.logger.error("Error saving ConveyorOutputMetricsSummary: " + e.getMessage(), e);
			}
			// --------------------- END SUMMARY LOGIC ---------------------
		 	    

		BayUtils bayUtils = new BayUtils();
		bayUtils.updateSummaryMetrics(conveyorOutputMetricsSummaryService, metrics, customerName, bayType, currentPalletTotalMeters, currentPalletPassedMeters, currentPalletFailedMeters);
