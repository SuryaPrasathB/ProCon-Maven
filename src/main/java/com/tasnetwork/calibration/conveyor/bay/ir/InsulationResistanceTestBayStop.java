...
public class InsulationResistanceTestBayStop extends TimerTask{

	//public static Logger logger = Logger.getLogger(InsulationResistanceTestBayStop.class.getPackage().getName());
	private IrtBayContext irtBayStopStateManager = new IrtBayContext();
	
	public void run() {
		Ir.logger.debug("InsulationResistanceTestBayStop : Entry"); 

		manageInsulationResistanceTestBayStopStates();
		//DevSysEnergyMeter.sendReadNeutralCurrentCommand();
	}

	private void manageInsulationResistanceTestBayStopStates() {

		Ir.logger.debug("InsulationResistanceTestBayStop : manageInsulationResistanceTestBayStopStates : Entry");

		//setTableStatePlanner_IrtBay(StatePlannerController.getTableStatePlannerIrtBay_UI());
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.IR_BAY_KEY, "STOP");//findByBayKey(ConstantConveyor.IR_BAY_KEY);
		...
		if(getTableStatePlanner_IrtBay().size()>0) {
			...
			Ir.logger.debug("InsulationResistanceTestBayStop : manageInsulationResistanceTestBayStopStates : No states found in the planner");
		}
	...
	public void setNextState(IrtBayState newState) {
		...
		// Set the new state
		irtBayStopStateManager.setState(newState);  
	...
	public BayResponse processCurrentState(){
		// Process the current state
		BayResponse bayStatus = irtBayStopStateManager.processPresentState();

		Ir.logger.debug("processCurrentState : " + irtBayStopStateManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		Ir.logger.debug("processCurrentState : " + irtBayStopStateManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}

	public IrtBayState getPreviousState(){
		return irtBayStopStateManager.getLastProcessedBayState();
	}
