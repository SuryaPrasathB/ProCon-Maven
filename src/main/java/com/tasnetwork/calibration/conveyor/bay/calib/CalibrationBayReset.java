...
public class CalibrationBayReset extends TimerTask{
	...
	private void manageCalibrationBayResetStates() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.CALIBRATION_BAY_KEY, "RESET");//findByBayKey(ConstantConveyor.CALIBRATION_BAY_KEY);
		...
		if(getTableStatePlanner_CalibBay().size()>0) {
			...
			Calib.logger.debug("CalibrationBayReset : manageCalibrationBayResetStates : getTableStatePlanner2 : Size : " + getTableStatePlanner_CalibBay().size());
			...
			Calib.logger.debug("CalibrationBayReset : manageCalibrationBayResetStates : No states found in the planner");
		}
	...
	public BayResponse processCurrentState(){
		...
		Calib.logger.debug("processCurrentState : " + calibBayResetStateManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		Calib.logger.debug("processCurrentState : " + calibBayResetStateManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}
