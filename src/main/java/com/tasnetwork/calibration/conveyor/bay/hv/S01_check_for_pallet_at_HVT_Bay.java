...
public class S01_check_for_pallet_at_HVT_Bay implements HvtBayState {
	...
	//private boolean simulateHvtBayHappyPath = true;
	...
	public BayResponse handleRequest() {
		...
		boolean isPalletAvailableAt_HVTBay = (boolean)responseReturn.get("status");

		while (!isPalletAvailableAt_HVTBay && 
				!HighVoltageTestBay2.isStopProcessRequestedHvtBay() &&
        		(!ConstantConveyor.ALL_LOOP_BREAK_FLAG)) {

			HighVoltageTestBay.logger.info("S01_check_for_pallet_at_HVT_Bay : No pallet Available at HVT Bay");
			BayUtils.delay(1000);
		...
		if (isPalletAvailableAt_HVTBay) {
			HighVoltageTestBay.logger.info("S01_check_for_pallet_at_HVT_Bay : Pallet Available");
			bayResponse.setStatus(true);
			bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
		} else {
			HighVoltageTestBay.logger.info("S01_check_for_pallet_at_HVT_Bay : Pallet Not Available");
	...
	private Map<String,Object> isPalletAvailableAt_HVTBay() {
		...
		if (portInfo != null) {
			if(logEnabled) {
				...
				Hv.logger.debug("isPalletAvailableAt_HVTBay : getClusterId: " +portInfo.getClusterId() + " -> getBayId: " + portInfo.getBayId() + " -> getPortId: " + portInfo.getPortId() );
