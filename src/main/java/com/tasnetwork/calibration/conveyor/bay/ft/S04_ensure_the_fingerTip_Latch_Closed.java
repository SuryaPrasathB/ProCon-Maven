...
/**
 * State class responsible for verifying that the fingertip latch has
 * successfully closed.
 * It polls the sensor input up to a maximum number of retries.
 */
public class S04_ensure_the_fingerTip_Latch_Closed implements FtBayState {
	...
	public BayResponse handleRequest() {
		...
		Ft.logger.info(String.format(
				"[%s] : [FINGERTIP_STATUS_CHECK] : [SEQUENCE_ENTRY] - Ensuring fingertip latch is closed.",
				getMyBayKey()));
		...
		while (try_count <= 3 && !Ft.isStopProcessRequestedFtBay() && !ConstantConveyor.ALL_LOOP_BREAK_FLAG) {
			Ft.logger.debug(String.format(
					"[%s] : [FINGERTIP_STATUS_CHECK] : [RETRY] - Attempt %d of 4 to verify fingertip latch status.",
					getMyBayKey(), try_count + 1));
			...
			if (Constant_IO_ActionMapping.OPEN.equals(fingerLatchPresentState)) { // Verify success
				// Log success
				Ft.logger.info(String.format(
						"[%s] : [FINGERTIP_STATUS_CHECK] : [SUCCESS] - Finger Tip Latch confirmed closed. State: %s",
						getMyBayKey(), fingerLatchPresentState));
				...
				if (ProconFeatureEnable.MOTOR_CONTROL_ENABLE) {
					Ft.logger.info(String.format(
							"[%s] : [MOTOR_CONTROL] : [ENABLE_CHECK] - Motor control is enabled. Setting motor not required.",
							getMyBayKey()));
					...
					if (set_motor_not_required) {
						Ft.logger.info(String.format("[%s] : [MOTOR_CONTROL] : [SUCCESS] - Motor set to not required.",
								getMyBayKey()));
						...
						Ft.logger.error(String.format(
								"[%s] : [MOTOR_CONTROL] : [FAILED] - Failed to set motor not required. Error: %s",
								getMyBayKey(), ConvErrorCodeMapping.ERROR_CODE_CALIB_026));
						bayResponse.setStatus(false);
						bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_CALIB_026);
					}
				} else {
					Ft.logger.debug(String.format(
							"[%s] : [MOTOR_CONTROL] : [DISABLED] - Motor control is not enabled. Skipping motor settings.",
							getMyBayKey()));
				}
				...
				Ft.logger.warn(String.format(
						"[%s] : [FINGERTIP_STATUS_CHECK] : [MISMATCH] - Finger Tip Latch state is not CLOSED. Expected: %s, Actual: %s. Retrying...",
						getMyBayKey(), Constant_IO_ActionMapping.OPEN, fingerLatchPresentState));
				bayResponse.setStatus(false);
		...
		if (!Constant_IO_ActionMapping.OPEN.equals(fingerLatchPresentState)) {
			...
			Ft.logger.error(String.format(
					"[%s] : [FINGERTIP_STATUS_CHECK] : [FINAL_FAILURE] - Failed to ensure fingertip latch is closed after %d attempts. Final state: %s. Error: %s",
					getMyBayKey(), try_count, fingerLatchPresentState, ConvErrorCodeMapping.ERROR_CODE_FT_003));
		}

		// Structured log for sequence exit
		Ft.logger.info(String.format(
				"[%s] : [FINGERTIP_STATUS_CHECK] : [SEQUENCE_EXIT] - Fingertip latch check sequence completed.",
				getMyBayKey()));
		return bayResponse;
	}
	// ============================================================================================================================================
	...
	private Map<String, Object> ftBay_FingerTipLatch_Status() {
		...
		Ft.logger.debug(String.format(
				"[%s] : [FINGERTIP_SENSOR_READ] : [REQUEST_ENTRY] - Reading fingertip latch sensor status.",
				getMyBayKey()));
		...
		if (portInfo != null) {
			Ft.logger.debug(
					String.format("[%s] : [FINGERTIP_SENSOR_READ] : [PORT_INFO] - PortId: %s, ClusterId: %s, BayId: %s",
			...
			Ft.logger.error(String.format(
					"[%s] : [CONFIG_ERROR] : [FINGERTIP_SENSOR_READ] - Input port not found for fingertip latch sensor: %s",
					getMyBayKey(), ConstantBayPortNameMapping.FT_PORT_NAME_SNSR_FINGER_TIP));
		...
		try {
			...
			Ft.logger.error(String.format(
					"[%s] : [COMMUNICATION_ERROR] : [FINGERTIP_SENSOR_READ] - Failed to get input data from fingertip latch sensor. Port: %s. Error: %s",
		...
		// Based on previous contexts, OLD_OFF_NEW_ON seems to represent 'closed' state
		// for active sensors.
		// Re-evaluating the logic here: if the sensor reports ON for closed, and OFF
		// for open.
		// If `state.equals(Constant_IO_ActionMapping.OLD_OFF_NEW_ON)` means the sensor
		// is ON (active),
		// and active implies closed, then the result should be OLD_CLOSE_NEW_OPEN.
		// If `state.equals(Constant_IO_ActionMapping.OLD_ON_NEW_OFF)` means the sensor
		// is OFF (inactive),
		// and inactive implies open, then the result should be OLD_OPEN_NEW_CLOSE.
		String interpretedState = state.equals(Constant_IO_ActionMapping.ON) ? Constant_IO_ActionMapping.OPEN
		...
		if (StateExecutorController.simulateFtBayHappyPath) {
			...
			Ft.logger.debug(
					String.format("[%s] : [SIMULATION] : Fingertip latch status overridden to CLOSED.", getMyBayKey()));
		}
		...
		if (portInfo.getPortId().equals(state)) { // This check seems to be for a timeout or invalid response
			...
			Ft.logger.warn(String.format(
					"[%s] : [FINGERTIP_SENSOR_READ] : [TIMEOUT] - Fingertip latch sensor read timed out or invalid response. Raw state: %s",
					getMyBayKey(), state));
		} else {
		...
		Ft.logger.debug(String.format(
				"[%s] : [FINGERTIP_SENSOR_READ] : [REQUEST_EXIT] - Fingertip latch sensor read completed. Interpreted state: %s",
				getMyBayKey(), interpretedState));
