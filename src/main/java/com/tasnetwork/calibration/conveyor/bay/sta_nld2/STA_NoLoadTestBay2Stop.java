...
public class STA_NoLoadTestBay2Stop extends TimerTask {
	...
	private void manageSta2StopStates() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService()
				.findByBayKeyAndExecutionMode(ConstantConveyor.STA_NLD2_BAY_KEY, "STOP");// findByBayKey(ConstantConveyor.STA_NLD2_BAY_KEY);
		...
		if (getTableStatePlanner_StaNldTestBay2().size() > 0) {
			...
			StaNld_Bay2.logger.debug("STA_NoLoadTestBay2Stop : manageSta2StopStates : getTableStatePlanner2 : Size : "
					+ getTableStatePlanner_StaNldTestBay2().size());
	 ...
	 * BayResponse bayStatus = processCurrentState();
	 * 
	 * ShortCircuit_NoLoadTestBay2.logger.
	...
	public BayResponse processCurrentState() {
		...
		StaNld_Bay2.logger
				.debug("processCurrentState : " + sctNltBay2StopStateManager.getState().getClass().getSimpleName()
