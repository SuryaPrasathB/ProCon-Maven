...
public class STA_NoLoadTestBay2Stop extends TimerTask{
	//public static Logger logger = Logger.getLogger(STA_NoLoadTestBay2.class.getPackage().getName()); 
	private STA_NoLoadTestBay2Context sctNltBay2StopStateManager = new STA_NoLoadTestBay2Context();  
	public static boolean abort_SCT_NLT_Bay2 = false ;
	public void run() {
		StaNld_Bay2.logger.debug("STA_NoLoadTestBay2Stop : Entry"); 

		manageSta2StopStates();
		//DevSysEnergyMeter.sendReadNeutralCurrentCommand();
	}

	private void manageSta2StopStates() {

		StaNld_Bay2.logger.debug("STA_NoLoadTestBay2Stop : manageSta2StopStates : Entry");

		//setTableStatePlanner_SctNltBay2(StatePlannerController.getTableStatePlannerSctNltBay2_UI());
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.STA_NLD2_BAY_KEY, "STOP");//findByBayKey(ConstantConveyor.STA_NLD2_BAY_KEY);
		...
		if(getTableStatePlanner_StaNldTestBay2().size()>0) {
			...
			StaNld_Bay2.logger.debug("STA_NoLoadTestBay2Stop : manageSta2StopStates : getTableStatePlanner2 : Size : " + getTableStatePlanner_StaNldTestBay2().size());
			...
			StaNld_Bay2.logger.debug("STA_NoLoadTestBay2Stop : manageSta2StopStates : No states found in the planner");
		}

		StaNld_Bay2.logger.debug("STA_NoLoadTestBay2Stop : manageSta2StopStates : Exit");
	...
	public void setNextState(STA_NoLoadTestBay2State newState) {
		...
		// Set the new state
		sctNltBay2StopStateManager.setState(newState);  
	...
	public BayResponse processCurrentState(){
		// Process the current state
		BayResponse bayStatus = sctNltBay2StopStateManager.processPresentState();

		StaNld_Bay2.logger.debug("processCurrentState : " + sctNltBay2StopStateManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		StaNld_Bay2.logger.debug("processCurrentState : " + sctNltBay2StopStateManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}

	public STA_NoLoadTestBay2State getPreviousState(){
		return sctNltBay2StopStateManager.getLastProcessedBayState();
	}
