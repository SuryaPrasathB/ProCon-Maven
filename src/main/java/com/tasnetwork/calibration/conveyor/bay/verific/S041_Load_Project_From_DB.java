...
public class S041_Load_Project_From_DB implements VerificTestBayState {
	...
	public BayResponse handleRequest() {
		...
		/*
		 * ProjectExecutionController projectExecutionController = new
		 * ProjectExecutionController();
		 * projectExecutionController.initiateLoadProjectFromDB();
		 * 
		 * while (!projectExecutionController.isProjectLoadedFromDB()){//
		 * isVoltageStartAcknowledged()) {
		 * CalibrationBay.logger.
		 * info("S041_Load_Project_From_DB : Project Load : awaiting..");
		 * BayUtils.delay(1000);
		 * }
		 * 
		 * if (projectExecutionController.isProjectLoadedFromDB()) {
		 * VerificationTestBay.logger.
		 * info("S041_Load_Project_From_DB : Project Load Success");
		 * bayResponse.setStatus(true);
		 * bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
		 * } else {
		 * VerificationTestBay.logger.
		 * info("S041_Load_Project_From_DB : Project Load Failed");
	 ...
	 * 
	 * VerificationTestBay.logger.
	 * debug("S041_Load_Project_From_DB : startCalibSource : Entry");
	 * 
	 ...
	 * VerificationTestBay.logger.
	 * debug("S041_Load_Project_From_DB : startCalibSource : Turned on Source Start Pin : Success"
	 * );
	 * BayUtils.delay(500);
	 * } else {
	 * VerificationTestBay.logger.
	 * debug("S041_Load_Project_From_DB : startCalibSource : Failed to turn on Source Start Pin"
	 * );
	 ...
	 * 
	 * VerificationTestBay.logger.
	 * debug("S08_functional_Test : startCalibSource : Exit");
