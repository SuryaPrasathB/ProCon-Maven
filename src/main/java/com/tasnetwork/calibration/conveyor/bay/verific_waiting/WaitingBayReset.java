...
public class WaitingBayReset extends TimerTask{
	...
	public void run() {
		VerificWaiting.logger.debug("WaitingBayReset : Entry"); 

		manageWaitingBaySResetStates();
		//DevSysEnergyMeter.sendReadNeutralCurrentCommand();
	}

	
	private void manageWaitingBaySResetStates() {

		VerificWaiting.logger.debug("WaitingBayReset : manageWaitingBaySResetStates : Entry");

		//setTableStatePlanner_WaitingBay(StatePlannerController.getTableStatePlannerWaitingBay_UI());
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.WAITING_BAY_KEY, "RESET");//findByBayKey(ConstantConveyor.WAITING_BAY_KEY);
		...
		if(getTableStatePlanner_WaitingBay().size()>0) {
			...
			VerificWaiting.logger.debug("WaitingBayReset : manageWaitingBaySResetStates : getTableStatePlanner2 : Size : " + getTableStatePlanner_WaitingBay().size());
			...
			VerificWaiting.logger.debug("WaitingBayReset : manageWaitingBaySResetStates : getTableStatePlanner2  : No states found in the planner");
		}
		
		VerificWaiting.logger.debug("WaitingBayReset : manageWaitingBaySResetStates : Exit");
