...
public class S051_stop_CALIB_source implements CalibrationBayState {
	...
	/*private Map<String,Object> stopCalibSource() {
		...
		CalibrationBay.logger.debug("S071_stop_CALIB_source : stopCalibSource : Entry");
		...
		if (isSrcStarted_CalibBay) {
			CalibrationBay.logger.debug("S071_stop_CALIB_source : stopCalibSource : Turned on Source Start Pin : Success");
			BayUtils.delay(500);
		} else {
			CalibrationBay.logger.debug("S071_stop_CALIB_source : stopCalibSource : Failed to turn on Source Start Pin");
			isSrcStarted_CalibBay = false;
		}

		status = turn_off_stop_pin_ft_bay();
		if (status) {
	        CalibrationBay.logger.debug("S08_functional_Test : stopCalibSource : Turned Off Source Start Pin : Success");

		} else {
	        CalibrationBay.logger.debug("S08_functional_Test : stopCalibSource : Failed to turn off Source Start Pin");
	        status = false;
		}
	...
	/*private Map<String,Object> turn_off_stop_pin_ft_bay() {
		CalibrationBay.logger.debug("S071_turn_off_stop_pin_ft_bay : ftBay_StartPin_Status : Entry");
		...
		if (portInfo != null) {
			...
			CalibrationBay.logger.debug("S071_turn_off_stop_pin_ft_bay : Output port not found");
			return responseReturn ;
        }
		...
		CalibrationBay.logger.debug("S071_turn_off_stop_pin_ft_bay : ftBay_StartPin_Status : status : " + status); 

		responseReturn.put("status", status);
		
		CalibrationBay.logger.debug("S071_turn_off_stop_pin_ft_bay : ftBay_StartPin_Status : Exit");
		return responseReturn;
	}*/
	//============================================================================================================================================  
