...
public class S041_start_CALIB_source implements CalibrationBayState {
	...
	/*private Map<String,Object> startCalibSource() {
		...
		CalibrationBay.logger.debug("S071_start_CALIB_source : startCalibSource : Entry");
		...
		if (isSrcStarted_CalibBay) {
			CalibrationBay.logger.debug("S071_start_CALIB_source : startCalibSource : Turned on Source Start Pin : Success");
			BayUtils.delay(500);
		} else {
			CalibrationBay.logger.debug("S071_start_CALIB_source : startCalibSource : Failed to turn on Source Start Pin");
			isSrcStarted_CalibBay = false;
		}

		status = turn_off_start_pin_ft_bay();
		if (status) {
	        CalibrationBay.logger.debug("S08_functional_Test : startCalibSource : Turned Off Source Start Pin : Success");

		} else {
	        CalibrationBay.logger.debug("S08_functional_Test : startCalibSource : Failed to turn off Source Start Pin");
	        status = false;
		}
	...
	//	Add Procal Source Start Here
		...
		Calib.logger.debug("S071_turn_on_start_pin_ft_bay : startCalibSource : status : " + responseReturn.get("status"));
	...
	/*private Map<String,Object> turn_off_start_pin_ft_bay() {
		CalibrationBay.logger.debug("S071_turn_off_start_pin_ft_bay : ftBay_StartPin_Status : Entry");

		boolean status = false;
        Map<String,Object> responseReturn = new HashMap<String,Object>();
		responseReturn.put("status", false);
        
		IoPortInfo portInfo = BayUtils.getOutputPortDetails(ConstantBayPortNameMapping.CALIB_PORT_NAME_SRC_START);
		...
		if (portInfo != null) {
			...
			CalibrationBay.logger.debug("S071_turn_off_start_pin_ft_bay : Output port not found");
			return responseReturn ;
        }
		...
		CalibrationBay.logger.debug("S071_turn_off_start_pin_ft_bay : ftBay_StartPin_Status : status : " + status); 

		responseReturn.put("status", status);
		
		CalibrationBay.logger.debug("S071_turn_off_start_pin_ft_bay : ftBay_StartPin_Status : Exit");
		return responseReturn;
	}*/
	//============================================================================================================================================  
