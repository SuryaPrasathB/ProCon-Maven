...
public class S02_qR_Code_Scanning_of_Pallet implements UnloadingBayState {
	...
	// Define the URL for your Python unloading application's single meter update API
    private static final String UNLOADING_SINGLE_METER_API_URL = "http://127.0.0.1:5002/api/unloading_meters/"; // Base URL for PUT
    ...
    //private String myBayKey; // This should be initialized when the state machine for a specific bay starts.
    ...
    public S02_qR_Code_Scanning_of_Pallet() {
        ...
        // For demonstration, let's assume it's set via a setter or passed during instantiation.
    }
...
/*	public void setMyBayKey(String myBayKey) {
	...
	public BayResponse handleRequest() {
		...
		long qrScanningWaitTimeInSec =  DeviceDataManagerController.getConveyorConfigParsedKey().getUnloadingQrScanningWaitTime_InSec();
		
		Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : isSta2PalletsExitInProgress()-X :" +ConveyorDataManager.isSta2PalletsExitInProgress());
		Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : isSta1PalletsExitInProgress() :" +ConveyorDataManager.isSta1PalletsExitInProgress());
		if (  (ConveyorDataManager.isSta1PalletsExitInProgress())  ) {
			...
			Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA1: qrScanningWaitTimeInSec: Entry :" +qrScanningWaitTimeInSec);
			while ( (qrScanningWaitTimeInSec>0) && 
					(!Unloading.isStopProcessRequestedUnloadingBay()) ) {
				Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA1: qrScanningWaitTimeInSec: waiting :" +qrScanningWaitTimeInSec);
				BayUtils.delay(1000);
				qrScanningWaitTimeInSec--;
				if(!ConveyorDataManager.isSta1PalletsExitInProgress()){
					Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA1: isSta1PalletsExitInProgress cleared: Exiting wait loop");	
					break;
				}
			}
			Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA1: qrScanningWaitTimeInSec: Exit :");
		}
		Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : isSta2PalletsExitInProgress()-Y :" +ConveyorDataManager.isSta2PalletsExitInProgress());
		...
		if (  (ConveyorDataManager.isSta2PalletsExitInProgress()) ) {
			...
			qrScanningWaitTimeInSec =  DeviceDataManagerController.getConveyorConfigParsedKey().getUnloadingQrScanningWaitTime_InSec();
			
			Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA2: qrScanningWaitTimeInSec: Entry :" +qrScanningWaitTimeInSec);
			while ( (qrScanningWaitTimeInSec>0) && 
					(!Unloading.isStopProcessRequestedUnloadingBay()) ) {
				Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA2: qrScanningWaitTimeInSec: waiting :" +qrScanningWaitTimeInSec);
				BayUtils.delay(1000);
				qrScanningWaitTimeInSec--;
				if(!ConveyorDataManager.isSta2PalletsExitInProgress()){
					Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : STA2: isSta2PalletsExitInProgress cleared: Exiting wait loop");	
					break;
				}
			}
		...
		if ((bayResponse.getStatus()) ||
			...
			if(bayResponse.getStatus()) {
				...
				try{
					if(BayUtils.getUnloadingBayPalletDistinctIdList().contains(palletQrId)) {
						...
						long palletCount = BayUtils.getUnloadingBayPalletDistinctIdList().stream().filter(e-> e.endsWith(palletQrIdFinal))
								.count();
						Unloading.logger.debug("S02_qR_Code_Scanning_of_Pallet: palletCount : " + palletCount + " for palletQrId :" + palletQrId);
						if(palletCount>1) {
							Unloading.logger.debug("S02_qR_Code_Scanning_of_Pallet: duplicate pallets found for " +  palletQrId +" . Kindly check BayUtils.getUnloadingBayPalletDistinctIdList():" +BayUtils.getUnloadingBayPalletDistinctIdList());
						}else {
							palletDistinctId = BayUtils.getUnloadingBayPalletDistinctIdList().stream().filter(e-> e.endsWith(palletQrIdFinal))
									.findFirst().get();
						}
			...
			if(palletDistinctId.isEmpty()) {
				Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : not success in fetching from getUnloadingBayPalletDistinctIdList");
		        ...
		        if(myPalletManageOptional.isPresent()){
		        	...
		        	Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : found result  in database myPalletManage : " + myPalletManage.getPalletDistinctId());
		        	
		        	palletDistinctId = myPalletManage.getPalletDistinctId();
		        	
		        }else {
		        	Optional<PalletManage> myPalletManageOptional2 =  MySqlServiceManager.getPalletManageService().findLastByPalletQrIdAndPresentBayKey(
		        			palletQrId,getMyBayKey());
			        if(myPalletManageOptional2.isPresent()){
			        	PalletManage myPalletManage = myPalletManageOptional2.get();
			        	Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : found result  in database myPalletManage-2 : " + myPalletManage.getPalletDistinctId());
			        	
			        	palletDistinctId = myPalletManage.getPalletDistinctId();
			        }else{
		        	//fg
			        	Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : result not found in database with today date for Pallet : " + palletQrId);
			        }
		        }
			}else {
				
				BayUtils.getUnloadingBayPalletDistinctIdList().remove(palletDistinctId);
				Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet : removed palletDistinctId from getUnloadingBayPalletDistinctIdList :" + BayUtils.getUnloadingBayPalletDistinctIdList());
			}
			if(palletDistinctId.isEmpty()) {
				
				if(ConveyorDataManager.isUnloadingBayScreenPalletResultDisplayUserRequested()) {
					palletDistinctId = new String (ConveyorDataManager.getUnloadingBayScreenPalletResultDisplayUserRequestedPalletDistinctId());
					...
					*/
					...
					Unloading.logger.info("S02_qR_Code_Scanning_of_Pallet :processing user request for palletDistinctId: " + palletDistinctId);
				}
			...
			if(!palletDistinctId.isEmpty()) {
				...
				if (metersData != null && !metersData.isEmpty()) {
					batchUpdateUnloadingMeters(palletDistinctId,palletQrId,metersData); // This will now update ConveyorOutputMetrics daily with average hourly output
					
					ReportUtilsV2 reportUtils = new ReportUtilsV2();
					reportUtils.processPalletMeterIndividualResult(palletDistinctId);
				} else {
					Unloading.logger.debug("S02_qR_Code_Scanning_of_Pallet: No meter data found for batch update for bay: " + getMyBayKey());
				}*/
				...
				if (palletMeterList != null && !palletMeterList.isEmpty()) {
					...
					metricsUpdateUnloadingMeters(palletDistinctId,palletQrId,palletMeterList); // This will now update ConveyorOutputMetrics daily with average hourly output
				...
				Unloading.logger.debug("S02_qR_Code_Scanning_of_Pallet: No Pallet details found for bay: " + getMyBayKey());
			...
			Unloading.logger.debug("S02_qR_Code_Scanning_of_Pallet: Qr scanning failed : getResponseData: " + bayResponse.getResponseData());
			sendIdleStatusUpdate();
		}
		...
		if (metersData != null && !metersData.isEmpty()) {
			batchUpdateUnloadingMeters(qrData,metersData); // This will now update ConveyorOutputMetrics daily with average hourly output
		} else {
			Unloading.logger.debug("handleRequest: No meter data found for batch update for bay: " + getMyBayKey());
		}*/
            ...
            try {
                ...
                if (positionNo != null) {
                    ...
                    ConveyorDataManager.getDashboardObject().
                			updatePalletMeterStatusByBayAndPositionWithSerialNo(getMyBayKey(), positionNo, serialNo, meterStatus, 
	...
	public List<Map<String, Object>> readPalletMetersData(String selectedBayTypeKey,String palletDistinctId) {
        ...
        try {
        	/*Unloading.logger.error("readPalletMetersData: getPresentPalletAtBayMap : " + PalletTrackerController.getPresentPalletAtBayMap());
            List<PalletManage> myPalletManage = MySqlServiceManager.getPalletManageService().findByPalletQrIdAndExitNotAppeared(palletQrCode);

            if (myPalletManage.size()==0) {
                Unloading.logger.error("readPalletMetersData: PalletManage not found for pallet Qr code " + palletQrCode);
                return null;
            }*/
            ...
            if(myPalletManageOptional.isPresent()){
            	...
            	Unloading.logger.debug("Unloading : readPalletMetersData: result found for " + selectedBayTypeKey + " : " + palletDistinctId);
            	Unloading.logger.debug("Unloading : readPalletMetersData: result found for " + palletDistinctId + " : getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
            	...
            	//Rejection.logger.debug("Rejection : readPalletMetersData: result found for " + palletQrId + " : getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
            	/*TestReportConveyorController testReportConveyorController = new TestReportConveyorController();
            	testReportConveyorController.exportPalletMeterResult(myPalletManage);*/
            }else{
            	Unloading.logger.debug("readPalletMetersData: result not found for " + selectedBayTypeKey + " : " + palletDistinctId);
                return null;
            }
            Unloading.logger.debug("readPalletMetersData: getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
            ...
            Unloading.logger.debug("readPalletMetersData: updated setExitAppeared to true for getPalletDistinctId: " + myPalletManage.getPalletDistinctId());
...
/*    public static void sendunloadingMeterUpdate(int positionNo, String serialNo, String status, String reason) {
        new Thread(() -> {
            try {
                ...
                System.out.println("Sending unloading API Update: " + Arrays.toString(command));
                Process process = pb.start();
                int exitCode = process.waitFor();
                System.out.println("Curl command for unloading app exited with code: " + exitCode);

            } catch (IOException | InterruptedException e) {
                System.err.println("Error sending unloading meter update: " + e.getMessage());
                e.printStackTrace();
    ...
    public void sendRestApiMeterStatusUpdate(int positionNo, String serialNo, String status, String reason) {
    	...
    	new Thread(() -> {
    		try {
    			Unloading.logger.debug("sendRestApiMeterStatusUpdate: status-1: <" + status + "> : positionNo: " + positionNo);
    			...
    			Unloading.logger.debug("sendRestApiMeterStatusUpdate: status-2: <" + status2 + "> : positionNo: " + positionNo);
    			...
    			Unloading.logger.error("sendRestApiMeterStatusUpdate : Error sending Unloading meter update: " + e.getMessage() + " : Position :" +positionNo);
    			e.printStackTrace();
    			if (e instanceof InterruptedException) {
    ...
    //public void sendPalletWithMetersStatusUpdate(ClusterServer clusterServer,String targetDisplay,String palletQrCode, List<Map<String, Object>> metersData) {
/*    public void sendRestApiMeterStatusUpdate(int positionNo, String serialNo, String status, String reason) {
        	Unloading.logger.debug("sendRestApiMeterStatusUpdate: Entry");
        	new Thread(() -> {
        		try {
    		...
    		try {
    			...
    			for (Map<String, Object> meter : metersData) {
    				try{
	                    ...
	                    Unloading.logger.debug("sendPalletWithMetersStatusUpdate: serial number : " + meterJson.get("meterSerialNo") + " , status: <" + meterJson.get("overallTestResultStatus") + "> : positionNo: " + meterJson.get("rackPositionNo"));
	                    
	    			} catch (Exception e) {
	        			Unloading.logger.error("sendPalletWithMetersStatusUpdate : Exception in result manipulation: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
	        			e.printStackTrace();
	        			
	        		}
                }
    			
    			Unloading.logger.debug("sendPalletWithMetersStatusUpdate: resultManipulatedMetersData: " + resultManipulatedMetersData);
    			//String status2 = status.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
    			//Unloading.logger.debug("sendPalletWithMetersStatusUpdate: status-2: <" + status2 + "> : positionNo: " + positionNo);
    			...
    			Unloading.logger.error("sendPalletWithMetersStatusUpdate : Error sending Unloading meter update: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
    			e.printStackTrace();
    			if (e instanceof InterruptedException) {
    				Thread.currentThread().interrupt(); // Restore interrupt status
    			}
    ...
    public void updatePalletStatusOnDisplayMonitor(String palletQrCode, ArrayList<PalletMeter> palletMeterList) {
    	...
    	new Thread(() -> {
    		try {
    			...
    			//Unloading.logger.debug("updatePalletStatusOnDisplayMonitor: metersData: <" + metersData + ">");
    			//ConveyorClientManager cluster1ClientManager = new ConveyorClientManager();
    			String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
    			String clusterId = ConstantConveyorConfig.CONVEYOR_UNLOADING_CLUSTER_ID;//"4";
    			BayUtils bayUtils = new BayUtils();
    			...
    			for(PalletMeter eachPalletMeter: palletMeterList) {
    				try{
	                    ...
	                    resultManipulatedMetersData.add(meterJson);
	                    Unloading.logger.debug("updatePalletStatusOnDisplayMonitor: serial number : " + meterJson.get("meterSerialNo") + " , status: <" + meterJson.get("overallTestResultStatus") + "> : positionNo: " + meterJson.get("rackPositionNo"));
	                    
	    			} catch (Exception e) {
	        			Unloading.logger.error("updatePalletStatusOnDisplayMonitor : Exception in result manipulation: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
	        			e.printStackTrace();
	        			
	        		}
                }
    			
    			Unloading.logger.debug("updatePalletStatusOnDisplayMonitor: resultManipulatedMetersData: " + resultManipulatedMetersData);
    			//String status2 = status.replace(ConstantReport.REPORT_POPULATE_WFR, ConstantReport.REPORT_POPULATE_PASS).toUpperCase();
    			//Unloading.logger.debug("updatePalletStatusOnDisplayMonitor: status-2: <" + status2 + "> : positionNo: " + positionNo);
    			
    			cluster4ClientManager.getRestConvClient().sendPalletWithMetersStatusUpdate( clusterServer, targetDisplay,palletQrCode,  resultManipulatedMetersData);
    			Unloading.logger.debug("updatePalletStatusOnDisplayMonitor: Exit");
    		} catch (Exception e) {
    			Unloading.logger.error("updatePalletStatusOnDisplayMonitor : Error sending Unloading meter update: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
    			e.printStackTrace();
    			if (e instanceof InterruptedException) {
    				Thread.currentThread().interrupt(); // Restore interrupt status
    			}
    ...
    public void sendRestApiPalletUpdate(String palletQrCode) {
    	...
    	new Thread(() -> {
    		try {
    			...
    			String terminalId = ConstantConveyorConfig.MY_TERMINAL_ID;
    			String clusterId = ConstantConveyorConfig.CONVEYOR_UNLOADING_CLUSTER_ID;//"4";
    			Unloading.logger.debug("sendRestApiPalletUpdate: clusterId: <" + clusterId + "> : palletQrCode: " + palletQrCode);
    			BayUtils bayUtils = new BayUtils();
    			ClusterServer clusterServer =  bayUtils.getServerDetails(terminalId, clusterId);
    			ConveyorClientManager cluster4ClientManager = ConveyorClientManager.getInstance(clusterId);
    			String targetDisplay= ConstantConveyorConfig.REST_API_TAIL_END_UNLOADING_DISPLAY;// "rejection_meters";
    			...
    			Unloading.logger.error("sendRestApiPalletUpdate : Error sending Unloading pallet update: " + e.getMessage() + " : palletQrCode :" +palletQrCode);
    			e.printStackTrace();
    			if (e instanceof InterruptedException) {
    				Thread.currentThread().interrupt(); // Restore interrupt status
    			}
    ...
    public void sendIdleStatusUpdate() {
    	...
    	new Thread(() -> {
    		try {
    			...
    			Unloading.logger.error("sendIdleStatusUpdate : Error sending Unloading meter update: " + e.getMessage() );
    			e.printStackTrace();
    			if (e instanceof InterruptedException) {
    				Thread.currentThread().interrupt(); // Restore interrupt status
    			}
    ...
    /**
     * Sends a batch update of meter statuses to the Python unloading app and updates daily ConveyorOutputMetrics.
     * This method now maintains a single daily record for customer/bay, accumulating totals and calculating
    ...
    public boolean metricsUpdateUnloadingMeters(String palletDistinctId,String palletQrCode,ArrayList<PalletMeter> palletMeterList) {
    	...
    	if (bayType == null || bayType.isEmpty()) {
    		Unloading.logger.error("metricsUpdateUnloadingMeters: bayType (myBayKey) is null or empty. Cannot update daily metrics.");
    		return false;
    	}
...
/*    	for (Map<String, Object> meterData : metersToUpdate) {
    		try {
    			...
    			if (positionNo != null) {
    				...
    				Unloading.logger.error("metricsUpdateUnloadingMeters: Meter ID is null, skipping update for: " + serialNo);
    				allUpdatesInitiatedSuccessfully = false;
    			}
    		} catch (Exception e) {
    			Unloading.logger.error("metricsUpdateUnloadingMeters: Error initiating update for meter data: " + meterData + ". Exception: " + e.getMessage(), e);
    			allUpdatesInitiatedSuccessfully = false;
    		}
    	}*/
...
/*    	if(!restApiSendIndividualMeterStatus) {
    	...
    	Unloading.logger.debug("metricsUpdateUnloadingMeters: Exit. All updates initiated successfully: " + allUpdatesInitiatedSuccessfully);
