...
public class STA_NoLoadTestBay2Reset extends TimerTask {
	...
	private void manageShortCircuit_NoLoadTestBay2ResetStates() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService()
				.findByBayKeyAndExecutionMode(ConstantConveyor.STA_NLD2_BAY_KEY, "RESET");// findByBayKey(ConstantConveyor.STA_NLD2_BAY_KEY);
		...
		if (getTableStatePlanner_StaNldTestBay2().size() > 0) {
			...
			StaNld_Bay2.logger.debug(
					"STA_NoLoadTestBay2Reset : manageShortCircuit_NoLoadTestBay2ResetStates : getTableStatePlanner2 : Size : "
			...
			StaNld_Bay2.logger.debug(
					"STA_NoLoadTestBay2Reset : manageShortCircuit_NoLoadTestBay2ResetStates : No states found in the planner");
		}
	...
	public BayResponse processCurrentState() {
		...
		StaNld_Bay2.logger
				.debug("processCurrentState : " + sctNltBay2ResetStateManager.getState().getClass().getSimpleName()
