...
public class  DevSysEnergyMeter{   
	...
	public boolean calibrationProcess(int positionNum){
		...
		terminalBayProfile = MySqlServiceManager.getTerminalProfileSettingService().findByBayKey(ConstantConveyor.CALIBRATION_BAY_KEY);
		...
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Unlock Command : Success ");
			//	status = devSysEnergyMeter.sendMemoryClearCommand(spManager);    // clear previous calibration values
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Unlock Command : Failed ");
		...
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Calib In Phase Circuit Command: Success ");
			status = sendDeviceCalibInNeutralCktCommand(positionNum,spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Calib In Phase Circuit Command: Failed ");
			spManager.disconnectDut();
			return status;
		}
		//=============================
		...
		/*		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral Circuit Command: Success ");
			status = sendCalibrationLockCommand(positionNum,spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral Circuit Command: Failed ");
			spManager.disconnectDut();
			return status;
		}*/
		...
		/*if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Calibration Lock Command: Success ");
			status = devSysEnergyMeter.sendReadPhaseCurrentCommand(spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Position No: " + positionNum +" : sending Device Calibration Lock Command: Failed ");
		...
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Reading Phase Current : Success ");
			status = devSysEnergyMeter.sendReadNeutralCurrentCommand(spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Reading Phase Current : Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}

		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Reading Neutral Current : Success ");
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : Reading Neutral Current : Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}*/
	...
	public boolean phaseCalibrationProcess(int positionNum){
		...
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Unlock Command : Success ");
			//	status = devSysEnergyMeter.sendMemoryClearCommand(spManager);    // clear previous calibration values
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Unlock Command : Failed ");
			spManager.disconnectDut();
			return status;
		}
		...
		// 3.2 device calibration in phase circuit
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : sending Memory Clear Command : Success ");
			status = sendDeviceCalibInPhaseCktCommand(positionNum,spManager); 
		} else {
			status = false ;
		...
		// 3.4 device calibration in neutral circuit
		/*if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Phase Circuit Command: Success ");
			status = sendDeviceCalibInNeutralCktCommand(positionNum,spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Phase Circuit Command: Failed ");
			spManager.disconnectDut();
			return status;
		}*/
		//=============================
		//3.5 calibration lock
		/*		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral Circuit Command: Success ");
			status = sendCalibrationLockCommand(positionNum,spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral Circuit Command: Failed ");
			spManager.disconnectDut();
			return status;
		}*/
		...
		// 4.1 read phase current 
		/*if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calibration Lock Command: Success ");
			status = devSysEnergyMeter.sendReadPhaseCurrentCommand(spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calibration Lock Command: Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}
		...
		// 4.2 read neutral current 
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Reading Phase Current : Success ");
			status = devSysEnergyMeter.sendReadNeutralCurrentCommand(spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Reading Phase Current : Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}

		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Reading Neutral Current : Success ");
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Reading Neutral Current : Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}*/
		...
		if (status) {
			...
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Phase Circuit: Success ");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
		} else {
			...
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : phaseCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Phase Circuit: Failed ");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			spManager.disconnectDut();
	...
	public boolean neutralCalibrationProcess(int positionNum){
		...
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Unlock Command : Success ");
			//	status = devSysEnergyMeter.sendMemoryClearCommand(spManager);    // clear previous calibration values
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Unlock Command : Failed ");
			spManager.disconnectDut();
			return status;
		}
		...
		// 3.2 device calibration in phase circuit
		/*if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : sending Memory Clear Command : Success ");
			status = sendDeviceCalibInPhaseCktCommand(positionNum,spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : sending Memory Clear Command : Failed ");
			spManager.disconnectDut();
			return status;
		}*/
		...
		// 3.4 device calibration in neutral circuit
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral Circuit Command: Success ");
			status = sendDeviceCalibInNeutralCktCommand(positionNum,spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral Circuit Command: Failed ");
			spManager.disconnectDut();
			return status;
		}
		//=============================
		...
		// 4.1 read phase current 
		/*if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Calibration Lock Command: Success ");
			status = devSysEnergyMeter.sendReadPhaseCurrentCommand(spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Calibration Lock Command: Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}
		...
		// 4.2 read neutral current 
		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Reading Phase Current : Success ");
			status = devSysEnergyMeter.sendReadNeutralCurrentCommand(spManager); 
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Reading Phase Current : Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}

		if (status) {
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Reading Neutral Current : Success ");
		} else {
			status = false ;
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Reading Neutral Current : Failed ");
			spManager.disconnectPwrSrc();
			return status;
		}*/
		...
		if (status) {
			...
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral: Success ");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
			eachBaylogger.debug("DevSysEnergyMeter : ********************************************************");
		} else {
			...
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : neutralCalibrationProcess : Position No: " + positionNum +" : sending Device Calib In Neutral: Failed ");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			eachBaylogger.debug("DevSysEnergyMeter : =========================================================");
			spManager.disconnectDut();
	...
	public BayResponse sendReadPhaseCurrentCommandV2(SpmDut spManager) {
 	...
 	//	receivedData = "02004B0000FFFFFFFF0000300000000000000200000002004B5F2F0031010101030D0A" ;
 		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendReadPhaseCurrentCommand : receivedData : " + receivedData);
		 */
		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVGINST ,DevSysEnergyMeter.ER_DATA_IN_HEX);
		...
		eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommandV2 : phaseCurrent_hexStr : " + phaseCurrent_hexStr);
		...
		/*if (currentValue.equals(null)) {
    		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendReadPhaseCurrentCommand : Send : Failed");
    		status = false;
    	} else {
    		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendReadPhaseCurrentCommand : Send : Success");
    		status = true;
    	}*/

		/*    	if (phaseCurrent>0.0f) {

    		eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommand : sendReadPhaseCurrentCommand : Send : Success");
    		status = true;
    	} else {
    		eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommand : sendReadPhaseCurrentCommand : Send : Failed");
    		status = false;
    	}*/
	...
	public boolean sendReadPhaseCurrentCommand(SpmDut spManager) {
 	...
 	//	receivedData = "02004B0000FFFFFFFF0000300000000000000200000002004B5F2F0031010101030D0A" ;
		...
		eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommand : sendReadNeutralCurrentCommand : receivedData" + receivedData);
		...
		String phaseCurrent_hexStr   = receivedData.substring(42, receivedData.length() - 24);
		eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommand : sendReadCurrentCommand : phaseCurrent_hexStr : " + phaseCurrent_hexStr);
		...
		try {
			...
			eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommand : sendReadPhaseCurrentCommand : Excception : " + e.getMessage());
		}

		float phaseCurrent = (float)phaseCurrent_dec / 100 ;
		eachBaylogger.debug("DevSysEnergyMeter : sendReadPhaseCurrentCommand : sendReadPhaseCurrentCommand : phaseCurrent : " + phaseCurrent);
	...
	public boolean sendReadNeutralCurrentCommand(SpmDut spManager) {
		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : Entry");
		boolean status = false;
 	...
 	//	receivedData = "02004B0000FFFFFFFF0000300000000000000200000002004B5F2F0031010101030D0A" ;
		...
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVGINST ,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendRelayOffCommand : Send : Failed");
			return status;
		}
		String receivedData = (String) responseMap.get("responseData");

		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : receivedData : " + receivedData);
		...
		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : neutralCurrent_hexStr : " + neutralCurrent_hexStr);
		...
		try {
			...
			eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : Excception : " + e.getMessage());
		}

		float neutralCurrent = (float)neutralCurrent_dec / 100 ;
		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadPhaseCurrentCommand : neutralCurrent : " + neutralCurrent);

		/*currentValue = extractAsciiData_DevSys(receivedData);
    	eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : extractedData : " + currentValue);

    	if (currentValue.equals(null)) {
    		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : Send : Failed");
    		status = false;
    	} else {
    		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommand : sendReadNeutralCurrentCommand : Send : Success");
    		status = true;
    	}*/
	...
	public BayResponse sendReadNeutralCurrentCommandV2(SpmDut spManager) {
 	...
 	//	receivedData = "02004B0000FFFFFFFF0000300000000000000200000002004B5F2F0031010101030D0A" ;
		...
		String neutralCurrent_hexStr   = receivedData.substring(46, receivedData.length() - 20);
		eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommandV2 : neutralCurrent_hexStr : " + neutralCurrent_hexStr);
		...
		try {
			neutralCurrent_dec = Integer.parseInt(neutralCurrent_hexStr, 16);
			float neutralCurrent = (float)neutralCurrent_dec / 100 ;
			eachBaylogger.debug("DevSysEnergyMeter : sendReadNeutralCurrentCommandV2 : neutralCurrent : " + neutralCurrent);
			bayResponse.setStatus(true);
	...
	public Map<String, Object> readSerialNumOfMeter(int meterNum, SpmDut spManager) {
		...
		for (int i = 0; i < numOfAttempts; i++) {
			Map<String, Object> responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVGSR, DevSysEnergyMeter.ER_DATA_IN_HEX);
			...
			eachBaylogger.debug("readSerialNumOfMeter : extractedData: " + meterSerialNumber+" :Position: " + meterNum);
	...
	public Map<String, Object> writeSerialNumOfMeter(int meterNum, String dutSerialNo, SpmDut spManager) {
		...
		for (int i = 0; i < numOfAttempts; i++) {
			targetCommand = DevSysEnergyMeter.DEVSSR +
					GuiUtils.asciiToHex(DeviceDataManagerController.getConveyorConfigParsedKey().getDutWriteSerialNoPassword()) +
			...
			eachBaylogger.debug("writeSerialNumOfMeter : targetCommand-Ascii : Position: " + GuiUtils.hexToAsciiV2(targetCommand ));
			Map<String, Object> responseMap = pwrSrcDirector.sendCommandToDut(targetCommand, DevSysEnergyMeter.ER_SERIAL_NO_WRITE_DATA_IN_HEX);
			...
			meterSerialNumber = extractAsciiData_DevSys(receivedData);
			eachBaylogger.debug("writeSerialNumOfMeter : extractedData: " + meterSerialNumber+" :Position: " + meterNum);
	...
	public boolean sendRelayOffCommand(SpmDut spManager) {
		...
		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVRELAYOFF ,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendRelayOffCommand : Send : Failed");
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendRelayOffCommand : receivedData: " + receivedData);

		//String extractedData = extractAsciiData_DevSys(receivedData);
		//eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendRelayOffCommand : extractedData: " + extractedData);
	...
	public boolean sendRelayOnCommand(SpmDut spManager) {
		...
		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVRELAYON,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendRelayOnCommand : Send : Failed");
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendRelayOnCommand : receivedData" + receivedData);

		//String extractedData = extractAsciiData_DevSys(receivedData);
		//eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendRelayOnCommand : extractedData" + extractedData);
	...
	public boolean sendMemoryClearCommand(SpmDut spManager) {
		...
		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVMER ,DevSysEnergyMeter.ER_DATA_IN_HEX);

		/*String receivedData = (String)responseMap.get("responseData");
     	eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendMemoryClearCommand : receivedData" + receivedData);

    	String extractedData = extractAsciiData(receivedData);
    	eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendMemoryClearCommand : extractedData" + extractedData);
    	...
    	if (extractedData.equals(START)) {
    		...
    		status = false;
    		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendMemoryClearCommand : Send : Failed");
    	}*/
	...
	public boolean sendCalibrationLockCommand(int meterNum, SpmDut spManager) {
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : Entry"+" : Position: " + meterNum);
		boolean status = false;

		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVSCALL,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : Send : Failed"+" : Position: " + meterNum);
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : receivedData: " + receivedData+" : Position: " + meterNum);

		String extractedData = extractAsciiData(receivedData);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : extractedData: " + extractedData+" : Position: " + meterNum);

		if (extractedData.equals("\u0000") || extractedData.isEmpty()) {
			status = true;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : Send : Success"+" : Position: " + meterNum);
		} else {
			status = false;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : Send : Failed"+" : Position: " + meterNum);
		}

		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : Exit"+" : Position: " + meterNum);
		return status;
	}
	//============================================================================================================================================  

	public boolean sendDeviceCalibInNeutralCktCommand(int meterNum, SpmDut spManager) {
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : Entry"+" : Position: " + meterNum );
		boolean status = false;

		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVCALN,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : Send : Failed"+" : Position: " + meterNum );
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : receivedData: " + receivedData+" : Position: " + meterNum );

		String extractedData = extractAsciiData(receivedData);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : extractedData: " + extractedData+" : Position: " + meterNum );

		if (extractedData.equals(START)) {
			status = true;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : Send : Success"+" : Position: " + meterNum );
		} else {
			status = false;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : Send : Failed"+" : Position: " + meterNum );
		}

		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInNeutralCktCommand : Exit"+" : Position: " + meterNum );
		return status;
	}
	//============================================================================================================================================  

	public boolean sendDeviceCalibInPhaseCktCommand(int meterNum, SpmDut spManager) {
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : Entry"+" : Position: " + meterNum );
		boolean status = false;

		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVCALP,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : Send : Failed"+" : Position: " + meterNum );
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : receivedData: " + receivedData+" : Position: " + meterNum );

		String extractedData = extractAsciiData(receivedData);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : extractedData: " + extractedData+" : Position: " + meterNum );

		if (extractedData.equals(START)) {
			status = true;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : Send : Success"+" : Position: " + meterNum );
		} else {
			status = false;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : Send : Failed"+" : Position: " + meterNum );
		}

		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceCalibInPhaseCktCommand : Exit"+" : Position: " + meterNum );
		return status;
	}
	...
	public boolean sendDeviceUnlockCommand(int meterNum,SpmDut spManager) {		
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceUnlockCommand : Entry"+" : Position: " + meterNum );
		boolean status = false;

		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVUNLOCK,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : responseMap Status : " + (boolean) responseMap.get("status")+" : Position: " + meterNum );
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendCalibrationLockCommand : Send : Failed" +" : Position: " + meterNum );
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");//sendCommand(DevSysEnergyMeterCommands.DEVUNLOCK);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceUnlockCommand : receivedData : " + receivedData +" : Position: " + meterNum );

		String extractedData = extractAsciiData(receivedData);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceUnlockCommand : extractedData : " + extractedData+" : Position: " + meterNum );


		//if (extractedData.equals(UNLOCKED)) { // while executing in parallel one time exception observed here 
		if (UNLOCKED.equals(extractedData)) {
			status = true;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceUnlockCommand : Send : Success"+" : Position: " + meterNum );
		} else {
			status = false;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceUnlockCommand : Send : Failed"+" : Position: " + meterNum );
		}

		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendDeviceUnlockCommand : Exit"+" : Position: " + meterNum );
		return status;
	}
	...
	public boolean sendDefaultCalibrationCommand(SpmDut spManager) {
		...
		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVDCAL ,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : sendDefaultCalibrationCommand : sendRelayOffCommand : Send : Failed");
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");
		eachBaylogger.debug("DevSysEnergyMeter : sendDefaultCalibrationCommand : sendRelayOffCommand : receivedData: " + receivedData);

		//String extractedData = extractAsciiData_DevSys(receivedData);
		//eachBaylogger.debug("DevSysEnergyMeter : sendDefaultCalibrationCommand : sendRelayOffCommand : extractedData: " + extractedData);

		if (receivedData.equals(DevSysEnergyMeter.ER_DATA_IN_HEX + DevSysEnergyMeter.TERMINATOR )) {
			status = true;
			eachBaylogger.debug("DevSysEnergyMeter : sendDefaultCalibrationCommand : sendRelayOffCommand : Send : Success");
		} else {
			status = false;
			eachBaylogger.debug("DevSysEnergyMeter : sendDefaultCalibrationCommand : sendRelayOffCommand : Send : Failed");
		}
	...
	public boolean sendGsmCheckWithoutSimCommand(int meterNum,SpmDut spManager) {		
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : Entry"+" : Position: " + meterNum );
		boolean status = false;

		DutDirector pwrSrcDirector = new DutDirector(spManager);
		Map<String,Object>  responseMap = pwrSrcDirector.sendCommandToDut(DevSysEnergyMeter.DEVGPRSSTATE,DevSysEnergyMeter.ER_DATA_IN_HEX);
		if (!((boolean) responseMap.get("status"))) {
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : responseMap Status : " + (boolean) responseMap.get("status")+" : Position: " + meterNum );
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : Send : Failed" +" : Position: " + meterNum );
			return status;
		}
		String receivedData = (String)responseMap.get("responseData");//sendCommand(DevSysEnergyMeterCommands.DEVUNLOCK);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : receivedData : " + receivedData +" : Position: " + meterNum );

		String extractedData = extractAsciiData(receivedData);
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : extractedData : " + extractedData+" : Position: " + meterNum );


		if (extractedData.startsWith(PASS)) {
			status = true;
			eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : Send : Success"+" : Position: " + meterNum );
		...
		eachBaylogger.debug("DevSysEnergyMeter : calibrationProcess : sendGsmCheckWithoutSimCommand : Exit"+" : Position: " + meterNum );
		return status;
	}
	...
	public SpmDut serialPortInit(String portCname) {
		...
		try{
			if( (!commBaudRate.isEmpty()) && (!commPortID.isEmpty())){
				//SerialPortManagerPwrSrc_V2 serialPortManagerQrScanner = new SerialPortManagerPwrSrc_V2("qrScn-1");
	...
	public SpmDut serialPortInitV2(DeviceSetting deviceSetting) {
		...
		/*		try {
        	...
        	//cmbBxLDU_PortSelection2.setValue(saved_ldu2_setting.getString("port_name")); 	
				...
				//cmbBxLDU_PortSelection2.setValue("");
				eachBaylogger.info("serialPortInitV2: port_name-1: Data not retrieved from DB");
			...
			//cmbBxLDU_PortSelection2.setValue("");
			eachBaylogger.info("serialPortInitV2: port_name-2: Data not retrieved from database");
		...
		/*		try {
			...
			eachBaylogger.error("serialPortInitV2: JSONException5-2:"+e.getMessage());
			//cmbBxLDU_PortSelection2.setValue("");
			eachBaylogger.info("serialPortInitV2: baudrate-2: Data not retrieved from database");
