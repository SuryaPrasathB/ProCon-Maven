...
public class S01_check_for_pallet_at_Unloading_Bay implements UnloadingBayState {
	...
	public BayResponse handleRequest() {
		...
		while (!stableDetection &&
			...
			if (isPalletAvailableAt_UnloadingBay) {
				...
				if (logEnabled) {
					Unloading.logger
							.info("S01_check_for_pallet_at_Unloading_Bay : No pallet Available at Unloading Bay");
				}
				BayUtils.delay(1000);
			}
			// logEnabled = false;
	...
	private Map<String, Object> isPalletAvailableAt_UnloadingBay() {
		...
		if (portInfo != null) {
			...
			if (logEnabled) {
				Unloading.logger.debug(
						"S01_check_for_pallet_at_Unloading_Bay : isPalletAvailableAt_UnloadingBay :Output port not found");
			}
		...
		if (logEnabled) {
			Unloading.logger.debug(
					"S01_check_for_pallet_at_Unloading_Bay : isPalletAvailableAt_UnloadingBay : state : " + state);
		}
		...
		if (logEnabled) {
			Unloading.logger.debug(
					"S01_check_for_pallet_at_Unloading_Bay : isPalletAvailableAt_UnloadingBay : status : " + status);
		}
