...
public class S058_02_Turn_On_Meter_Relays implements CalibrationBayState {
	...
	public BayResponse handleRequest() {
		...
		for (int i = 0; i < MAX_NUM_OF_METERS; i++) {     // do it for all 6 meters
			...
			Calib.logger.debug("S046_Turn_On_Meter_Relays : portCname : " + portCname + " : Position No: " +positionNum);
			Calib.logger.debug("S046_Turn_On_Meter_Relays: getPortName: " + deviceSetting.getPortName() + " : Position No: " +positionNum);	
			...
			SpmDut spManager = null;
				if(deviceSetting!=null) {
					...
					Calib.logger.debug("S046_Turn_On_Meter_Relays: " + positionNum +  " Failed : Com port not configured");	  
			...
			if(spManager == null){
				...
				if(status){
					status = devSysEnergyMeter.sendRelayOnCommand(spManager);  //dfgvdv status = sendDeviceUnlockCommand(positionNum,spManager); 
					if(status){
