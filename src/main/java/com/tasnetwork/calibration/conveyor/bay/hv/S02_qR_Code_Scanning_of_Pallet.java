    ...
    public BayResponse handleRequest() {
       ...
       /* setSequencePathId("p1");
        ...
        if (status.equals("GOOD")) {
            HighVoltageTestBay.logger.debug("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Successful");
            ...
            HighVoltageTestBay.logger.debug("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Failed");
            HighVoltageTestBay.logger.debug("S02_qR_Code_Scanning_of_Pallet : Issue with Pallet Side");
            // Logic for failure case (status is false)
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_HVT_006);  // Failure error code
        }
        else if(status.equals("SCNR_NW")){
            HighVoltageTestBay.logger.debug("S02_qR_Code_Scanning_of_Pallet : QR Code Scanning Failed");
            HighVoltageTestBay.logger.debug("S02_qR_Code_Scanning_of_Pallet : Issue with Scanner Side");
            // Logic for failure case (status is false)
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_HVT_006);  // Failure error code
        }
