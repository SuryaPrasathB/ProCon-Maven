...
public class S043_Get_Test_Point_Immediately_Bay2 implements STA_NoLoadTestBay2State  {
	...
	public BayResponse handleRequest() {
		StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : Entry");
		...
		while (!(myProcalRemoteResponse.getTestPointStatus().isAllTestExecutionCompleted()) && 
				!(S046_Stop_Execution_Bay2.isStopExecutionRequested()) && i < 60 &&
        		(!ConstantConveyor.ALL_LOOP_BREAK_FLAG)){

			StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : Getting test point status...");		
			...
			StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : presentTpResult : " + presentTpResult);	
		...
		if (myProcalRemoteResponse.getTestPointStatus().isAllTestExecutionCompleted()) {
			StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : Execution Success");

			String myProcalRemoteResult = procalRemoteSender.sendCommAllResultToProcal(stdNldt1ClusterServer, endpoint);

			StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : All results : " + myProcalRemoteResult);
			...
			StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : Execution Failed");
			bayResponse.setStatus(false);
			bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_026);
		}

		StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : Exit");
		return bayResponse;
	}
	...
	private void addTestPointMeterResults(String myProcalRemoteResult) {
		StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : addTestPointMeterResults : Entry ");
		...
		try {
			...
			StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : myPalletManageList : " + myPalletManageList);
			...
			for (int i = 0; i < results.length(); i++) {
				...
				if (selectedPallet != null) {
					StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : addTestPointMeterResults : Adding result to Meter : " + positionNo);
					...
					StaNld_Bay2.logger.warn("S043_Get_Test_Point_Immediately : addTestPointMeterResults : No matching pallet found for position: " + positionNo);
				}
			}
		...
		StaNld_Bay2.logger.info("S043_Get_Test_Point_Immediately : addTestPointMeterResults : Exit ");
	}
