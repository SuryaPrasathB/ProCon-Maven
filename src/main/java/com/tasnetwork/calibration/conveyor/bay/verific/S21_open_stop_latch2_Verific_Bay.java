...
public class S21_open_stop_latch2_Verific_Bay implements VerificTestBayState {

    @Override
    public BayResponse handleRequest() {
    	Verification.logger.info("S21_open_stop_latch2_Verific_Bay : Entry");
       ...
       // BayUtils.delay(2000);
        ...
        if (openStopLatch_Verific_Bay) {
            Verification.logger.info("S21_open_stop_latch2_Verific_Bay : Stop Latch Opened");
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
            
            BayUtils.delay(3000);
        } else {
            Verification.logger.info("S21_open_stop_latch2_Verific_Bay : Failed to Open Stop Latch");
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_VERIFIC_021 );
        }

        Verification.logger.info("S21_open_stop_latch2_Verific_Bay : Exit");
        return bayResponse;
    }

    private Map<String, Object> open_StopLatch_Verific_Bay() {
        Verification.logger.debug("S21_open_stop_latch2_Verific_Bay : verificBay_StopLatch_Status : Entry");
        ...
        if (portInfo != null) {
            ...
            Verification.logger.debug("S21_open_stop_latch2_Verific_Bay : open_StopLatch_Verific_Bay : state : " + state);
            ...
            Verification.logger.debug("S21_open_stop_latch2_Verific_Bay : open_StopLatch_Verific_Bay : status : " + status);

        } else {
            Verification.logger.debug("S21_open_stop_latch2_Verific_Bay : Output port not found");
        ...
        Verification.logger.debug("S21_open_stop_latch2_Verific_Bay : verificBay_StopLatch_Status : status : " + status);
        Verification.logger.debug("S21_open_stop_latch2_Verific_Bay : verificBay_StopLatch_Status : Exit");
        return responseReturn;
    }
}
