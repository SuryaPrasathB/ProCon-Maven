...
public class STA_NoLoadTestBay2Bypass extends TimerTask{
	...
	public BayResponse processCurrentState(){
		...
		StaNld_Bay2.logger.debug("processCurrentState : " + staBay2BypassStateManager.getState().getClass().getSimpleName() + " : Status     : " + bayStatus.isStatus());
		StaNld_Bay2.logger.debug("processCurrentState : " + staBay2BypassStateManager.getState().getClass().getSimpleName() + " : Error Code : " + bayStatus.getErrorCode());
		return bayStatus ;
	}
	//====================================================================================================================
	...
	public void manageSTA_NoLoadTestBay2BypassStates2() {
		...
		ArrayList<StateFlow> statePlanner = (ArrayList<StateFlow>) MySqlServiceManager.getStateFlowService().findByBayKeyAndExecutionMode(ConstantConveyor.STA_NLD2_BAY_KEY, ConstantStateModes.BAY_BYPASS);
		...
		if(getTableStatePlanner_StaBay2().size()>0) {
			...
			STA_NoLoadTestBay2State currentState = createStateInstance(currentStateName, errorCode); // Create the state instance
			setNextState(currentState); // Set the first state
	
			StaNld_Bay2.logger.debug("STA_NoLoadTestBay2 : manageSTA_NoLoadTestBay2BypassStates2 : getTableStatePlanner2 : Size : " + getTableStatePlanner_StaBay2().size());
			...
			StaNld_Bay2.logger.debug("STA_NoLoadTestBay2 : manageSTA_NoLoadTestBay2BypassStates2 :  : No states found in the planner");
		}
		//=================================================================================================	
	...
	public STA_NoLoadTestBay2State createStateInstance(String stateName, String errorCode) {
		...
		try {
			...
			try {
				...
				e.printStackTrace();
				throw new IllegalArgumentException("STA_NoLoadTestBay2Bypass : Exception: Unknown state2: " + stateName);
			}
