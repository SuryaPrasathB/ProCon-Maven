...
public class S055_Phase_Calibration implements CalibrationBayState {
	...
	public BayResponse handleRequest() {
		Calib.logger.info("S055_Phase_Calibration : Entry");

		//BayUtils.delay(15000); // 15sec wait time for current to set
		
		for(int i = 1; i <= 15; i ++) {
			Calib.logger.info("S055_Phase_Calibration : Waiting for Main CT Current Set: " + i + "secs");
			BayUtils.delay(1000);
		}
		...
		if (status) {
			Calib.logger.info("S055_Phase_Calibration : Calibration Test Successful");
			bayResponse.setStatus(true);
			...
			for(int i = 1; i <= 60; i ++) {
				Calib.logger.info("S055_Phase_Calibration : Waiting for Phase Calib: T-" + (60-i) + "secs");
				BayUtils.delay(1000);
			}

		} else {
			Calib.logger.info("S055_Phase_Calibration : Calibration Test Failed");
		...
		Calib.logger.info("S055_Phase_Calibration : Exit");
		return bayResponse;
	}
	...
	public Map<String,Object> phaseCalibrationTask() {
		Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Entry");
		...
		if(ProconFeatureEnable.CALIB_DUT_EXECUTION_PROCESS_IN_PARALLEL){
			...
			if(dutManager.isDutAllControlProcessCompleted()){
				...
				Calib.logger.debug("phaseCalibrationTask: All dut task completed");
				for(int positionNo =1; positionNo<= maxDeviceConnected; positionNo++){
			...
			if(ProconFeatureEnable.DEV_SYS_EM1_CONNECTED) {
				...
				Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Meter 1 : status : " + status1);
				Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Meter 2 : status : " + status2);
				Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Meter 3 : status : " + status3);
				Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Meter 4 : status : " + status4);
				Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Meter 5 : status : " + status5);
				Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationTask : Meter 6 : status : " + status6);
			}
		...
		Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationProcess : status : " + status);

		Calib.logger.debug("S055_Phase_Calibration : phaseCalibrationProcess : Exit");
