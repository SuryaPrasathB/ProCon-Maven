...
public class S191_open_stop_latch_Verific_Bay implements VerificTestBayState {
    ...
    public BayResponse handleRequest() {
    	Verification.logger.info("S191_open_stop_latch_Verific_Bay : Entry");
       ...
       // BayUtils.delay(2000);
        ...
        if (openStopLatch_Verific_Bay) {
            Verification.logger.info("S191_open_stop_latch_Verific_Bay : Stop Latch Opened");
            bayResponse.setStatus(true);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
            
            if(S044_Close_Run_Project.isVerificationTestCompleted()) {
            	//if(getPalletsPassedVerificBay()<4){
	            	palletsPassedVerificBay++ ;
	            	Verification.logger.info("S191_open_stop_latch_Verific_Bay : Stop Latch : palletsPassedVerificBay-1 : " +palletsPassedVerificBay);
            	/*}else{
            		VerificationTestBay.logger.info("S191_open_stop_latch_Verific_Bay : Stop Latch Opened-hit1-A");
            	}*/
            } else {
                palletsPassedVerificBay++ ; // REMOVE LATER    
                Verification.logger.info("S191_open_stop_latch_Verific_Bay : Stop Latch : palletsPassedVerificBay-2 : " +palletsPassedVerificBay);
            ...
            if (ProconFeatureEnable.MOTOR_CONTROL_ENABLE) {
            	if(getPalletsPassedVerificBay()<2){
            		Verification.logger.info("S191_open_stop_latch_Verific_Bay : Stop Latch Opened-hit3");
            	...
            	Verification.logger.info("S191_open_stop_latch_Verific_Bay : Stop Latch Opened-hit4");
			} else {
				bayResponse.setStatus(true);
				bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_601);
			}
                   
            Verification.logger.info("S191_open_stop_latch_Verific_Bay : palletsPassedVerificBay : " + palletsPassedVerificBay);
        ...
        Verification.logger.info("S191_open_stop_latch_Verific_Bay : Exit");
        return bayResponse;
    }

    private Map<String, Object> open_StopLatch_Verific_Bay() {
        Verification.logger.debug("S191_open_stop_latch_Verific_Bay : verificBay_StopLatch_Status : Entry");
        ...
        if (portInfo != null) {
            ...
            Verification.logger.debug("S191_open_stop_latch_Verific_Bay : open_StopLatch_Verific_Bay : state : " + state);
            ...
            Verification.logger.debug("S191_open_stop_latch_Verific_Bay : open_StopLatch_Verific_Bay : status : " + status);

        } else {
            Verification.logger.debug("S191_open_stop_latch_Verific_Bay : Output port not found");
        ...
        Verification.logger.debug("S191_open_stop_latch_Verific_Bay : verificBay_StopLatch_Status : status : " + status);
        Verification.logger.debug("S191_open_stop_latch_Verific_Bay : verificBay_StopLatch_Status : Exit");
        return responseReturn;
    }
	...
	public static void setPalletsPassedVerificBay(int palletsPassedVerificBay) {
		S191_open_stop_latch_Verific_Bay.palletsPassedVerificBay = palletsPassedVerificBay;
	}
}
