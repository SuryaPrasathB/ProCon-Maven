...
public class S07_qR_Code_Scanning_of_Meters implements FtBayState {
	...
	private String sequencePathId = "p1"; // Renamed from 'String sequencePathId' to match class member
	
	private String meterSerialNumber = ""; // This field does not seem to be used at class level, consider localizing.

	private String resultStatus = ""; // This field does not seem to be used at class level, consider localizing.
	private String resultValue = ""; // This field does not seem to be used at class level, consider localizing.
	private String testType = ""; // This field does not seem to be used at class level, consider localizing.
	private String testCaseName = ""; // This field does not seem to be used at class level, consider localizing.
	
	// Adjusted as per your request: `myBayKey` is now a class member and returned directly.
    ...
    public BayResponse handleRequest() {
        Ft.logger.info(String.format("[%s] : [METER_QR_SCAN] : [SEQUENCE_ENTRY] - Starting QR code scanning for meters.", getMyBayKey()));
        ...
        // This block handles user input for placing optical readers,
        // it includes UI updates (Platform.runLater) and blocking delays (BayUtils.delay).
        // It is assumed that handleRequest() itself is called from a background thread
        ...
        if(bayResponse.getStatus()){
	        if (ProconFeatureEnable.WAIT_FOR_USER_INPUT) {
	            ...
	            while (!ConstantConveyor.isFT_OPTICAL_PLACED()) {
	                ...
	                // Alternate the tower lamp state (I/O operation, should be on background thread)
	                if (toggle) {
	    	...
	    	Ft.logger.error(String.format("[%s] : [METER_QR_SCAN] : [OVERALL_FAILED] - Meter QR scanning overall status failed. Error: %s", getMyBayKey(), bayResponse.getErrorCode()));
	    }

		// Structured log for sequence exit
		Ft.logger.info(String.format("[%s] : [METER_QR_SCAN] : [SEQUENCE_EXIT] - QR code scanning for meters sequence completed.", getMyBayKey()));
        return bayResponse;
    }
   //============================================================================================================================================  

    private void turn_on_tower_lamp1() {
    	Ft.logger.debug(String.format("[%s] : [TOWER_LAMP_1] : [TURN_ON] - Sending command to turn on Tower Lamp 1 (HV_PORT_NAME_TWR_LAMP2).", getMyBayKey()));
    ...
    private void turn_off_tower_lamp1() {
    	Ft.logger.debug(String.format("[%s] : [TOWER_LAMP_1] : [TURN_OFF] - Sending command to turn off Tower Lamp 1 (HV_PORT_NAME_TWR_LAMP2).", getMyBayKey()));
    	IoPortInfo portInfo = BayUtils.getOutputPortDetails(ConstantBayPortNameMapping.HV_PORT_NAME_TWR_LAMP2);
    ...
    public BayResponse processQrCodeScanningMeterSerialNoFtBay() {
    	Ft.logger.info(String.format("[%s] : [METER_QR_SCAN_PROCESS] : [ENTRY] - Starting parallel/sequential QR scanning for meters.", getMyBayKey()));
        ...
        if(ProconFeatureEnable.FT_QR_SCANNER_EXECUTION_PROCESS_IN_PARALLEL){
            Ft.logger.info(String.format("[%s] : [METER_QR_SCAN_PROCESS] : [PARALLEL] - Executing QR scan in parallel mode.", getMyBayKey()));
            ...
            if(dutManager.isDutAllControlProcessCompleted()){
                ...
                for(int positionNo = 1; positionNo <= maxDutSupported; positionNo++){
                    ...
                    Ft.logger.debug(String.format("[%s] : [PARALLEL_SCAN_RESULT] : Position: %d, Summary: <%s>, SerialNo: %s, HexSerialNo: %s",
                                       getMyBayKey(), positionNo, resultSummary, readQrMeterSerialNo, GuiUtils.StringToHex(readQrMeterSerialNo)));
                    ...
                    if(resultSummary.trim().equals(ConstantReport.RESULT_STATUS_FAIL.trim())) {	
                        ...
                        Ft.logger.debug(String.format("[%s] : [PARALLEL_SCAN_RESULT] : overAllBayResponse set to false", getMyBayKey()));
                    ...
                    if(resultStatus.equals(ConstantReport.REPORT_POPULATE_FAIL)){
                    	PalletMeter responsePalletMeter = palletTracker.updateMetersToPallet(getMyBayKey(), positionNo, resultStatus, ErrorCode.ERR_DUT_QR_READ_SERIAL_NO);
                    }
                }
            } else {
            ...
            Ft.logger.info(String.format("[%s] : [METER_QR_SCAN_PROCESS] : [SEQUENTIAL] - Executing QR scan in non-parallel (sequential) mode.", getMyBayKey()));
            for(int positionNo = 1; positionNo <= maxDutSupported; positionNo++) {
                Ft.logger.debug(String.format("[%s] : [SEQUENTIAL_SCAN] : [INITIATING] - Starting QR scan for position: %d", getMyBayKey(), positionNo));
                BayResponse individualBayResponse = dutQrScanSerialNoProcess(positionNo);	 
                
                Ft.logger.info(String.format("[%s] : [SEQUENTIAL_SCAN_RESULT] : Position: %d, Status: %s, Response Data: %s",
                                   getMyBayKey(), positionNo, individualBayResponse.getStatus(), individualBayResponse.getResponseData()));
                
                String readQrMeterSerialNo = individualBayResponse.getResponseData();
                ConveyorPalletTracking.setPositionToMeterSerialNoMap(positionNo, readQrMeterSerialNo);
                
                if(!individualBayResponse.getStatus()) {
                    Ft.logger.error(String.format("[%s] : [SEQUENTIAL_SCAN] : [FAILED] - QR scan failed for position: %d. Status: %s", getMyBayKey(), positionNo, individualBayResponse.getStatus()));
                    overAllBayResponse.setStatus(false);
    	...
    	Ft.logger.info(String.format("[%s] : [METER_QR_SCAN_PROCESS] : [EXIT] - QR scanning process for meters completed. Overall Status: %s", getMyBayKey(), overAllBayResponse.getStatus()));
	...
	public BayResponse dutQrScanSerialNoProcess(int positionNum){
		Ft.logger.info(String.format("[%s] : [DUT_QR_SCAN] : [ENTRY] - Processing QR scan for DUT at position: %d", getMyBayKey(), positionNum));
		BayResponse bayResponse = new BayResponse();
        ...
        try {
            ...
            if (terminalBayProfile == null) {
                Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [CONFIG_ERROR] - Terminal profile setting not found for bay key: %s", getMyBayKey(), ConstantConveyor.FT_BAY_KEY));
                bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006); // Use specific error code
                return bayResponse;
            }
        } catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [DB_ERROR] - Failed to retrieve terminal profile for bay: %s. Error: %s", getMyBayKey(), ConstantConveyor.FT_BAY_KEY, e.getMessage()), e);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006); // Use specific error code
            return bayResponse;
        }
        ...
        try {
            ...
            if (qrScannerDeviceSetting == null) {
                Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [CONFIG_ERROR] - Device setting not found for QR scanner ID: %s at position %d", getMyBayKey(), qrScannerDeviceId, positionNum));
                bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
                return bayResponse;
            }
            qrScannerPortCname = qrScannerDeviceSetting.getCanName();
        } catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [DEVICE_MANAGER_ERROR] - Failed to get device setting for QR scanner ID: %s. Error: %s", getMyBayKey(), qrScannerDeviceId, e.getMessage()), e);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
            return bayResponse;
        }
		...
		Ft.logger.debug(String.format("[%s] : [DUT_QR_SCAN] : [TEST_STATUS_GUI_ADD] - Added TestInterfaceStatus to GUI for position %d. SerialNo: %s", getMyBayKey(), positionNum, serialNo));

    	String scannedData = "";
        try {
            scannedData = qrScannerObj.scan_QR_code(positionNum);
            Ft.logger.debug(String.format("[%s] : [DUT_QR_SCAN] : [RAW_SCANNED_DATA] - Raw scanned data for position %d: '%s'", getMyBayKey(), positionNum, scannedData));
        } catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [SCAN_EXCEPTION] - Exception during QR scan for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
        ...
        if(scannedData.equals("NO_QR_CODE_AVAILABLE")){  
            ...
            Ft.logger.warn(String.format("[%s] : [DUT_QR_SCAN] : [NO_DATA] - No QR code available for position %d.", getMyBayKey(), positionNum));
			...
			bayResponse.setStatus(false);
			bayResponse.setResponseData(scannedData);
            Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [NETWORK_ERROR] - Scanner network error for position %d.", getMyBayKey(), positionNum));
			...
			bayResponse.setStatus(false);
			bayResponse.setResponseData(scannedData);
            Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [COMM_FAILED] - Communication access failed for position %d.", getMyBayKey(), positionNum));
			...
			bayResponse.setStatus(false);
			bayResponse.setResponseData(scannedData);
            Ft.logger.warn(String.format("[%s] : [DUT_QR_SCAN] : [BAD_READ] - Incomplete/malformed QR read for position %d: '%s'", getMyBayKey(), positionNum, scannedData));
			...
			qrScannerTestIntefaceStatus.setDeviceResponseStatus("Failed");
			qrScannerTestIntefaceStatus.setSerialStatus("Success"); // Communication was fine, but scan failed for specific reason
			qrScannerTestIntefaceStatus.setDeviceResponseData(scannedData);
			bayResponse.setStatus(false);
			bayResponse.setResponseData(scannedData);
            Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [SCAN_FAILED_SPECIFIC] - QR meter serial number scan failed for position %d: '%s'", getMyBayKey(), positionNum, scannedData));
		} else {
            ...
            Ft.logger.info(String.format("[%s] : [DUT_QR_SCAN] : [GOOD_READ] - Successfully scanned QR for position %d: '%s'", getMyBayKey(), positionNum, dutSerialNo));
			...
			Callable<Boolean> taskAddMetersToPalletDb = () -> {
				...
				Ft.logger.debug(String.format("[%s] : [DUT_QR_SCAN] : [DB_ADD_STATUS] - Add meters to pallet DB for position %d: %s", getMyBayKey(), positionNum, queueStatus));
				return queueStatus;
	        };
			...
			try {
			    ...
			    Ft.logger.debug(String.format("[%s] : [DUT_QR_SCAN] : [DB_TASK_COMPLETE] - DB add task for position %d completed with status: %s", getMyBayKey(), positionNum, responseStatus));
			} catch (TimeoutException e) {
				Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [DB_TASK_TIMEOUT] - DB add task for position %d timed out after %d seconds. Error: %s", getMyBayKey(), positionNum, timeoutInSec, e.getMessage()), e);
				...
				Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [DB_TASK_FAILED] - DB add task for position %d failed. Cause: %s", getMyBayKey(), positionNum, e.getCause() != null ? e.getCause().getMessage() : "Unknown"), e.getCause());
                bayResponse.setStatus(false); // Mark as failed due to DB execution error
                bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
			} catch (InterruptedException e) {
			    Ft.logger.error(String.format("[%s] : [DUT_QR_SCAN] : [DB_TASK_INTERRUPTED] - DB add task for position %d interrupted. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
			    Thread.currentThread().interrupt(); // Restore the interrupted status
			...
			Platform.runLater(() -> {
                if (ConveyorDataManager.getDashboardObject() != null) {
                    ConveyorDataManager.getDashboardObject().updateSerialByBayAndPosition(getMyBayKey(), positionNum, finalDutSerialNo);
                    Ft.logger.debug(String.format("[%s] : [UI_UPDATE] : Dashboard updated for position %d with serial: %s", getMyBayKey(), positionNum, finalDutSerialNo));
                } else {
        ...
        if(!qrResponseStatus.equals("GOOD")) {
            ...
            Platform.runLater(() -> {
                if (ConveyorDataManager.getDashboardObject() != null) {
                    ConveyorDataManager.getDashboardObject().updateSerialByBayAndPosition(getMyBayKey(), positionNum, "Scan Failed");
                    Ft.logger.warn(String.format("[%s] : [UI_UPDATE] : Dashboard updated for position %d with 'Scan Failed'.", getMyBayKey(), positionNum));
                } else {
                    Ft.logger.warn(String.format("[%s] : [UI_UPDATE_WARNING] : Dashboard object is null, cannot update 'Scan Failed' for position %d.", getMyBayKey(), positionNum));
                }
            });
        }
    	...
    	if(StateExecutorController.simulateFtBayHappyPath){
    		...
    		Ft.logger.debug(String.format("[%s] : [SIMULATION] : DUT QR scan status overridden to GOOD for position %d.", getMyBayKey(), positionNum));
    	}
        ...
        Ft.logger.info(String.format("[%s] : [DUT_QR_SCAN] : [EXIT] - Processing QR scan for DUT at position %d completed. Status: %s", getMyBayKey(), positionNum, bayResponse.getStatus()));
		return bayResponse;
	}
	
	public BayResponse dutOpticalSerialNoWriteProcess(int positionNum){
		Ft.logger.info(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [ENTRY] - Starting optical serial number write process for position: %d", getMyBayKey(), positionNum));
		BayResponse bayResponse = new BayResponse();
		bayResponse.setStatus(false); // Default to false
		...
		try {
			terminalBayProfile = MySqlServiceManager.getTerminalProfileSettingService().findByBayKey(ConstantConveyor.FT_BAY_KEY);
			if (terminalBayProfile == null) {
                Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [CONFIG_ERROR] - Terminal profile setting not found for bay key: %s", getMyBayKey(), ConstantConveyor.FT_BAY_KEY));
                bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006); // Generic config error
                return bayResponse;
            }
		} catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DB_ERROR] - Failed to retrieve terminal profile for bay: %s. Error: %s", getMyBayKey(), ConstantConveyor.FT_BAY_KEY, e.getMessage()), e);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
            return bayResponse;
		}
		...
		Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DEVICE_ID] - Optical device ID: %s for Position: %d", getMyBayKey(), opticalReaderDeviceId, positionNum));
        ...
        try {
            ...
            if (opticalReaderDeviceSetting == null) {
                Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [CONFIG_ERROR] - Device setting not found for optical reader ID: %s at position %d", getMyBayKey(), opticalReaderDeviceId, positionNum));
                bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
                return bayResponse;
            }
            opticalReaderPortCname = opticalReaderDeviceSetting.getCanName();
            
            Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [PORT_INFO] - Optical port CName: %s, PortName: %s for Position: %d", getMyBayKey(), opticalReaderPortCname, opticalReaderDeviceSetting.getPortName(), positionNum));
        } catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DEVICE_MANAGER_ERROR] - Failed to get device setting for optical reader ID: %s. Error: %s", getMyBayKey(), opticalReaderDeviceId, e.getMessage()), e);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
            return bayResponse;
        }
		...
		opticalReaderTestIntefaceStatus = new TestInterfaceStatus(
				ConstantConveyor.FT_BAY_KEY,
				ConstantBayStateManage.BAY_HP_SEQ_07, // Using BAY_HP_SEQ_07, assuming it's for QR/Optical scan
				ConstantConveyor.DEVICE_TYPE_DUT,
				getSequencePathId(),
				"" + positionNum,
		...
		Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [TEST_STATUS_GUI_ADD] - Added TestInterfaceStatus to GUI for position %d. SerialNo: %s", getMyBayKey(), positionNum, serialNo));

		SpmDut spManager = null;
		try {
			spManager = devSysEnergyMeter.serialPortInitV2(opticalReaderDeviceSetting);
		} catch (Exception e) {
			Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [SERIAL_PORT_EXCEPTION] - Exception during serial port initialization for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
		...
		if(spManager == null){
            ...
            Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [INIT_FAILED] - Serial port initialization failed for position %d.", getMyBayKey(), positionNum));
			return bayResponse;
		} else {
			opticalReaderTestIntefaceStatus.setSerialStatus("Success");
			bayResponse.setStatus(true);
			StateExecutorController.updateTestStatusGui(opticalReaderTestIntefaceStatus);
            Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [INIT_SUCCESS] - Serial port initialized successfully for position %d.", getMyBayKey(), positionNum));
		}

		// 1. Communication Check (Placeholder)
		boolean status = checkAckForCommands(); // This method needs actual implementation
        if (!status) {
            Ft.logger.warn(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [ACK_CHECK_FAILED] - Acknowledgment check failed for position %d. Proceeding with caution or returning.", getMyBayKey(), positionNum));
            // Depending on criticality, you might want to return here:
        ...
        try {
            ...
            Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [UNLOCK_CMD] - Device unlock command sent for position %d.", getMyBayKey(), positionNum));
        } catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [UNLOCK_CMD_ERROR] - Failed to send device unlock command for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
            bayResponse.setStatus(false);
        }
        ...
        try {
            ...
            if (myPalletManage == null) {
                Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DB_ERROR] - Pallet distinct ID: %s not found in DB.", getMyBayKey(), myPalletDistinctId));
                bayResponse.setStatus(false);
            ...
            Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DB_ERROR] - Exception while fetching pallet details for ID: %s. Error: %s", getMyBayKey(), myPalletDistinctId, e.getMessage()), e);
            bayResponse.setStatus(false);
            bayResponse.setErrorCode(ConvErrorCodeMapping.ERROR_CODE_FT_006);
            spManager.disconnectDut();
            return bayResponse;
		...
		if (status) { // This `status` comes from `checkAckForCommands()`. Consider if it needs to be combined with previous failures.
			Ft.logger.info(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [COMM_CHECK_SUCCESS] - Communication check success for position: %d", getMyBayKey(), positionNum));
			...
			try {
		        ...
		        Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [SERIAL_READ_ERROR] - Failed to read serial number for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
		        status = false;
			...
			if (status) {
		        ...
		        Ft.logger.info(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [SERIAL_READ_SUCCESS] - Serial number read for position %d: %s", getMyBayKey(), positionNum, meterSerialNumber));
			} else {
				opticalReaderTestIntefaceStatus.setDeviceResponseStatus("Failed");
				bayResponse.setStatus(false);
				resultStatus = ConstantReport.REPORT_POPULATE_FAIL;
		        Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [SERIAL_READ_FAILED] - Failed to read serial number for position %d. Status: %s", getMyBayKey(), positionNum, resultStatus));
			}

			resultValue = resultStatus; // This seems to be reusing resultStatus for resultValue, confirm intent.
		    ...
		    try {
		        ...
		        Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [ADD_METER_DB] - Meter %s added to pallet for position %d.", getMyBayKey(), meterSerialNumber, positionNum));
		    } catch (Exception e) {
		        Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [ADD_METER_DB_ERROR] - Failed to add meter %s to pallet DB for position %d. Error: %s", getMyBayKey(), meterSerialNumber, positionNum, e.getMessage()), e);
		    ...
		    try {
		        ...
		        Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [ADD_RESULT_DB] - Result added to meter for position %d. Status: %s", getMyBayKey(), positionNum, resultStatus));
		    } catch (Exception e) {
		        Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [ADD_RESULT_DB_ERROR] - Failed to add result to meter DB for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
		        // Decide if this failure should make bayResponse.setStatus(false)
		    }
			...
			Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [SERIAL_READ_PRE_CHECK_FAILED] - Communication check failed or prior step failed for position: %d", getMyBayKey(), positionNum));
			bayResponse.setStatus(false);
			spManager.disconnectDut();
			return bayResponse; 
		}
		...
		if (status) { // This `status` is from the serial number read.
			Ft.logger.info(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [RELAY_ON_CMD] - Sending Relay ON command for position: %d", getMyBayKey(), positionNum));
			...
			opticalReaderTestIntefaceStatus.setSerialNo(String.valueOf(serialNo));
		    Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [TEST_STATUS_GUI_ADD] - Added TestInterfaceStatus for Relay ON. Position %d. SerialNo: %s", getMyBayKey(), positionNum, serialNo));

		    boolean relayCommandStatus = false;
		    try {
		        relayCommandStatus = devSysEnergyMeter.sendRelayOnCommand(spManager);
		    } catch (Exception e) {
		        Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [RELAY_ON_ERROR] - Failed to send Relay ON command for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
			...
			if(relayCommandStatus){
				...
				resultStatus = ConstantReport.REPORT_POPULATE_PASS;
				bayResponse.setStatus(true);
		        Ft.logger.info(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [RELAY_ON_SUCCESS] - Relay ON command successful for position %d.", getMyBayKey(), positionNum));
			}else{
				opticalReaderTestIntefaceStatus.setDeviceResponseStatus("Failed");
				resultStatus = ConstantReport.REPORT_POPULATE_FAIL;
				bayResponse.setStatus(false);
		        Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [RELAY_ON_FAILED] - Relay ON command failed for position %d.", getMyBayKey(), positionNum));
			}
			...
			Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [RELAY_ON_SKIPPED] - Relay ON command skipped for position %d due to prior failure (Serial Number Read Failed).", getMyBayKey(), positionNum));
			bayResponse.setStatus(false);
			spManager.disconnectDut();
		}
        ...
        try {
		    spManager.disconnectDut();
            Ft.logger.debug(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DUT_DISCONNECT] - DUT disconnected for position %d.", getMyBayKey(), positionNum));
        } catch (Exception e) {
            Ft.logger.error(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [DUT_DISCONNECT_ERROR] - Exception during DUT disconnect for position %d. Error: %s", getMyBayKey(), positionNum, e.getMessage()), e);
        }

		if (StateExecutorController.simulateFtBayHappyPath) {
			bayResponse.setStatus(true); // Overriding status for happy path simulation
			Ft.logger.debug(String.format("[%s] : [SIMULATION] : Optical serial write process status overridden to TRUE for position %d.", getMyBayKey(), positionNum));
		}
		
		Ft.logger.info(String.format("[%s] : [OPTICAL_WRITE_PROCESS] : [EXIT] - Optical serial number write process completed for position %d. Final status: %s", getMyBayKey(), positionNum, bayResponse.getStatus()));
		return bayResponse;
	}
