						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							...
							for (int i = 0; i < timezones.length; i++) {
								ApplicationLauncher.logger
										.info("lstamper_Scan_Available_timezones: timezones:" + i + ":" + timezones[i]);
							}
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (response.getStatusCode() == 200) {
						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							...
							ApplicationLauncher.logger
									.info("lstamper_getFirmwareVersion : FirmwareVersionData:" + FirmwareVersionData);
			...
			c.prepareRequest(request).execute(new AsyncCompletionHandler<Response>() {
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (response.getStatusCode() == 200) {
						...
						String msgSuccess = new String();
						msgSuccess = "200";
						// ApplicationLauncher.logger.info("lstamper_Scan_Available_LogFolders:
						// test2:");
						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							...
							for (int i = 0; i < pi_drives.length; i++) {
								System.out
										.println("lstamper_Scan_Available_drives:pi_drives:" + i + ":" + pi_drives[i]);
							}
							 ...
							 * System.out.println("lstamper_Scan_Available_LogFolders:pi_drives:" +i+":"+
							 * Log_Folders[i]);
							 * }
							 */
							// Platform.runLater(() -> {
					...
					if (response.getStatusCode() == 200) {
						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							...
							for (int i = 0; i < Available_files_in_drive.length; i++) {
								System.out.println("lstamper_Scan_files_in_drives:Available_files_in_drive:" + i + ":"
										+ Available_files_in_drive[i]);
							}
	...
	public void lstamper_Scan_Available_LogFoldersV2(Request request) {
		...
		try {
			c.prepareRequest(request).execute(new AsyncCompletionHandler<Response>() {
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (response.getStatusCode() == 200) {
						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							...
							/*
							 * ApplicationLauncher.logger.
							 * debug("lstamper_Scan_Available_LogFolders: Success");
							 * 
							 * try {
							 * Thread.sleep(2000);
							 * } catch (InterruptedException e) {
				...
				public void onThrowable(Throwable t) {
					ApplicationLauncher.logger
							.info("lstamper_Scan_Available_LogFolders: onThrowable:" + t.getMessage());
					LogViewController.EnableScanLogFolderButton();
					LogViewController.EnableCmbBxListOfLogFolder();
					c.close();
				}
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (response.getStatusCode() == 200) {
						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							...
							// {
							if (StringUtils.indexOfAny(myCurrentAPIResponse.getDevice_name(),
									ConstantProTamp.DEVICE_NAME_LIST) != -1) {
						...
						} else if (myCurrentAPIResponse.getStatusCode()
								.equals(ServerProperties.SERVER_CONNECTION_SLAVE_COMM_FAILED)) {
							// else if
							// (myCurrentAPIResponse.getStatusCode().equals(ConstantProTamp.SLAVE_COMM_FAILED)
							// ) {
							// if(myCurrentAPIResponse.getDevice_name().equals(ModeConstant.DEVICE_NAME_LS_PROTAMP))
							// {
							if (StringUtils.indexOfAny(myCurrentAPIResponse.getDevice_name(),
								...
								ScanDeviceController
										.getValidateCredResponseSlaveCommFailedTaskTrigger(myCurrentAPIResponse);
								// ScanDeviceController.ScanDeviceCompletedPostProcess();
								...
								.equals(ServerProperties.SERVER_CONNECTION_SLAVE_WAITING)) {
							// else if
							// (myCurrentAPIResponse.getStatusCode().equals(ConstantProTamp.SLAVE_WAITING)
							// ){
							// if(myCurrentAPIResponse.getDevice_name().equals(ModeConstant.DEVICE_NAME_LS_PROTAMP))
							// {
							if (StringUtils.indexOfAny(myCurrentAPIResponse.getDevice_name(),
	...
	public void lsTamperManual3PhaseStartWithRefStd(Request request) {
		...
		try {
			c.prepareRequest(request).execute(new AsyncCompletionHandler<Response>() {
				...
				public void onThrowable(Throwable t) {
					ApplicationLauncher.logger
							.info("lsTamperManual3PhaseStartWithRefStd: onThrowable:" + t.getMessage());
					c.close();
				}
			});
		} catch (Exception e1) {
			...
			c.prepareRequest(request).execute(new AsyncCompletionHandler<Response>() {
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (response.getStatusCode() == 200) {
						...
						if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
							// ApplicationLauncher.logger.info("lsTamperRefStdPreRequisite
							// :testid:"+myCurrentAPIResponse.getTestid());
							ApplicationLauncher.logger.info("lsTamperRefStdPreRequisite : getRefStdStatus(): "
									+ myCurrentAPIResponse.getRefStdStatus());
							if (myCurrentAPIResponse.getRefStdStatus().equals("RefStdSuccess")) {
								setPreRequisiteResponseStatus(true);

								ApplicationLauncher.logger
										.info("lsTamperRefStdPreRequisite : API-ResponseF3 - Ref Success");
	...
	public void lstamper_viewlog_from_selected_file(Request request) {
		...
		try {
			c.prepareRequest(request).execute(new AsyncCompletionHandler<Response>() {
				...
				public void onThrowable(Throwable t) {
					ApplicationLauncher.logger
							.info("lstamper_viewlog_from_selected_file: onThrowable:" + t.getMessage());
					LogViewController.EnableScanLogFolderButton();
					LogViewController.EnableCmbBxListOfLogFolder();
					LogViewController.EnableScanLogFilesButton();
					LogViewController.EnableViewLogFileButton();
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
						...
						for (int i = 0; i < Available_files_in_folder.length; i++) {
							System.out.println("lstamper_Scan_files_in_logfolder:Available_files_in_folder:" + i + ":"
									+ Available_files_in_folder[i]);
						}
			...
			c.prepareRequest(request).execute(new AsyncCompletionHandler<Response>() {
				...
				public Response onCompleted(Response response) throws IOException {
					...
					if (myCurrentAPIResponse.getStatusCode().equals(msgSuccess)) {
						...
						WindowManager.InformUser("TimeZone Success", "Updated Server TimeZone success",
								AlertType.INFORMATION);

					} else {
						WindowManager.InformUser("TimeZone Failed",
								"Updated Server TimeZone failed, Kindly retry after some time", AlertType.ERROR);
