...
public class SerialRxMessageQ {
	...
	public void ProcessRxMsg() {
		...
		if (lastMessage != null) {
			...
			// ApplicationLauncher.logger.debug("ProcessRxMsg: getExpectedResult hex :"+
			// GuiUtils.asciiToHex(serialPortObj.getExpectedResult()));
			 ...
			 * }
			 */
			if ((lastMessage.startsWith(serialPortObj.getExpectedResult()))) {// && (lastMessage.endsWith("0D"))){
				// if(lastMessage.equals(serialPortObj.getExpectedResult())) {
				ApplicationLauncher.logger.debug("ProcessRxMsg: lastMessage2:" + lastMessage);
				setExpectedResponseRecieved(true);
				setResponseRecieved(true);
				 ...
				 * else {
				 * //ApplicationLauncher.logger.
				 * debug("ProcessRxMsg: Unexpected message dropping the packet4:"+lastMessage);
				 * //removeProcessedMsgFromQueue();
				 * ApplicationLauncher.logger.debug("ProcessRxMsg: Unexpected message7: "
				 * +lastMessage);
				 * setUnknownResponseRecieved(true);
				 * setResponseRecieved(true);
				 * }
