...
public class DutMessenger {
	...
	public Map<String, Object> sendCommandToDut(String payLoadInHex, String expectedDataInHex) {
		...
		String responseStatus = dutMsngrSendCommandProcess(payLoadInHex, timeDelayInMilliSec, expectedDataInHex,
				isResponseExpected, "test-T1");
	...
	public String dutMsngrSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec,
		...
		if (getPwrSrcSpmObj().isDeviceSerialStatusConnected()) {
			...
			if (status) {
				...
				ApplicationLauncher.logger.debug("dutMsngrSendCommandProcess : responseData <" + sourceThread
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					ApplicationLauncher.logger
							.debug("dutMsngrSendCommandProcess :retryCount<" + sourceThread + ">:" + retryCount);
					 ...
					 * ApplicationLauncher.logger.info("dutMsngrSendCommandProcess V2 <"
	...
	public String dutMsngrSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec,
		...
		if (getPwrSrcSpmObj().isDeviceSerialStatusConnected()) {
			...
			if (status) {
				String responseData = (String) responseReturn.get("result");
				ApplicationLauncher.logger.debug(
						"dutMsngrSendCommandProcessWithLength : responseData <" + sourceThread + ">: " + responseData);
				ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessWithLength : responseData <" + sourceThread
						+ "> hex : " + GUIUtils.asciiToHex(responseData));
				// if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)))
				// {
				// if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.ER_CMD_STOP_VOLT_CURRENT_POSITIVE_IN_HEX)))
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					// VI_SendStartCommand(CommandVI_PayLoad,SelectedPhase);
					ApplicationLauncher.logger.debug(
							"dutMsngrSendCommandProcessWithLength :retryCount<" + sourceThread + ">:" + retryCount);
					// int TimeOutInSec= 60;vjhvjgv
					// SerialDataLDU lduData =
					// VICI_ReadDataWithErrorResponse(ExpectedData.length(),ExpectedData,ExpectedError1Data,ExpectedError2Data,TimeOutInSec);
					// ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessWithLength
					// :Hit2");
					if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessWithLength: <" + sourceThread
									+ ">:Ack Response Success");
							// String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							// ApplicationLauncher.logger.info("dutMsngrSendCommandProcessWithLength: VICI
							...
							ApplicationLauncher.logger.debug(
									"dutMsngrSendCommandProcessWithLength <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							// ApplicationLauncher.logger.info("dutMsngrSendCommandProcessWithLength:
							...
							String CurrentReadData = getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug(
									"dutMsngrSendCommandProcessWithLength : isUnknownResponseRecieved : CurrentReadData<"
											+ sourceThread + ">: " + CurrentReadData);
							responseStatus = CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if (responseStatus != null) { // Gopi2
								ApplicationLauncher.logger.debug(
										"dutMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"
												+ sourceThread + ">:" + responseStatus);
								ApplicationLauncher.logger.debug(
										"dutMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"
												+ sourceThread + ">:" + GUIUtils.HexToString(responseStatus));
						...
						ApplicationLauncher.logger.info(
								"dutMsngrSendCommandProcessWithLength : <" + sourceThread + ">: Ack No Data Received");
					}
			...
			ApplicationLauncher.logger.debug("dutMsngrSendCommandProcessWithLength V2 <" + sourceThread
		...
		if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage() == null) {
			ApplicationLauncher.logger
					.info("dutMsngrSendCommandProcessWithLength<" + sourceThread + ">: last read null");
		} else {
			ApplicationLauncher.logger.info("dutMsngrSendCommandProcessWithLength: last read<" + sourceThread + ">:"
					+ getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage());
		}
