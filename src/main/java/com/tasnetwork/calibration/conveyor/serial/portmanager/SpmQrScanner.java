...
public class SpmQrScanner {
	...
	public SpmQrScanner(String traceName, boolean terminatorMandatory){
		...
		//rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
		 rxPhysical_PwrSrc = new SerialRxPhysical(commQrScanner,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
	...
	public SpmQrScanner(String traceName, int expectedResponseHexLength){
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commQrScanner,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
	...
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commQrScanner,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
...
/*		commLDU = new Communicator(ConstantApp.SERIAL_PORT_LDU);
	...
	public void disconnectQrScanner(){
		...
		if(isDeviceSerialStatusConnected()){
			ApplicationLauncher.logger.debug("disconnectQrScanner :isDeviceSerialStatusConnected: " + getMyTraceName() + " : "+ isDeviceSerialStatusConnected());
...
/*				String eachDataInHex = "";
	...
	public Map<String,Object> qrSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,String expectedError1Data,
		...
		if(isDeviceSerialStatusConnected()){
				...
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("qrSendCommandProcess <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("qrSendCommandProcess <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("qrSendCommandProcess :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("qrSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("qrSendCommandProcess <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
						...
						if(ProjectExecutionController.getUserAbortedFlag()){
							...
							ApplicationLauncher.logger.debug("qrSendCommandProcess <"+sourceThread +">: user aborted - detected");
						}else if(!isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("qrSendCommandProcess <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("qrSendCommandProcess <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			...
			//ApplicationLauncher.logger.info("qrSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
	...
	public Map<String,Object> qrSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
		...
		if(isDeviceSerialStatusConnected()){
			...
			if(isResponseExpected){
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("qrSendCommandProcessWithLength <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("qrSendCommandProcessWithLength <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							rxMsgQ_PwrSrc.removeProcessedMsgFromQueue();
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("qrSendCommandProcessWithLength :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("qrSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("qrSendCommandProcessWithLength <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
					     ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_CURRENT_IN_HEX)) ){
						if(ProjectExecutionController.getUserAbortedFlag()){
			...
			ApplicationLauncher.logger.debug("qrSendCommandProcessWithLength <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
