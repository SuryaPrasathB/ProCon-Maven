...
public class VoltPmMessenger {

	//DeviceDataManagerController displayDataObj =  new DeviceDataManagerController();
	SpmVoltPm pwrSrcSpmObj =  new SpmVoltPm();//null;//new DeviceDataManagerController().getSerialPortManagerPwrSrc_V2();
	
	public VoltPmMessenger(SpmVoltPm qrScannerSerialPort){
		this.pwrSrcSpmObj = qrScannerSerialPort;
	}
	
	public VoltPmMessenger(){
...
//==========================================================================================================================	
	...
	public boolean sendVoltageCurrentStopOutputCommand(){
  	...
  	public Map<String,Object> sendReadCommandVoltPm(String slaveId){
  		...
  		//boolean isResponseExpected = true;
           ...
           String frame = slaveIdHex + Elmeasure_MultiMeter.FUNCTION_CODE + Elmeasure_MultiMeter.REGISTER_ADDRESS + Elmeasure_MultiMeter.NUM_BYTES;
         ...
         //  String commandFrame = frame + crc;
  		...
  		ApplicationLauncher.logger.info("sendReadCommandElmeasureMultiMeter: payLoadInHex: " + payLoadInHex  );
  ...
  /*		String addressStr = BofaManager.asciiToHex(String.valueOf((char)(address + ConstantPowerSourceBofa.LDU_ADDRESS_ADDITION))) ;
  		...
  		String responseStatus = elmeasurePanelMeterMsngrSendCommandProcessWithLength(payLoadInHex,timeDelayInMilliSec,expectedDataInHex,expectedDataLengthInHex,isResponseExpected,"test-T2");
  		...
  		if (responseStatus.equals(DeleteMeConstant.SUCCESS_RESPONSE)) {
  			...
  			if(!isResponseExpected){
  				if(responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
  					ApplicationLauncher.logger.info("sendReadCommandElmeasureMultiMeter : no response expected success");
  					status = true;
  				}
  			}
  		}
  ...
  //==================================================================================================================================
	...
	public SpmVoltPm getPwrSrcSpmObj() {
		return pwrSrcSpmObj;
	}
	public void setPwrSrcSpmObj(SpmVoltPm pwrSrcSpmObj) {
	...
	public String elmeasurePanelMeterMsngrSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, 
		...
		//ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess : expectedDataInHex: " + expectedDataInHex);
		String expectedData = "";
		if(isResponseExpected){
		...
		if(getPwrSrcSpmObj().isDeviceSerialStatusConnected()){
			...
			if(status){
				String responseData = (String)responseReturn.get("result");
				ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess : responseData <"+sourceThread +">: " + responseData);
				ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess : responseData <"+sourceThread +"> hex : " + GUIUtils.asciiToHex(responseData));
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))) {
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.ER_CMD_STOP_VOLT_CURRENT_POSITIVE_IN_HEX))) {
					status = true;
				//}else {
			...
			if(isResponseExpected){
				while (retryCount>0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess :retryCount<"+sourceThread +">:" + retryCount);
					...
					//ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess :Hit2");
					if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess: <"+sourceThread +">:Ack Response Success");
							//String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							//ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess: VICI Received Data1:"+CurrentLDU_Data);
							...
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							//ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess: ErrorResponse Received:"+CurrentLDU_Data);
							//ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess: ExpectedError1Data:"+ExpectedError1Data);
							...
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess : isUnknownResponseRecieved : CurrentReadData<"+sourceThread +">: "+ CurrentReadData);
							responseStatus=CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if(responseStatus!=null) { //Gopi2
								ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess :Unexpected Message: dropping the message<"+sourceThread +">:"+responseStatus);
								ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess :Unexpected Message: dropping the message<"+sourceThread +">:"+GUIUtils.HexToString(responseStatus));
							}
							
						}
					}
					else{
						ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess : <"+sourceThread +">: Ack No Data Received");
					}
					
					
					ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess : <"+sourceThread +">test1: ");
...
/*					if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
							...
							if(ProjectExecutionController.getUserAbortedFlag()){
								...
								ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess V2 <"+sourceThread +">: user aborted - detected");
							}
						}else if(!getPwrSrcSpmObj().isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseStatus = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess V2 <"+sourceThread +">: serial port not connected or disconnected");
			
		}
		//ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcess : test2: ");
		if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage()==null) {
			ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess<"+sourceThread +">: last read null");
		}else {
			ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcess: last read<"+sourceThread +">:"+getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage());
		}
	...
	public String elmeasurePanelMeterMsngrSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec, 
		...
		if(getPwrSrcSpmObj().isDeviceSerialStatusConnected()){
			...
			if(status){
				String responseData = (String)responseReturn.get("result");
				ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength : responseData <"+sourceThread +">: " + responseData);
				ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength : responseData <"+sourceThread +"> hex : " + GUIUtils.asciiToHex(responseData));
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))) {
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.ER_CMD_STOP_VOLT_CURRENT_POSITIVE_IN_HEX))) {
					status = true;
				//}else {
			...
			if(isResponseExpected){
				while (retryCount>0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
					retryCount--;
					//VI_SendStartCommand(CommandVI_PayLoad,SelectedPhase);
					ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength :retryCount<"+sourceThread +">:" + retryCount);
					//int TimeOutInSec= 60;vjhvjgv
					//SerialDataLDU lduData = VICI_ReadDataWithErrorResponse(ExpectedData.length(),ExpectedData,ExpectedError1Data,ExpectedError2Data,TimeOutInSec);
					//ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength :Hit2");
					if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength: <"+sourceThread +">:Ack Response Success");
							//String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							//ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength: VICI Received Data1:"+CurrentLDU_Data);
							//StripLDU_SerialData(lduData.getReceivedLength());
							responseStatus=DeleteMeConstant.SUCCESS_RESPONSE;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
						}else if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedErrorResponseReceived()){
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							//ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength: ErrorResponse Received:"+CurrentLDU_Data);
							//ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength: ExpectedError1Data:"+ExpectedError1Data);
							//StripLDU_SerialData(CurrentLDU_Data.length());
							...
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength : isUnknownResponseRecieved : CurrentReadData<"+sourceThread +">: "+ CurrentReadData);
							responseStatus=CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if(responseStatus!=null) { //Gopi2
								ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"+sourceThread +">:"+responseStatus);
								ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"+sourceThread +">:"+GUIUtils.HexToString(responseStatus));
							}
							
						}
					}
					else{
						ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength : <"+sourceThread +">: Ack No Data Received");
					}
					
					
					ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength : <"+sourceThread +">test1: ");
...
/*					if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
							...
							if(ProjectExecutionController.getUserAbortedFlag()){
								retryCount = 0;
								responseStatus = DeleteMeConstant.ERROR_RESPONSE;
								ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength V2 <"+sourceThread +">: user aborted - detected");
							}
						}else if(!getPwrSrcSpmObj().isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseStatus = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength V2 <"+sourceThread +">: serial port not connected or disconnected");
			
		}
		//ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength : test2: ");
		if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage()==null) {
			ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength<"+sourceThread +">: last read null");
		}else {
			ApplicationLauncher.logger.info("elmeasurePanelMeterMsngrSendCommandProcessWithLength: last read<"+sourceThread +">:"+getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage());
		}
		ApplicationLauncher.logger.debug("elmeasurePanelMeterMsngrSendCommandProcessWithLength <"+sourceThread +">:Exit");
		return responseStatus;
	}
