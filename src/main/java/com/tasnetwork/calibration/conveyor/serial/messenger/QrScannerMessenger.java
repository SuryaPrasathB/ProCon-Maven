...
public class QrScannerMessenger {
	...
	SpmQrScanner pwrSrcSpmObj =  new SpmQrScanner();//null;//new DeviceDataManagerController().getSerialPortManagerPwrSrc_V2();
	...
	public boolean sendAnalogTrigger(){
		...
		responseReturn = getPwrSrcSpmObj().qrSendCommandProcess( payLoadInHex, timeDelayInMilliSec,
				expectedData,error1ExpectedDataInHex,isResponseExpected, rxMessageTerminator,"sendVoltageCurrentStopOutputCommand");
...
//==========================================================================================================================	
	...
	public Map<String,Object> sendReadCommandQrCodeScanner(){
		...
		String payLoadInHex = NewlandQRCodeScanner.ANALOG_TRIGGER_SETTING ; //ANALOG_TRIGGER_SETTING;//startTestEndFrame ;xcvxc
...
/*		String addressStr = BofaManager.asciiToHex(String.valueOf((char)(address + ConstantPowerSourceBofa.LDU_ADDRESS_ADDITION))) ;
		...
		String responseStatus = qrMsngrSendCommandProcess(payLoadInHex,timeDelayInMilliSec,expectedDataInHex,error1ExpectedDataInHex,isResponseExpected,"test-T1");
	...
	public String qrMsngrSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, 
		...
		//ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess : expectedDataInHex: " + expectedDataInHex);
		String expectedData = "";
		...
		if(getPwrSrcSpmObj().isDeviceSerialStatusConnected()){
			...
			if(status){
				String responseData = (String)responseReturn.get("result");
				ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess : responseData <"+sourceThread +">: " + responseData);
				ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess : responseData <"+sourceThread +"> hex : " + GUIUtils.asciiToHex(responseData));
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))) {
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.ER_CMD_STOP_VOLT_CURRENT_POSITIVE_IN_HEX))) {
					status = true;
				//}else {
			...
			if(isResponseExpected){
				while (retryCount>0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess :retryCount<"+sourceThread +">:" + retryCount);
					...
					if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess: <"+sourceThread +">:Ack Response Success");
							//String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							//ApplicationLauncher.logger.info("qrMsngrSendCommandProcess: VICI Received Data1:"+CurrentLDU_Data);
							...
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							//ApplicationLauncher.logger.info("qrMsngrSendCommandProcess: ErrorResponse Received:"+CurrentLDU_Data);
							//ApplicationLauncher.logger.info("qrMsngrSendCommandProcess: ExpectedError1Data:"+ExpectedError1Data);
							...
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess : isUnknownResponseRecieved : CurrentReadData<"+sourceThread +">: "+ CurrentReadData);
							responseStatus=CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if(responseStatus!=null) { //Gopi2
								ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess :Unexpected Message: dropping the message<"+sourceThread +">:"+responseStatus);
								ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess :Unexpected Message: dropping the message<"+sourceThread +">:"+GUIUtils.HexToString(responseStatus));
							}
							
						}
					}
					else{
						ApplicationLauncher.logger.info("qrMsngrSendCommandProcess : <"+sourceThread +">: Ack No Data Received");
					}
...
/*					if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
							...
							if(ProjectExecutionController.getUserAbortedFlag()){
								...
								ApplicationLauncher.logger.info("qrMsngrSendCommandProcess V2 <"+sourceThread +">: user aborted - detected");
							}
						}else if(!getPwrSrcSpmObj().isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseStatus = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("qrMsngrSendCommandProcess V2 <"+sourceThread +">: serial port not connected or disconnected");
		...
		if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage()==null) {
			...
			ApplicationLauncher.logger.info("qrMsngrSendCommandProcess: last read<"+sourceThread +">:"+getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage());
		}
	...
	public String qrMsngrSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec, 
		...
		if(getPwrSrcSpmObj().isDeviceSerialStatusConnected()){
			...
			if(status){
				String responseData = (String)responseReturn.get("result");
				ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength : responseData <"+sourceThread +">: " + responseData);
				ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength : responseData <"+sourceThread +"> hex : " + GUIUtils.asciiToHex(responseData));
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))) {
				//if(responseData.equals(GUIUtils.hexToAsciiV2(ConstantPowerSourceBofa.ER_CMD_STOP_VOLT_CURRENT_POSITIVE_IN_HEX))) {
					status = true;
				//}else {
			...
			if(isResponseExpected){
				while (retryCount>0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)){
					retryCount--;
					//VI_SendStartCommand(CommandVI_PayLoad,SelectedPhase);
					ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength :retryCount<"+sourceThread +">:" + retryCount);
					//int TimeOutInSec= 60;vjhvjgv
					//SerialDataLDU lduData = VICI_ReadDataWithErrorResponse(ExpectedData.length(),ExpectedData,ExpectedError1Data,ExpectedError2Data,TimeOutInSec);
					//ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength :Hit2");
					if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength: <"+sourceThread +">:Ack Response Success");
							//String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							//ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength: VICI Received Data1:"+CurrentLDU_Data);
							//StripLDU_SerialData(lduData.getReceivedLength());
							responseStatus=DeleteMeConstant.SUCCESS_RESPONSE;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
						}else if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedErrorResponseReceived()){
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							//ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength: ErrorResponse Received:"+CurrentLDU_Data);
							//ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength: ExpectedError1Data:"+ExpectedError1Data);
							//StripLDU_SerialData(CurrentLDU_Data.length());
							...
							String CurrentReadData =getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength : isUnknownResponseRecieved : CurrentReadData<"+sourceThread +">: "+ CurrentReadData);
							responseStatus=CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if(responseStatus!=null) { //Gopi2
								ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"+sourceThread +">:"+responseStatus);
								ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"+sourceThread +">:"+GUIUtils.HexToString(responseStatus));
							}
							
						}
					}
					else{
						ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength : <"+sourceThread +">: Ack No Data Received");
					}
					
					
					ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength : <"+sourceThread +">test1: ");
...
/*					if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
							...
							if(ProjectExecutionController.getUserAbortedFlag()){
								retryCount = 0;
								responseStatus = DeleteMeConstant.ERROR_RESPONSE;
								ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength V2 <"+sourceThread +">: user aborted - detected");
							}
						}else if(!getPwrSrcSpmObj().isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseStatus = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength V2 <"+sourceThread +">: serial port not connected or disconnected");
			
		}
		//ApplicationLauncher.logger.debug("qrMsngrSendCommandProcessWithLength : test2: ");
		if(getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage()==null) {
			ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength<"+sourceThread +">: last read null");
		}else {
			ApplicationLauncher.logger.info("qrMsngrSendCommandProcessWithLength: last read<"+sourceThread +">:"+getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage());
		}
