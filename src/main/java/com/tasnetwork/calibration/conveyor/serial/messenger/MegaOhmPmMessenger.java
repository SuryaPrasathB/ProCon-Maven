...
public class MegaOhmPmMessenger {
	...
	SpmMegaOhmPm pwrSrcSpmObj = new SpmMegaOhmPm();// null;//new
	...
	public Map<String, Object> sendReadCommandEicMegaOhmMeter(String slaveId) {
		...
		String frame = slaveIdHex + EIC_MegaOhmMeter.FUNCTION_CODE + EIC_MegaOhmMeter.REGISTER_ADDRESS
				+ EIC_MegaOhmMeter.NUM_BYTES;
		...
		String responseStatus = eicPanelMeterMsngrSendCommandProcessWithLength(payLoadInHex, timeDelayInMilliSec,
	...
	public String eicPanelMeterMsngrSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec,
		...
		// ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcess :
		...
		if (getPwrSrcSpmObj().isDeviceSerialStatusConnected()) {
			...
			if (status) {
				...
				ApplicationLauncher.logger.debug(
						"eicPanelMeterMsngrSendCommandProcess : responseData <" + sourceThread + ">: " + responseData);
				ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcess : responseData <" + sourceThread
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					ApplicationLauncher.logger.debug(
							"eicPanelMeterMsngrSendCommandProcess :retryCount<" + sourceThread + ">:" + retryCount);
					...
					if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcess: <" + sourceThread
									+ ">:Ack Response Success");
							// String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							// ApplicationLauncher.logger.info("eicPanelMeterMsngrSendCommandProcess: VICI
							...
							ApplicationLauncher.logger.debug(
									"eicPanelMeterMsngrSendCommandProcess <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							// ApplicationLauncher.logger.info("eicPanelMeterMsngrSendCommandProcess:
							...
							String CurrentReadData = getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug(
									"eicPanelMeterMsngrSendCommandProcess : isUnknownResponseRecieved : CurrentReadData<"
											+ sourceThread + ">: " + CurrentReadData);
							responseStatus = CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if (responseStatus != null) { // Gopi2
								ApplicationLauncher.logger.debug(
										"eicPanelMeterMsngrSendCommandProcess :Unexpected Message: dropping the message<"
						...
						ApplicationLauncher.logger.info(
								"eicPanelMeterMsngrSendCommandProcess : <" + sourceThread + ">: Ack No Data Received");
					}

					ApplicationLauncher.logger
							.debug("eicPanelMeterMsngrSendCommandProcess : <" + sourceThread + ">test1: ");
					 ...
					 * ApplicationLauncher.logger.info("eicPanelMeterMsngrSendCommandProcess V2 <"
					 * +sourceThread +">: user aborted - detected");
					 * }
					 * }else if(!getPwrSrcSpmObj().isPwrSrcSerialStatusConnected()){
					 * retryCount = 0;
					 * responseStatus = DeleteMeConstant.ERROR_RESPONSE;
					 * ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcess V2 <"
			...
			ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcess V2 <" + sourceThread
		...
		if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage() == null) {
			ApplicationLauncher.logger
					.info("eicPanelMeterMsngrSendCommandProcess<" + sourceThread + ">: last read null");
		} else {
			ApplicationLauncher.logger.info("eicPanelMeterMsngrSendCommandProcess: last read<" + sourceThread + ">:"
	...
	public String eicPanelMeterMsngrSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec,
		...
		if (getPwrSrcSpmObj().isDeviceSerialStatusConnected()) {
			...
			if (status) {
				String responseData = (String) responseReturn.get("result");
				ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcessWithLength : responseData <"
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseStatus.equals(DeleteMeConstant.NO_RESPONSE)) {
					retryCount--;
					// VI_SendStartCommand(CommandVI_PayLoad,SelectedPhase);
					ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcessWithLength :retryCount<"
							+ sourceThread + ">:" + retryCount);
					// int TimeOutInSec= 60;vjhvjgv
					// SerialDataLDU lduData =
					// VICI_ReadDataWithErrorResponse(ExpectedData.length(),ExpectedData,ExpectedError1Data,ExpectedError2Data,TimeOutInSec);
					// ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcessWithLength
					// :Hit2");
					if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().IsResponseReceived()) {
						if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcessWithLength: <"
							...
							// String CurrentLDU_Data =lduData.getLDU_ReadSerialData();
							// ApplicationLauncher.logger.info("eicPanelMeterMsngrSendCommandProcessWithLength:
							...
							String CurrentReadData = getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage();
							ApplicationLauncher.logger.debug(
									"eicPanelMeterMsngrSendCommandProcessWithLength : isUnknownResponseRecieved : CurrentReadData<"
											+ sourceThread + ">: " + CurrentReadData);
							responseStatus = CurrentReadData;
							getPwrSrcSpmObj().getRxMsgQ_PwrSrc().removeProcessedMsgFromQueue();
							if (responseStatus != null) { // Gopi2
								ApplicationLauncher.logger.debug(
										"eicPanelMeterMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"
												+ sourceThread + ">:" + responseStatus);
								ApplicationLauncher.logger.debug(
										"eicPanelMeterMsngrSendCommandProcessWithLength :Unexpected Message: dropping the message<"
												+ sourceThread + ">:" + GUIUtils.HexToString(responseStatus));
					...
					ApplicationLauncher.logger
							.debug("eicPanelMeterMsngrSendCommandProcessWithLength : <" + sourceThread + ">test1: ");
					 ...
					 * responseStatus = DeleteMeConstant.ERROR_RESPONSE;
					 * ApplicationLauncher.logger.
					 * info("eicPanelMeterMsngrSendCommandProcessWithLength V2 <"+sourceThread
					 ...
					 * responseStatus = DeleteMeConstant.ERROR_RESPONSE;
					 * ApplicationLauncher.logger.
					 * debug("eicPanelMeterMsngrSendCommandProcessWithLength V2 <"+sourceThread
			...
			ApplicationLauncher.logger.debug("eicPanelMeterMsngrSendCommandProcessWithLength V2 <" + sourceThread
					+ ">: serial port not connected or disconnected");
		...
		if (getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage() == null) {
			ApplicationLauncher.logger
					.info("eicPanelMeterMsngrSendCommandProcessWithLength<" + sourceThread + ">: last read null");
		} else {
			ApplicationLauncher.logger.info("eicPanelMeterMsngrSendCommandProcessWithLength: last read<" + sourceThread
					+ ">:" + getPwrSrcSpmObj().getRxMsgQ_PwrSrc().getLastReadMessage());
		}
