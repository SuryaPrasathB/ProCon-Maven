...
public class SpmMegaOhmPm {
	...
	public SpmMegaOhmPm(String traceName, boolean terminatorMandatory) {
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commEic, rxMsgQ_PwrSrc, false, getMyTraceName(),
	...
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commEic, rxMsgQ_PwrSrc, false, getMyTraceName(),
			DeleteMeConstant.HV_ER_TERMINATOR, "");
	...
	public void disconnectMegaOhmPm() {
		...
		if (isDeviceSerialStatusConnected()) {
			ApplicationLauncher.logger.debug("disconnectMegaOhmPm :isDeviceSerialStatusConnected: " + getMyTraceName()
	...
	public Map<String, Object> eicSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
		...
		if (isDeviceSerialStatusConnected()) {
				...
				while (retryCount > 0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					if (rxMsgQ_PwrSrc.IsResponseReceived()) {
						if (rxMsgQ_PwrSrc.isExpectedResponseReceived()) {
							ApplicationLauncher.logger
									.debug("eicSendCommandProcess V2 <" + sourceThread + ">:Ack Response Success");
							...
							ApplicationLauncher.logger
									.debug("eicSendCommandProcess V2 <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = new String(rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if (responseData != null) { // Gopi2
								ApplicationLauncher.logger.debug("eicSendCommandProcess V2 :Unexpected Message<"
										+ sourceThread + ">: dropping the message:" + responseData);
								// ApplicationLauncher.logger.debug("eicSendCommandProcess :Unexpected Message:
						...
						ApplicationLauncher.logger
								.debug("eicSendCommandProcess V2 <" + sourceThread + ">: Ack No Data Received");
					}
					/*
					 * if ( (
					 ...
					 * ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread
			...
			ApplicationLauncher.logger
					.debug("eicSendCommandProcess V2 <" + sourceThread + "> serial port not connected or disconnected");
		}
		if (rxMsgQ_PwrSrc.getLastReadMessage() == null) {
			...
			// ApplicationLauncher.logger.info("eicSendCommandProcess: last
	...
	public Map<String, Object> eicSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec,
			String expectedData,
		...
		if (isDeviceSerialStatusConnected()) {
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					if (rxMsgQ_PwrSrc.IsResponseReceived()) {
						if (rxMsgQ_PwrSrc.isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug(
									"eicSendCommandProcessWithLength <" + sourceThread + ">:Ack Response Success");
							...
							ApplicationLauncher.logger
									.debug("eicSendCommandProcessWithLength <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = new String(rxMsgQ_PwrSrc.getLastReadMessage());
							...
							rxMsgQ_PwrSrc.removeProcessedMsgFromQueue();
							if (responseData != null) { // Gopi2
								ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength :Unexpected Message<"
										+ sourceThread + ">: dropping the message:" + responseData);
								// ApplicationLauncher.logger.debug("eicSendCommandProcess :Unexpected Message:
								// dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger
								.debug("eicSendCommandProcessWithLength <" + sourceThread + ">: Ack No Data Received");
					}
					/*
					 * if ( (
					 * !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))
			...
			ApplicationLauncher.logger.debug(
					"eicSendCommandProcessWithLength <" + sourceThread + "> serial port not connected or disconnected");
		}
		if (rxMsgQ_PwrSrc.getLastReadMessage() == null) {
