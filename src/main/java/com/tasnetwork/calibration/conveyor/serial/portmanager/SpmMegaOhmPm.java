...
public class SpmMegaOhmPm {
	
	private String myTraceName = "Eic-Pm";
	public SpmMegaOhmPm(String traceName){
	...
	public SpmMegaOhmPm(String traceName, boolean terminatorMandatory){
		setMyTraceName(traceName);
		...
		//rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
		 rxPhysical_PwrSrc = new SerialRxPhysical(commEic,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
		 rxPhysical_PwrSrc.setTerminatorMandatory(terminatorMandatory);
		 
		 
	}
	
	public SpmMegaOhmPm(String traceName, int expectedResponseHexLength){
		setMyTraceName(traceName);
		boolean validateByRxDataLength  = true;
		ApplicationLauncher.logger.debug("SerialPortManagerPwrSrc_V2 : expectedResponseHexLength: " + expectedResponseHexLength);
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commEic,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
	...
	public Communicator commEic= null;
	
	public SerialTxMessageQ txMsgQ_PwrSrc = new SerialTxMessageQ(commEic);
	
	public SerialRxMessageQ rxMsgQ_PwrSrc = new SerialRxMessageQ(commEic);
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commEic,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
	...
	{
		ApplicationLauncher.logger.debug("EicPanelMeterSpm: createObjects :Entry");
...
/*		commLDU = new Communicator(ConstantApp.SERIAL_PORT_LDU);
		...
		commEic = new Communicator(ConstantApp.SERIAL_PORT_EIC);//SERIAL_PORT_POWER_SOURCE);//ConstantApp.SERIAL_PORT_REF_STD);//ConstantProGEN_App.SERIAL_PORT_HVCI);
		txMsgQ_PwrSrc = new SerialTxMessageQ(commEic);
		
		rxMsgQ_PwrSrc = new SerialRxMessageQ(commEic);
			
		rxPhysical_PwrSrc = new SerialRxPhysical(commEic,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
		
		ApplicationLauncher.logger.debug("MegaOhmPmSpm: createObjects :Exit");
	...
	public void scanForSerialCommPort(){
		...
		//commPowerSrc.searchForPorts();  
		commEic.searchForPorts();  

	}
	
	public void startSerialRxPhysical_MegaOhmPm() {
	...
	public void enableSerialRxPhysical_MegaOhmPmMonitor() {
		rxPhysical_PwrSrc.setReadRxPhysicalFlag(true);
		
	}
	
	public void disableSerialRxPhysical_EicMonitor() {
		rxPhysical_PwrSrc.setReadRxPhysicalFlag(false);
		

	}
	
	public void disconnectMegaOhmPm(){
		ApplicationLauncher.logger.debug("disconnectMegaOhmPm V2 :Entry");
		if(isDeviceSerialStatusConnected()){
			ApplicationLauncher.logger.debug("disconnectMegaOhmPm :isDeviceSerialStatusConnected: " + getMyTraceName() + " : "+ isDeviceSerialStatusConnected());
			disableSerialRxPhysical_EicMonitor();
			//DisplayDataObj.setPwrSrcReadDataFlag(false);  
			Sleep(10);
			disconnectMegaOhmPmSerialComm();
			setDeviceSerialStatusConnected(false);
			
		}else {
			ApplicationLauncher.logger.debug("disconnectMegaOhmPm V2 : Path2");
		}
	}
	
	public void disconnectMegaOhmPmSerialComm(){
		ApplicationLauncher.logger.debug("disconnectMegaOhmPmSerialComm V2 :Entry");
		commEic.disconnect();
	}
	...
	public boolean powerSourceCommSetting(String CommInput, String BaudRate) throws UnsupportedEncodingException{
		...
		//InitSerialCommPort();
		ApplicationLauncher.logger.debug("powerSourceComm: " +commEic );
		ApplicationLauncher.logger.debug("CommInput: " + CommInput);
		ApplicationLauncher.logger.debug("BaudRate: " + BaudRate);
		boolean status = setSerialCommV2(commEic,CommInput,Integer.valueOf(BaudRate),true);
...
/*				String eachDataInHex = "";
				...
				commEic.writeStringMsgToPortV1(myStr);
...
/*				String eachDataInHex = "";
	...
	public void powerSourceSetExpectedData(String ExpectedResponse){
		...
		Communicator SerialPortObj =commEic;
	...
	public Map<String,Object> eicSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
			boolean isResponseExpected, String rxMessageTerminatorInHex,String sourceThread){
		ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">:Entry");
		boolean status=false;
		...
		if(isDeviceSerialStatusConnected()){
				...
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("eicSendCommandProcess V2 :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("eicSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
						...
						if(ProjectExecutionController.getUserAbortedFlag()){
							...
							ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">: user aborted - detected");
						}else if(!isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">: last read null");
		}else {
			//ApplicationLauncher.logger.info("eicSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
		responseReturn.put("status", status);
		responseReturn.put("result", responseData);
		ApplicationLauncher.logger.debug("eicSendCommandProcess V2 <"+sourceThread +">:Exit");
	...
	public Map<String,Object> eicSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
			boolean isResponseExpected, int rxMessageHexLength,String sourceThread){
		ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +">:Entry");
		boolean status=false;
		Map<String,Object> responseReturn = new HashMap<String,Object>();
		String responseData=DeleteMeConstant.NO_RESPONSE;
		int retryCount= 5;//10//ConstantHV_Src.HV_READ_RESPONSE_RETRY_COUNT+10;//1;
		...
		if(isDeviceSerialStatusConnected()){
			...
			if(isResponseExpected){
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							rxMsgQ_PwrSrc.removeProcessedMsgFromQueue();
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("eicSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
					     ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_CURRENT_IN_HEX)) ){
						if(ProjectExecutionController.getUserAbortedFlag()){
			...
			ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +">: last read null");
		}else {
			//ApplicationLauncher.logger.info("eicSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
		responseReturn.put("status", status);
		responseReturn.put("result", responseData);
		ApplicationLauncher.logger.debug("eicSendCommandProcessWithLength <"+sourceThread +">:Exit");
