...
/*	String messageTerminator = ConstantHV_Src.HV_ER_TERMINATOR;
	...
	public SerialRxPhysical(Communicator inpSerialPortObj,Object inputProductType, boolean comCheckStatus,String inpLabel,boolean valByDataLength, int expectedDataLengthInHex, String errorTerminator){//,String inputComCheckSuccess){
		SerialPortObj = inpSerialPortObj;
		//rxMessageQ.productType = inputProductType;
		productType = inputProductType;
		comCheckEnabled = comCheckStatus;
		...
		ApplicationLauncher.logger.debug("SerialRxPhysical_V2 : expectedResponseDataInAsciiLength: " + expectedResponseDataInAsciiLength);
		// comCheckSuccess = inputComCheckSuccess;
	}
...
/*			if(MaintenanceModeExecController.isSerialDisplayProcess()) {
			...
			ApplicationLauncher.logger.debug("ReadRxPhysicalData V2: "+ rxLabel+": expectedResponseDataInAsciiLength: "+expectedResponseDataInAsciiLength);
			ApplicationLauncher.logger.debug("ReadRxPhysicalData V2: "+ rxLabel+": ReadRxPhysicalData.length(): "+ReadRxPhysicalData.length());
			ApplicationLauncher.logger.debug("ReadRxPhysicalData V2: "+ rxLabel+": terminatorMandatory: "+ terminatorMandatory);
			*/
			if(ReadRxPhysicalData.endsWith(responseMessageTerminator)){
				...
				ApplicationLauncher.logger.debug("ReadRxPhysicalData V2: "+ rxLabel+": hit4 ");
				Sleep(300);
				terminatorOptionalCheckCounter++;
				if(terminatorOptionalCheckCounter==3){
					ApplicationLauncher.logger.debug("SerialRxPhysical_V2 counter hit: "+ ReadRxPhysicalData);
			...
			if(isReadRxPhysicalFlag()) {
				...
				ApplicationLauncher.logger.debug("SerialRxPhysicalTask: Stopped :"  + rxLabel + " : Port: " + SerialPortObj.getSystemPortName());
			}
		}
