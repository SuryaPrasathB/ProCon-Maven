...
public class SpmDut {
	...
	private String myTraceName = "SpmDut";
	public SpmDut(String traceName){
	...
	public SpmDut(String traceName, boolean terminatorMandatory){
		setMyTraceName(traceName);
		...
		//rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
		 rxPhysical_PwrSrc = new SerialRxPhysical(commDut,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
		 rxPhysical_PwrSrc.setTerminatorMandatory(terminatorMandatory);
		 
		 
	}
	
	public SpmDut(String traceName, int expectedResponseHexLength){
		setMyTraceName(traceName);
		boolean validateByRxDataLength  = true;
		ApplicationLauncher.logger.debug("SerialPortManagerPwrSrc_V2 : expectedResponseHexLength: " + expectedResponseHexLength);
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commDut,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
	...
	public Communicator commDut= null;
	
	public SerialTxMessageQ txMsgQ_PwrSrc = new SerialTxMessageQ(commDut);
	
	public SerialRxMessageQ rxMsgQ_PwrSrc = new SerialRxMessageQ(commDut);
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commDut,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
...
/*		commLDU = new Communicator(ConstantApp.SERIAL_PORT_LDU);
		...
		commDut = new Communicator(ConstantApp.SERIAL_PORT_DUT);//SERIAL_PORT_POWER_SOURCE);//ConstantApp.SERIAL_PORT_REF_STD);//ConstantProGEN_App.SERIAL_PORT_HVCI);
		txMsgQ_PwrSrc = new SerialTxMessageQ(commDut);
		
		rxMsgQ_PwrSrc = new SerialRxMessageQ(commDut);
	...
	public void disconnectDut(){
		...
		if(isDeviceSerialStatusConnected()){
			ApplicationLauncher.logger.debug("disconnectDut :isDeviceSerialStatusConnected: " + getMyTraceName() + " : "+ isDeviceSerialStatusConnected());
	...
	public boolean powerSourceCommSetting(String CommInput, String BaudRate) throws UnsupportedEncodingException{
		...
		//InitSerialCommPort();
		ApplicationLauncher.logger.debug("powerSourceComm: " +commDut );
		ApplicationLauncher.logger.debug("CommInput: " + CommInput);
		ApplicationLauncher.logger.debug("BaudRate: " + BaudRate);
		boolean status = setSerialCommV2(commDut,CommInput,Integer.valueOf(BaudRate),true);
...
/*				String eachDataInHex = "";
	...
	public void powerSourceSetExpectedData(String ExpectedResponse){
		...
		Communicator SerialPortObj =commDut;
	...
	public Map<String,Object> dutSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
		...
		int retryCount= 2;//#Version0.5.1.4//5;//10//ConstantHV_Src.HV_READ_RESPONSE_RETRY_COUNT+10;//1;
		...
		if(isDeviceSerialStatusConnected()){
			...
			ApplicationLauncher.logger.debug("dutSendCommandProcess V2 <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			...
			//ApplicationLauncher.logger.info("dutSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
	...
	public Map<String,Object> dutSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
			boolean isResponseExpected, int rxMessageHexLength,String sourceThread){
		ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +">:Entry");
		boolean status=false;
		Map<String,Object> responseReturn = new HashMap<String,Object>();
		String responseData=DeleteMeConstant.NO_RESPONSE;
		...
		if(isDeviceSerialStatusConnected()){
			...
			if(isResponseExpected){
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							rxMsgQ_PwrSrc.removeProcessedMsgFromQueue();
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("dutSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
					     ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_CURRENT_IN_HEX)) ){
						if(ProjectExecutionController.getUserAbortedFlag()){
			...
			ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +">: last read null");
		}else {
			//ApplicationLauncher.logger.info("dutSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
		responseReturn.put("status", status);
		responseReturn.put("result", responseData);
		ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength <"+sourceThread +">:Exit");
