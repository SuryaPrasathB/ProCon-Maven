...
public class SpmDut {
	...
	public SpmDut(String traceName, boolean terminatorMandatory) {
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commDut, rxMsgQ_PwrSrc, false, getMyTraceName(),
	...
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commDut, rxMsgQ_PwrSrc, false, getMyTraceName(),
			DeleteMeConstant.HV_ER_TERMINATOR, "");
	...
	public void disconnectDut() {
		...
		if (isDeviceSerialStatusConnected()) {
			ApplicationLauncher.logger.debug("disconnectDut :isDeviceSerialStatusConnected: " + getMyTraceName() + " : "
	...
	public Map<String, Object> dutSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
		...
		if (isDeviceSerialStatusConnected()) {
			...
			ApplicationLauncher.logger
					.debug("dutSendCommandProcess V2 <" + sourceThread + "> serial port not connected or disconnected");
		}
	...
	public Map<String, Object> dutSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec,
			String expectedData,
		...
		if (isDeviceSerialStatusConnected()) {
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					if (rxMsgQ_PwrSrc.IsResponseReceived()) {
						if (rxMsgQ_PwrSrc.isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug(
									"dutSendCommandProcessWithLength <" + sourceThread + ">:Ack Response Success");
							...
							ApplicationLauncher.logger
									.debug("dutSendCommandProcessWithLength <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = new String(rxMsgQ_PwrSrc.getLastReadMessage());
							...
							rxMsgQ_PwrSrc.removeProcessedMsgFromQueue();
							if (responseData != null) { // Gopi2
								ApplicationLauncher.logger.debug("dutSendCommandProcessWithLength :Unexpected Message<"
										+ sourceThread + ">: dropping the message:" + responseData);
								// ApplicationLauncher.logger.debug("dutSendCommandProcess :Unexpected Message:
								// dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger
								.debug("dutSendCommandProcessWithLength <" + sourceThread + ">: Ack No Data Received");
					}
					/*
					 * if ( (
					 * !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))
			...
			ApplicationLauncher.logger.debug(
					"dutSendCommandProcessWithLength <" + sourceThread + "> serial port not connected or disconnected");
		}
		if (rxMsgQ_PwrSrc.getLastReadMessage() == null) {
