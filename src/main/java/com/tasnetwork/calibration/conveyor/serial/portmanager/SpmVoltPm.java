...
public class SpmVoltPm {
	...
	public SpmVoltPm(String traceName, boolean terminatorMandatory) {
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commElmeasure, rxMsgQ_PwrSrc, false, getMyTraceName(),
	...
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commElmeasure, rxMsgQ_PwrSrc, false,
	...
	public void disconnectVoltPm() {
		...
		if (isDeviceSerialStatusConnected()) {
			ApplicationLauncher.logger.debug("disconnectElmeasure :isDeviceSerialStatusConnected: " + getMyTraceName()
	...
	public Map<String, Object> elmeasureSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec,
			String expectedData,
		...
		if (isDeviceSerialStatusConnected()) {
				...
				while (retryCount > 0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					if (rxMsgQ_PwrSrc.IsResponseReceived()) {
						if (rxMsgQ_PwrSrc.isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug(
									"elmeasureSendCommandProcess V2 <" + sourceThread + ">:Ack Response Success");
							...
							ApplicationLauncher.logger
									.debug("elmeasureSendCommandProcess V2 <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = new String(rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if (responseData != null) { // Gopi2
								ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 :Unexpected Message<"
										+ sourceThread + ">: dropping the message:" + responseData);
								// ApplicationLauncher.logger.debug("elmeasureSendCommandProcess :Unexpected
								// Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}

						}
					} else {
						ApplicationLauncher.logger
								.debug("elmeasureSendCommandProcess V2 <" + sourceThread + ">: Ack No Data Received");
					}
					/*
					 * if ( (
					 ...
					 * ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"
			...
			ApplicationLauncher.logger.debug(
					"elmeasureSendCommandProcess V2 <" + sourceThread + "> serial port not connected or disconnected");
		}
		if (rxMsgQ_PwrSrc.getLastReadMessage() == null) {
			...
			// ApplicationLauncher.logger.info("elmeasureSendCommandProcess: last
	...
	public Map<String, Object> elmeasureSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec,
			String expectedData,
		...
		if (isDeviceSerialStatusConnected()) {
			...
			if (isResponseExpected) {
				while (retryCount > 0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)) {
					...
					if (rxMsgQ_PwrSrc.IsResponseReceived()) {
						if (rxMsgQ_PwrSrc.isExpectedResponseReceived()) {
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <" + sourceThread
							...
							ApplicationLauncher.logger.debug(
									"elmeasureSendCommandProcessWithLength <" + sourceThread + ">:Ack Response Error");
							String CurrentReadData = new String(rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if (responseData != null) { // Gopi2
								ApplicationLauncher.logger
										.debug("elmeasureSendCommandProcessWithLength :Unexpected Message<"
												+ sourceThread + ">: dropping the message:" + responseData);
								// ApplicationLauncher.logger.debug("elmeasureSendCommandProcess :Unexpected
								// Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug(
								"elmeasureSendCommandProcessWithLength <" + sourceThread + ">: Ack No Data Received");
					}
					/*
					 * if ( (
					 * !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX))
		...
		if (rxMsgQ_PwrSrc.getLastReadMessage() == null) {
			ApplicationLauncher.logger
					.debug("elmeasureSendCommandProcessWithLength <" + sourceThread + ">: last read null");
		} else {
			// ApplicationLauncher.logger.info("elmeasureSendCommandProcess: last
			// read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
