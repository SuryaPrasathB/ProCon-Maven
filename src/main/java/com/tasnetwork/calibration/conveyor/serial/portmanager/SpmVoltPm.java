...
public class SpmVoltPm {
	
	private String myTraceName = "Elm-Pm";
	public SpmVoltPm(String traceName){
	...
	public SpmVoltPm(String traceName, boolean terminatorMandatory){
		...
		//ApplicationLauncher.logger.debug("SerialPortManagerPwrSrc_V2 : expectedResponseHexLength: " + expectedResponseHexLength);
		...
		//rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
		 rxPhysical_PwrSrc = new SerialRxPhysical(commElmeasure,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
		 rxPhysical_PwrSrc.setTerminatorMandatory(terminatorMandatory);
		 
		 
	}
	
	public SpmVoltPm(String traceName, int expectedResponseHexLength){
		setMyTraceName(traceName);
		boolean validateByRxDataLength  = true;
		ApplicationLauncher.logger.debug("SerialPortManagerPwrSrc_V2 : expectedResponseHexLength: " + expectedResponseHexLength);
		...
		rxPhysical_PwrSrc = new SerialRxPhysical(commElmeasure,rxMsgQ_PwrSrc,false,getMyTraceName(),validateByRxDataLength,expectedResponseHexLength,"");
	...
	public Communicator commElmeasure= null;
	
	public SerialTxMessageQ txMsgQ_PwrSrc = new SerialTxMessageQ(commElmeasure);
	
	public SerialRxMessageQ rxMsgQ_PwrSrc = new SerialRxMessageQ(commElmeasure);
	public SerialRxPhysical rxPhysical_PwrSrc = new SerialRxPhysical(commElmeasure,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
	...
	public SpmVoltPm(){
		if(!bInitOccured){
			InitSerialCommPort();
			bInitOccured= true;
			//rxPhysical_PwrSrc = new SerialRxPhysical_V2(commPowerSource,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
	...
	{
		ApplicationLauncher.logger.debug("ElmeasurePanelMeterSpm: createObjects :Entry");
...
/*		commLDU = new Communicator(ConstantApp.SERIAL_PORT_LDU);
		...
		commElmeasure = new Communicator(ConstantApp.SERIAL_PORT_ELMEASURE);//SERIAL_PORT_POWER_SOURCE);//ConstantApp.SERIAL_PORT_REF_STD);//ConstantProGEN_App.SERIAL_PORT_HVCI);
		txMsgQ_PwrSrc = new SerialTxMessageQ(commElmeasure);
		
		rxMsgQ_PwrSrc = new SerialRxMessageQ(commElmeasure);
			
		rxPhysical_PwrSrc = new SerialRxPhysical(commElmeasure,rxMsgQ_PwrSrc,false,getMyTraceName(),DeleteMeConstant.HV_ER_TERMINATOR,"");
		
		ApplicationLauncher.logger.debug("ElmeasurePanelMeterSpm: createObjects :Exit");
	...
	public void scanForSerialCommPort(){
		...
		//commPowerSrc.searchForPorts();  
		commElmeasure.searchForPorts();  

	}
	
	public void startSerialRxPhysical_VoltPm() {
	...
	public void enableSerialRxPhysical_VoltPmMonitor() {
		rxPhysical_PwrSrc.setReadRxPhysicalFlag(true);
		
	}
	
	public void disableSerialRxPhysical_ElmeasureMonitor() {
		rxPhysical_PwrSrc.setReadRxPhysicalFlag(false);
		

	}
	
	public void disconnectVoltPm(){
		ApplicationLauncher.logger.debug("disconnectElmeasure V2 :Entry");
		if(isDeviceSerialStatusConnected()){
			ApplicationLauncher.logger.debug("disconnectElmeasure :isDeviceSerialStatusConnected: " + getMyTraceName() + " : "+ isDeviceSerialStatusConnected());
			disableSerialRxPhysical_ElmeasureMonitor();
			//DisplayDataObj.setPwrSrcReadDataFlag(false);  
			Sleep(10);
			disconnectElmeasureSerialComm();
			setDeviceSerialStatusConnected(false);
			
		}else {
			ApplicationLauncher.logger.debug("disconnectElmeasure V2 : Path2");
		}
	}
	
	public void disconnectElmeasureSerialComm(){
		ApplicationLauncher.logger.debug("disconnectElmeasureSerialComm V2 :Entry");
		commElmeasure.disconnect();
	}
	...
	public void disconnectPwrSrcSerialCommIfConnected(){
		...
		try {
		if(commElmeasure.isDeviceConnected()){
	...
	public boolean powerSourceCommSetting(String CommInput, String BaudRate) throws UnsupportedEncodingException{
		...
		//InitSerialCommPort();
		ApplicationLauncher.logger.debug("powerSourceComm: " +commElmeasure );
		ApplicationLauncher.logger.debug("CommInput: " + CommInput);
		ApplicationLauncher.logger.debug("BaudRate: " + BaudRate);
		boolean status = setSerialCommV2(commElmeasure,CommInput,Integer.valueOf(BaudRate),true);
		...
		if (status){
			...
			ApplicationLauncher.logger.info("powerSourceCommSetting: " + getMyTraceName() + " : " +CommInput+" : access failed");
		}
		return status;
		...
		try {
			...
			if(SerialPortObj.isDeviceConnected()==true){
				...
				//if (SerialPortObj.initIOStream() == true){
					...
					ApplicationLauncher.logger.info("setSerialCommV2: PortDeviceMapping: " + getMyTraceName() + " : " + SerialPort_ID + " : "+  SerialPortObj.getPortDeviceMapping()+":"+ReadHexFormat);
				...
				ApplicationLauncher.logger.debug("setSerialCommV2 : device com port : " + getMyTraceName() + " : "  + SerialPort_ID + " not connected");
			}
		}catch(Exception e) {
			e.printStackTrace();
...
/*				String eachDataInHex = "";
				...
				commElmeasure.writeStringMsgToPortV1(myStr);
...
/*				String eachDataInHex = "";
				...
				ApplicationLauncher.logger.debug("writeStringToPwrSrcV2 : "+ getMyTraceName() +" : Data<"+sourceThread +">: "  + Data);
				String myStr = GUIUtils.hexToAsciiV2(Data);
	...
	public void powerSourceSetExpectedData(String ExpectedResponse){
		...
		Communicator SerialPortObj =commElmeasure;
	...
	public Map<String,Object> elmeasureSendCommandProcess(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
			boolean isResponseExpected, String rxMessageTerminatorInHex,String sourceThread){
		ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">:Entry");
		boolean status=false;
		...
		if(isDeviceSerialStatusConnected()){
				...
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("elmeasureSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
						...
						if(ProjectExecutionController.getUserAbortedFlag()){
							...
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">: user aborted - detected");
						}else if(!isPwrSrcSerialStatusConnected()){
							retryCount = 0;
							responseData = DeleteMeConstant.ERROR_RESPONSE;
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">: serial port - disconnected");
			...
			ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">: last read null");
		}else {
			//ApplicationLauncher.logger.info("elmeasureSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
		responseReturn.put("status", status);
		responseReturn.put("result", responseData);
		ApplicationLauncher.logger.debug("elmeasureSendCommandProcess V2 <"+sourceThread +">:Exit");
	...
	public Map<String,Object> elmeasureSendCommandProcessWithLength(String payLoadInHex, int timeDelayInMilliSec, String expectedData,
			boolean isResponseExpected, int rxMessageHexLength,String sourceThread){
		ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +">:Entry");
		boolean status=false;
		Map<String,Object> responseReturn = new HashMap<String,Object>();
		String responseData=DeleteMeConstant.NO_RESPONSE;
		int retryCount= 5;//10//ConstantHV_Src.HV_READ_RESPONSE_RETRY_COUNT+10;//1;
		...
		if(isDeviceSerialStatusConnected()){
			...
			if(isResponseExpected){
				while (retryCount>0 && responseData.equals(DeleteMeConstant.NO_RESPONSE)){
					...
					if(rxMsgQ_PwrSrc.IsResponseReceived()) {
						if(rxMsgQ_PwrSrc.isExpectedResponseReceived()){
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +">:Ack Response Success");
							...
							ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +">:Ack Response Error");
							String CurrentReadData =new String (rxMsgQ_PwrSrc.getLastReadMessage());
							...
							rxMsgQ_PwrSrc.removeProcessedMsgFromQueue();
							if(responseData!=null) { //Gopi2
								ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength :Unexpected Message<"+sourceThread +">: dropping the message:"+responseData);
								//ApplicationLauncher.logger.debug("elmeasureSendCommandProcess :Unexpected Message: dropping the message:"+GuiUtils.HexToString(responseData));
							}
						...
						ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +">: Ack No Data Received");
					}
					/*if ( ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_VOLT_CURRENT_IN_HEX)) && 
					     ( !payLoadInHex.equals(ConstantPowerSourceBofa.CMD_STOP_CURRENT_IN_HEX)) ){
						if(ProjectExecutionController.getUserAbortedFlag()){
			...
			ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +"> serial port not connected or disconnected");
		}
		if(rxMsgQ_PwrSrc.getLastReadMessage()==null) {
			ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +">: last read null");
		}else {
			//ApplicationLauncher.logger.info("elmeasureSendCommandProcess: last read:"+rxMsgQ_PwrSrc.getLastReadMessage());
		}
		responseReturn.put("status", status);
		responseReturn.put("result", responseData);
		ApplicationLauncher.logger.debug("elmeasureSendCommandProcessWithLength <"+sourceThread +">:Exit");
