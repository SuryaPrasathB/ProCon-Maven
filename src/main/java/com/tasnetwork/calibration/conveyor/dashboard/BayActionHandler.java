	...
	public void handleActionByBayType(PalletController.BayActionType actionType) {
		...
		new Thread(() -> {
			...
			case ConstantConveyor.UNLOADING_BAY_KEY :
				...
				break;
			default:
				ApplicationLauncher.logger.warn("handleActionByBayType: Unknown bayType " + bayTypeKey + " for action " + actionType);
			}
			...
			// Platform.runLater(() -> {
			//     // DashboardController.updateStatusLabel("Action for " + bayTypeKey + " completed.");
...
/*		case FINGERTIP_ENGAGE:
	...
	private void disengageSTANLD2Fingertip() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().setPalletsLockedImageDisplayOn(bayTypeKey,false);
	}
	...
	private void engageSTANLD2Fingertip() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().setPalletsLockedImageDisplayOn(bayTypeKey,true);
	}
	...
	private void closeSTANLD2BayStopper1() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.STA_NLD2_PP1_BAY_KEY,false);
	}
	...
	private void openSTANLD2BayStopper1() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.STA_NLD2_PP1_BAY_KEY,true);
	}
	...
	private void closeSTANLD1BayStopper1() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.STA_NLD1_PP1_BAY_KEY,false);
	}
	...
	private void openSTANLD1BayStopper1() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.STA_NLD1_PP1_BAY_KEY,true);
	}
...
/*		case PALLETS_BLOCK:
	...
	private void closeVerificationBayStopper1() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.VERIFICATION_PP1_BAY_KEY,false);
	}
	...
	private void openVerificationBayStopper1() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.VERIFICATION_PP1_BAY_KEY,true);
	}
...
/*		case PALLETS_BLOCK:
	...
	/*public void refreshPalletsInWaitingVerific1Bay() {
		...
		ConveyorDeviceDataManagerController.getDashboardObject().removeAllPalletsFromWaitingVerific1Bays();
		ApplicationLauncher.logger.info("refreshPalletsInWaitingVerific1Bay: batch update: all WaitingVerific1 removed");

		BayUtils.delay(100);
		ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: batch update : delay done :for removal: " );
		...
		ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: refreshDashBoard: palletManageList size: " + palletManageList.size());
		boolean scannedPalletQrIdExist = false;
		for(PalletManage eachPalletManage : palletManageList ) {
			ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: fetchPalletsByBayState: getPalletDistinctId:    " + eachPalletManage.getPalletDistinctId());
			//ApplicationLauncher.logger.debug("refreshDashBoard : fetchPalletsByBayState: palletQrId:    " + palletQrId);

			if(eachPalletManage.getPalletDistinctId().contains(palletQrId)) {
				scannedPalletQrIdExist = true;
				ApplicationLauncher.logger.debug("refreshDashBoard : fetchPalletsByBayState: scannedPalletQrIdExist in fetch list");
			}
		}
		String palletName = "";
		//ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: batch update ");
		for(PalletManage eachPalletManage : palletManageList ) {
			...
			ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: batch update : palletName: " + palletName);

			//ConveyorDeviceDataManagerController.getDashboardObject().addPalletToFirstAvailableVerificationBay(palletName, meterListWithSerialNoMap);
			ConveyorDeviceDataManagerController.getDashboardObject().addPalletToFirstAvailableWaitingBay(palletName, meterListWithSerialNoMap);
			ApplicationLauncher.logger.info("refreshPalletsInWaitingVerific1Bay: batch update: WaitingVerific1 added");
			...
			errorCodeMap.put(6, "");
			//Platform.runLater(()->{
			ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: batch update : updateDashBoardPalletStatus : " + palletName);
			ConveyorDeviceDataManagerController.getDashboardObject().updateDashBoardPalletStatus(palletName, statusMap, errorCodeMap);
			
			BayUtils.delay(50);
			
			ApplicationLauncher.logger.debug("refreshPalletsInWaitingVerific1Bay: batch update : delay done :palletName: " + palletName);
	...
	public void refreshMultiplePalletInBay(String bayKey) {
		...
		Set<PalletMeter> palletMeterSetList =  new HashSet<PalletMeter>();
		List<PalletManage> palletManageList = bayUtils.fetchPalletsByBayState(bayKey);
		ApplicationLauncher.logger.debug("refreshMultiplePalletInBay: palletManageList size: " + palletManageList.size());
		//boolean scannedPalletQrIdExist = false;
		//for(PalletManage eachPalletManage : palletManageList ) {
		//	ApplicationLauncher.logger.debug("refreshMultiplePalletInBay: fetchPalletsByBayState: getPalletDistinctId:    " + eachPalletManage.getPalletDistinctId());
			//ApplicationLauncher.logger.debug("refreshDashBoard : fetchPalletsByBayState: palletQrId:    " + palletQrId);
		...
		if(bayKey.equals(ConstantConveyor.VERIFICATION_BAY_KEY)) {
			noOfPalletsAcceptedInBay = DeviceDataManagerController.getConveyorConfigParsedKey().getMaxNoOfPalletsInVerific1();
			...
			noOfPalletsAcceptedInBay = DeviceDataManagerController.getConveyorConfigParsedKey().getMaxNoOfPalletsInStaNld1();
		}else if(bayKey.equals(ConstantConveyor.STA_NLD2_BAY_KEY)) {
			ApplicationLauncher.logger.debug("refreshMultiplePalletInBay: sta2"); 
			noOfPalletsAcceptedInBay = DeviceDataManagerController.getConveyorConfigParsedKey().getMaxNoOfPalletsInStaNld2();
			...
			noOfPalletsAcceptedInBay = DeviceDataManagerController.getConveyorConfigParsedKey().getMaxNoOfPalletsInWaitingVerific1();
		}
		
		ApplicationLauncher.logger.debug("refreshMultiplePalletInBay: noOfPalletsAcceptedInBay: " + noOfPalletsAcceptedInBay);
		...
		if(palletManageList.size()>noOfPalletsAcceptedInBay){
			...
			for(PalletManage eachPalletManage : palletManageList ) {
				...
				if(bayKey.startsWith(ConstantConveyor.WAITING_BAY_KEY)) {
					...
					ApplicationLauncher.logger.debug("refreshMultiplePalletInBay: delay done :palletName: " + palletName);
				}
	...
	public void refreshSinglePalletsInBay(String myBayKey) {
		...
		if ( (myBayKey.equals(ConstantConveyor.REJECTION_BAY_KEY)) || (myBayKey.equals(ConstantConveyor.UNLOADING_BAY_KEY)) ) {
			palletManageList = MySqlServiceManager.getPalletManageService().findTopByPresentBayKeyAndPalletActiveFalseToday(myBayKey);
			
			
		}
		ApplicationLauncher.logger.debug("refreshSinglePalletsInBay: refreshDashBoard: palletManageList size: " + palletManageList.size());
		boolean scannedPalletQrIdExist = false;
		List<String> palletDistinctIdList = new ArrayList<String>();
		for(PalletManage eachPalletManage : palletManageList ) {
			ApplicationLauncher.logger.debug("refreshSinglePalletsInBay: fetchPalletsByBayState: getPalletDistinctId:    " + eachPalletManage.getPalletDistinctId());
			palletDistinctIdList.add(eachPalletManage.getPalletDistinctId());
			//ApplicationLauncher.logger.debug("refreshDashBoard : fetchPalletsByBayState: palletQrId:    " + palletQrId);
		...
		if(palletManageList.size()>1){
			...
			for(PalletManage eachPalletManage : palletManageList ) {
				...
				//ApplicationLauncher.logger.debug("refreshSinglePalletsInBay: batch update : palletName: " + palletName);
				//ConveyorDeviceDataManagerController.getDashboardObject().addPalletToFirstAvailableVerificationBay(palletName, meterListWithSerialNoMap);
				ConveyorDataManager.getDashboardObject().addNewPalletViewDashboard(myBayKey,palletName, meterListWithSerialNoMap);
				ApplicationLauncher.logger.info("refreshSinglePalletsInBay: added : " + myBayKey + " , palletName: " + palletName);
				...
				errorCodeMap.put(6, "");
				//Platform.runLater(()->{
				ApplicationLauncher.logger.debug("refreshSinglePalletsInBay : updateDashBoardPalletStatus : " + palletName);
				ConveyorDataManager.getDashboardObject().updateDashBoardPalletStatus(palletName, statusMap, errorCodeMap);
	...
	private void closeWaitingBayStopper() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.WAITING_PP1_BAY_KEY,false);
	}
	...
	private void openWaitingBayStopper() {
		...
		ConveyorDataManager.getDashboardObject().getBayIndicatorManager().updateBayExitStopper(ConstantConveyor.WAITING_PP1_BAY_KEY,true);
	}
...
//	}
	...
	/*private Map<String, Object> voltage_current_start() {
		IoPortInfo portInfo = BayUtils.getOutputPortDetails(ConstantBayPortNameMapping.CALIB_PORT_NAME_BOFA_VOLTAGE_CURRENT_START);
		...
		CalibrationBay.logger.debug("S059_02_Power_Source_Start_Main_CT : voltage_current_start : status : " + status);
		// ============================================================================================

		responseReturn.put("status", status);

		CalibrationBay.logger.debug("S059_02_Power_Source_Start_Main_CT : voltage_current_start : Exit");
