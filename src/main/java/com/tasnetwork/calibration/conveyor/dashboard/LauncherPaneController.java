 ...
 * for command locations and file names, including screen numbers for Python apps.
 */
public class LauncherPaneController {
    ...
    // Python application configurations (assuming these constants exist in ConstantConveyorConfig)
    ...
    /**
     * Handles the action event for launching the Rejection Screen (Python application).
     * Launches the Python script using predefined constants, including the screen number.
     * @param event The ActionEvent triggered by the button click.
     */
    @FXML
    void launchRejectionScreen(ActionEvent event) {
        //launchPythonApp(rejectionCmdLocation, rejectionPythonScript, rejectionPythonExecutablePath, (rejectionScreenNumber), btn_rejectionLaunch);
        //launchPythonAppV2( rejectionCmdLocation, rejectionPythonScript, rejectionPythonExecutablePath, rejectionMonitorDisplayName, rejectionBayNameDisplay, rejectionPort ,btn_rejectionLaunch);
        //launchPythonAppV3( rejectionCmdLocation, rejectionPythonScript, rejectionPythonExecutablePath, rejectionMonitorManufacturerName,
    	//    	rejectionMonitorPidNo, rejectionMonitorSerialNo, rejectionBayNameDisplay, rejectionPort ,btn_rejectionLaunch);
    ...
    /**
     * Handles the action event for launching the Unloading Screen (Python application).
     * Launches the Python script using predefined constants, including the screen number.
     * @param event The ActionEvent triggered by the button click.
     */
    @FXML
    void launchUnloadingScreen(ActionEvent event) {
        /*S08_functional_Test.sendRejectionMeterUpdate(1,"01000001", "FAIL", "ERR_RELAY");
        S08_functional_Test.sendRejectionMeterUpdate(2,"01000002", "PASS", "NO_RELAY");
        S08_functional_Test.sendRejectionMeterUpdate(3,"01000003", "PASS", "NO_RELAY");
        S08_functional_Test.sendRejectionMeterUpdate(4,"01000004", "FAIL", "ERR_COMM");
        S08_functional_Test.sendRejectionMeterUpdate(5,"01000005", "FAIL", "ERR_PULSE");
        S08_functional_Test.sendRejectionMeterUpdate(6,"01000006", "FAIL", "ERR_PhaseCurrent");*/
       ...
       // launchPythonApp(unloadingCmdLocation, unloadingPythonScript, unloadingPythonExecutablePath, (unloadingScreenNumber), btn_unloadingLaunch);
    	//launchPythonAppV2( unloadingCmdLocation, unloadingPythonScript, unloadingPythonExecutablePath, unloadingMonitorDisplayName, unloadingBayNameDisplay, unloadingPort ,btn_unloadingLaunch);
    	// launchPythonAppV3( unloadingCmdLocation, unloadingPythonScript, unloadingPythonExecutablePath, unloadingMonitorManufacturerName,
    	//    	unloadingMonitorPidNo, unloadingMonitorSerialNo, unloadingBayNameDisplay, unloadingPort ,btn_unloadingLaunch);
			 ...
			 ApplicationLauncher.logger.debug("UnloadingGetScreenNumberTask : screenNumber: " + screenNumber);
		    	if(!screenNumber.isEmpty()) {
		    		...
		    		if(Integer.parseInt(screenNumber)==-1) {
		    			...
		    			ApplicationLauncher.logger.debug("UnloadingGetScreenNumberTask : launchPythonApp : screenNumberIndex: " +screenNumberIndex);
		    			unloadingPythonScript = ConstantConveyorConfig.CONVEYOR_UNLOADING_PYTHON_SCRIPT_SCREEN_NUMBER;
		    			ApplicationLauncher.logger.debug("UnloadingGetScreenNumberTask : launchPythonApp : unloadingPythonScript:" +unloadingPythonScript);
		    			//launchPythonApp(unloadingCmdLocation, unloadingPythonScript, unloadingPythonExecutablePath, (unloadingScreenNumber), btn_unloadingLaunch);
		    			
		    			launchPythonAppV3_1(unloadingCmdLocation, unloadingPythonScript, unloadingPythonExecutablePath, screenNumberIndex,unloadingBayNameDisplay, unloadingPort, btn_unloadingLaunch);
			 ...
			 ApplicationLauncher.logger.debug("RejectionGetScreenNumberTask : screenNumber: " + screenNumber);
		    	if(!screenNumber.isEmpty()) {
		    		...
		    		if(Integer.parseInt(screenNumber)==-1) {
		    			...
		    			int screenNumberIndex = Integer.parseInt(screenNumber)-1;
		    			ApplicationLauncher.logger.debug("RejectionGetScreenNumberTask : launchPythonApp : screenNumberIndex : " +screenNumberIndex);
		    			rejectionPythonScript = ConstantConveyorConfig.CONVEYOR_REJECTION_PYTHON_SCRIPT_SCREEN_NUMBER;
		    			ApplicationLauncher.logger.debug("RejectionGetScreenNumberTask : launchPythonApp : rejectionPythonScript:" +rejectionPythonScript);
		    			//launchPythonApp(rejectionCmdLocation, rejectionPythonScript, rejectionPythonExecutablePath, (rejectionScreenNumber), btn_rejectionLaunch);
		    			launchPythonAppV3_1(rejectionCmdLocation, rejectionPythonScript, rejectionPythonExecutablePath, screenNumberIndex,rejectionBayNameDisplay, rejectionPort, btn_rejectionLaunch);
    ...
    private String getUnloadingUserMonitorScreenNumber() {
    	...
    	ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber : setUnloadingBayScreenNameUserInputReceived: false");
        	Platform.runLater(()->{
				...
				String title  = "Unloading Bay";
				    ...
				    dialog.resultProperty().addListener((obs, oldVal, newVal) -> {
				        if (Boolean.TRUE.equals(newVal)) {
				        	ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumberStable: prompt user hit: YES");
				        } else {
				        	ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber: Stable prompt user hit: NO");
				        }
				        ConveyorDeviceDataManagerController.setUnloadingBayScreenNameUserInputReceived(true);
				        ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber : setUnloadingBayScreenNameUserInputReceived: true");
				    });*/
				    ...
				    ApplicationLauncher .logger.debug("getUnloadingUserMonitorScreenNumber: Prompt shown, returning immediately");
				
			});
        	

        	ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber : awaiting for isUnloadingBayScreenNameUserInputReceived: Entry");
        	while (!ConveyorDataManager.isUnloadingBayScreenNameUserInputReceived() && !Unloading.isStopProcessRequestedUnloadingBay()) {
        		ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber: Waiting for isUnloadingBayScreenNameUserInputReceived");
                BayUtils.delay(1000);
            }
        	ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber : awaiting for isUnloadingBayScreenNameUserInputReceived: Exit");
        	if(ConveyorDataManager.isUnloadingBayScreenNameUserInputReceived()) {
        		//if(ConveyorDeviceDataManagerController.getUnloadingBayUserEntryScreenName()!=-1) {
        			...
        			ApplicationLauncher.logger.debug("getUnloadingUserMonitorScreenNumber : userEnteredScreenNumber: " + userEnteredScreenNumber);
    ...
    private String getRejectionUserMonitorScreenNumber() {
    	...
    	ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber : setRejectionBayScreenNameUserInputReceived: false");
        	Platform.runLater(()->{
				...
				String title  = "Rejection Bay";
				    ...
				    dialog.resultProperty().addListener((obs, oldVal, newVal) -> {
				        if (Boolean.TRUE.equals(newVal)) {
				        	ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumberStable: prompt user hit: YES");
				        } else {
				        	ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber: Stable prompt user hit: NO");
				        }
				        ConveyorDeviceDataManagerController.setRejectionBayScreenNameUserInputReceived(true);
				        ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber : setRejectionBayScreenNameUserInputReceived: true");
				    });*/
				    ...
				    dialog.show(); // This will NOT block the JavaFX thread 
				    ApplicationLauncher .logger.debug("getRejectionUserMonitorScreenNumber: Prompt shown, returning immediately");
				
			});
        	

        	ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber : awaiting for isRejectionBayScreenNameUserInputReceived: Entry");
        	while (!ConveyorDataManager.isRejectionBayScreenNameUserInputReceived() && !Rejection.isStopProcessRequestedRejectionBay()) {
        		ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber: Waiting for isRejectionBayScreenNameUserInputReceived");
                BayUtils.delay(1000);
            }
        	ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber : awaiting for isRejectionBayScreenNameUserInputReceived: Exit");
        	if(ConveyorDataManager.isRejectionBayScreenNameUserInputReceived()) {
        		//if(ConveyorDeviceDataManagerController.getRejectionBayUserEntryScreenName()!=-1) {
        			...
        			ApplicationLauncher.logger.debug("getRejectionUserMonitorScreenNumber : userEnteredScreenNumber: " + userEnteredScreenNumber);
	...
	/**
     * Launches a JAR file from a specified command prompt location in a new CMD window
     * and updates the button's state.
     *
     * @param cmdLocation The directory where the JAR is located and the new command prompt should start.
        ...
        new Thread(() -> {
            try {
                ...
                if (!workingDir.exists() || !workingDir.isDirectory()) {
                    throw new IOException("The specified directory does not exist or is not a directory: " + cmdLocation);
                }

                // Get the absolute path to the java.exe that is currently running THIS application
                String javaExecutablePath = System.getProperty("java.home") + File.separator + "bin" + File.separator + "java.exe";
                ...
                // Build the command using ProcessBuilder, now using the absolute path to java.exe
    ...
    /**
     * Launches a Python application from a specified command prompt location in a new CMD window
     * and updates the button's state.
     *
     * @param cmdLocation The directory where the Python script is located and the new command prompt should start.
     * @param pythonScript The name of the Python script file to launch.
     * @param pythonExecutablePath The explicit path to the Python executable (e.g., "C:\\Python\\Python39\\python.exe").
     * If null or empty, "python.exe" is assumed to be in the system's PATH.
     * @param screenNumber The screen index (e.g., 0 for primary, 1 for secondary) on which to open the Python app.
     * @param button The button associated with the launch action.
     */
    private void launchPythonApp(String cmdLocation, String pythonScript, String pythonExecutablePath, int screenNumber, Button button) {
        // Basic validation for cmdLocation and pythonScript
        if (cmdLocation == null || cmdLocation.trim().isEmpty() || pythonScript == null || pythonScript.trim().isEmpty()) {
        ...
        new Thread(() -> {
            try {
                ...
                String fullCommand = driveLetter + ": && cd \"" + cmdLocation + "\" && \"" +
                                     (pythonExecutablePath != null && !pythonExecutablePath.trim().isEmpty() ? pythonExecutablePath : "python.exe") +
                ...
                // IMPORTANT: Remove pb.directory() as the 'cd' command in fullCommand handles directory change
                // pb.directory(workingDir); // REMOVED

                pb.inheritIO();           // Inherit I/O for debugging (optional)

                // --- Debugging output ---
                ApplicationLauncher.logger.info("Launching Python app command array: " + Arrays.toString(pb.command().toArray()));
                ...
                ApplicationLauncher.logger.error("Error launching Python app: " + pythonScript + " from " + cmdLocation);
                e.printStackTrace();
                Platform.runLater(() -> {
                    updateButtonOnError(button);
                });
    ...
    private void launchPythonAppV3_1(String cmdLocation, String pythonScript, String pythonExecutablePath, int screenNumber,String bayNameDisplay, String port,Button button) {
        // Basic validation for cmdLocation and pythonScript
        if (cmdLocation == null || cmdLocation.trim().isEmpty() || pythonScript == null || pythonScript.trim().isEmpty()) {
            ApplicationLauncher.logger.error("launchPythonAppV3_1: Cannot launch Python app: cmdLocation or pythonScript is null/empty.");
            Platform.runLater(() -> updateButtonOnError(button));
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                // --- Debugging output ---
                ApplicationLauncher.logger.info("launchPythonAppV3_1: Launching Python app command array: " + Arrays.toString(pb.command().toArray()));
                ApplicationLauncher.logger.info("launchPythonAppV3_1: Full command string passed to inner cmd.exe: " + fullCommand);
                // --- End Debugging output ---
                ...
                ApplicationLauncher.logger.error("launchPythonAppV3_1: Error launching Python app: " + pythonScript + " from " + cmdLocation);
                e.printStackTrace();
                Platform.runLater(() -> {
                    updateButtonOnError(button);
                });
    ...
    private void launchPythonAppV2(String cmdLocation, String pythonScript, String pythonExecutablePath, String monitorDisplayName, String bayNameDisplay, String port ,Button button) {
        // Basic validation for cmdLocation and pythonScript
    	ApplicationLauncher.logger.info("launchPythonAppV2: Entry");
        if (cmdLocation == null || cmdLocation.trim().isEmpty() || pythonScript == null || pythonScript.trim().isEmpty()) {
            ApplicationLauncher.logger.error("launchPythonAppV2: Cannot launch Python app: cmdLocation or pythonScript is null/empty.");
            Platform.runLater(() -> updateButtonOnError(button));
            return;
        }
        ...
        new Thread(() -> {
            try {
               ...
               /* String fullCommand = driveLetter + ": && cd \"" + cmdLocation + "\" && \"" +
                        (pythonExecutablePath != null && !pythonExecutablePath.trim().isEmpty() ? pythonExecutablePath : "python.exe") +
                        "\" \"" + pythonScript + " --port " + port + " --bay-name \"" + bayNameDisplay + "\" --screen-name \"" + monitorDisplayName + "\"";*/
                ...
                // --- Debugging output ---
                ApplicationLauncher.logger.info("launchPythonAppV2: Launching Python app command array: " + Arrays.toString(pb.command().toArray()));
                ApplicationLauncher.logger.info("launchPythonAppV2: Full command string passed to inner cmd.exe: " + fullCommand);
                // --- End Debugging output ---
                ...
                ApplicationLauncher.logger.error("launchPythonAppV2: Error launching Python app: " + pythonScript + " from " + cmdLocation);
                e.printStackTrace();
                Platform.runLater(() -> {
                    updateButtonOnError(button);
                });
    ...
    private void launchPythonAppV3(String cmdLocation, String pythonScript, String pythonExecutablePath, String  monitorManufacturerName,
    	...
    	//ApplicationLauncher.logger.info("launchPythonAppV3: monitorManufacturerName: " + monitorManufacturerName);
        if (cmdLocation == null || cmdLocation.trim().isEmpty() || pythonScript == null || pythonScript.trim().isEmpty()) {
            ApplicationLauncher.logger.error("launchPythonAppV3: Cannot launch Python app: cmdLocation or pythonScript is null/empty.");
            Platform.runLater(() -> updateButtonOnError(button));
            return;
        }
        ...
        new Thread(() -> {
            try {
               ...
               /* String fullCommand = driveLetter + ": && cd \"" + cmdLocation + "\" && \"" +
                ...
                // --- Debugging output ---
                ApplicationLauncher.logger.info("launchPythonAppV3: Launching Python app command array: " + Arrays.toString(pb.command().toArray()));
                ApplicationLauncher.logger.info("launchPythonAppV3: Full command string passed to inner cmd.exe: " + fullCommand);
                // --- End Debugging output ---
                ...
                ApplicationLauncher.logger.error("launchPythonAppV3: Error launching Python app: " + pythonScript + " from " + cmdLocation);
                e.printStackTrace();
                Platform.runLater(() -> {
                    updateButtonOnError(button);
                });
