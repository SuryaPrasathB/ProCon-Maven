	...
	public void showInlineBayError(String bayKey, String errorCode) {
		Platform.runLater(() -> {
			...
			if (ConstantConveyor.VERIFICATION_BAY_KEY.equals(bayKey)) {
				...
				targetContainers.add(ref_bay11Container);
				targetContainers.add(ref_bay12Container);
			} else if (ConstantConveyor.WAITING_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay5Container);
				targetContainers.add(ref_bay6Container);
				targetContainers.add(ref_bay7Container);
				targetContainers.add(ref_bay8Container);
			} else if (ConstantConveyor.STA_NLD1_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay13Container);
				targetContainers.add(ref_bay14Container);
				targetContainers.add(ref_bay15Container);
				targetContainers.add(ref_bay16Container);
			} else if (ConstantConveyor.STA_NLD2_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay17Container);
				targetContainers.add(ref_bay18Container);
				targetContainers.add(ref_bay19Container);
				targetContainers.add(ref_bay20Container);
			} else if (ConstantConveyor.FT_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay1Container);
			} else if (ConstantConveyor.HV_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay2Container);
			} else if (ConstantConveyor.IR_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay3Container);
			} else if (ConstantConveyor.CALIBRATION_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_bay4Container);
			} else if (ConstantConveyor.REJECTION_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_rejectionBayContainer);
			} else if (ConstantConveyor.UNLOADING_BAY_KEY.equals(bayKey)) {
				targetContainers.add(ref_unloadingBayContainer);
			} else if (ConstantConveyor.LOADING_BAY_KEY.equals(bayKey)) {
				// No specific loading bay container in ref_ mapped for pallets explicitly
			}
			...
			for (AnchorPane container : targetContainers) {
				...
				javafx.scene.control.Label titleLabel = new javafx.scene.control.Label("BAY HALTED");
				titleLabel.setStyle("-fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 14px;");

				javafx.scene.control.Label errorLabel = new javafx.scene.control.Label(errorCode);
